...
function getPool(): Pool | null {
  ...
  pool.on('error', () => { /* pg discards the client and reconnects on demand */ });
  return pool;
}
...
async function lookup(hash: string): Promise<KeyPrincipal | null> {
  const p = getPool();
  if (!p) return null;
  const { rows } = await p.query<{ customer: string }>(
    `SELECT customer FROM tenancy.api_keys WHERE key_hash = $1 AND status = 'active'`,
    [hash],
  );
 ...
 */
export async function resolveApiKey(presented: string): Promise<KeyPrincipal | null> {
  ...
  if (!keyStoreConfigured()) return null;

  const hash = hashKey(presented);
  const hit = cache.get(hash);
  if (hit && hit.expiresAt > Date.now()) {
    if (hit.principal) touch(hash);
    return hit.principal;
  }

  let principal: KeyPrincipal | null;
  try {
