...
function getAtPath(obj, path6) {
  if (!obj || !path6) return void 0;
  return path6.split(".").reduce((acc, key) => acc == null ? void 0 : acc[key], obj);
}
...
function manifestPath(metadataDir) {
  return path2.join(metadataDir, EXTRACT_MANIFEST_FILENAME);
}
function readExtractedCustomModels(metadataDir) {
  try {
    const raw = fs3.readFileSync(manifestPath(metadataDir), "utf-8");
    const parsed = JSON.parse(raw);
    if (Array.isArray(parsed.customModels)) {
      return parsed.customModels.filter((m) => typeof m === "string");
    }
    return void 0;
  } catch {
    return void 0;
...
async function discoverLabelFiles(modelDir) {
  const results = [];
  let axLabelDir = path3.join(modelDir, "AxLabelFile", "LabelResources");
  if (!fsSync.existsSync(axLabelDir)) {
    axLabelDir = path3.join(modelDir, "axlabelfile", "LabelResources");
    if (!fsSync.existsSync(axLabelDir)) {
      axLabelDir = path3.join(modelDir, "axlabelfile", "labelresources");
    }
  }
  ...
  let locales;
  try {
    locales = await fs4.readdir(axLabelDir);
  } catch {
    return results;
  }
  for (const locale of locales) {
    ...
    const localeDir = path3.join(axLabelDir, locale);
    let files;
    try {
      files = await fs4.readdir(localeDir);
    } catch {
      continue;
    }
    for (const file of files) {
      ...
      results.push({
        ...
        filePath: path3.join(localeDir, file)
      });
    }
  }
  return results;
...
async function indexAllLabels(symbolIndex, packagesPath, modelFilter, opts) {
  ...
  for (const packageOrModel of models) {
    const packageDir = path3.join(packagesPath, packageOrModel);
    const modelDirs = [];
    try {
      ...
      for (const subDir of subDirs) {
        const candidateDir = path3.join(packageDir, subDir);
        const axLabelDirOriginal = path3.join(candidateDir, "AxLabelFile");
        const axLabelDirLower = path3.join(candidateDir, "axlabelfile");
    ...
    if (modelDirs.length === 0) {
      const flatAxLabel = path3.join(packageDir, "AxLabelFile");
      const flatAxLabelLower = path3.join(packageDir, "axlabelfile");
...
var XppConfigProvider = class {
  ...
  constructor(configDir) {
    this.configDir = configDir || path4.join(
      process.env.LOCALAPPDATA || path4.join(process.env.USERPROFILE || "", "AppData", "Local"),
      "Microsoft",
      "Dynamics365",
      "XPPConfig"
    );
  ...
  async listConfigs() {
    ...
    const withStats = await Promise.all(
      jsonFiles.map(async (entry) => {
        const fullPath = path4.join(this.configDir, entry.name);
        try {
...
// src/knowledge/formPatterns/catalog/topLevel/common.ts
function actionPane(occurrence = "required") {
  return {
    id: "ActionPane",
...
function filterGroup(occurrence = "optional") {
  return {
    id: "FilterGroup",
    controlTypes: ["Group"],
    occurrence,
...
function mainGrid(occurrence = "required") {
  return {
    id: "Grid",
    controlTypes: ["Grid"],
    occurrence,
...
// src/knowledge/formPatterns/catalog/topLevel/simpleList.ts
var simpleList = {
  id: "SimpleList",
  xmlName: "SimpleList",
  displayName: "Simple List",
  versions: ["1.1", "1.0"],
  purpose: "Maintains data for simple entities as a single editable grid with fewer than ~10 fields per record. The default pattern for setup/group tables.",
  whenToUse: [
...
// src/knowledge/formPatterns/catalog/topLevel/simpleListDetails.ts
var navigationListPanel = {
  id: "NavigationList",
  controlTypes: ["Group"],
  occurrence: "required",
...
var simpleListDetailsListGrid = {
  ...
  purpose: "Maintains data for entities of medium complexity: a left navigation list (2-3 fields) plus a right details panel. The default Simple List & Details variant.",
  whenToUse: [
  ...
  notes: [
    "Tabular Grid and Tree variants share the SimpleListDetails xmlName in metadata \u2014 the variant is determined by the list panel content (tabular grid / tree control).",
    "Mining confirmed: newer forms serialize as SimpleListDetails-Grid; both forms resolve to this entry."
  ]
};
var simpleListDetailsTree = {
  ...
  purpose: "Simple List & Details variant where the left navigation panel contains a tree control instead of a flat grid (for hierarchical entities).",
...
// src/knowledge/formPatterns/catalog/topLevel/detailsMaster.ts
var fastTabs = {
  id: "FastTabs",
  controlTypes: ["Tab"],
  occurrence: "required",
...
var detailsMaster = {
  ...
  purpose: "Displays the details of a complex master entity on FastTabs, with a grid view and a details view (e.g. customers, vendors, products).",
  whenToUse: [
...
var detailsMasterTabs = {
  id: "DetailsMasterTabs",
  xmlName: "DetailsMasterTabs",
  variantOf: "DetailsMaster",
  displayName: "Details Master w/ Standard Tabs",
  versions: ["1.4", "1.0"],
  purpose: "Details Master variant for forms with a large number of FastTabs (>15) grouped into categories using standard tabs.",
...
// src/knowledge/formPatterns/catalog/topLevel/detailsTransaction.ts
var navigationListPanel2 = {
  id: "NavigationList",
  controlTypes: ["Group"],
  occurrence: "optional",
...
var detailsTransaction = {
  id: "DetailsTransaction",
  xmlName: "DetailsTransaction",
  displayName: "Details Transaction",
  versions: ["1.4", "1.1", "1.0"],
  purpose: "Displays the details of a complex transaction entity and its lines \u2014 an order header plus order lines (e.g. sales orders, purchase orders).",
  whenToUse: [
  ...
  root: [actionPane("required"), navigationListPanel2, filterGroup("optional"), headerLinesTabs],
  extraRootChildren: "none",
  lifecycleGuidance: [
  ...
  notes: [
    "From v1.4 the platform pattern requires a Navigation List (left SidePanel grid of header records); the generator emits one. The catalog keeps it optional so clones of older v1.x forms without one are not rejected."
  ]
};

// src/knowledge/formPatterns/catalog/topLevel/dialog.ts
var dialogBody = {
  id: "DialogBody",
  controlTypes: ["Group"],
  occurrence: "required",
...
var dropDialog = {
  ...
  versions: ["1.2", "1.1", "1.0"],
  purpose: "Lightweight dialog dropped from a button to gather a small set of inputs (<5 fields) that provide context for an action.",
...
// src/knowledge/formPatterns/catalog/topLevel/tableOfContents.ts
var tableOfContents = {
  id: "TableOfContents",
  xmlName: "TableOfContents",
  displayName: "Table of Contents",
  versions: ["1.1", "1.0"],
  purpose: "Displays setup/parameters information or loosely related information sets as a vertical table-of-contents navigation with one content region per entry.",
  whenToUse: [
...
var advancedSelection = {
  id: "AdvancedSelection",
  xmlName: "AdvancedSelection",
  displayName: "Advanced Selection",
  versions: ["1.1", "1.0"],
  purpose: 'Multi-select dialog allowing users to choose from a list with optional filtering \u2014 the "Add" dialog pattern used when selecting multiple records (e.g. adding items to a group).',
  whenToUse: [
...
// src/knowledge/formPatterns/catalog/topLevel/lookup.ts
var lookupBasic = {
  id: "Lookup",
  xmlName: "Lookup",
  displayName: "Lookup - Basic",
  versions: ["1.2", "1.1", "1.0"],
  purpose: "Form used as a lookup: a grid (or tree) optimized for picking a value, with optional filters or buttons.",
  whenToUse: [
...
// src/knowledge/formPatterns/catalog/topLevel/listPage.ts
var listPage = {
  id: "ListPage",
  xmlName: "ListPage",
  ...
  purpose: "Read-optimized grid entry point for browsing records and acting on them, typically with FactBoxes in the Parts node and a corresponding Details form.",
  whenToUse: [
...
// src/knowledge/formPatterns/catalog/topLevel/workspace.ts
var workspacePanorama = {
  id: "Workspace",
  xmlName: "TabbedWorkspace",
  xmlAliases: ["Workspace"],
  displayName: "Workspace (Tabbed Panorama)",
  versions: ["1.0"],
  purpose: "Activity overview page: a horizontally scrolling panorama with a tile/KPI summary section followed by list/chart/link sections. Primary means of navigation for an activity.",
  whenToUse: [
...
var formPartSectionList = {
  ...
  versions: ["1.2", "1.1", "1.0"],
  purpose: "A list shown in a workspace section \u2014 modeled as a separate form and rendered in the workspace via a Form Part control.",
...
var formPartSectionListDouble = {
  ...
  versions: ["1.2", "1.1", "1.0"],
  purpose: 'A workspace section list with two lists \u2014 the primary visible list and a secondary hidden list that becomes visible on toggle (e.g. "My" vs "All" tasks).',
...
var workspaceOperational = {
  id: "WorkspaceOperational",
  xmlName: "WorkspaceOperational",
  variantOf: "Workspace",
  displayName: "Operational Workspace",
  versions: ["1.1", "1.0"],
  purpose: "The preferred, performance-enhanced workspace variant: sections render via Form Part controls so content loads on demand.",
...
// src/knowledge/formPatterns/catalog/topLevel/factBox.ts
var factBoxGrid = {
...
var factBoxPatterns = [factBoxGrid, factBoxCard];

// src/knowledge/formPatterns/catalog/topLevel/simpleDetails.ts
var simpleDetailsToolbarFields = {
  id: "SimpleDetails",
  ...
  notes: [
    "Variants (FastTabs / Standard Tabs / Panorama) share the Simple Details class; the variant is the body container style. Mining confirmed: newer forms serialize as SimpleDetails-FastTabsContainer or SimpleDetails-StandardTabsContainer."
  ]
};
var simpleDetailsFastTabs = {
...
// src/knowledge/formPatterns/catalog/topLevel/task.ts
var taskSingle = {
  id: "TaskSingle",
  xmlName: "TaskSingle",
...
var taskPatterns = [taskSingle, taskDouble];

// src/knowledge/formPatterns/catalog/topLevel/wizard.ts
var wizard = {
  id: "Wizard",
  xmlName: "Wizard",
  displayName: "Wizard",
  versions: ["1.2", "1.1", "1.0"],
  purpose: "Displays a sequence of tab pages gathering information in a predetermined order, navigated with Back/Next/Finish buttons (backed by a SysWizard class).",
  whenToUse: [
...
// src/knowledge/formPatterns/catalog/subPatterns/customFilters.ts
var customAndQuickFilters = {
  id: "CustomAndQuickFilters",
  xmlName: "CustomAndQuickFilters",
  displayName: "Custom and Quick Filters",
  versions: ["1.1", "1.0"],
  appliesToControlTypes: ["Group"],
  purpose: "Filter group above a grid containing a QuickFilter plus optional modeled custom filter fields. Used when a QuickFilter is required (the default for list-style patterns).",
...
var customFilterSubPatterns = [customAndQuickFilters, customFilters];

// src/knowledge/formPatterns/types.ts
var INPUT_CONTROL_TYPES = [
  "String",
  "Int",
  "Integer",
  "Int64",
  ...
  "ListBox",
  "Control"
  // extension/custom controls (e.g. dimension controls)
];

// src/knowledge/formPatterns/catalog/subPatterns/fields.ts
var fieldsFieldGroups = {
  id: "FieldsFieldGroups",
  xmlName: "FieldsFieldGroups",
  displayName: "Fields and Field Groups",
  versions: ["1.2", "1.1", "1.0"],
  appliesToControlTypes: ["Group", "TabPage"],
  purpose: "Responsive column layout for containers that contain only fields and one level of field groups. The pattern sets WidthMode/HeightMode to SizeToContent \u2014 manual widths and SizeToAvailable are not allowed.",
...
var horizontalFieldsButtonGroup = {
...
// src/knowledge/formPatterns/catalog/subPatterns/panels.ts
var sidePanel = {
  id: "SidePanel",
  xmlName: "SidePanel",
  ...
  purpose: "Left-hand navigation list of a Simple List & Details form: an optional QuickFilter above a list-style grid with 2-3 fields per row.",
...
var workspaceSummaryNumbersUnboundFields = {
...
var panelSubPatterns = [
  sidePanel,
  workspaceSummaryNumbersUnboundFields
];

// src/knowledge/formPatterns/catalog/subPatterns/toolbar.ts
var toolbarAndList = {
  id: "ToolbarAndList",
  xmlName: "ToolbarList",
...
var nestedSimpleListDetails = {
  id: "NestedSimpleListDetails",
  xmlName: "NestedSimpleListDetails",
  displayName: "Nested Simple List and Details",
  versions: ["1.0"],
  appliesToControlTypes: ["Group", "TabPage"],
  purpose: "Embeds a simpler Simple List & Details layout (list panel + details panel) inside a tab or group of a larger form.",
  referenceForms: ["HcmJob (TaskTabPage)"],
  root: [],
  extraRootChildren: "any"
};
var toolbarAndListDouble = {
...
var dimensionExpressionBuilder = {
...
// src/knowledge/formPatterns/catalog/subPatterns/workspaceSections.ts
function workspaceSection(id, xmlName, aliases, displayName, purpose, referenceForms) {
  return {
    id,
    xmlName,
    xmlAliases: aliases,
...
var workspaceSectionSubPatterns = [
  workspaceSection(
...
// src/knowledge/formPatterns/index.ts
var FORM_PATTERN_CATALOG = {
  patterns: [
    simpleList,
...
var patternByKey = /* @__PURE__ */ new Map();
...
var subPatternByKey = /* @__PURE__ */ new Map();
...
function resolvePatternExact(name) {
  if (!name) return void 0;
  return patternByKey.get(name.trim().toLowerCase());
}
function resolveSubPattern(name) {
  if (!name) return void 0;
  return subPatternByKey.get(name.trim().toLowerCase());
}

// src/knowledge/formPatterns/crossCheck.ts
function hasMinedPatternData(db) {
  try {
...
function crossCheckPatternCatalog(db) {
  ...
  report.catalogGaps = [...gapCounts.entries()].map(([pattern, forms]) => ({ pattern, forms })).sort((a, b) => b.forms - a.forms);
...
function formatCrossCheckReport(report) {
  const lines = [];
  ...
  if (report.unusedCatalogEntries.length > 0) {
    lines.push(
      `   [i] Catalog entries with zero mined usage (possible xmlName mismatch or unused area): ` + report.unusedCatalogEntries.join(", ")
    );
  }
  return lines.join("\n");
}
...
var INPUT_PATH = process.env.METADATA_PATH || "./extracted-metadata";
...
var FORCE_VACUUM = process.env.VACUUM === "true";
...
var SKIP_FTS = process.env.SKIP_FTS === "true";
var RESUME = process.env.RESUME === "true";
async function buildDatabase() {
  ...
  if (extractManifestCustomModels !== void 0) {
    ...
    console.log(kv("Non-MS models", extractManifestCustomModels.length > 0 ? c.dim(`${extractManifestCustomModels.length} from extract manifest (excluded from property stats)`) : c.dim("none recorded by extract manifest")));
  }
  ...
  if (RESUME) {
    ...
    const allModels = fsSync2.readdirSync(INPUT_PATH, { withFileTypes: true }).filter((e) => e.isDirectory()).map((e) => e.name);
    const manifestCustomModels = CUSTOM_MODELS.length > 0 ? void 0 : extractManifestCustomModels;
    if (CUSTOM_MODELS.length > 0) {
      ...
      modelsToRebuild = [...new Set(expandedModels)];
    ...
    } else if (manifestCustomModels !== void 0) {
  ...
  if (SKIP_FTS) {
    ...
    if (validRoots.length === 0) {
      ...
      if (isIncrementalCustomBuild) {
        ...
        log.detail(`Label indexes rebuilt in ${(indexMs / 1e3).toFixed(2)}s`);
        if (ftsPending) {
          const ftsStart = Date.now();
          symbolIndex.rebuildLabelsFts();
          log.detail(`Labels FTS rebuilt in ${((Date.now() - ftsStart) / 1e3).toFixed(2)}s`);
        }
      }
