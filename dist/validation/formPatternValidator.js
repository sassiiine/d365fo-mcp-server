...
import { resolvePatternExact, resolveSubPattern, knownPatternNames, } from '../knowledge/formPatterns/index.js';
// Helpers
...
function isOpaqueControl(node) {
    if (OPAQUE_CONTROL_TYPES.has(node.type))
        return true;
...
function typeMatches(node, allowed) {
...
function checkVersion(declared, knownVersions, patternLabel, path, violations) {
    if (!declared) {
        violations.push({
            rule: 'FP002',
            severity: 'warning',
 ...
 */
function matchChildren(actual, specs, extra, ordered, parentPath, patternLabel, violations) {
    ...
    for (const spec of specs) {
        ...
        for (let i = 0; i < actual.length; i++) {
            if (consumed[i])
                continue;
            if (!typeMatches(actual[i], spec.controlTypes))
                continue;
            indices.push(i);
            if (spec.occurrence === 'required' || spec.occurrence === 'optional')
                break;
        }
        if (indices.length === 0) {
        ...
        if (!allowedExtra) {
            violations.push({
                ...
                fix: extra === 'none'
                    ...
                    : `Move "${child.name}" elsewhere — allowed extra types here: ${extra.join(', ')}.`,
            });
        }
    }
    return matched;
...
function applySpecToNode(pair, parentPath, patternLabel, topPatternId, violations) {
    ...
    if (node.pattern) {
        if (spec.allowedSubPatterns && spec.allowedSubPatterns.length > 0) {
            ...
            const allowed = spec.allowedSubPatterns.some((name) => name.toLowerCase() === node.pattern.toLowerCase()
        ...
        violations.push({
            ...
            fix: spec.allowedSubPatterns && spec.allowedSubPatterns.length > 0
    ...
    if (hasChildSpecs || restrictsExtras) {
        const matched = matchChildren(node.children, spec.children ?? [], spec.extraChildren ?? 'any', spec.childrenOrdered ?? true, path, patternLabel, violations);
 ...
 */
function validateSubPatternsDeep(nodes, parentPath, topPatternId, violations) {
    for (const node of nodes) {
        ...
        if (node.pattern) {
            ...
            if (!sp) {
                ...
                const matched = matchChildren(node.children, sp.root, sp.extraRootChildren ?? 'any', true, path, `sub-pattern ${sp.xmlName}`, violations);
...
function countContainers(nodes) {
    ...
    const visit = (node) => {
        if (isOpaqueControl(node))
            return; // managed interior — not pattern containers
...
export function validateFormTree(facts) {
    ...
    if (spec) {
        ...
        const matched = matchChildren(design.controls, spec.root, spec.extraRootChildren ?? 'none', true, 'Design', label, violations);
        for (const pair of matched) {
...
export async function validateFormPatternXml(xml) {
    ...
    if (dataSourcesNode && typeof dataSourcesNode === 'object') {
        ...
        if (ds)
            dataSourceCount = Array.isArray(ds) ? ds.length : 1;
    }
...
export function hasPatternErrors(report) {
    return report.violations.some((v) => v.severity === 'error');
}
//# sourceMappingURL=formPatternValidator.js.map