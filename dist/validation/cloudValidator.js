 ...
 */
async function ruleFieldTypeMatchesEdt(xml, ctx) {
    if (!ctx.edtTypes)
        return [];
    const findings = [];
    for (const f of parseTableFields(xml)) {
        ...
        if (!declared)
            continue; // unrecognised element type - not this rule's business
        const actual = await ctx.edtTypes.baseTypeOf(f.edt);
        if (!actual)
            continue; // unknown EDT: silence beats a false accusation
        if (actual !== declared) {
            findings.push({
                severity: 'error',
                rule: 'field-type-mismatches-edt',
                location: `Fields/${f.name}`,
                message: `Field '${f.name}' is declared i:type="${f.elementType}" (${declared}) but its ` +
 ...
 */
function ruleDataGroupNeedsDataSource(xml) {
    const findings = [];
 ...
 * half file to disk.
 */
function ruleRootElementCloses(xml) {
 ...
 */
export async function validateGeneratedXml(xml, ctx = {}) {
    if (!xml || !xml.trim()) {
 ...
 */
export function validateObjectSet(docs) {
    const findings = [];
    const tables = new Map();
    for (const d of docs) {
        if (/^table$/i.test(d.objectType))
            tables.set(d.name.toLowerCase(), tableFieldGroups(d.xml));
    }
    for (const d of docs) {
        if (!/^form$/i.test(d.objectType))
            continue;
        for (const ref of formDataGroupRefs(d.xml)) {
            const groups = ref.dataSource ? tables.get(ref.dataSource.toLowerCase()) : undefined;
            if (!groups)
                continue; // that table is not in this set - cannot judge
            if (!groups.has(ref.group.toLowerCase())) {
                findings.push({
                    severity: 'error',
                    rule: 'datagroup-missing-on-table',
                    location: `${d.name}.Design/${ref.control}`,
                    message: `Form '${d.name}' binds field group '${ref.group}' on table '${ref.dataSource}', ` +
                        `but that table declares no such group (${[...groups].join(', ') || 'none'}).`,
                    hint: `Add a '${ref.group}' field group to ${ref.dataSource}, or bind an existing one. ` +
...
//# sourceMappingURL=cloudValidator.js.map