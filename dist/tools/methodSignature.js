 ...
 */
async function tryBridgeMethodSignature(bridge, className, methodName, modelName, includeCoc) {
 ...
 */
async function tryXmlMethodSignature(parser, filePath, className, methodName, modelName, includeCoc, objectType) {
    if (!parser || !filePath)
        return null;
    try {
 ...
 */
function parseMethodSignature(source, methodName) {
    if (!source)
        return null;
...
function formatOutput(className, methodName, signature, modelName, includeCocTemplate = false, obsoleteWarning = '') {
    let output = `# Method: \`${className}.${methodName}\`\n`;
