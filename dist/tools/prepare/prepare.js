...
const PrepareArgsSchema = z
    ...
    mode: z
        .enum(PREPARE_MODES)
        .default('change')
        .describe('change (default) → aggregate context for extending/modifying an existing object; ' +
...
export function pruneRecentPrepares(now = Date.now()) {
    for (const [key, prior] of recentPrepares.entries()) {
        if (now - prior.at > REPEAT_TTL_MS)
            recentPrepares.delete(key);
    }
}
/**
 ...
 */
export function resetRecentPrepares() {
    ...
    if (prior) {
        ...
        return {
            content: [
                {
                    type: 'text',
                    text: `ℹ️ Already prepared — this is the same request as ${minutes} min ago ` +
