...
import { hasTableDataMethods, lookupTableDataMethod, renderTableDataMethodEligibility, renderTableDataMethodSignature, } from '../../knowledge/tableDataMethods.js';
...
export const prepareChangeArgsSchema = z.object({
    goal: z.string().describe('One-sentence description of the intended change. ' +
        'Example: "Add CoC on CustTable.validateWrite to enforce a custom rule."'),
    objectName: z.string().describe('Name of the D365FO object to extend or modify (class, table, form, etc.). ' +
        'Example: "CustTable", "SalesFormLetter", "CustPostInvoice".'),
    methodName: z.string().optional().describe('Target method name when the change involves a specific method (CoC or event handlers). ' +
        'Example: "validateWrite", "insert", "post".'),
    objectType: z.enum([
        'class', 'table', 'form', 'query', 'view', 'enum', 'edt',
        'data-entity', 'map', 'report', 'security-duty', 'security-role',
    ]).optional().describe('D365FO object type. Auto-detected from the symbol index when omitted.'),
    proposedName: z.string().optional().describe('Proposed name for the new extension class/object. ' +
...
function lookupObjectNocase(objectName, context) {
...
async function resolveObject(objectName, context) {
    try {
 ...
 */
function readMethodRow(context, objectName, methodName) {
    try {
        ...
        const stmt = db.prepare(`SELECT signature, tags FROM symbols
       WHERE parent_name = ? AND type = 'method' AND name = ? COLLATE NOCASE
       LIMIT 1`);
        const own = stmt.get(objectName, methodName);
        if (own)
            return { row: own, owner: objectName, inherited: false };
        const declaring = findDeclaringAncestor(db, objectName, methodName);
        if (!declaring)
            return undefined;
        const inheritedRow = stmt.get(declaring, methodName);
...
async function fetchMethodSignature(objectName, methodName, objectType, context) {
    ...
    if (found) {
        ...
        if (inherited) {
            lines.push(`ℹ️  Inherited — \`${objectName}\` does not declare \`${methodName}\`; ` +
                `it is declared on \`${owner}\`, which is where this signature comes from.`);
        }
        const tags = row.tags ?? '';
        if (tags.includes('hookable:false'))
            lines.push('⛔ [Hookable(false)] — CoC is blocked.');
        if (tags.includes('wrappable:false'))
            lines.push('⛔ [Wrappable(false)] — wrapping is blocked.');
    ...
    if (inherited)
        return renderTableDataMethodSignature(inherited, objectName);
    return '(not found in symbol index)';
}
/** Fetch existing CoC extensions — bridge-first, index fallback. */
async function fetchCocExtensions(objectName, methodName, context) {
    ...
    try {
        const db = context.symbolIndex.getReadDb();
        const rows = db.prepare(`SELECT name, model FROM symbols
       WHERE type IN ('class', 'class-extension')
         AND (name LIKE ? OR name LIKE ?)
       LIMIT 20`).all(`${objectName}%Extension`, `${objectName}%_Extension`);
        if (rows.length > 0) {
...
async function fetchEligibility(objectName, methodName, objectType, context) {
    if (!methodName)
        return 'No specific method targeted — check base class documentation.';
    const found = readMethodRow(context, objectName, methodName);
    if (found) {
        const { row, owner, inherited } = found;
        const tags = row.tags ?? '';
        if (tags.includes('hookable:false'))
            return '⛔ [Hookable(false)] — CoC is blocked on this method.';
        if (tags.includes('wrappable:false'))
            return '⛔ [Wrappable(false)] — wrapping is blocked on this method.';
        if (/\bfinal\b/i.test(row.signature ?? '')) {
...
async function fetchNamingValidation(proposedName, context) {
    const issues = [];
...
async function fetchPatterns(objectName, objectType, context) {
    try {
        const db = context.symbolIndex.getReadDb();
        ...
        const rows = db.prepare(`SELECT name, description FROM symbols INDEXED BY idx_type_name
       WHERE type = ? AND name LIKE ? AND description != ''
       ORDER BY LENGTH(name)
       LIMIT 3`).all(objectType ?? 'class', `%${objectName}%`);
        if (rows.length > 0) {
...
// Tool handler
export async function prepareChangeTool(request, context) {
    ...
    if (resolvedType)
        lines.push(`**Object type (resolved):** ${resolvedType}`);
    lines.push('');
    if (sigText !== null) {
    ...
    lines.push('');
    lines.push(process.env.GROUNDING_ENFORCE === 'true'
...
//# sourceMappingURL=prepareChange.js.map