...
export const prepareCreateArgsSchema = z.object({
    goal: z.string().describe('One-sentence description of what the new object is for. ' +
        'Example: "Parameter table for the Contoso import feature."'),
    objectName: z.string().describe('Proposed BASE name of the new object WITHOUT model prefix ' +
    ...
    ]).describe('Type of the new D365FO object. Wholly new standalone objects only — for ' +
    ...
    fieldsHint: z.array(z.string()).optional().describe('For tables/views: planned field names (e.g. ["CustAccount", "ImportDate", "Qty"]). ' +
        'Each gets EDT suggestions from the index.'),
});
// Lookups below are all index-only, run in parallel.
/** Exact + prefixed collision check. */
function checkCollisions(finalName, baseName, context) {
    try {
...
function validateNaming(baseName, finalName, modelName) {
    const issues = [];
    ...
    if (!/^[A-Za-z][A-Za-z0-9_]*(\.[A-Za-z][A-Za-z0-9_]*)?$/.test(finalName)) {
        issues.push(`❌ Final name "${finalName}" is not a valid AOT name — letters, digits and underscores only ` +
...
function findSimilarObjects(baseName, objectType, context) {
    try {
        const db = context.symbolIndex.getReadDb();
        ...
        const rows = db.prepare(`SELECT name, model FROM symbols INDEXED BY idx_type_name
       WHERE type = ? AND name LIKE ? AND parent_name IS NULL
       ORDER BY LENGTH(name) LIMIT 5`).all(objectType, `%${needle}%`);
        if (rows.length > 0) {
...
function suggestEdtsForFields(fieldsHint, context) {
    const lines = [];
    ...
    if (reservedHits.length > 0) {
        lines.push(`⛔ **Reserved system field names — do NOT use as custom fields:**`, ...reservedHits.map(f => `  • \`${f}\` — reserved by the platform (auto-tracked). Rename to a non-reserved name (e.g. "NoteDateTime" instead of "CreatedDateTime").`), `  The platform auto-provides: CreatedDateTime, ModifiedDateTime, CreatedBy, ModifiedBy, RecId, RecVersion, DataAreaId, Partition.`, ``);
    }
    try {
        const db = context.symbolIndex.getReadDb();
        // INDEXED BY keeps the LIKE on the index (runs once per hinted field).
        const stmt = db.prepare(`SELECT name, signature FROM symbols INDEXED BY idx_type_name
       WHERE type = 'edt' AND name LIKE ? ORDER BY LENGTH(name) LIMIT 3`);
        for (const field of fieldsHint.slice(0, 10)) {
            if (RESERVED_SYSTEM_FIELD_NAMES.has(field.toLowerCase()))
                continue;
            const tokens = field.split(/(?=[A-Z])/).filter(t => t.length >= 3);
            const needle = tokens.length > 0 ? tokens[tokens.length - 1] : field;
            const rows = stmt.all(`%${needle}%`);
...
async function findReusableLabels(baseName, context) {
    try {
...
function minedPropertyDefaults(objectType, context) {
    if (objectType !== 'table')
        return '';
    try {
        const idx = context.symbolIndex;
        if (typeof idx.getPropertyPresenceRatio !== 'function')
            return '';
        const lines = [];
...
export async function prepareCreateTool(request, context) {
    ...
    lines.push('');
    lines.push('Next: generate the object, run `validate_code(mode="references")` + `validate_code(mode="syntax")` on the result, ' +
...
//# sourceMappingURL=prepareCreate.js.map