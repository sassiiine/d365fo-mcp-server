...
export async function getMacroInfoTool(request, context) {
    try {
        const { macroName, filter } = GetMacroInfoArgsSchema.parse(request.params.arguments);
        const db = context.symbolIndex.getReadDb();
        const symbol = db.prepare(`SELECT name, model, file_path FROM symbols WHERE name = ? AND type = 'macro' LIMIT 1`).get(macroName);
        if (!symbol) {
            return {
        ...
        let defines = db.prepare(`SELECT define_name, define_value FROM macro_defines WHERE macro_name = ? ORDER BY define_name`).all(macroName);
        if (filter) {
...
//# sourceMappingURL=macroInfo.js.map