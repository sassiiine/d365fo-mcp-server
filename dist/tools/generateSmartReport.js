...
- generate_object(mode="scaffold", objectType="report", name="CustOpenItems", fieldsHint="CustAccount, Amount, DueDate", callerTableName="CustTable", aotQuery="CustOpenTrans")`,
    inputSchema: {
        ...
        properties: {
            ...
            fieldsHint: {
                type: 'string',
                description: 'Comma-separated field names for the TmpTable (e.g. "ItemId, ItemName, Qty, Zone"). ' +
...
// Handler
// ─────────────────────────────────────────────────────────────────────────────
export async function handleGenerateSmartReport(args, symbolIndex) {
    const { name, caption, fieldsHint, fields: structuredFields, contractParams = [], generateController = true, designStyle = 'SimpleList', copyFrom, aotQuery, callerTableName, preProcess = false, controllerType = 'simple', additionalDatasets = [], modelName, projectPath, solutionPath, packagePath: argPackagePath, } = args;
    const log = (msg) => console.error(`[generateSmartReport] ${msg}`);
    ...
    const tmpTableLabel = (suffix) => reportCaption.startsWith('@') ? reportCaption : `${reportCaption}${suffix}`;
    ...
    if (copyFrom) {
        ...
        try {
            ...
            const dpSearch = db.prepare(`SELECT name FROM symbols WHERE type = 'class' AND name LIKE ? LIMIT 1`).get(`${copyFrom}%DP`);
            let srcTmpTable;
            if (dpSearch) {
                // Search for a TmpTable-like table referenced by the DP class
                const tmpSearch = db.prepare(`SELECT name FROM symbols WHERE type = 'table' AND name LIKE ? LIMIT 1`).get(`${copyFrom}%Tmp`);
                srcTmpTable = tmpSearch?.name;
            }
            if (!srcTmpTable) {
                // Try the direct convention: <ReportName>Tmp
                const directTmp = db.prepare(`SELECT name FROM symbols WHERE type = 'table' AND name = ? LIMIT 1`).get(`${copyFrom}Tmp`);
                srcTmpTable = directTmp?.name;
            }
            if (srcTmpTable) {
                const dbFields = db.prepare(`SELECT name, signature FROM symbols WHERE type = 'field' AND parent_name = ? ORDER BY name`).all(srcTmpTable);
    ...
    const resolvedExtraDatasets = additionalDatasets.map((ds) => {
        ...
        if (ds.fields && ds.fields.length > 0) {
            ...
            const hints = ds.fieldsHint.split(',').map((s) => s.trim()).filter(Boolean);
            dsFields = hints.map((h) => {
                const edt = suggestEdtFromFieldName(h);
    ...
    // ──────────────────────────────────────────────────────────────────────────
    const tableFields = reportFields.map(f => ({
        name: f.name,
        edt: f.edt,
        type: resolveFieldType(f.edt, rdb),
    }));
    ...
    const contractXml = XmlTemplateGenerator.generateAxClassXml(contractClassName, contractSourceCode);
    generatedObjects.push({
        objectType: 'class',
    ...
    if (aotQuery)
        dpAttrLines.push(`    SRSReportQueryAttribute(queryStr(${aotQuery}))`);
    ...
    const querySourceTable = aotQuery
        ...
        : undefined;
    if (querySourceTable)
        log(`Resolved aotQuery "${aotQuery}" first datasource: ${querySourceTable}`);
    let processReportBody;
    if (aotQuery) {
        ...
        const queryAssignments = reportFields.map(f => `            ${tmpTableVarName}.${f.name} = sourceRecord.${f.name}; // TODO: map from ${querySourceTable ?? 'query result'}`).join('\n');
        processReportBody = [
            contractFetch,
            ``,
        ...
        const fieldComments = reportFields.map(f => `        //       ${tmpTableVarName}.${f.name} = ''; // TODO: populate from data source`).join('\n');
        processReportBody = [
            contractFetch,
            ``,
    ...
    if (generateController) {
        ...
        const controllerXml = XmlTemplateGenerator.generateAxClassXml(controllerClassName, controllerSourceCode);
        generatedObjects.push({
            objectType: 'class',
        ...
        const menuItemXml = XmlTemplateGenerator.generateAxMenuItemXml('menu-item-output', finalName, {
    ...
    if (!resolvedModel) {
        return {
            content: [{
                    type: 'text',
                    text: `❌ Cannot write report files: model name could not be resolved.\n\n` +
    ...
    for (const obj of generatedObjects) {
        ...
        if (effectiveProjectPath) {
            try {
                ...
                const wasAdded = await projectManager.addToProject(effectiveProjectPath, obj.objectType, obj.objectName, normalizedPath);
 ...
 */
function injectRdlPageHeader(axReportXml, caption) {
    const pageHeaderXml = [
        `      <PageHeader>`,
 ...
 */
function injectGroupedTablix(axReportXml, fields, datasetName) {
    if (fields.length === 0)
        return axReportXml;
    // Classify fields
    ...
    const cols = fields.map(() => `            <TablixColumn><Width>${colW}in</Width></TablixColumn>`).join('\n');
    ...
    const tablixRe = new RegExp(`        <Tablix Name="Tablix_${datasetName}">[\\s\\S]*?        </Tablix>`, 'g');
 ...
 */
function suggestEdtFromFieldName(fieldName) {
    const n = fieldName.toLowerCase();
    if (n === 'recid')
        return 'RecId';
    if (n === 'accountnum' || n === 'accountnumber')
        return 'CustAccount';
    if (n.includes('custaccount') || n.includes('customeraccount'))
        return 'CustAccount';
    if (n.includes('vendaccount') || (n.includes('vendor') && n.includes('account')))
        return 'VendAccount';
    if (n === 'name' || n === 'itemname')
        return 'Name';
    if (n.includes('name'))
        return 'Name';
    ...
    if (n.includes('description'))
        return 'Description';
    if (n.includes('amount') || n.includes('balance'))
        return 'AmountMST';
    if (n.includes('quantity') || n.includes('qty'))
        return 'Qty';
    if (n.includes('price'))
        return 'PriceUnit';
    if (n === 'fromdate' || n === 'validfrom')
        return 'TransDate';
    if (n === 'todate' || n === 'validto')
        return 'TransDate';
    if (n.includes('date'))
        return 'TransDate';
    if (n.includes('itemid') || n === 'item')
        return 'ItemId';
    if (n.includes('custgroup'))
        return 'CustGroupId';
    if (n.includes('cust'))
        return 'CustAccount';
    if (n.includes('vend'))
        return 'VendAccount';
    if (n.includes('percent') || n.includes('pct'))
        return 'Percent';
    if (n.includes('zone'))
        return 'WHSZoneId';
    if (n.includes('warehouse') || n === 'whs')
 ...
 */
function resolveRdlDataType(edtName, db) {
    if (!edtName)
 ...
 */
function resolveEdtBaseType(edtName, db, depth = 0) {
 ...
 */
function resolveFieldType(edtName, db) {
    if (!edtName)
        return undefined;
 ...
 */
function contractParamToRdlType(xppType) {
    const t = xppType.toLowerCase();
    if (t === 'real' || t === 'amountmst' || t === 'qty' || t === 'percent')
        return 'System.Double';
    if (t === 'int' || t === 'integer')
        return 'System.Int32';
    if (t === 'int64' || t === 'recid' || t === 'refrecid')
        return 'System.Int64';
    if (t.includes('date'))
        return 'System.DateTime';
    return 'System.String';
}
//# sourceMappingURL=generateSmartReport.js.map