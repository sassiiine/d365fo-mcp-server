...
`;
     ...
     */
    static generateAxFormXml(formName, _properties) {
...
\t<SubRoles />
    ...
    static refContainer(container, childTag, names) {
        if (names.length === 0)
            return `\t<${container} />`;
...
export async function handleGenerateD365Xml(request) {
    try {
        ...
        if (args.objectType === 'report') {
            xmlContent = xmlContent.replace(/<Text><!\[CDATA\[([\s\S]*?)\]\]><\/Text>/g, (_m, inner) => `<Text>${inner.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')}</Text>`);
        }
