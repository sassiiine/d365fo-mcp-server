...
const GetFormPatternsArgsSchema = z.object({
    ...
    recommend: RecommendSchema.optional().describe('Pattern advisor mode: describe the requirements and get a recommended pattern + reference forms to clone. ' +
...
export function recommendPattern(input) {
    ...
    const pick = (id) => {
        ...
        if (!spec)
            throw new Error(`Catalog inconsistency: pattern "${id}" not found`);
        return spec;
    };
    const alt = (id, why) => {
        const spec = resolvePattern(id);
        if (spec)
            alternatives.push({ spec, why });
    };
    ...
    reasons.push(fieldCount !== undefined
...
export async function handleGetFormPatterns(args, symbolIndex) {
...
export async function getFormPatternsTool(request, context) {
    try {
...
async function renderRecommendation(symbolIndex, input) {
    ...
    if (enriched.tableName && enriched.fieldCount === undefined) {
      ...
      `).get(enriched.tableName);
            if (row && row.c > 0)
                enriched.fieldCount = row.c;
        }
      ...
      `).all(rec.spec.xmlName);
            if (mined.length > 0) {
                ...
                for (const m of mined) {
                    if (!referenceForms.includes(m.form_name))
                        referenceForms.push(m.form_name);
                }
            }
            else {
...
async function analyzeSimilarForm(symbolIndex, formName, limit) {
    const rdb = symbolIndex.getReadDb();
    // Get form info
...
async function analyzeFormsUsingTable(symbolIndex, tableName, limit) {
    const rdb = symbolIndex.getReadDb();
    // Find forms using this table
...
async function analyzeFormPattern(symbolIndex, formPattern, limit) {
    const rdb = symbolIndex.getReadDb();
...
//# sourceMappingURL=getFormPatterns.js.map