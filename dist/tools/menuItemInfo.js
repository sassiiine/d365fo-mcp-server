...
export async function menuItemInfoTool(request, context) {
    try {
        ...
        if (symbols.length === 0) {
            // Try FTS fallback
            const ftsResult = db.prepare(`SELECT name, type, model FROM symbols
         WHERE name LIKE ? AND type IN ('menu-item-display', 'menu-item-action', 'menu-item-output')
         LIMIT 10`).all(`%${args.name}%`);
       ...
       ORDER BY entry_point_name, privilege_name`).all(...symNames);
        const privilegesBySymbol = new Map();
        for (const pe of allPrivEntries) {
            if (!privilegesBySymbol.has(pe.entry_point_name))
                privilegesBySymbol.set(pe.entry_point_name, []);
            privilegesBySymbol.get(pe.entry_point_name).push(pe);
        }
        const allPrivNames = [...new Set(allPrivEntries.map(pe => pe.privilege_name))];
        const dutiesByPriv = new Map();
        const dutiesCountByPriv = new Map();
        const rolesByDutyName = new Map();
        const rolesCountByDuty = new Map();
        if (allPrivNames.length > 0) {
         ...
         WHERE privilege_name IN (${privPH}) ORDER BY duty_name`).all(...allPrivNames);
            for (const d of allDutyRows) {
                if (!dutiesByPriv.has(d.privilege_name))
                    dutiesByPriv.set(d.privilege_name, []);
           ...
           WHERE duty_name IN (${dutyPH}) ORDER BY role_name`).all(...allDutyNames);
                for (const r of allRoleRows) {
                    if (!rolesByDutyName.has(r.duty_name))
                        rolesByDutyName.set(r.duty_name, []);
        ...
        for (const symbol of symbols) {
            ...
            const target = db.prepare(`SELECT target_object, target_type, security_privilege, label FROM menu_item_targets
         WHERE menu_item_name = ? AND menu_item_type = ?`).get(symbol.name, symbol.type.replace('menu-item-', ''));
            if (target) {
                if (target.target_object) {
                    output += `Target: ${target.target_object}`;
                    if (target.target_type)
                        output += ` (${target.target_type})`;
                    output += '\n';
                }
            ...
            const matchingObject = db.prepare(`SELECT name, type, model FROM symbols WHERE name = ? AND type IN ('form', 'class', 'query', 'report') LIMIT 1`).get(symbol.name);
            if (matchingObject) {
...
//# sourceMappingURL=menuItemInfo.js.map