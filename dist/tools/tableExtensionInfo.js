...
export async function tableExtensionInfoTool(request, context) {
    try {
        ...
        const baseTable = db.prepare(`SELECT name, model, file_path FROM symbols WHERE name = ? AND type = 'table' LIMIT 1`).get(tableName);
        ...
        let extensionRows = [];
        try {
            extensionRows = db.prepare(`SELECT extension_name, model, added_fields, added_indexes, added_methods, coc_methods, event_subscriptions
         FROM extension_metadata
         WHERE base_object_name = ? AND extension_type = 'table-extension'
         ORDER BY model, extension_name`).all(tableName);
        }
        catch (e) {
            // extension_metadata table may not exist in older databases — non-fatal
            if (process.env.DEBUG_LOGGING === 'true')
                console.warn('[tableExtensionInfo] extension_metadata query failed:', e);
        }
        // Fallback: symbols with extends_class pointing to the table
        const symbolExts = db.prepare(`SELECT name, model FROM symbols
       WHERE type = 'table-extension' AND (extends_class = ? OR name LIKE ?)
       ORDER BY model, name`).all(tableName, `${tableName}.%`);
        const seen = new Set();
        const allExtensions = [];
        for (const row of extensionRows) {
            if (!seen.has(row.extension_name)) {
                ...
                let eventSubs = [];
                try {
                    addedFields = JSON.parse(row.added_fields || '[]');
                }
                catch { /**/ }
                try {
                    addedIndexes = JSON.parse(row.added_indexes || '[]');
                }
                catch { /**/ }
                try {
                    addedMethods = JSON.parse(row.added_methods || '[]');
                }
                catch { /**/ }
                try {
                    cocMethods = JSON.parse(row.coc_methods || '[]');
                }
                catch { /**/ }
                try {
                    eventSubs = JSON.parse(row.event_subscriptions || '[]');
                }
                catch { /**/ }
        ...
        if (args.includeEffectiveSchema) {
            output += '\nEffective Schema:\n';
            // Base fields
            const baseFields = db.prepare(`SELECT name FROM symbols WHERE parent_name = ? AND type = 'field' ORDER BY name`).all(tableName);
            ...
            if (totalExtFields > 0)
                output += ` (base: ${baseFields.length}, extensions: ${totalExtFields})`;
            output += '\n';
            // Base indexes
            const baseIndexes = db.prepare(`SELECT COUNT(*) as cnt FROM symbols WHERE parent_name = ? AND type = 'index'`).get(tableName);
            const baseIndexCount = baseIndexes?.cnt ?? 0;
            output += `  Total indexes: ${baseIndexCount + totalExtIndexes}`;
            if (totalExtIndexes > 0)
                output += ` (base: ${baseIndexCount}, extensions: ${totalExtIndexes})`;
            output += '\n';
            if (allExtensions.length > 0) {
...
function makeObjectExtensionTool(extensionType, objectLabel) {
    return async function objectExtensionInfoTool(request, context) {
        try {
            const raw = (request.params.arguments ?? {});
            // Accept baseName or tableName for compat; strip dot notation if present.
            const rawName = (raw.baseName ?? raw.tableName ?? raw.name ?? '');
            ...
            let metaRows = [];
            try {
                metaRows = db.prepare(`SELECT extension_name, model, added_fields, added_indexes, added_methods, coc_methods, event_subscriptions
           FROM extension_metadata
           WHERE base_object_name = ? AND extension_type = ?
           ORDER BY model, extension_name`).all(baseName, extensionType);
            }
            catch { /* older DB without extension_metadata — non-fatal */ }
            // ── symbols fallback ───────────────────────────────────────────────
            const symbolRows = db.prepare(`SELECT name, model FROM symbols
         WHERE type = ? AND (extends_class = ? OR name LIKE ?)
         ORDER BY model, name`).all(extensionType, baseName, `${baseName}.%`);
            const seen = new Set();
            const extensions = [];
            for (const row of metaRows) {
                if (!seen.has(row.extension_name)) {
                    seen.add(row.extension_name);
                    let addedFields = [], addedIndexes = [];
                    let addedMethods = [], cocMethods = [], eventSubs = [];
                    try {
                        addedFields = JSON.parse(row.added_fields || '[]');
                    }
                    catch { /**/ }
...
//# sourceMappingURL=tableExtensionInfo.js.map