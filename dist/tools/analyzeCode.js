...
export const ANALYZE_MODES = ['patterns', 'implementations', 'completeness', 'api-usage'];
const ANALYZE_DISPATCH = {
...
const AnalyzeCodeArgsSchema = z
    .object({
    mode: z.enum(ANALYZE_MODES).describe('patterns (scenario → common classes/methods), implementations (className+methodName → real examples), ' +
        'completeness (className → missing standard methods), api-usage (apiName → init/call patterns).'),
})
    .passthrough();
export async function analyzeCodeTool(request, context) {
    ...
    const dispatch = ANALYZE_DISPATCH[mode];
    if (!dispatch) {
        return {
