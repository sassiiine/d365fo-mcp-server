...
function getEdtHierarchy(db, edtName, modelName) {
    ...
    const fieldUsages = db.prepare(`SELECT parent_name, name, model FROM symbols WHERE type = 'field' AND signature LIKE ? ORDER BY model, parent_name LIMIT 50`).all(`%${edtName}%`);
