 ...
 */
import { z } from 'zod';
import { searchLabelsTool, REUSABLE_MARKER, NO_HITS_MARKER, NO_REUSE_ADVICE, SOME_REUSE_ADVICE, } from './analysis/searchLabels.js';
import { mapWithConcurrency } from '../utils/concurrency.js';
import { recordLabelSearchCall, repeatSearchNotice, searchBudgetNotice, } from './analysis/labelSearchHistory.js';
...
export const LABEL_ACTIONS = ['search', 'info', 'create', 'update', 'rename'];
export const LABEL_DISPATCH = {
...
const LabelsArgsSchema = z
    .object({
    action: z.enum(LABEL_ACTIONS).describe('Which label operation to run: ' +
...
export async function labelsTool(request, context) {
    ...
    const incoming = { ...(request.params.arguments ?? {}) };
    ...
    if (typeof rawAction === 'string') {
        if (LABEL_FILE_ACTIONS.has(rawAction)) {
            return {
                content: [{
                        type: 'text',
                        text: `❌ labels: "${rawArgs.action}" is not a labels action — d365fo_file has no "label-file" object type. ` +
        ...
        if (!LABEL_ACTIONS.includes(rawAction) && ACTION_ALIASES[rawAction]) {
    ...
    if (!parsed.success) {
        return {
            content: [{
                    type: 'text',
                    text: `❌ labels: invalid arguments — action must be one of: ${LABEL_ACTIONS.join(', ')} ` +
    ...
    const dispatch = LABEL_DISPATCH[action];
    if (!dispatch) {
        return {
 ...
 */
async function batchSearch(args, search, context) {
    const all = args.query.map(q => String(q).trim()).filter(q => q !== '');
    ...
    const runs = await mapWithConcurrency(queries, BATCH_CONCURRENCY, async (query) => {
        let result;
        try {
            result = await search({ method: 'tools/call', params: { name: 'search_labels', arguments: { ...args, query } } }, context);
        }
        catch (error) {
    ...
    if (missed.length > 0) {
        sections.push(`## No match — ${missed.length} phrasing(s)\n\n` +
