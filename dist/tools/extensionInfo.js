...
function withArgs(request, args) {
    return { ...request, params: { ...request.params, arguments: args } };
}
function err(text) {
    return { content: [{ type: 'text', text }], isError: true };
}
export async function extensionInfoTool(request, context) {
    ...
    switch (mode) {
        case 'coc':
            if (!target)
                return err('extension_info(mode="coc") requires `target` (the base class or table name).');
        ...
        case 'events':
            if (!target)
                return err('extension_info(mode="events") requires `target` (the class or table whose handlers to find).');
        ...
        case 'table-merge':
            if (!target)
                return err('extension_info(mode="table-merge") requires `target` (the base table name).');
        ...
        case 'points':
            if (!target)
                return err('extension_info(mode="points") requires `target` (the class, table, or form name).');
        ...
        case 'strategy':
            if (!a.goal)
                return err('extension_info(mode="strategy") requires `goal`.');
