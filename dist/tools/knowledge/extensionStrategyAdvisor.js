...
const ExtensionStrategyArgsSchema = z.object({
    goal: z.string().describe('What you want to achieve — e.g. "validate that SalesLine quantity is positive", ' +
        '"send order confirmation to external ERP", "add field to CustTable form"'),
    objectName: z.string().optional().describe('Target D365FO object if known — e.g. "SalesTable", "CustTable", "SalesFormLetter"'),
    scenario: z.enum(scenarioTypes).optional().describe('Scenario category (auto-detected from goal if omitted). One of: ' +
        scenarioTypes.join(', ')),
});
const STRATEGY_RULES = [
    // Data validation
    {
        ...
        reasoning: 'Table data events fire consistently for ALL data entry paths (forms, services, data entities, batch). ' +
    ...
    // Field defaulting / initialization
    {
        ...
        reasoning: 'initValue() is the standard entry point for field defaults on new records. ' +
    ...
    // Reacting to a user/field value change
    {
        ...
        reasoning: 'modifiedField() fires every time a field value changes on an EXISTING or new record — including ' +
    ...
    // Business logic modification
    {
        ...
        reasoning: 'CoC wraps the original method. You call next() to execute the original and can add pre/post logic. ' +
    ...
    // Outbound integration
    {
        ...
        reasoning: 'Business Events are the D365FO-native mechanism for outbound notifications. ' +
    ...
    // Inbound data / external data transfer
    {
        ...
        reasoning: 'Data entities are the standard D365FO interface for inbound data. ' +
    ...
    // UI modification
    {
        ...
        reasoning: 'Form extensions modify the UI declaratively (add controls, change properties) without touching the base form. ' +
    ...
    // Document output / printing
    {
        ...
        reasoning: 'ER is the strategic direction for business documents (invoices, statements). ' +
    ...
    // Number sequence
    {
        ...
        reasoning: 'D365FO number sequences are configured per legal entity/company. ' +
    ...
    // Security / access control
    {
        ...
        reasoning: 'D365FO security follows a strict hierarchy: Menu Item → Privilege → Duty → Role. ' +
    ...
    // Batch processing
    {
        ...
        reasoning: 'SysOperation separates contract (parameters), controller (scheduling), and service (execution). ' +
...
// Rule Matching
function findMatchingRules(goal, scenario) {
    const lower = goal.toLowerCase();
    ...
    const scored = candidates.map(rule => {
        ...
        if (scenario && rule.scenarios.includes(scenario))
            score += 3;
        return { rule, score };
    });
...
// Formatting
function formatRecommendation(goal, resolvedScenario, primary, alternatives, objectName) {
    let out = `# Extension Strategy Recommendation\n\n`;
    out += `**Goal:** ${goal}\n`;
    if (objectName)
        out += `**Target object:** ${objectName}\n`;
...
function formatNoMatch(goal, objectName) {
    let out = `# Extension Strategy Recommendation\n\n`;
    out += `**Goal:** ${goal}\n`;
    if (objectName)
        out += `**Target object:** ${objectName}\n\n`;
...
// Tool Handler
export async function extensionStrategyAdvisorTool(request, _context) {
    try {
        ...
        if (matchedRules.length === 0) {
            return {
                content: [{ type: 'text', text: formatNoMatch(goal, objectName) }],
                isError: false,
            };
        }
...
//# sourceMappingURL=extensionStrategyAdvisor.js.map