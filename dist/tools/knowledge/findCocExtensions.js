 ...
 */
const COC_BASE_TYPES = ['class', 'table', 'form', 'view', 'data-entity', 'map', 'query'];
...
export async function findCocExtensionsTool(request, context) {
    try {
        ...
        if (methodName)
            output += `Filtering by method: ${methodName}\n`;
        output += '\n';
        // 1. Query extension_metadata table for class-extension records
        let extensionRows = [];
        try {
            extensionRows = rdb.prepare(`SELECT extension_name, model, base_object_name, coc_methods, added_methods, event_subscriptions
         FROM extension_metadata
         WHERE base_object_name = ? AND extension_type = 'class-extension'
         ORDER BY model, extension_name`).all(className);
           ...
           ORDER BY model, extension_name`).all(args.className);
            }
        }
        catch {
        ...
        const symbolExtensions = rdb.prepare(`SELECT name, model, file_path FROM symbols
       WHERE type = 'class-extension' AND extends_class = ?
       ORDER BY model, name`).all(className);
        ...
        const filtered = methodName
            ? allExtensions.filter(e => e.cocMethods.some(m => m.toLowerCase() === methodName.toLowerCase()) ||
            ...
            if (process.platform === 'win32') {
                ...
                if (packagePath) {
                    ...
                    try {
                        ...
                        const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('filesystem scan timeout')), FS_SCAN_TIMEOUT_MS));
                        ...
                        const filtered2 = methodName
                            ? fsExts.filter(e => e.cocMethods.some(m => m.toLowerCase() === methodName.toLowerCase()) ||
                                e.addedMethods.some(m => m.toLowerCase() === methodName.toLowerCase()) ||
        ...
        if (args.includeEventHandlers) {
            ...
            let eventHandlerRows = [];
            try {
                eventHandlerRows = rdb.prepare(`SELECT extension_name, model, event_subscriptions FROM extension_metadata
           WHERE (event_subscriptions LIKE ? OR event_subscriptions LIKE ?)
           ORDER BY model, extension_name`).all(`%classStr(${className}%`, `%tableStr(${className}%`);
            }
            catch { /**/ }
            // FTS5 search for SubscribesTo patterns (replaces LIKE full-table scan)
            let ftsHandlers = [];
            try {
                ftsHandlers = rdb.prepare(`SELECT s.name, s.parent_name, s.model, s.source_snippet
           ...
           LIMIT 20`).all(`%classStr(${className}%`, `%tableStr(${className}%`);
            }
            catch {
                // FTS5 fallback — use LIKE on source_snippet
                ftsHandlers = rdb.prepare(`SELECT s.name, s.parent_name, s.model, s.source_snippet FROM symbols s
           WHERE s.type = 'method'
           ...
           LIMIT 20`).all(`%classStr(${className}%`, `%tableStr(${className}%`);
            ...
            for (const row of eventHandlerRows) {
                let subs = [];
                try {
                    subs = JSON.parse(row.event_subscriptions || '[]');
                }
                catch { /**/ }
            ...
            if (allHandlers.length > 0) {
                ...
                for (const h of allHandlers) {
                    ...
                    if (h.event)
                        output += ` — event: ${className}.${h.event}`;
                    output += '\n';
                }
            }
            else {
...
//# sourceMappingURL=findCocExtensions.js.map