...
const D365foErrorHelpArgsSchema = z.object({
    errorText: z.string().describe('Full error or warning message text — paste from Error List, Output window, or infolog. ' +
        'Examples: "CSUV1 The field … cannot be assigned", "TTS level is not 0", ' +
        '"BPUpgradeCodeToday", "SYS10028 you must call next …"'),
    errorCode: z.string().optional().describe('Optional: error code prefix, e.g. "CSUV1", "SYS10028", "BPError", "BP". ' +
        'Improves matching accuracy.'),
    context: z.string().optional().describe('Optional: the X++ code snippet where the error occurs — helps produce a targeted suggestion.'),
});
const ERROR_DB = [
    // TTS / Transaction errors
    {
        ...
        explanation: 'The transaction scope (ttsbegin/ttscommit) is unbalanced. ' +
...
catch (Exception::UpdateConflict)
{
    ...
    },
    {
        ...
        explanation: 'Two transactions tried to update the same record simultaneously. ' +
...
while (!done && retryCount < 5)
{
    ...
    // Compilation errors
    {
        ...
        explanation: 'Assigning a value of an incompatible type to a variable. ' +
    ...
    },
    {
        ...
        explanation: 'A Chain of Command extension method is missing the required "next methodName()" call. ' +
...
final class CustTable_MyModel_Extension
{
    ...
    },
    {
        ...
        explanation: 'D365FO cannot parse the XML file for a class, table, form, or other AOT object. ' +
    ...
    },
    {
        ...
        explanation: 'D365FO completely blocks overlayering (modifying Microsoft or ISV source code directly). ' +
    ...
    },
    {
        ...
        explanation: 'The today() function is deprecated in D365FO and triggers a BP (Best Practices) error. ' +
...
TransDate systemDate = DateTimeUtil::getSystemDate(
    ...
    },
    {
        ...
        explanation: 'A while-select loop is nested inside another while-select loop, causing N×M database round-trips. ' +
...
while select custTable
    ...
    },
    {
        ...
        explanation: 'Calling .update() on a record that was not selected with the forupdate keyword. ' +
...
ttscommit;`,
    ...
    },
    {
        ...
        explanation: 'Referencing a field that does not exist on the table. ' +
    ...
    },
    {
        ...
        explanation: 'The compiler resolved the receiver (a table buffer, a class instance) but found no such method on it. ' +
...
final class MyTable_MyModel_Extension
{
    ...
    },
    {
        ...
        explanation: 'A form extension declares a control whose name already exists on the base form. The usual cause is ' +
...
d365fo_file(action="modify", objectType="form-extension", operation="add-control",
    ...
    },
    {
        ...
        explanation: 'A label reference (e.g. "@MyModel:LabelId") points to a label ID that does not exist in the label file. ' +
    ...
    },
    {
        ...
        explanation: 'A .NET (CLR) method threw an exception. In X++, this surfaces as Exception::CLRError. ' +
...
catch (Exception::CLRError)
{
    ...
    },
    {
        ...
        explanation: 'A select statement returned no record (the buffer is empty). ' +
    ...
    },
    {
        ...
        explanation: 'The code tries to allocate a number from a number sequence that is not registered or not set up for the current company.',
        fix: [
...
function scoreError(entry, errorText, errorCode) {
    const lowerText = errorText.toLowerCase();
    const lowerCode = (errorCode ?? '').toLowerCase();
    let score = 0;
    for (const pattern of entry.patterns) {
        if (lowerCode && lowerCode.includes(pattern))
            score += 20;
 ...
 */
export function lookupErrorFix(errorText) {
...
export function d365foErrorHelpTool(request) {
    try {
        ...
        if (scored.length === 0 || scored[0].score < MIN_CONFIDENT_SCORE) {
            ...
            return {
                content: [{
                        type: 'text',
                        text: `❌ No matching error pattern found for:\n\n> ${errorText}\n\n` +
...
//# sourceMappingURL=d365foErrorHelp.js.map