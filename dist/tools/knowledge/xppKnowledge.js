                ...
                code: `public boolean validateWrite()
{
    ...
    // an agent could only discover them by shipping an enum that xppc rejects.
    {
        ...
        summary: 'Base enums and extensible enums have incompatible XML shapes. IsExtensible=true REQUIRES UseEnumValue=No and forbids explicit <Value> elements — xppc rejects any other combination. ' +
...
// if (any2Int(category) == 3) { … }`,
    ...
    // These functions disagree about their arity, so the arity IS the topic.
    {
        ...
        summary: 'Converting an enum to text has two answers and they are not interchangeable: enum2Str returns the value\'s translated LABEL, enum2Symbol returns its untranslated AOT NAME. ' +
...
public void post()
{
    ...
    // ── Class Inheritance ───────────────────────────────────────────────────
    {
        ...
        summary: 'X++ has single class inheritance and every non-final method is virtual. The trap in D365FO is that ' +
...
select firstonly lookup
    ...
    // ── X++ collections & containers ────────────────────────────────────────
    {
        ...
        summary: 'X++ has two families of in-memory collections: the kernel collection classes (List, Map, Set, Struct, Array), ' +
...
while (mapEnumerator.moveNext())
{
    ...
    // ── Date/time & time zones ──────────────────────────────────────────────
    {
        ...
        summary: 'D365FO stores every utcdatetime in UTC and converts to a time zone only for display or user input. ' +
...
if (deserialized == effective.ValidFrom)
{
    ...
    // ── .NET interop ────────────────────────────────────────────────────────
    {
        ...
        summary: 'X++ can call .NET types directly. The three things that go wrong are: the call runs on the wrong tier, ' +
...
public static server str buildCsvLine(container _values)
{
    ...
    // ── Reflection / Dict* metadata API ─────────────────────────────────────
    {
        ...
        summary: 'The Dict* kernel classes expose the AOT at runtime: enumerate a table\'s fields, resolve labels, ' +
...
public static void runIfPresent(ClassId _classId, str _methodName)
{
    ...
    // ── Tiles & KPIs ────────────────────────────────────────────────────────
    {
        ...
        summary: 'A Tile is an AOT element that renders a count (or a static link) on a workspace, driven by an AOT query ' +
...
<AxKPI xmlns:i="http://www.w3.org/2001/XMLSchema-instance">
    ...
    // ── Macros ──────────────────────────────────────────────────────────────
    {
        ...
        summary: 'Macros are a text preprocessor inherited from AX. A macro library is an AOT element (AxMacroDictionary) ' +
...
public class MyPostingLimits
{
    ...
    // ── Aggregate measurements ──────────────────────────────────────────────
    {
        ...
        summary: 'An aggregate measurement is the star schema D365FO ships to analytics: measure groups (facts, each bound ' +
...
<AxAggregateMeasurement xmlns:i="http://www.w3.org/2001/XMLSchema-instance" xmlns="Microsoft.Dynamics.AX.Metadata.V2">
    ...
    // ── License codes ───────────────────────────────────────────────────────
    {
        ...
        summary: 'A license code is the ISV licensing anchor: configuration keys point at it, and the code is enabled by a ' +
 ...
 */
function distinctiveTokens(topic) {
    const out = [];
    const seen = new Set();
    for (const raw of topic.split(/[\s,;/()[\]]+/)) {
        ...
        if (tok.length < 4)
            continue;
        const identifierLike = /[A-Za-z][0-9]|[0-9][A-Za-z]/.test(tok) || /[a-z][A-Z]/.test(tok) || tok.includes('_');
        if (!identifierLike)
            continue;
 ...
 * and a wrong answer.
 */
function isDocumentedByName(token) {
    const t = token.toLowerCase();
    return KNOWLEDGE_BASE.some(entry => entry.id === t ||
...
export function unknownDistinctiveTokens(topic) {
 ...
 */
function unknownTokenNotice(topic) {
    ...
    return (`⚠️ ${named}${rest} ${isPlural ? 'are' : 'is'} not documented by name in this knowledge base. ` +
...
export async function xppKnowledgeTool(request) {
    try {
        ...
        if (isListAll) {
            ...
            if (entries.length > 0 && unknownNotice)
                notices.push(unknownNotice);
            ...
            if (entries.length > 0 && topScore < CONFIDENT_SCORE) {
                notices.push(`⚠️ No strong match for "${args.topic}" — showing the closest entries below, which may be ` +
