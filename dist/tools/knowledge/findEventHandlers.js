 ...
 */
function delegateSearch(rdb, targetName, pattern) {
    ...
    if (safe) {
        try {
            return rdb.prepare(`${SELECT}
           AND id IN (SELECT rowid FROM symbols_fts WHERE symbols_fts MATCH ?)
           AND source_snippet LIKE ?${TAIL}`).all(`{source_snippet} : "${safe}"`, pattern);
        }
        catch {
    ...
    try {
        return rdb.prepare(`${SELECT} AND source_snippet LIKE ?${TAIL}`).all(pattern);
    }
    catch {
        return [];
    }
