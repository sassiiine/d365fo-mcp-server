 ...
 */
import { z } from 'zod';
import { validateMoniker, searchMonikers, buildSuppressionXml, BP_MONIKER_CATALOG, } from '../../knowledge/bpMonikers/index.js';
const ELEMENT_TYPES = [
...
const BpMonikerArgsSchema = z.object({
    action: z.enum(['validate', 'search', 'suppress']).describe('validate = confirm an exact moniker is real; search = free-text query for a scenario with no moniker yet; ' +
...
export async function bpMonikerHelpTool(request) {
    ...
    if (args.action === 'validate') {
        ...
        lines.push(e.message ? `Message template: ${e.message}` : 'Message template: (not found in a resource class)', e.description ? `Description: ${e.description}` : 'Description: (not found in a resource class)');
    ...
    if (args.action === 'search') {
        ...
        for (const r of results) {
            lines.push(`• ${r.entry.moniker}${r.entry.canonical ? '' : ' (not in any AxRuleSet — less certain)'}  [matched: ${r.matchedIn.join(', ')}]`);
            if (r.entry.description)
                lines.push(`    ${r.entry.description}`);
            else if (r.entry.message)
                lines.push(`    ${r.entry.message}`);
        }
        return { content: [{ type: 'text', text: lines.join('\n') }] };
    }
    // suppress
...
//# sourceMappingURL=bpMonikerHelp.js.map