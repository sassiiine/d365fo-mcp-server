...
export async function getMethodSignatureTool(request, context) {
    try {
        ...
        const classRow = loadOwnerRow(rdb, className, modelName);
        if (!classRow) {
    ...
    `);
        ...
        const bridgeSignature = await tryBridgeMethodSignature(context.bridge, className, methodName, classRow.model, includeCoc);
        if (bridgeSignature)
            return bridgeSignature;
        // Fallback: parse XML file from disk (same pattern as classInfo.ts)
        const xmlSignature = await tryXmlMethodSignature(parser, classRow.file_path, className, methodName, classRow.model, includeCoc, classRow.type);
        if (xmlSignature)
            return xmlSignature;
        ...
        if (!methodRow) {
            ...
            for (const ancestor of inheritedOwnerCandidates(rdb, className, methodName, bridgeUsable)) {
                const row = loadOwnerRow(rdb, ancestor);
                const model = row?.model ?? classRow.model;
                const inheritedBridge = await tryBridgeMethodSignature(context.bridge, ancestor, methodName, model, includeCoc, className);
                if (inheritedBridge)
                    return inheritedBridge;
                const inheritedXml = await tryXmlMethodSignature(parser, row?.file_path, ancestor, methodName, model, includeCoc, row?.type, className);
                if (inheritedXml)
                    return inheritedXml;
            }
        }
        ...
        if (methodRow?.signature) {
            const sigText = methodRow.signature;
            let output = `# Method: \`${className}.${methodRow.name ?? methodName}\`\n`;
        ...
        if (!methodRow && hasTableDataMethods(classRow.type)) {
            ...
            if (inherited) {
                return {
                    content: [{
                            type: 'text',
                            text: `# Method: \`${className}.${inherited.name}\`\n` +
 ...
 */
function loadOwnerRow(rdb, name, modelName) {
    try {
        if (modelName) {
            return rdb.prepare(`
 ...
 */
async function tryBridgeMethodSignature(bridge, className, methodName, modelName, includeCoc, inheritedBy) {
 ...
 */
async function tryXmlMethodSignature(parser, filePath, className, methodName, modelName, includeCoc, objectType, inheritedBy) {
    if (!parser || !filePath)
        return null;
    try {
        ...
        if (!parseResult.success || !parseResult.data)
            return null;
        const method = parseResult.data.methods.find((m) => m.name.toLowerCase() === methodName.toLowerCase());
        if (!method?.source)
            return null;
 ...
 */
function parseByObjectType(parser, filePath, modelName, objectType) {
    switch (objectType) {
 ...
 */
export function parseMethodSignature(source, methodName) {
 ...
 */
function buildSignatureString(modifiers, returnType, methodName, parameters) {
    let sig = '';
 ...
 */
function buildCoCTemplate(modifiers, returnType, methodName, parameters) {
    let template = '';
 ...
 */
function formatOutput(className, signature, modelName, includeCocTemplate = false, obsoleteWarning = '', inheritedBy) {
