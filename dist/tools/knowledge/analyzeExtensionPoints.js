 ...
 */
async function readVisibility(bridge, classNames) {
    ...
    for (const name of classNames) {
        ...
        for (const m of info?.members ?? []) {
            ...
            if (!key || byMethod.has(key))
                continue;
            const visibility = parseVisibility(m.signature);
            if (visibility)
                byMethod.set(key, visibility);
        }
    }
    return byMethod;
}
...
function isPrivateMethod(name, signature, sourceSnippet, visibility) {
 ...
 */
function isPrivate(signature, sourceSnippet, methodName) {
    if (!sourceSnippet)
        return false;
    ...
    for (const rawLine of sourceSnippet.split('\n')) {
        ...
        if (at === -1)
            continue;
        if (!/^\s*\(/.test(line.slice(at + needle.length)))
            continue; // not the declaration
...
export async function analyzeExtensionPointsTool(request, context) {
    try {
        ...
        const objName = canonicalSymbolName(rdb, args.objectName, args.objectType === 'auto' ? ['class', 'table', 'form'] : [args.objectType]) ?? args.objectName;
        // Resolve object type
        let resolvedType = args.objectType;
        if (resolvedType === 'auto') {
            const sym = rdb.prepare(`SELECT type FROM symbols WHERE name = ? AND type IN ('class','table','form')
         ORDER BY CASE type WHEN 'class' THEN 0 WHEN 'table' THEN 1 WHEN 'form' THEN 2 END LIMIT 1`).get(objName);
            if (sym)
                resolvedType = sym.type;
        }
        const baseSymbol = rdb.prepare(`SELECT name, type, model, file_path FROM symbols WHERE name = ? AND type = ? LIMIT 1`).get(objName, resolvedType === 'auto' ? 'class' : resolvedType);
        let output = `Extension Points for: ${objName}`;
        if (resolvedType !== 'auto')
            output += ` (${resolvedType})`;
        if (baseSymbol)
            output += ` — ${baseSymbol.model}`;
        output += '\n\n';
        ...
        if (args.showExistingExtensions) {
            try {
                ...
                existingExtensions = rdb.prepare(`SELECT extension_name, model, coc_methods, event_subscriptions, added_methods
           FROM extension_metadata
           WHERE base_object_name = ? AND extension_type LIKE ?
           ORDER BY model, extension_name`).all(objName, extType);
            }
            catch (e) {
                // extension_metadata table may not exist in older databases — non-fatal
                if (process.env.DEBUG_LOGGING === 'true')
                    console.warn('[analyzeExtensionPoints] extension_metadata query failed:', e);
            }
            ...
            if (existingExtensions.length === 0 && context.bridge?.isReady && context.bridge.xrefAvailable) {
                try {
                    ...
                    if (bridgeExts && bridgeExts.count > 0) {
                        existingExtensions = bridgeExts.extensions.map((ext) => ({
        ...
        const extensionsFromFs = existingExtensions.some((e) => e._fromFs);
        const extensionsFromBridge = existingExtensions.some((e) => e._fromBridge);
        // Build maps of already-extended methods and subscribed events
        const alreadyWrapped = new Map(); // methodName → [extName, ...]
        const alreadySubscribed = new Map(); // eventName → [handlerClass, ...]
        for (const ext of existingExtensions) {
            let cocMethods = [];
            let eventSubs = [];
            try {
                cocMethods = JSON.parse(ext.coc_methods || '[]');
            }
            catch { /**/ }
            try {
                eventSubs = JSON.parse(ext.event_subscriptions || '[]');
            }
            catch { /**/ }
            for (const m of cocMethods) {
                const key = m.toLowerCase();
                if (!alreadyWrapped.has(key))
                    alreadyWrapped.set(key, []);
            ...
            for (const ev of eventSubs) {
                ...
                if (delegateMatch) {
                    ...
                    if (!alreadySubscribed.has(evName))
                        alreadySubscribed.set(evName, []);
        ...
        try {
            const subHandlers = rdb.prepare(`SELECT s.name, s.parent_name, s.source_snippet FROM symbols_fts fts
         JOIN symbols s ON s.id = fts.rowid
         WHERE symbols_fts MATCH 'source_snippet:SubscribesTo' AND s.type = 'method'
         AND s.source_snippet LIKE ?
         LIMIT 30`).all(`%${objName}%`);
            ...
            if (process.env.DEBUG_LOGGING === 'true')
                console.warn('[analyzeExtensionPoints] symbols SubscribesTo scan failed:', e);
        }
        // For classes — analyze methods
        if (resolvedType === 'class' || resolvedType === 'auto') {
            const methodStmt = rdb.prepare(`SELECT name, signature, source_snippet FROM symbols
         WHERE parent_name = ? AND type = 'method'
         ORDER BY name`);
            const methods = methodStmt.all(objName);
            ...
            for (const ancestor of ancestorNames) {
                for (const m of methodStmt.all(ancestor)) {
        ...
        if (resolvedType === 'table' || resolvedType === 'auto') {
            ...
            const eventHandlerCounts = new Map();
            try {
                const allEvHandlers = rdb.prepare(`SELECT s.source_snippet FROM symbols_fts fts
           JOIN symbols s ON s.id = fts.rowid
           WHERE symbols_fts MATCH 'source_snippet:SubscribesTo'
           AND s.type = 'method'
           AND s.source_snippet LIKE ?
           LIMIT 200`).all(`%${objName}%`);
            ...
            const tableMethods = rdb.prepare(`SELECT name, source_snippet FROM symbols
         WHERE parent_name = ? AND type = 'method'
         AND (source_snippet LIKE '%delegate %' OR signature LIKE '%delegate %')
         LIMIT 10`).all(objName);
        ...
        if (resolvedType === 'form') {
            const formDataSources = rdb.prepare(`SELECT name FROM symbols WHERE parent_name = ? AND type = 'datasource' ORDER BY name`).all(objName);
            ...
            const formMethods = rdb.prepare(`SELECT name FROM symbols WHERE parent_name = ? AND type = 'method' ORDER BY name LIMIT 20`).all(objName);
            if (formMethods.length > 0) {
                output += `Form methods (${formMethods.length} shown, CoC-eligible via form extension):\n`;
                output += `  ${formMethods.slice(0, 10).map((m) => m.name + '()').join(', ')}`;
                if (formMethods.length > 10)
                    output += ` (+${formMethods.length - 10} more)`;
                output += '\n';
            }
        }
...
//# sourceMappingURL=analyzeExtensionPoints.js.map