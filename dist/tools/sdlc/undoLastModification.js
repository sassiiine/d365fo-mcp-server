...
import { lookupCreatedArtifact, forgetCreatedArtifact } from '../../workspace/createdArtifactLedger.js';
 ...
 */
export function isInsideRepo(repoRoot, targetPath) {
 ...
 * Exported for unit tests.
 */
export function toRepoRelative(repoRoot, absolutePath) {
...
export const undoLastModificationTool = async (params, context) => {
 ...
 */
async function undoViaLedger(absolutePath, context) {
 ...
 */
async function removeFromProjectAndForget(absolutePath) {
    const entry = lookupCreatedArtifact(absolutePath);
    if (!entry)
        return '';
 ...
 */
async function removeFromProjectSafe(entry) {
    if (!entry.projectPath || !entry.objectType || !entry.objectName)
        return;
    try {
 ...
 */
async function cleanupIndexAfterUndo(context, filePath, action) {
...
//# sourceMappingURL=undoLastModification.js.map