 ...
 */
const SYNCABLE_AOT_FOLDERS = new Map([
    ['AxTable', 'table'],
 ...
 */
function runWithStreaming(exe, args, opts) {
    return new Promise((resolve, reject) => {
        ...
        let stderr = '';
        let killed = false;
        const appendBounded = (which, text) => {
        ...
        child.on('close', (code) => {
            if (timer)
                clearTimeout(timer);
            if (killed) {
                reject(Object.assign(new Error(`SyncEngine timed out after ${opts.timeout / 60000} minutes`), { stdout, stderr }));
            }
            else {
 ...
 */
export async function extractTablesFromProject(projectPath) {
 ...
 */
export function classifySyncTargets(names, db) {
    const targets = [];
    const unresolved = [];
 ...
 */
async function checkStaticMetadata(packagesRoot, modelName) {
 ...
 */
export function classifySyncOutcome(rawOutput) {
    ...
    const hardFailure = rawOutput.match(/^.*\b(sync failed|synchronization failed|aborting sync|unhandled exception|fatal error)\b.*$/mi)?.[0]?.trim();
    if (hardFailure) {
...
export const dbSyncTool = async (params, context) => {
    ...
    try {
        ...
        const { stdout, stderr } = await withOperationLock('dbsync', // single lock key — SyncEngine can't run in parallel on same DB
