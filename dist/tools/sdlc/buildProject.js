...
async function buildLog(level, message) {
...
function assertSafePath(value, label) {
    if (/[&|<>^`!;$%"'\n\r]/.test(value)) {
        throw new Error(`${label} contains potentially dangerous characters and cannot be used in a build command: ${value}`);
    }
}
...
const XPPC_DIAG_DYNAMICS_RE = new RegExp(String.raw `^${DIAG_PREFIX}:\s*(?:(.*?)\s+)?dynamics:\/\/([^/\s:]+)\/([^/\s:]+)(?:\/([^\s:]+))?\s*:?\s*\[\((\d+),(\d+)\)(?:,\(\d+,\d+\))?\]\s*:\s*(.*)$`);
/** `<Kind> <Severity>: AxFormExtension/Name/Design/Controls/…: message` — no line/col. */
const XPPC_DIAG_PATH_RE = new RegExp(String.raw `^${DIAG_PREFIX}:\s*(Ax[A-Za-z]+)\/([^\s:]+)\s*:\s*(.*)$`);
 ...
 */
export function renderUnexplainedFailure(parsed, logContent) {
    const parsedErrors = parsed.filter(d => d.severity === 'error').length;
    const reported = xppcReportedErrorCount(logContent);
    if (parsedErrors > 0 && (reported === null || parsedErrors >= reported))
        return '';
    const lines = [];
    if (parsedErrors === 0) {
        lines.push(`⚠️ The build FAILED but this server parsed **no error diagnostic** from the log` +
            (reported !== null ? ` (xppc's own tally says: Errors: ${reported})` : '') + '.');
        lines.push(`Any warnings listed below are NOT the failure — do not treat them as the cause. ` +
            `Read the raw log at the end of this response; the failing line is in there in a ` +
            `format this parser did not recognise.`);
    }
    else {
        lines.push(`⚠️ xppc counted ${reported} error(s) but only ${parsedErrors} could be parsed into the list below. ` +
            `The rest are in the raw log.`);
    }
    lines.push(`⛔ Do NOT delete, undo or unregister an object to make the build pass. A green build ` +
 ...
 */
export function formatStructuredDiagnostics(diagnostics, maxItems = 25) {
...
function stateFilePath(targetModel, customPackagesPath) {
    const hash = crypto
        .createHash('md5')
...
function logFilePath(targetModel, queueIndex, customPackagesPath) {
    const hash = crypto
        .createHash('md5')
 ...
 */
export async function hasSourceChangesSince(modelDir, since, budgetMs = 3000) {
    ...
    while (stack.length > 0) {
        ...
        try {
            ...
            if ((await stat(dir)).mtimeMs > since)
                return true;
        ...
        for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
                if (dir === modelDir && BUILD_OUTPUT_DIRS.has(entry.name.toLowerCase()))
                    continue;
                stack.push(full);
                continue;
            }
            try {
                if ((await stat(full)).mtimeMs > since)
                    return true;
            }
 ...
 */
export async function finishedResultStillDescribesDisk(state, targetModel, customPackagesPath) {
    if (!state.endTime)
        return false; // no idea when it finished — do not trust it
...
async function readBuildState(targetModel, customPackagesPath) {
    try {
...
async function writeBuildState(state, customPackagesPath) {
    await writeFile(stateFilePath(state.targetModel, customPackagesPath), JSON.stringify(state, null, 2), 'utf-8');
}
async function clearBuildState(targetModel, customPackagesPath) {
...
async function readLogTail(logFile, lines = 60) {
    try {
...
async function readWholeLog(logFile) {
    try {
 ...
 */
async function readFullLog(logFile, maxLines = 300) {
    ...
    try {
        ...
        for (let i = 0; i < all.length; i++) {
            if (DIAG_LINE_TEST.test(all[i].trim()))
                diagIndices.push(i);
        }
...
async function getModelFromRnrproj(projectPath) {
    try {
...
async function findXppcExe(microsoftPackagesPath) {
 ...
 * silently skipped.
 */
async function resolveBuildQueue(targetModel, customPackagesPath, _microsoftPackagesPath) {
    const visited = new Set();
    const order = [];
    async function visit(modelName) {
        if (visited.has(modelName.toLowerCase()))
            return;
...
async function killOrphanedBuildProcesses() {
 ...
 */
export function describeLabelCompilation(modelName, result) {
    if (result.success) {
 ...
 */
async function spawnXppcForState(ctx, state) {
    ...
    const seenRefFolders = new Set();
    const referenceFolderArgs = [];
    ...
    child.on('error', async (err) => {
        closeSyncFs(logFd);
        const failed = { ...liveState, status: 'failed', exitCode: -1, endTime: new Date().toISOString() };
    ...
    child.on('close', async (code) => {
        ...
        const allResults = [...(liveState.queueResults ?? []), newResult];
        if (!succeeded) {
        ...
        const metaResult = await generateRuntimeMetadata(microsoftPackagesPath, customPackagesPath, liveState.targetModel);
...
// when appropriate.
// ---------------------------------------------------------------------------
async function renderFinishedBuildResult(finalState, targetModel, 
 ...
 */
function incrementalScopeCaveat(succeeded, fullBuild) {
    if (!succeeded || fullBuild)
        return '';
...
async function waitForBuildCompletion(targetModel, customPackagesPath, timeoutMs, onProgress, startedAt = Date.now()) {
    ...
    while (Date.now() < deadline) {
        ...
        if (state) {
            ...
            if (settled) {
                ...
                if (pidDeadSince === null)
                    pidDeadSince = Date.now();
                const grace = finalizing ? FINALIZING_GRACE_MS : ORPHAN_GRACE_MS;
                if (Date.now() - pidDeadSince > grace)
                    return { outcome: 'orphaned', state };
            }
...
function describeBuildProgress(state, startedAt) {
 ...
 */
function renderWaitTimeoutGuidance(elapsedSec, timeoutMs) {
...
// Tool handler
// ---------------------------------------------------------------------------
export const buildProjectTool = async (params, context, onProgress) => {
    const dataDir = context?.symbolIndex?.dataDir;
    try {
        ...
        // ------------------------------------------------------------------
        let customPackagesPath = null;
        let microsoftPackagesPath = null;
        let extraReferenceFolders = [];
        ...
        // Resolve model name
        // ------------------------------------------------------------------
        let modelName = params.modelName || configManager.getModelName();
            ...
            if (fullBuildNeedsFreshRun) {
                ...
                if (existingState.status === 'running' && alive) {
                    ...
                    if (waitForFinish) {
                        ...
                        const wait = await waitForBuildCompletion(targetModel, customPackagesPath, timeoutMs, onProgress, startedAt);
                        ...
                        return {
                            content: [{
                                    type: 'text',
                                    text: `⏳ ${queueProgress} (PID: ${existingState.pid}, running ${elapsed}s; wait timeout reached)${completedLine}\n\n` +
                ...
                // this call's success.
                const stillCurrent = await finishedResultStillDescribesDisk(existingState, targetModel, customPackagesPath);
                await clearBuildState(targetModel, customPackagesPath);
                if (stillCurrent) {
                    ...
                    const collected = `ℹ️  Collected the result of the build that ended ${existingState.endTime} ` +
                ...
                await buildLog('WARN', `discarding finished build state for ${targetModel}: sources changed after ${existingState.endTime}`);
