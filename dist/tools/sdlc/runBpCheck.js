...
import { lookupSymbolsNocase, lookupSymbolNocase } from '../../utils/symbolLookup.js';
import { compileModelLabels } from '../write/compileLabels.js';
import { buildFreshness } from '../../utils/buildMarker.js';
 ...
 */
const INDEX_TYPE_TO_ELEMENT_TYPE = {
    class: 'class',
 ...
 * caller to ignore it.
 */
function passVerdict(status) {
    if (status === 'never')
        return '⚠️ BP clean, NOT compiled';
    if (status === 'stale')
        return '⚠️ BP clean, build is STALE';
 ...
 */
export function normalizeElementType(raw) {
 ...
 * caller will act on.
 */
export function describeNonRun(output) {
 ...
 */
async function tryXppbp(xppbpPath, args) {
 ...
 */
export function extractReportedElements(output) {
    const names = new Set();
    for (const m of output.matchAll(/[\\/]Ax[A-Za-z]+[\\/]([A-Za-z0-9_]+)\.xml/g))
        names.add(m[1]);
 ...
 */
export function parseBpFindings(output) {
    const findings = [];
 ...
 */
export function renderFindingsSection(output) {
    const findings = parseBpFindings(output);
    if (findings.length === 0)
        return '';
    const lines = findings.map(f => {
        if (f.moniker === null)
            return `  • ${f.target} (rule not named on this line)`;
 ...
 */
export function describeScope(targetFilter, output) {
 ...
 */
export function normalizeTargets(params) {
 ...
 */
export function resolveElementType(name, db) {
    if (!db) {
        return {
            error: `Cannot determine the element type of "${name}" — the symbol index is not available here.\n` +
    ...
    if (elementTypes.length === 1)
        return { elementType: elementTypes[0] };
    if (elementTypes.length > 1) {
        return {
            error: `"${name}" is ambiguous — the index has it as ${elementTypes.join(' and ')}.\n` +
                `Say which one, e.g. { objects: [{ objectType: "${elementTypes[0]}", objectName: "${name}" }] }.`,
        };
    }
    return {
        error: `Cannot determine the element type of "${name}" — no object of a checkable type is indexed under that name.\n` +
            `Pass the type explicitly, e.g. { objects: [{ objectType: "table", objectName: "${name}" }] }, ` +
 ...
 */
export function splitSharedPreamble(outputs) {
    const lineSets = outputs.map(o => o.split(/\r?\n/));
    if (lineSets.length < 2)
        return { preamble: [], bodies: lineSets };
    const shape = (line) => line.replace(/\d+/g, '#').trimEnd();
    const preamble = [];
    let i = 0;
    scan: while (true) {
        const first = lineSets[0][i];
        if (first === undefined || FINDING_LINE_PATTERN.test(first))
            break;
        const key = shape(first);
        for (const lines of lineSets) {
            if (lines[i] === undefined || shape(lines[i]) !== key)
                break scan;
        }
        preamble.push(first);
        i++;
...
export const runBpCheckTool = async (params, context) => {
    try {
        ...
        for (const t of requested) {
            ...
            if (elementType)
                targets.push({ name: t.name, elementType });
            else
        ...
        if (db) {
            for (const t of targets) {
                try {
                    ...
                    if (hit?.file_path)
                        targetFiles.push(hit.file_path);
                }
                catch {
        ...
        const selector = (target) => target ? `${target.elementType}:${target.name}` : '-all';
        ...
        const buildArgsColonStyle = (metadataFlag, compilerMetadataFlag, target) => [
        ...
        const buildArgsEqStyle = ({ compilerMetadata }, target) => [
        ...
        const buildArgsFallbackStyle = (target) => [
            `-metadata:${metadataPath}`,
        ...
        const runTargets = targets.length > 0 ? targets : [null];
        const combinedByTarget = await withOperationLock(`bp:${modelName}`, async () => {
        ...
        const hasIssues = (output) => /BPError|<Diagnostic|severity="error"|BestPractices (Warning|Error):/i.test(output)
        ...
        const groups = runTargets.map((target, i) => {
            ...
            return (`── ${selector(target)} ── ${hasIssues(combined) ? '⚠️ issues' : '✅ clean'}` +
                (target ? describeScope(target.name, combined) : '') +
                `\n${body || '(no findings)'}` +
                renderFindingsSection(combined));
        });
        const verdict = notRunCount > 0 ? `❌ BP Check incomplete — ${notRunCount} object(s) were NOT checked`
...
//# sourceMappingURL=runBpCheck.js.map