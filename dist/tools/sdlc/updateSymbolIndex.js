 ...
 */
const EXTENSION_OBJECT_TYPES = new Set(Object.values(AOT_FOLDER_TYPE_MAP).filter(t => t.endsWith('-extension')));
/** True for an AOT element folder segment — mapped types plus everything else Ax*. */
export function isAotFolder(segment) {
 ...
 */
export function classifyAotFolder(aotFolder) {
    ...
    if (AOT_FOLDER_PATTERN.test(aotFolder)) {
        ...
        return key.replace(/^ax/, '');
    }
    return 'class';
}
function isLabelTextFile(filePath) {
...
function parseLabelFileName(filePath) {
 ...
 */
function normalizeFilePaths(filePath) {
 ...
 */
const REDUNDANT_CALL_NOTE = 'ℹ️ This call was not needed. The bridge provider was already refreshed after these file(s) ' +
 ...
 */
async function refreshBridgeForBatch(context, filePaths) {
...
export const updateSymbolIndexTool = async (params, context) => {
    ...
    return {
        content: [{
                type: 'text',
                text: `${lead}Indexed ${results.length - failed}/${results.length} file(s) in one pass.` +
                    `${trail ? ` ${trail}` : ''}\n\n` +
...
async function refreshOnly(context) {
    context.workspaceScanner?.invalidate?.();
    ...
    return {
        content: [{
                type: 'text',
                text: `🔄 **Bridge/cache refresh** (no filePath supplied).\n\n` +
                    `${bridgeNote}\n` +
 ...
 */
async function reindexExtensionMetadata(filePath, objectType, model, symbolIndex, parser) {
...
export async function indexOneFile(filePath, context) {
    ...
    try {
        ...
        const parts = filePath.replace(/\//g, '\\').split('\\');
        const aotFolder = parts.find((p) => isAotFolder(p)) ?? '';
        const objectType = classifyAotFolder(aotFolder);
        ...
        if (!fs.existsSync(filePath)) {
            ...
            const metaCount = (symbolIndex.removeExtensionMetadata?.(objectName, objectType, deletedModel) ?? 0) +
                (objectType === 'class'
            ...
            const parts_cleaned = [];
            if (deletedCount > 0)
                parts_cleaned.push(`${deletedCount} symbol(s)`);
            if (labelCount > 0)
                parts_cleaned.push(`${labelCount} label(s)`);
            if (metaCount > 0)
                parts_cleaned.push(`${metaCount} extension record(s)`);
        ...
        if (isLabelTextFile(filePath)) {
            ...
            if (!parsedFileName) {
                return err(`❌ Error updating label index: invalid label filename format for ${path.basename(filePath)} (expected {LabelFileId}.{locale}.label.txt).`);
            }
            ...
            symbolIndex.touchLastIndexed?.();
            return ok(`✅ Label index updated for **${path.basename(filePath)}** (model: ${model}, language: ${language}).\n\n` +
        ...
        if (objectType === 'class') {
          ...
          `).run(edtName, edtData.extends ?? null, edtData.enumType ?? null, edtData.referenceTable ?? null, edtData.relationType ?? null, edtData.stringSize ?? null, edtData.databaseStringSize ?? null, edtData.displayLength ?? null, edtData.label ?? null, model);
                }
                catch (e) {
        ...
        `);
        ...
        else if (objectType === 'menu-item-display' ||
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?)
        `).run(miData.name ?? objectName, objectType, miData.targetObject ?? null, miData.targetType ?? null, miData.securityPrivilege ?? null, miData.label ?? null, model);
                insertedCount++;
            }
            else {
                tx();
        ...
        symbolIndex.touchLastIndexed?.();
        return ok(`✅ Symbol index updated for **${objectName}** (${objectType}, model: ${model}).\n\n` +
...
//# sourceMappingURL=updateSymbolIndex.js.map