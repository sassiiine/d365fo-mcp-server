    ...
    try {
        ...
        if (!filePath || (typeof filePath === 'string' && filePath.trim().length === 0)) {
            ...
            return {
                content: [{
                        type: 'text',
                        text: `🔄 **Index refresh** (no filePath supplied).\n\n` +
                            `${bridgeNote}\n` +
        ...
        const parts = filePath.replace(/\//g, '\\').split('\\');
        const aotFolder = parts.find((p) => p.toLowerCase() in AOT_FOLDER_TYPE_MAP) ?? '';
        const objectType = AOT_FOLDER_TYPE_MAP[aotFolder.toLowerCase()] ?? 'class';
