...
export async function getSecurityPolicyInfoTool(request, context) {
    try {
       ...
       FROM security_policies WHERE policy_name = ? LIMIT 1`).get(policyName);
        if (!policy) {
            return {
        ...
        const file = db.prepare(`SELECT file_path FROM symbols WHERE name = ? AND type = 'security-policy' LIMIT 1`).get(policyName);
        const lines = [];
