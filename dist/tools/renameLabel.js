...
const RenameLabelArgsSchema = z.object({
    ...
    searchPaths: z
        .array(z.string())
        .optional()
        .describe('Additional absolute directory paths to scan for X++ / XML references. ' +
        'The model package directory is always included automatically.'),
    allowExtensionLabelFile: z
        .boolean()
        .optional()
        .default(false)
        .describe('Escape hatch to allow operating on a label file EXTENSION (a labelFileId ' +
    ...
    dryRun: z
        ...
        .optional()
        .default(false)
        .describe('If true, report all files that would be changed without writing anything. ' +
 ...
 */
function renameLabelInTxt(content, oldId, newId) {
 ...
 */
function replaceReferences(content, labelFileId, oldId, newId) {
    const pattern = new RegExp(`@${escapeRegex(labelFileId)}:${escapeRegex(oldId)}(?=[^A-Za-z0-9_]|$)`, 'g');
    let count = 0;
...
// ── Tool implementation ───────────────────────────────────────────────────────
export async function renameLabelTool(request, context) {
    try {
        ...
        if (!foundInAny) {
            return {
                content: [{
                        type: 'text',
                        text: `❌ Label "${oldLabelId}" not found in any .label.txt file of ` +
            ...
            try {
                ...
                if (content.includes(`\n${newLabelId}=`) || content.includes(`${UTF8_BOM}${newLabelId}=`)) {
                    return {
                        content: [{
                                type: 'text',
                                text: `❌ Target label ID "${newLabelId}" already exists in ` +
        ...
        const allXppFiles = (await Promise.all(scanRoots.map(d => collectFiles(d, ['.xpp'])))).flat();
        const allXmlFiles = (await Promise.all(scanRoots.map(d => collectFiles(d, ['.xml'])))).flat();
        const labelTxtChanges = [];
        const xppChanges = [];
        const xmlChanges = [];
        for (const lang of existingLanguages) {
            const txtPath = path.join(labelResourcesDir, lang, `${labelFileId}.${lang}.label.txt`);
        ...
        for (const xmlFile of allXmlFiles) {
            ...
            if (xmlFile.includes('AxLabelFile'))
                continue;
            let content;
            try {
        ...
        const totalFiles = labelTxtChanges.length + xppChanges.length + xmlChanges.length;
        const totalReplacements = labelTxtChanges.reduce((s, c) => s + c.replacements, 0) +
...
//# sourceMappingURL=renameLabel.js.map