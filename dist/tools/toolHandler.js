 ...
 */
function extractWorkspaceFromMeta(meta) {
    if (!meta)
        return null;
    let rawUri;
...
export function capToolResponse(toolName, result) {
    const cap = getCapForTool(toolName);
    if (cap === 'uncapped' || !result?.content)
        return result;
    const content = result.content.map((item) => {
        if (item.type !== 'text' || typeof item.text !== 'string')
            return item;
        if (item.text.length <= cap)
            return item;
        ...
        const kept = truncateOnBlockBoundary(item.text, cap);
        return {
            ...item,
...
export function registerToolHandler(server, context) {
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request, extra) => {
        ...
        const workspacePath = extractWorkspaceFromMeta(request.params?._meta) ??
            extractWorkspaceFromMeta(request.params._meta);
        ...
        if (context.dbReady && !LOCAL_TOOLS.has(toolName)) {
            ...
            const timeoutPromise = new Promise(resolve => setTimeout(() => resolve('timeout'), DB_WAIT_TIMEOUT_MS));
            const result = await Promise.race([
                context.dbReady.then(() => 'ready'),
                timeoutPromise,
            ]);
            if (result === 'timeout') {
                return {
        ...
        if (!DEDUP_EXCLUDED_TOOLS.has(toolName)) {
            ...
            if (cached !== undefined) {
                ...
                return appendNote(cached, `> ♻️ Duplicate call — this exact ${toolName} call was answered moments ago; ` +
            ...
            if (inFlight) {
                ...
                return appendNote(inFlightResult, `> ♻️ Parallel duplicate — coalesced with a concurrent identical call.`);
            }
        }
        ...
        try {
            result = await runWithBridgeFailureScope(bridgeFailures, async () => {
                ...
                const reportProgress = createProgressReporter(server, extra);
                ...
                return (async () => {
                    switch (toolName) {
                        ...
                        case 'suggest_edt': {
                            const r = await handleSuggestEdt(request.params.arguments, context.symbolIndex);
                        ...
                        case 'update_symbol_index':
                            return await updateSymbolIndexTool(request.params.arguments, context);
                        case 'build_d365fo_project':
                            ...
                            return await buildProjectTool(request.params.arguments, context, reportProgress);
                        case 'trigger_db_sync':
                            return await dbSyncTool(request.params.arguments, context);
                        case 'run_bp_check':
                            return await runBpCheckTool(request.params.arguments, context);
                        case 'run_systest_class':
                            return await sysTestRunnerTool(request.params.arguments, context);
                        case 'review_workspace_changes':
                            return await reviewWorkspaceChangesTool(request.params.arguments, context);
                        case 'undo_last_modification':
                            return await undoLastModificationTool(request.params.arguments, context);
                        case 'get_knowledge':
        ...
        try {
            ...
            if (bridgeFailures.length > 0) {
                const alreadyReported = capped?.content?.some((item) => typeof item?.text === 'string' && item.text.includes(BRIDGE_FAILURE_MARKER));
                if (!alreadyReported) {
            ...
            if (!DEDUP_EXCLUDED_TOOLS.has(toolName)) {
                ...
                if (occurrences >= 3) {
                    capped = appendNote(capped, `> ⚠️ Loop detected: this is occurrence #${occurrences} of the exact same ${toolName} call. ` +
