...
function checkCollisions(finalName, baseName, context) {
    try {
        ...
        const rows = db.prepare(`SELECT DISTINCT name, type, model FROM symbols
       WHERE name IN (?, ?) COLLATE NOCASE
          AND parent_name IS NULL
       LIMIT 10`).all(finalName, baseName);
        if (rows.length > 0) {
            return rows
...
function suggestEdtsForFields(fieldsHint, context) {
    ...
    try {
        const db = context.symbolIndex.getReadDb();
        const stmt = db.prepare(`SELECT name, signature FROM symbols
...
function findReusableLabels(baseName, context) {
    try {
