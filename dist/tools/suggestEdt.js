...
export async function handleSuggestEdt(args, symbolIndex) {
  ...
  `).get(fieldName);
    ...
    LIMIT ?
  `).all(`%${fieldName}%`, `%${fieldName.toLowerCase()}%`, limit * 2);
 ...
 */
function recommendNewEdt(fieldName, _context) {
 ...
 */
function calculateConfidence(fieldName, edtName, context) {
 ...
 */
function getHeuristicSuggestions(fieldName, context) {
    const nameLower = fieldName.toLowerCase();
    const suggestions = [];
    // Common patterns
    const patterns = [
...
//# sourceMappingURL=suggestEdt.js.map