...
const FindMethodsArgsSchema = z.object({
    ...
    keyFields: z
        .array(z.string())
        .optional()
        .describe('Explicit key field names for find()/exists(), in order. Overrides index detection. ' +
...
export function bufferName(table) {
 ...
 */
export function resolveKeyFields(table, override) {
    const typeOf = (fieldName) => {
        const f = table.fields.find((x) => x.name.toLowerCase() === fieldName.toLowerCase());
        return f?.extendedDataType || f?.fieldType || 'str';
    };
    const toKeys = (names) => names.map((n) => ({ field: n, type: typeOf(n) }));
    if (override && override.length > 0)
        return toKeys(override);
 ...
 */
export function buildFindMethods(table, keys, opts = {}) {
...
export async function generateFindMethodsTool(request, context) {
    let args;
    try {
        ...
        try {
            ...
            if (rows.length === 0) {
                return text(`❌ Table "${name}" not found via bridge or symbol index.\n\n` +
                    `If it was just created, call update_symbol_index first, then retry.`, true);
            }
            table = {
                name,
    ...
    if (keys.length === 0 && !includeFindRecId) {
        return text(`❌ Nothing to generate for "${name}": no key fields resolved and includeFindRecId=false.` +
            `\n\nPass keyFields explicitly, or enable findRecId.`, true);
    }
    const code = buildFindMethods(table, keys, { includeExists, includeFindRecId });
    const keySummary = keys.length > 0 ? `Keyed on: ${keys.map((k) => k.field).join(', ')}` : 'RecId only';
    return text(`✅ Find methods for **${name}** (${keySummary})\n\n` +
...
//# sourceMappingURL=generateFindMethods.js.map