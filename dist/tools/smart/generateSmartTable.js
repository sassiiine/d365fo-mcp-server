 ...
 */
import { SmartXmlBuilder } from '../../utils/smartXmlBuilder.js';
...
export const generateSmartTableTool = {
    ...
    inputSchema: {
        ...
        properties: {
            ...
            tableGroup: {
                type: 'string',
                description: 'Table group (business role). Defined by the system enum TableGroup (source: MSDN). ' +
                    'Valid values: ' +
            ...
            tableType: {
                type: 'string',
                description: 'Table storage type (TableType property, source: MSDN). Valid values: ' +
            ...
            fieldsHint: {
                type: 'string',
                description: 'REQUIRED when the user mentions any fields, columns, or data. ' +
            ...
            primaryKeyFields: {
                type: 'array',
                items: { type: 'string' },
                description: 'REQUIRED when user specifies a composite primary key (multiple fields). ' +
            ...
            packagePath: {
                type: 'string',
                description: 'Base packages directory path (e.g. "C:\\AosService\\PackagesLocalDirectory"). ' +
                    'Auto-detected from .mcp.json; only needed when the default K: fallback is wrong.',
            },
            methods: {
                type: 'array',
                items: { type: 'string' },
                description: 'ALWAYS pass ["find", "exist"] when the user asks for those methods. ' +
...
export async function handleGenerateSmartTable(args, symbolIndex, bridge) {
    ...
    if (fieldsHint && !copyFrom && explicitFields.length === 0) {
        ...
        for (const hint of hintFields) {
            ...
            if (fields.find(f => f.name === fieldName)) {
                let suffix = 2;
                while (fields.find(f => f.name === `${hint}${suffix}`))
                    suffix++;
            ...
            const isMandatory = hintLower === 'recid' ||
                /id$/.test(hintLower) ||
    ...
    // from edt_metadata.reference_table and generate matching relations.
    {
        ...
        for (const f of fields) {
            ...
            if (relations.some(r => r.constraints.some(c => c.field === f.name)))
                continue;
            try {
                const edtRow = db.prepare(`SELECT reference_table FROM edt_metadata WHERE edt_name = ? AND reference_table IS NOT NULL AND reference_table != '' LIMIT 1`).get(f.edt);
    ...
    if (!resolvedModel) {
        ...
        if (resolvedModel) {
            ...
            throw new Error('Could not resolve model name. Provide modelName, projectPath, or solutionPath, ' +
    ...
    if (requestedMethods && requestedMethods.length > 0) {
        ...
        const buildParams = (withType) => pkFields.map(f => {
    ...
    if (resolvedModel) {
        ...
        if (fs.existsSync(bridgeTargetPath)) {
            throw new Error(`⚠️ Table "${finalName}" already exists at:\n${bridgeTargetPath}\n\n` +
    ...
    if (bridge && resolvedModel) {
        ...
        if (isBridgeFailure(bridgeAttempt))
            bridgeFailure = bridgeAttempt;
        ...
        if (bridgeResult?.success && bridgeResult.filePath) {
            ...
            if (effectiveProjectPath) {
                try {
                    ...
                    const wasAdded = await projectManager.addToProject(effectiveProjectPath, 'table', finalName, bridgeResult.filePath);
    ...
    if (process.platform !== 'win32' && /^[A-Z]:\\/.test(normalizedPath)) {
        throw new Error(`❌ Cannot create D365FO file on non-Windows system!\n\n` +
    ...
    if (effectiveProjectPath) {
        try {
            const projectManager = new ProjectFileManager();
            const wasAdded = await projectManager.addToProject(effectiveProjectPath, 'table', finalName, normalizedPath);
            projectMessage = wasAdded
                ? `\n✅ Added to Visual Studio project:\n📋 Project: ${effectiveProjectPath}`
                : `\n✅ Already in Visual Studio project:\n📋 Project: ${effectiveProjectPath}`;
 ...
 */
const edtBaseTypeCache = new Map();
/** Forget memoized EDT base types (test isolation). */
export function resetEdtBaseTypeCache() {
 ...
 * in a scaffold.
 */
export async function bridgeEdtBaseType(bridge, edtName) {
    if (!bridge?.isReady)
        return undefined;
    ...
    try {
        ...
        const bt = info?.baseType;
        if (typeof bt !== 'string' || bt.length === 0 || bt === 'Enum')
            return undefined;
 ...
 */
export function resolveEdtBaseType(edtName, db, depth = 0) {
    ...
    if (depth > 8)
        return 'String'; // guard against circular chains
    if (PRIMITIVES.has(edtName))
        return edtName;
    try {
        const row = db.prepare(`SELECT extends, enum_type, string_size FROM edt_metadata WHERE edt_name = ? LIMIT 1`).get(edtName);
 ...
 */
export function resolveEdtEnumType(edtName, db, depth = 0) {
    if (depth > 8)
        return undefined; // guard against circular chains
    try {
        ...
        if (row.enum_type && !row.extends)
            return row.enum_type;
        if (!row.extends)
            return undefined; // root EDT of a non-enum primitive
        if (row.extends === 'Enum')
            return undefined; // extends the bare primitive, no named enum
 ...
 */
export function heuristicEdtBaseType(edtName) {
    ...
    if (e.includes('amount') || e.includes('price') || e.includes('qty') || e.includes('quantity')
        || e.includes('percent') || e.includes('rate') || e === 'real' || e.endsWith('mst'))
        return 'Real';
 ...
 */
export function isEnumName(name, db) {
    try {
 ...
 */
export function selectUnbuildableEdts(missingEdts, modelDir, existsOnDisk = fs.existsSync) {
    return missingEdts.filter(({ edt }) => {
        const edtFile = path.join(modelDir, 'AxEdt', `${edt}.xml`);
        return !existsOnDisk(edtFile);
    });
}
function validateEdtExists(edtName, db) {
    ...
    if (SYSTEM_EDTS.has(edtName))
        return true;
    try {
        // Check edt_metadata first (most reliable)
        const edtRow = db.prepare(`SELECT 1 FROM edt_metadata WHERE edt_name = ? LIMIT 1`).get(edtName);
        if (edtRow)
            return true;
        // Fallback: check symbols table for EDT type entries
        const symRow = db.prepare(`SELECT 1 FROM symbols WHERE name = ? AND type = 'edt' LIMIT 1`).get(edtName);
        if (symRow)
            return true;
        return false;
    }
...
export function isInfrastructureField(fieldName) {
    const INFRA = new Set([
 ...
 */
export function resolveBestEdt(fieldName, db) {
    try {
        ...
        const edtExactStmt = db.prepare(`SELECT edt_name FROM edt_metadata WHERE edt_name = ? LIMIT 1`);
        let exact = edtExactStmt.get(fieldName);
        if (!exact) {
            const canonical = lookupSymbolNocase(db, fieldName, ['edt'])?.name;
            if (canonical && canonical !== fieldName) {
                exact = edtExactStmt.get(canonical);
            }
        }
        if (exact)
            return exact.edt_name;
        ...
        const candidates = db.prepare(`SELECT edt_name FROM edt_metadata WHERE edt_name LIKE ? COLLATE NOCASE ORDER BY LENGTH(edt_name) ASC LIMIT 30`).all(`%${fieldName}%`);
        let best = '';
        let bestConf = 0;
        ...
        if (acceptFuzzy && best && bestConf >= 0.8)
            return best;
        ...
        if (validateEdtExists(heuristic, db))
            return heuristic;
        if (acceptFuzzy && best && bestConf >= 0.6)
            return best;
    }
    catch {
...
export function suggestEdtFromFieldName(fieldName) {
    const nameLower = fieldName.toLowerCase();
    // Common patterns
    if (nameLower === 'recid')
        return 'RecId';
    if (nameLower === 'accountnum' || nameLower === 'accountnumber')
        return 'CustAccount';
    if (nameLower.includes('custaccount') || nameLower.includes('customeraccount'))
        return 'CustAccount';
    if (nameLower.includes('vendaccount') || (nameLower.includes('vendor') && nameLower.includes('account')))
        return 'VendAccount';
    if (nameLower === 'name')
        return 'Name';
    ...
    if (nameLower === 'description')
        return 'Description';
    if (nameLower.includes('description') || nameLower === 'desc')
        return 'Description';
    if (nameLower.includes('subject') || nameLower === 'title')
        return 'Name';
    ...
    if (nameLower === 'notes' || nameLower === 'note' || nameLower === 'notetext' ||
        ...
        nameLower === 'body' || nameLower === 'content' || nameLower === 'message')
        return 'Notes';
    if (nameLower.includes('amount'))
        return 'AmountMST';
    if (nameLower.includes('rate'))
        return 'AmountMST';
    if (nameLower.includes('quantity') || nameLower.includes('qty'))
        return 'Qty';
    if (nameLower.includes('price'))
        return 'PriceUnit';
    // Only the bare effectivity names map to the *DateTime EDTs; other "*date" → TransDate.
    if (nameLower === 'validfrom')
        return 'ValidFromDateTime';
    if (nameLower === 'validto')
        return 'ValidToDateTime';
    if (nameLower.includes('datetime') || nameLower.includes('time'))
        return 'TransDateTime';
    if (nameLower.includes('date'))
        return 'TransDate';
    if (nameLower.includes('account'))
        return 'LedgerAccount';
    if (nameLower.includes('customer') || nameLower.includes('cust'))
        return 'CustAccount';
    if (nameLower.includes('vendor') || nameLower.includes('vend'))
        return 'VendAccount';
    if (nameLower.includes('item'))
        return 'ItemId';
    if (nameLower.includes('percent') || nameLower.includes('pct'))
        return 'Percent';
    if (nameLower.includes('enabled') || nameLower.includes('active') || nameLower.includes('flag'))
        return 'NoYesId';
...
//# sourceMappingURL=generateSmartTable.js.map