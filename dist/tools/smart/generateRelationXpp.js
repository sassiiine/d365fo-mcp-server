 ...
 */
export function buildRelationSelect(sourceTable, rel) {
    ...
    return (`${rel.relatedTable} ${relBuf};\n` +
 ...
 */
export function buildRelationQuery(sourceTable, rel) {
    const sourceBuf = bufferName(sourceTable);
...
export async function generateRelationXppTool(request, context) {
    let args;
    try {
    ...
    if (!bridge?.isReady || !bridge.metadataAvailable) {
        return text(`❌ relation-xpp needs the C# metadata bridge — relation metadata is not stored in the symbol index.\n\n` +
            `Run on the Windows VM with the bridge built (bridge\\D365MetadataBridge), then retry.`, true);
    }
    let t;
    try {
    ...
    let relations = t.relations.map((r) => ({
        name: r.name,
    ...
    if (relationName) {
        ...
        if (!match) {
            ...
            return text(`❌ Relation "${relationName}" not found on "${name}".\n\nAvailable relations: ${available}`, true);
        }
        relations = [match];
    }
    ...
    return text(`✅ X++ for ${relations.length} relation(s) on **${name}** (style: ${style})\n\n` +
        `\`\`\`xpp\n${code}\n\`\`\``);
}
//# sourceMappingURL=generateRelationXpp.js.map