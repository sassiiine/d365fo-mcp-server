 ...
 */
const DATASOURCE_TYPES = ['table', 'view'];
 ...
 */
export function resolveFormCaption(explicitCaption, explicitLabel, tableLabel, fallbackName) {
 ...
 */
export function lookupTableLabel(symbolIndex, table) {
    if (!table)
        return undefined;
    try {
...
export async function handleGenerateSmartForm(args, symbolIndex) {
    ...
    if (dataSource && !copyFrom) {
        ...
        try {
            ...
            if (directHit) {
                ...
                if (matchedHit) {
                    ...
                    const alt = db.prepare(`SELECT name FROM symbols WHERE type IN ('table', 'view') AND name LIKE ? COLLATE NOCASE ORDER BY LENGTH(name) ASC LIMIT 1`).get(`${dataSource.replace(/s$/i, '')}%`);
        ...
        console.log(`[generateSmartForm] Creating datasource for ${dataSourceIsView ? 'view' : 'table'}: ${dataSourceResolved}`);
        dataSources.push({
    ...
    if (linesTableResolved) {
        ...
        try {
            const db = symbolIndex.getReadDb();
            const exists = (n) => lookupSymbolNocase(db, n, ['table']);
    ...
    `);
        ...
        if (tableMapping && Object.keys(tableMapping).length > 0) {
            ...
            const { unknownTargets, poorOverlap } = checkTableMappingCoverage(tableMapping, (table) => {
                const canonical = lookupSymbolNocase(db, table)?.name ?? table;
                const rows = fieldStmt.all(canonical);
    ...
    if (/<DataGroup>Overview<\/DataGroup>/.test(xml)) {
        const overviewDsTable = xml.match(/<AxFormDataSource[^>]*>\s*<Name>[^<]+<\/Name>\s*<Table>([^<]+)<\/Table>/)?.[1]
