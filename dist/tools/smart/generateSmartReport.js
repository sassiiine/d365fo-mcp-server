...
import { resolveBestEdt, suggestEdtFromFieldName as heuristicEdtFromFieldName, resolveEdtBaseType as sharedResolveEdtBaseType, heuristicEdtBaseType, bridgeEdtBaseType, } from './generateSmartTable.js';
...
export async function handleGenerateSmartReport(args, symbolIndex, bridge) {
    ...
    const edtPrimitiveCache = new Map();
    const primitiveOf = async (edt) => {
        if (!edt)
            return undefined;
    ...
    const primitiveOfSync = (edt) => edt ? edtPrimitiveCache.get(edt) : undefined;
    const reportFields = [];
        ...
        try {
            ...
            if (srcTmpTable) {
                ...
                for (const f of dbFields) {
                    if (['RecId', 'RecVersion', 'DataAreaId', 'Partition', 'TableId'].includes(f.name))
                        continue;
                    reportFields.push({
                        name: f.name,
    ...
    const resolvedExtraDatasets = [];
    for (const ds of additionalDatasets) {
 ...
 */
function suggestEdtFromFieldName(fieldName, db) {
    if (db) {
        try {
 ...
 */
function resolveRdlDataType(baseType) {
    switch (baseType) {
 ...
 */
async function resolveEdtPrimitive(edtName, db, bridge) {
    if (!edtName)
        return undefined;
 ...
 */
function resolveFieldType(base) {
 ...
 */
function resolveAotQueryFirstTable(requestedQueryName, db) {
    try {
