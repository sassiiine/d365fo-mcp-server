...
export const GENERATE_SMART_TYPES = ['table', 'form', 'report'];
export const GENERATE_SMART_DISPATCH = {
...
const GenerateSmartArgsSchema = z
    .object({
    objectType: z.enum(GENERATE_SMART_TYPES).describe('Kind of object to generate: table (AxTable + indexes + relations + methods), ' +
        'form (controls + datasources + pattern), report (TmpTable + Contract + DP + Controller + AxReport).'),
})
    .passthrough();
export async function generateSmartTool(request, context) {
    ...
    const handler = GENERATE_SMART_DISPATCH[objectType];
    if (!handler) {
        return {
...
//# sourceMappingURL=generateSmart.js.map