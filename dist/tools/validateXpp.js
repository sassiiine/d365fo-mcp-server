...
function checkFunctionInWhere(code) {
    ...
    lines.forEach((rawLine, i) => {
        ...
        if (/\bwhere\b/i.test(rawLine))
            inWhere = true;
        if (inWhere && /{/.test(rawLine))
            inWhere = false;
        if (!inWhere)
            return;
        ...
        while ((m = callPattern.exec(rawLine)) !== null) {
            ...
            if (['if', 'while', 'for', 'switch', 'catch', 'str', 'int', 'new'].includes(fnName))
                continue;
            violations.push({
                rule: 'SEL005',
                severity: 'warning',
...
function runRules(code, codeType, stats) {
    const violations = [];
    if (codeType === 'xpp') {
...
export async function validateXppTool(request, serverContext) {
    ...
    const lines = [];
    lines.push(`${errors.length > 0 ? '❌' : '⚠️'} validate_xpp: ` +
