 ...
 */
import { z } from 'zod';
import { resolveBestEdt, resolveEdtBaseType, heuristicEdtBaseType, isEnumName, } from './generateSmartTable.js';
...
export function axTableFieldType(edt, type, enumType) {
    if (enumType)
    ...
    if (edt) {
        ...
        if (e.includes('amount') || e.includes('mst') || e.includes('price') || e.includes('qty') || e.includes('percent') || e === 'real')
            return 'AxTableFieldReal';
        if (e === 'noyesid' || e.endsWith('noyesid') || e === 'noyes')
            return 'AxTableFieldEnum';
        if ((e.endsWith('int') || e.includes('count') || e.includes('level')) && !e.includes('account') && !e.includes('name'))
...
export function parseFieldsHint(hint) {
    return hint
        .split(/[\n,;]+/)
        .map((s) => s.trim())
        .filter(Boolean);
 ...
 */
export function resolveField(input, db) {
    const field = { ...input };
    let warning;
...
export async function generateTableFieldsTool(request, context) {
    let args;
    try {
    ...
    const inputs = fields && fields.length > 0
        ? fields
        : fieldsHint
    ...
    if (warnings.length > 0)
        parts.push('', ...warnings);
    parts.push('', `Insert each field with \`d365fo_file(action="modify", objectType="table", objectName="${name}", ...)\` add-field.`, '', '```xml', fieldFragments, '```');
    if (groupFragment) {
        parts.push('', `Field group **${fieldGroup}**:`, '```xml', groupFragment, '```');
    }
    return text(parts.join('\n'));
}
//# sourceMappingURL=generateTableFields.js.map