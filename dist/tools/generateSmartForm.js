...
import { getFieldControlMap } from '../utils/fieldControlTypes.js';
export const generateSmartFormTool = {
 ...
 */
export function cloneFromPatternMismatchWarning(requestedPattern, clonedXml, sourceFormName) {
    if (!requestedPattern)
        return null;
    const intended = resolvePattern(requestedPattern);
    const clonedPattern = clonedXml.match(/<Pattern xmlns="">([^<]+)<\/Pattern>/)?.[1];
    if (!intended || !clonedPattern)
        return null;
    if (intended.xmlName.toLowerCase() === clonedPattern.toLowerCase())
        return null;
    const ref = intended.referenceForms?.[0];
    return (`🛑 PATTERN MISMATCH — you requested "${intended.xmlName}" but "${sourceFormName}" is a "${clonedPattern}" form. ` +
 ...
 */
export function checkTableMappingCoverage(tableMapping, getTableFields) {
    const unknownTargets = [];
    const poorOverlap = [];
    for (const [srcTable, tgtTable] of Object.entries(tableMapping)) {
        if (!tgtTable || srcTable.toLowerCase() === tgtTable.toLowerCase())
            continue;
        ...
        if (!srcFields || srcFields.length < 3)
            continue;
        ...
        if (ratio < 0.3) {
            poorOverlap.push(`${srcTable} → ${tgtTable}: ${shared.length}/${srcFields.length} fields shared (${Math.round(ratio * 100)} %)`);
        }
    }
    return { unknownTargets: [...new Set(unknownTargets)], poorOverlap };
}
export async function handleGenerateSmartForm(args, symbolIndex) {
    const { name, label, caption, dataSource, linesTable, linesDataSource, formPattern, copyFrom, cloneFrom, tableMapping, includeMethodStubs, generateControls, modelName, projectPath, solutionPath, } = args;
    ...
    if (dataSource && !copyFrom) {
        ...
        try {
            const db = symbolIndex.getReadDb();
            const directHit = db.prepare(`SELECT name FROM symbols WHERE type = 'table' AND name = ? COLLATE NOCASE LIMIT 1`).get(dataSource);
            if (!directHit) {
                const add = (n) => {
                    const v = (n ?? '').trim();
                    if (v && !candidates.some((c) => c.toLowerCase() === v.toLowerCase()))
                        candidates.push(v);
                };
                const candidates = [];
                ...
                const matched = candidates
                    .map(c => db.prepare(`SELECT name FROM symbols WHERE type = 'table' AND name = ? COLLATE NOCASE LIMIT 1`).get(c))
                    .find(r => r)?.name;
                if (matched) {
                    ...
                    const alt = db.prepare(`SELECT name FROM symbols WHERE type = 'table' AND name LIKE ? COLLATE NOCASE ORDER BY LENGTH(name) ASC LIMIT 1`).get(`${dataSource.replace(/s$/i, '')}%`);
        ...
        if (formPattern) {
            try {
                await handleGetFormPatterns({ formPattern }, symbolIndex);
    ...
    const collectGridFields = (db, table) => {
        ...
        try {
            ...
            if (gridFields.length > 0) {
                if (generateControls) {
                    ...
                    const gridControl = builder.buildGridControl(`${dataSourceEffective}Grid`, dataSourceEffective, gridFields, fieldTypes);
    ...
    if (linesTableResolved) {
        ...
        try {
            const db = symbolIndex.getReadDb();
            const exists = db.prepare(`SELECT name FROM symbols WHERE type = 'table' AND name = ? COLLATE NOCASE LIMIT 1`);
            const direct = exists.get(linesTableResolved);
            if (!direct) {
                ...
                if (matched) {
                    ...
                    const alt = db.prepare(`SELECT name FROM symbols WHERE type = 'table' AND name LIKE ? COLLATE NOCASE ORDER BY LENGTH(name) ASC LIMIT 1`).get(`${stem}%`);
                    ...
                    return {
                        content: [{
                                type: 'text',
                                text: `❌ Lines table "${linesTableResolved}" not found in the symbol index.` +
                                    suggestion +
                                    `\n\nIf the table was just created in this session, call \`update_symbol_index\` first, then retry.`,
                            }],
                        isError: true,
        ...
        const linesTbl = linesTableResolved;
        const explicitDsIsStalePlural = !!linesDataSource &&
        ...
        const isDuplicateDs = dataSources.some(ds => ds.table.toLowerCase() === linesTbl.toLowerCase() ||
    ...
    if (cloneFrom) {
        ...
        if (!sourceXml) {
            return {
                content: [{
                        type: 'text',
                        text: `❌ cloneFrom: form "${cloneFrom}" could not be read from the metadata store.\n\n` +
                            `Options:\n` +
    ...
    `);
        ...
        if (tableMapping && Object.keys(tableMapping).length > 0) {
            ...
            const { unknownTargets, poorOverlap } = checkTableMappingCoverage(tableMapping, (table) => {
                const rows = fieldStmt.all(table);
            ...
            if (unknownTargets.length > 0) {
                return {
                    content: [{
                            type: 'text',
                            text: `❌ PRE-CLONE CHECK — target table${unknownTargets.length > 1 ? 's' : ''} not found in the symbol index: ` +
                                `${unknownTargets.join(', ')}.\n\n` +
            ...
            if (poorOverlap.length > 0) {
                const targetList = Object.values(tableMapping).filter(Boolean).join(', ');
                return {
                    content: [{
                            type: 'text',
                            text: `❌ PRE-CLONE CHECK — Poor field overlap between source and target tables:\n` +
        ...
        xml = cloneResult.xml;
        const noteLines = [`   Cloned from: ${cloneResult.sourceFormName}`];
        ...
        if (poorMatches.length > 0) {
            ...
            return {
                content: [{
                        type: 'text',
                        text: `❌ POOR CLONE MATCH — ${matchSummary}.\n\n` +
        ...
        if (designPattern) {
            ...
            if (envVersion) {
                ...
                if (current && current !== envVersion) {
                    xml = xml.replace(/(<PatternVersion xmlns="">)[^<]*(<\/PatternVersion>)/, `$1${envVersion}$2`);
    ...
    if (includeMethodStubs) {
        ...
        const stubDsName = xml.match(/<AxFormDataSource[^>]*>\s*<Name>([^<]+)<\/Name>/)?.[1] ?? primaryDs?.name ?? '';
        const stubLinesDsName = linesTableResolved ? (linesDsNameResolved || linesTableResolved) : undefined;
        const stubResult = injectMethodStubs(xml, methodStubsForPattern(patternInXml, stubDsName, stubLinesDsName), stubDsName, stubLinesDsName);
        xml = stubResult.xml;
    ...
    if (patternErrors.length > 0) {
        ...
        if (!cloneFrom) {
            return {
                content: [{
                        type: 'text',
                        text: `❌ generate internal error: the generated XML violates its own pattern ` +
    ...
    try {
        const db = symbolIndex.getReadDb();
        const tableExists = db.prepare(`SELECT 1 FROM symbols WHERE type = 'table' AND name = ? COLLATE NOCASE LIMIT 1`);
        const seenTables = new Set();
        for (const m of xml.matchAll(/<Table>([^<]+)<\/Table>/g)) {
            const table = m[1].trim();
            if (!table || seenTables.has(table.toLowerCase()))
                continue;
    ...
    if (!resolvedModel) {
        return {
            content: [{
                    type: 'text',
                    text: `❌ Cannot write form file: model name could not be resolved.\n\n` +
    ...
    if (effectiveProjectPath) {
        try {
            ...
            const wasAdded = await projectManager.addToProject(effectiveProjectPath, 'form', finalName, normalizedPath);
 ...
 */
export function resolveEnvPatternVersion(db, patternXmlName) {
    try {
...
//# sourceMappingURL=generateSmartForm.js.map