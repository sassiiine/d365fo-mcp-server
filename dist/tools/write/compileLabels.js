...
async function findCscDir() {
    for (const dir of CSC_DIRS) {
        try {
            await access(path.join(dir, 'csc.exe'));
            return dir;
        }
        catch { /* next */ }
    }
...
async function findSdkToolsDir() {
 ...
 */
export async function findLabelFileDirs(packageDir) {
    let entries;
    try {
 ...
 */
export async function labelAssembliesAreStale(labelDirs, resourcesDir, moduleName) {
    let builtAt;
    try {
...
export function labelcArgs(customPackagesPath, modelName, resourcesDir, cscDir, sdkToolsDir) {
    return [
 ...
 */
export async function compileModelLabels(microsoftPackagesPath, customPackagesPath, modelName, force = false) {
    ...
    if (!labelcExe) {
        return {
            skipped: true,
            success: false,
            message: `labelc.exe not found (looked in ${microsoftPackagesPath}\\bin) — labels will not be ` +
                `compiled, so references to them may be reported as unknown labels`,
        };
    }
    const args = labelcArgs(customPackagesPath, modelName, resourcesDir, await findCscDir(), await findSdkToolsDir());
    try {
...
//# sourceMappingURL=compileLabels.js.map