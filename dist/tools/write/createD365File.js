...
import { buildAxSecurityDutyXml, buildAxSecurityRoleXml, buildAxSecurityDutyExtensionXml, buildAxSecurityRoleExtensionXml, } from '../xml/securityDutyRoleXml.js';
...
import { assertKnownEnumValue, resolveEnumValueMode, RELATED_TABLE_CARDINALITIES, RELATION_CARDINALITIES, RELATIONSHIP_TYPES, SECURITY_POLICY_CONTEXT_TYPES, } from '../../utils/axEnumProperties.js';
...
import { buildAxMenuItemExtensionXml } from '../xml/menuItemExtensionXml.js';
...
import { reconcileTableCreateProperties, renderTableCreateHonestyReport, } from '../xml/createTablePropertyHonesty.js';
/**
 ...
 * wrong project.
 */
function buildNoProjectPathWarning() {
...
const CreateD365FileArgsSchema = z.object({
    ...
    projectPath: z
        .string()
        .optional()
        .describe('Path to .rnrproj file. Required for addToProject in any workspace holding more than one .rnrproj: ' +
    ...
    xmlContent: z
        .string()
        .optional()
        .describe('Custom XML content to write verbatim instead of generating a template. ' +
    ...
    overwrite: z
        ...
        .optional()
        .default(false)
        .describe('Allow overwriting an existing file. Use together with xmlContent when you need to completely ' +
        'rewrite an object (e.g. table with corrupted field names). Default: false (returns error if file already exists).'),
    groundingToken: z
        .string()
        .optional()
        .describe('Provenance token returned by prepare(mode="change"). Proves the change was grounded in the indexed codebase. ' +
 ...
 */
export function normalizeFieldSpecsForBridge(fields) {
    return fields.map((f) => {
        let fieldType = f.type ?? f.fieldType;
        if (typeof fieldType === 'string')
            fieldType = fieldType.replace(/^AxTableField/, '');
        const out = { name: f.name };
        if (fieldType != null && fieldType !== '')
            out.type = fieldType;
        if (f.edt != null)
            out.edt = f.edt;
        else if (f.extendedDataType != null)
            out.edt = f.extendedDataType;
        if (f.enumType != null)
        ...
        if (f.label != null)
            out.label = f.label;
        if (f.stringSize != null)
            out.stringSize = f.stringSize;
        return out;
    });
}
/**
 ...
 */
export function normalizeIndexSpecsForBridge(indexes) {
    return indexes.map((idx) => {
        const name = idx.name ?? idx.indexName;
        const rawFields = (idx.fields ?? idx.indexFields);
        ...
        if (fields && fields.length > 0)
            out.fields = fields;
        if (idx.alternateKey != null)
            out.alternateKey = idx.alternateKey;
        if (idx.allowDuplicates != null)
            out.allowDuplicates = idx.allowDuplicates;
        return out;
    });
}
/**
...
export class XmlTemplateGenerator {
     ...
     */
    static splitXppClassSource(fullSource) {
        // Find the '{' that opens the class body
        const firstBrace = fullSource.indexOf('{');
        if (firstBrace === -1)
            return { declaration: fullSource, methods: [] };
        ...
        if (!rest.trim()) {
            ...
            if (innerResult) {
                console.error('[splitXppClassSource] Inner-class methods detected — extracting into ' +
        ...
        if (nextBraceInRest !== -1) {
            ...
            if (varLines.length > 0) {
                ...
                console.error(`[splitXppClassSource] Rescued ${varLines.length} member variable declaration(s) ` +
        ...
        while (pos < rest.length) {
            ...
            const nameMatch = parenIdx !== -1 ? sigText.substring(0, parenIdx).match(/(\w+)\s*$/) : null;
        ...
        if (methods.length === 0) {
            const innerResult = XmlTemplateGenerator.extractInnerClassMethods(declaration);
            if (innerResult) {
                console.error('[splitXppClassSource] Extracted inner class methods — moving them from ' +
     ...
     */
    static normalizeSelfReferenceName(className, declaration, methods) {
     ...
     */
    static parseSourceForBridge(sourceCode, className) {
     ...
     */
    static generateAxClassXml(className, sourceCode, properties) {
        ...
        const { declaration, methods } = XmlTemplateGenerator.normalizeSelfReferenceName(className, rawSplit.declaration, rawSplit.methods);
        ...
        const implementsAttr = properties?.implements
            ...
            : '';
        const methodsXml = methods.length === 0
            ? '\t\t<Methods />\n'
            : `\t\t<Methods>\n${methods
...
`;
     ...
     */
    static generateAxClassExtensionXml(extensionName, sourceCode, properties) {
     ...
     */
    static generateAxTableXml(tableName, properties, sourceCode) {
        return buildAxTableXml(tableName, properties, sourceCode?.trim()
     ...
     */
    static generateAxEnumXml(enumName, properties) {
        ...
        const enumValueSpecs = Array.isArray(properties?.enumValues) ? properties.enumValues : [];
        // D365FO hard limit: max 251 elements (0–250). Warn early — compiler rejects beyond this.
        if (enumValueSpecs.length > 251) {
            throw new Error(`Enum '${enumName}' has ${enumValueSpecs.length} values but D365FO supports a maximum of 251 (0–250). ` +
        ...
        if (enumValueSpecs.length === 0) {
            ...
            for (const v of enumValueSpecs) {
                ...
                if (v.label)
                    enumValuesXml += `\t\t\t<Label>${escapeXml(v.label)}</Label>\n`;
                if (v.helpText)
                    enumValuesXml += `\t\t\t<HelpText>${escapeXml(v.helpText)}</HelpText>\n`;
                // Omit <Value> when UseEnumValue=No (position-based ordering) or for implicit 0
                if (intValue !== 0 && !suppressExplicitValues)
                    enumValuesXml += `\t\t\t<Value>${intValue}</Value>\n`;
                enumValuesXml += `\t\t</AxEnumValue>\n`;
            }
            enumValuesXml += '\t</EnumValues>\n';
        }
...
`;
     ...
     */
    static generateAxFormXml(formName, properties) {
     ...
     */
    static generateAxQueryXml(queryName, properties) {
     ...
     */
    static generateAxViewXml(viewName, properties) {
     ...
     * property contract.
     */
    static generateAxMapXml(mapName, properties) {
     ...
     * matter).
     */
    static generateAxDataEntityXml(entityName, properties) {
     ...
     *   </AxReport>
     */
    static generateAxReportXml(reportName, properties) {
        ...
        if (properties?.datasets && Array.isArray(properties.datasets)) {
            ...
            datasets = [{
                    ...
                    aotQuery: properties?.aotQuery,
                    contractParams: properties?.contractParams,
                }];
        }
...
\t</DefaultParameterGroup>`;
        ...
        const buildRdlSkeleton = () => {
            ...
            const rdlDataSourcesXml = `  <DataSources>
...
${rdlFields}      <rd:DataSetInfo>
        ...
        <rd:TableAdapterName>FieldsTableAdapter</rd:TableAdapterName>
            ...
            const buildRdlTablix = (ds) => {
                ...
                const cols = ds.fields.map(() => `            <TablixColumn><Width>${colW}in</Width></TablixColumn>`).join('\n');
            ...
            const cellDefs = rdlParamDefs
                .map((p, i) => `        <CellDefinition>\n          <ColumnIndex>${i}</ColumnIndex>\n          <RowIndex>0</RowIndex>\n          <ParameterName>${p.name}</ParameterName>\n        </CellDefinition>`)
                .join('\n');
            const rdlParamLayoutXml = `  <ReportParametersLayout>
...
${rdlParamLayoutXml}
  ...
  <rd:ReportID>${uuid()}</rd:ReportID>
        ...
        const rdlContent = properties?.rdlContent;
...
\t<EmbeddedImages />
     ...
     */
    static generate(objectType, objectName, sourceCode, properties) {
        switch (objectType) {
            case 'class':
            ...
            case 'data-entity': {
                ...
                const entitySource = sourceCode ?? properties?.sourceCode;
     ...
     */
    static sanitizeQueryXml(xml) {
        return xml.replace(/<AxQuery(\s[^>]*)?>/, (_match, attrs) => {
            let a = attrs || '';
            if (!a.includes('xmlns=""')) {
                a += ' xmlns=""';
            }
    ...
    static sanitizeEnumXml(xml) {
        ...
        if (!xml.includes('xmlns:i=')) {
            xml = xml.replace(/(<AxEnum)(\s|>)/, '$1 xmlns:i="http://www.w3.org/2001/XMLSchema-instance"$2');
        ...
        if (valueCount > 251) {
            console.error(`[sanitizeEnumXml] ⚠️ WARNING: ${valueCount} enum values detected — D365FO supports max 251 (0–250). ` +
    ...
    static sanitizeTableXml(xml) {
        ...
        for (const ft of fieldTypes) {
            ...
            xml = xml.replace(openRe, (_match, attrs) => {
    ...
    static sanitizeReportXml(xml) {
        ...
        xml = xml.replace(/<AxReportDesign(\s[^>]*)?>/, (match, attrs) => {
            const current = attrs || '';
            let updated = current;
            ...
            if (updated === current)
                return match; // nothing changed — idempotent
        ...
        if (xml.includes('<DataSourceType>') && !xml.includes('<Parameters>')) {
            const axDatasetParams = '\t\t\t<Parameters>\n' +
                '\t\t\t\t<AxReportDataSetParameter>\n' +
                '\t\t\t\t\t<Name>AX_PartitionKey</Name>\n' +
                '\t\t\t\t\t<Alias>AX_PartitionKey</Alias>\n' +
                '\t\t\t\t\t<DataType>System.String</DataType>\n' +
        ...
        if (xml.includes('<DataSourceType>') && !xml.includes('<DefaultParameterGroup>') && xml.includes('<Designs>')) {
            const defaultParamGroup = '\t<DefaultParameterGroup>\n' +
                '\t\t<Name xmlns="">Parameters</Name>\n' +
                '\t\t<ReportParameterBases xmlns="">\n' +
                '\t\t\t<AxReportParameterBase xmlns=""\n' +
                '\t\t\t\t\ti:type="AxReportParameter">\n' +
        ...
        xml = xml.replace(/(<Text><!\[CDATA\[)([\s\S]*?)(\]\]><\/Text>)/, (_whole, open, rdl, close) => {
            ...
            if (is2010 && !rdl.includes('<ReportSections>')) {
                ...
                let sectionContent = '';
                if (bodyMatch) {
                    sectionContent += bodyMatch[0];
                    fixedRdl = fixedRdl.replace(bodyMatch[0], '');
                }
                if (pageEl)
        ...
        xml = xml.replace(/(<Text><!\[CDATA\[)([\s\S]*?)(\]\]><\/Text>)/, (_whole, open, rdl, close) => {
            ...
            const fixedRdl = rdl.replace(/<Body>\s*\n(\s*)((?!<ReportItems)[\s\S]*?)<\/Body>/, (_bodyMatch, indent, bodyContent) => {
                // Only add <ReportItems /> when the body has no report items at all
                if (bodyContent.includes('<ReportItems'))
                    return _bodyMatch;
        ...
        xml = xml.replace(/(<Text><!\[CDATA\[)([\s\S]*?)(\]\]><\/Text>)/, (_whole, open, rdl, close) => {
            const fixedRdl = rdl.replace(/<\/(\w+)><\/\1>/g, (_m, tag) => {
        ...
        xml = xml.replace(/(<Text><!\[CDATA\[)([\s\S]*?)(\]\]><\/Text>)/, (_whole, open, rdl, close) => {
            ...
            const fixedRdl = rdl.replace(/(<Textbox\b[^>]*>)([\s\S]*?)(<\/Textbox>)/g, (tbMatch, tbOpen, tbContent, tbClose) => {
                // Only act if there is a <Value> but no <Paragraphs> wrapping yet
                if (!tbContent.includes('<Value>') && !tbContent.includes('<Value ='))
                    return tbMatch;
                if (tbContent.includes('<Paragraphs>'))
                    return tbMatch;
                const fixedContent = tbContent.replace(/<Value>([\s\S]*?)<\/Value>/, (_vMatch, val) => {
                    console.error('[sanitizeReportXml] Wrapped bare <Value> in <Textbox> into <Paragraphs> structure');
                    return `<Paragraphs>\n            <Paragraph>\n              <TextRuns>\n                <TextRun>\n                  <Value>${val}</Value>\n                  <Style />\n                </TextRun>\n              </TextRuns>\n              <Style />\n            </Paragraph>\n          </Paragraphs>`;
                });
                if (fixedContent === tbContent)
                    return tbMatch;
        ...
        xml = xml.replace(/(<Text><!\[CDATA\[)([\s\S]*?)(\]\]><\/Text>)/, (_whole, open, rdl, close) => {
            const fixedRdl = rdl.replace(/(<TablixCell>)([\s\S]*?)(<\/TablixCell>)/g, (tcMatch, tcOpen, tcContent, tcClose) => {
        ...
        xml = xml.replace(/(<Text><!\[CDATA\[)([\s\S]*?)(\]\]><\/Text>)/g, (_whole, open, rdl, close) => {
            ...
            for (const [prefix, wrapper] of groups) {
                ...
                if (!new RegExp(`<(?:${st}|${ct}|${wt})>`).test(fixedRdl))
                    continue;
                ...
                const singleTag = `(?:<${st}>([^<]*)<\\/${st}>|<${ct}>([^<]*)<\\/${ct}>|<${wt}>([^<]*)<\\/${wt}>)`;
                const clusterRe = new RegExp(`${singleTag}(?:\\s*${singleTag})?(?:\\s*${singleTag})?`, 'g');
                fixedRdl = fixedRdl.replace(clusterRe, (match) => {
                    // Extract each flat-tag value from the matched cluster via side-effect callbacks.
                    let bStyle = '', bColor = '', bWidth = '';
                    match.replace(new RegExp(`<${st}>([^<]*)<\\/${st}>`), (_, v) => { bStyle = v; return ''; });
                    match.replace(new RegExp(`<${ct}>([^<]*)<\\/${ct}>`), (_, v) => { bColor = v; return ''; });
                    match.replace(new RegExp(`<${wt}>([^<]*)<\\/${wt}>`), (_, v) => { bWidth = v; return ''; });
                    let inner = '';
                    if (bStyle)
                        inner += `<Style>${bStyle}</Style>`;
                    if (bColor)
                        inner += `<Color>${bColor}</Color>`;
                    if (bWidth)
                        inner += `<Width>${bWidth}</Width>`;
                    changed = true;
        ...
        xml = xml.replace(/(<Text><!\[CDATA\[)([\s\S]*?)(\]\]><\/Text>)/, (_whole, open, rdl, close) => {
            ...
            while ((csMatch = csRe.exec(fixedRdl)) !== null) {
                ...
                for (let i = 0; i < span - 1; i++) {
                    // Skip whitespace
                    while (pos < fixedRdl.length && /\s/.test(fixedRdl[pos]))
                        pos++;
                    ...
                    if (!fixedRdl.startsWith('<TablixCell>', pos))
                        break; // Not a TablixCell
        ...
        xml = xml.replace(/(<Text><!\[CDATA\[)([\s\S]*?)(\]\]><\/Text>)/, (_whole, open, rdl, close) => {
            const fixedRdl = rdl.replace(/<Paragraph>([\s\S]*?)<\/Paragraph>/g, (match, inner) => {
        ...
        xml = xml.replace(/(<Text><!\[CDATA\[)([\s\S]*?)(\]\]><\/Text>)/g, (_whole, open, rdl, close) => {
            ...
            const processTablix = (block) => {
                ...
                let result = block.replace(/(<TablixCells>)([\s\S]*?)(<\/TablixCells>)/g, (m, o, inner, c) => {
     ...
     */
    static encodeReportTextElement(xml) {
        return xml.replace(/<Text><!\[CDATA\[([\s\S]*?)\]\]><\/Text>/g, (_match, rdlInner) => {
     ...
     */
    static generateAxEdtXml(name, properties) {
        const edtTypeRaw = properties?.edtType || 'AxEdtString';
        const edtTypeNormMap = {
            string: 'AxEdtString',
            integer: 'AxEdtInt',
            int: 'AxEdtInt',
            int64: 'AxEdtInt64',
...
\t<TableReferences />${stringSize}
     ...
     */
    static generateAxEdtExtensionXml(name, properties) {
     ...
     */
    static generateAxDataEntityViewExtensionXml(name, properties) {
     ...
     */
    static generateAxMenuItemExtensionXml(rootElement, name, properties) {
     ...
     */
    static generateAxEnumExtensionXml(name, properties) {
        ...
        if (enumValueSpecs.length === 0) {
            ...
            for (const v of enumValueSpecs) {
                ...
                if (v.value !== undefined && v.value !== 0)
                    enumValuesXml += `\n\t\t\t<Value>${v.value}</Value>`;
                enumValuesXml += `\n\t\t</AxEnumValue>`;
            }
            enumValuesXml += '\n\t</EnumValues>';
...
\t<ValueModifications />
     ...
     */
    static generateAxTableExtensionXml(name, properties) {
        // ── Fields ───────────────────────────────────────────────────────────────
        const fieldSpecs = Array.isArray(properties?.fields) ? properties.fields : [];
        let fieldsXml;
        if (fieldSpecs.length === 0) {
            ...
            for (const f of fieldSpecs) {
                ...
                if (f.edt)
                    fieldsXml += `\t\t\t<ExtendedDataType>${f.edt}</ExtendedDataType>\n`;
                if (f.label)
                    fieldsXml += `\t\t\t<Label>${escapeXml(f.label)}</Label>\n`;
                if (f.mandatory)
                    fieldsXml += `\t\t\t<Mandatory>Yes</Mandatory>\n`;
                if (f.enumType)
                    fieldsXml += `\t\t\t<EnumType>${f.enumType}</EnumType>\n`;
                fieldsXml += `\t\t</AxTableField>\n`;
            }
            fieldsXml += '\t</Fields>';
        }
        // ── FieldGroups (new groups defined in this extension) ───────────────────
        const fgSpecs = Array.isArray(properties?.fieldGroups) ? properties.fieldGroups : [];
        let fieldGroupsXml;
        if (fgSpecs.length === 0) {
            ...
            for (const fg of fgSpecs) {
                ...
                if (fg.label)
                    fieldGroupsXml += `\t\t\t<Label>${escapeXml(fg.label)}</Label>\n`;
        ...
        const fgeSpecs = Array.isArray(properties?.fieldGroupExtensions) ? properties.fieldGroupExtensions : [];
        let fieldGroupExtensionsXml;
        ...
        // ── Indexes ──────────────────────────────────────────────────────────────
        const idxSpecs = Array.isArray(properties?.indexes) ? properties.indexes : [];
        let indexesXml;
        if (idxSpecs.length === 0) {
            ...
            for (const idx of idxSpecs) {
                ...
                if (idx.allowDuplicates !== undefined)
                    indexesXml += `\t\t\t<AllowDuplicates>${idx.allowDuplicates ? 'Yes' : 'No'}</AllowDuplicates>\n`;
                if (idx.alternateKey)
                    indexesXml += `\t\t\t<AlternateKey>Yes</AlternateKey>\n`;
                ...
                const idxFields = (Array.isArray(idx.fields) ? idx.fields : [])
                    ...
                    .filter((f) => typeof f.fieldName === 'string' && f.fieldName.length > 0);
                if (idxFields.length === 0) {
                    ...
                    for (const f of idxFields) {
                        ...
                        if (f.direction)
                            indexesXml += `\t\t\t\t\t<Direction>${f.direction}</Direction>\n`;
                        indexesXml += `\t\t\t\t</AxTableIndexField>\n`;
                    }
                    indexesXml += `\t\t\t</Fields>\n`;
                }
        ...
        // ── Relations ────────────────────────────────────────────────────────────
        const relSpecs = Array.isArray(properties?.relations) ? properties.relations : [];
        let relationsXml;
...
`;
                ...
                const constraints = (Array.isArray(rel.constraints) ? rel.constraints : [])
                    ...
                    .filter((c) => typeof c.field === 'string' && typeof c.relatedField === 'string');
...
${relationsXml}
     ...
     */
    static generateAxFormExtensionXml(name) {
        return `<?xml version="1.0" encoding="utf-8"?>
...
\t<PropertyModifications />
     ...
     */
    static generateAxSecurityPrivilegeXml(name, properties) {
     ...
     */
    static generateAxSecurityDutyXml(name, properties) {
     ...
     */
    static generateAxSecurityRoleXml(name, properties) {
     ...
     */
    static generateAxSecurityDutyExtensionXml(name, properties) {
     ...
     */
    static generateAxSecurityRoleExtensionXml(name, properties) {
     ...
     */
    static generateBusinessEventXml(name, properties) {
        ...
        const source = `[BusinessEvents(classStr(${contractName}),
    '${name}',
    '${name}',
    ${module})]
...
public final class ${contractName} extends BusinessEventsContract
{
     ...
     */
    static generateAxTileXml(name, properties) {
        const label = properties?.label || `@TODO:${name}Label`;
        const helpText = properties?.helpText || `@TODO:${name}HelpText`;
...
\t<RefreshFrequency>600</RefreshFrequency>
     ...
     */
    static generateAxKpiXml(name, properties) {
        const label = properties?.label || `@TODO:${name}Label`;
        const helpText = properties?.helpText || `@TODO:${name}HelpText`;
...
\t<Trend>None</Trend>
     ...
     */
    static generateAxMacroXml(name, sourceCode, properties) {
...
\t<Macros />
     ...
     */
    static generateAxConfigurationKeyXml(name, properties) {
        const label = properties?.label || `@TODO:${name}Label`;
...
\t<Label>${escapeXml(label)}</Label>${parentKey ? `\n\t<ParentKey>${parentKey}</ParentKey>` : ''}${licenseCode ? `\n\t<LicenseCode>${licenseCode}</LicenseCode>` : ''}${tags ? `\n\t<Tags>${escapeXml(tags)}</Tags>` : ''}
     ...
     */
    static generateAxSecurityPolicyXml(name, properties) {
        const label = properties?.label || `@TODO:${name}Label`;
        ...
        const contextType = assertKnownEnumValue(`Security policy '${name}': contextType`, properties?.contextType, SECURITY_POLICY_CONTEXT_TYPES, '');
        const roleName = properties?.roleName || '';
        const constrained = Array.isArray(properties?.constrainedTables) ? properties.constrainedTables : [];
...
\t<ConstrainedTables>${constrainedXml ? `\n${constrainedXml}\n\t` : ''}</ConstrainedTables>
     ...
     * out to be dropped.
     */
    static resolveDefaultAggregate(value, measureName) {
        ...
        if (!resolved) {
            throw new Error(`Measure "${measureName}": DefaultAggregate "${value}" is not a legal aggregation. ` +
     ...
     */
    static generateAxAggregateMeasurementXml(name, properties) {
        const usage = properties?.usage || 'StagedEntityStore';
        const groups = Array.isArray(properties?.measureGroups) ? properties.measureGroups : [];
...
\t\t\t\t\t<DefaultAggregate>${XmlTemplateGenerator.resolveDefaultAggregate(measure.defaultAggregate ?? measure.aggregateFunction, measure.name)}</DefaultAggregate>
...
\t<MeasureGroups>${groupXml ? `\n${groupXml}\n\t` : ''}</MeasureGroups>
     ...
     */
    static generateAxLicenseCodeXml(name, properties) {
        const label = properties?.label || `@TODO:${name}Label`;
        ...
        if (publicKey === undefined || publicKey === null || publicKey === '') {
            throw new Error('license-code requires properties.publicKey — the ISV key slot, which must be globally unique ' +
...
\t<PublicKey>${publicKey}</PublicKey>
     ...
     * Generate AxMenu XML.
     */
    static generateAxMenuXml(name, properties) {
        const label = properties?.label || '@TODO:LabelId';
        return `<?xml version="1.0" encoding="utf-8"?>
...
\t<Elements />
     ...
     */
    static generateAxMenuExtensionXml(name) {
        return `<?xml version="1.0" encoding="utf-8"?>
...
\t<PropertyModifications />
     ...
     */
    static generateAxMenuItemXml(itemType, name, properties) {
        ...
        const explicitObjType = properties?.objectType || properties?.targetType;
        let objType;
...
\t<Object>${targetObject}</Object>${objectTypeXml}
     ...
     */
    static sanitizeMenuItemXml(xml) {
        // 1. Ensure xmlns namespace attributes on root element
        xml = xml.replace(/<(AxMenuItem(?:Action|Display|Output))(\s[^>]*)?>/, (_match, tag, attrs) => {
            let a = attrs || '';
 ...
 */
async function normalizeCreatedArtifactEol(filePath) {
    try {
 ...
 */
export function normalizeEnumValuesAlias(objectType, properties) {
    if (objectType !== 'enum' && objectType !== 'enum-extension')
        return;
    if (!properties)
        return;
    if (properties.enumValues !== undefined)
        return;
    if (Array.isArray(properties.values))
        properties.enumValues = properties.values;
}
/**
 ...
 */
function rawLabelBpWarning(properties, objectName) {
 ...
 */
export function sourceAsWritten(sourceCode, finalObjectName) {
    if (!sourceCode)
        return sourceCode;
    try {
 ...
 */
function extensibleEnumOrderingWarning(objectType, properties, enumName) {
    if (objectType !== 'enum')
        return '';
    if (!properties?.isExtensible)
        return '';
 ...
 */
async function reconcileCreatedTableProperties(filePath, properties) {
    if (!filePath || !properties || typeof properties !== 'object')
        return '';
    try {
        const onDisk = await fs.readFile(filePath, 'utf-8');
        const reconciled = reconcileTableCreateProperties(onDisk, properties);
 ...
 */
export async function handleCreateD365File(request, context) {
    ...
    if (args.objectType.endsWith('-extension')) {
        const groundingError = enforceGrounding(args.groundingToken, `d365fo_file(action="create", objectType="${args.objectType}", objectName="${args.objectName}")`, args.objectName);
        if (groundingError)
    ...
    const referenceError = gateOnReferenceErrors(args.sourceCode, context?.symbolIndex, `d365fo_file(action="create", objectType="${args.objectType}", objectName="${args.objectName}")`);
    if (referenceError)
    ...
    try {
        ...
        console.error(`[create_d365fo_file] Initial modelName: ${actualModelName}`);
        ...
        if (!projectPathToUse && !solutionPathToUse) {
            ...
            if (projectPathToUse) {
                console.error(`[create_d365fo_file] Using projectPath (auto-detected or from .mcp.json): ${projectPathToUse}`);
            }
            else if (solutionPathToUse) {
                console.error(`[create_d365fo_file] Using solutionPath (auto-detected or from .mcp.json): ${solutionPathToUse}`);
            }
        }
        // If projectPath is available, extract model name from it
        if (projectPathToUse) {
            const projectManager = new ProjectFileManager();
            const extractedModelName = await projectManager.extractModelName(projectPathToUse);
            if (extractedModelName) {
                actualModelName = extractedModelName;
                wasAutoExtracted = true;
                console.error(`[create_d365fo_file] Extracted ModelName from projectPath: ${actualModelName}`);
        ...
        // If solutionPath is available, try to find .rnrproj and extract model name
            ...
            const foundProjectPath = await ProjectFileFinder.findProjectInSolution(solutionPathToUse, actualModelName ?? '');
            if (foundProjectPath) {
                const projectManager = new ProjectFileManager();
                const extractedModelName = await projectManager.extractModelName(foundProjectPath);
                if (extractedModelName) {
                    actualModelName = extractedModelName;
                    wasAutoExtracted = true;
                    console.error(`[create_d365fo_file] Extracted ModelName from solutionPath .rnrproj: ${actualModelName}`);
                    // ✨ Register extracted model as custom (since it came from user's project)
                    registerCustomModel(actualModelName);
                }
            }
        ...
        if (!actualModelName) {
            const errorMsg = '❌ ERROR: modelName could not be resolved.\n\n' +
        ...
        if (!projectPathToUse && !solutionPathToUse) {
            console.error(`[create_d365fo_file] ⚠️ WARNING: No projectPath or solutionPath available (not in args, not in .mcp.json)!`);
            console.error(`[create_d365fo_file] ⚠️ Using modelName AS-IS: "${actualModelName}"`);
            console.error(`[create_d365fo_file] ⚠️ If "${actualModelName}" is a Microsoft model (e.g., ApplicationSuite), this will create the file in the WRONG location!`);
            console.error(`[create_d365fo_file] ⚠️ Add projectPath or solutionPath to .mcp.json config to auto-extract correct ModelName from .rnrproj!`);
        ...
        console.error(`[create_d365fo_file] Final ModelName to use: ${actualModelName}${wasAutoExtracted ? ' (auto-extracted ✓)' : ' (as-is, NOT auto-extracted ⚠️)'}`);
        ...
        if (actualModelName && PLACEHOLDER_MODELS.has(actualModelName.toLowerCase())) {
            return {
                content: [
                    {
                        type: 'text',
                        text: `❌ Model name "${actualModelName}" looks like a placeholder — file creation aborted.\n\n` +
        ...
        const finalObjectName = normalizeObjectName(args.objectName, args.objectType, actualModelName, (note) => console.error(`[create_d365fo_file] ${note}`));
        ...
        if (args.packageName) {
            ...
            const roots = [customWritePath, msPath].filter(Boolean);
            ...
            const roots = [customWritePath, configPackagePath].filter(Boolean);
            const resolver = new PackageResolver(roots);
        ...
        console.error(`[create_d365fo_file] Environment: ${envType}, Package: ${resolvedPackageName}, Model: ${actualModelName}, BasePath: ${basePath}`);
        const modelPath = path.join(basePath, resolvedPackageName, actualModelName, objectFolder);
        ...
        if (!resolvedTarget.startsWith(resolvedBase + path.sep) && resolvedTarget !== resolvedBase) {
            throw new Error(`❌ Security error: resolved path "${resolvedTarget}" is outside base directory "${resolvedBase}".\n` +
        ...
        if (driveOrRoot) {
            try {
                ...
                const nonWindowsHint = process.platform !== 'win32'
                    ...
                    : '';
                throw new Error(`❌ Drive or root path does not exist: ${driveOrRoot}\n\n` +
        ...
        try {
            ...
            console.error(`[create_d365fo_file] Failed to create directory:`, mkdirError);
            const hint = (mkdirError instanceof Error && mkdirError.message.includes('\\?'))
                ...
                : '';
            throw new Error(`Failed to create directory ${directory}: ${mkdirError instanceof Error ? mkdirError.message : 'Unknown error'}${hint}`);
        }
        ...
        if (fileExisted) {
            if (!args.overwrite) {
                ...
                const projectNote = await registerFileInActiveProject(args.objectType, finalObjectName, actualModelName, projectPathToUse);
                return {
                    content: [
                        {
                            type: 'text',
        ...
        if (!args.xmlContent && !skipBridgeForEnum && context?.bridge && actualModelName && canBridgeCreate(args.objectType)) {
            try {
                ...
                const scalarProperties = args.properties
                ...
                if ((args.objectType === 'table' || args.objectType === 'table-extension') && args.properties) {
                    const props = args.properties;
                    if (props.fields)
                        bridgeParams.fields = normalizeFieldSpecsForBridge(props.fields);
                    if (props.fieldGroups)
                        bridgeParams.fieldGroups = props.fieldGroups;
                    if (props.indexes)
                        bridgeParams.indexes = normalizeIndexSpecsForBridge(props.indexes);
                    if (props.relations)
                        bridgeParams.relations = props.relations;
                    if (props.methods) {
                        bridgeParams.methods = props.methods.map(m => ({
                            ...m,
                            source: m.source !== undefined ? xppMethodSourceForXml(m.source) : m.source,
                        }));
                    }
                ...
                if ((args.objectType === 'table' || args.objectType === 'table-extension') &&
                    args.sourceCode &&
                    !args.sourceCode.trim().startsWith('{') &&
                    !bridgeParams.methods) {
                    const parsedTableSource = XmlTemplateGenerator.parseSourceForBridge(args.sourceCode, finalObjectName);
                ...
                if ((args.objectType === 'table' || args.objectType === 'table-extension') && args.properties) {
                    ...
                    if (bridgeParams.fields && bridgeParams.fields.length > 0) {
                        ...
                        const fieldsToResolve = bridgeParams.fields.filter(f => {
                            const edt = f.edt;
                            if (!edt || f.type)
                                return false;
                        ...
                        await Promise.all([...new Set(fieldsToResolve.map(f => f.edt))].map(async (edt) => {
                ...
                if ((args.objectType === 'enum' || args.objectType === 'enum-extension') && args.properties) {
                    const props = args.properties;
                    const enumVals = (props.enumValues ?? props.values);
                    if (enumVals)
                        bridgeParams.values = enumVals;
                }
                // For views: pass fields from properties
                if (args.objectType === 'view' && args.properties) {
                    const props = args.properties;
                    if (props.fields)
                        bridgeParams.fields = props.fields;
                }
                ...
                if (args.objectType === 'table') {
                    const rawTableProps = args.properties ?? {};
                    ...
                    const smartExtraProperties = scalarProperties
                        ? Object.fromEntries(Object.entries(scalarProperties).filter(([k]) => !['tableGroup', 'tableType', 'label'].includes(k)))
                        : undefined;
                    try {
                        ...
                        if (isBridgeFailure(smartAttempt))
                            bridgeFailure = smartAttempt;
                        ...
                        if (smartResult?.success && smartResult.filePath) {
                            ...
                            if (args.addToProject !== false) {
                                if (projectPathToUse) {
                                    try {
                                        const projectManager = new ProjectFileManager();
                                        await projectManager.addToProject(projectPathToUse, args.objectType, finalObjectName, smartResult.filePath);
                                    ...
                                    try {
                                        const detectedPath = await ProjectFileFinder.findProjectInSolution(solutionPathToUse, actualModelName);
                                        if (detectedPath) {
                                            const projectManager = new ProjectFileManager();
                                            await projectManager.addToProject(detectedPath, args.objectType, finalObjectName, smartResult.filePath);
                            ...
                            const honestyReport = await reconcileCreatedTableProperties(smartResult.filePath, args.properties);
                            ...
                            const verifyNote = renderWriteVerification(await verifyWrittenFile(smartResult.filePath, projectPathToUse, membershipOf(args.objectType, finalObjectName, actualModelName)));
                            const bpNote = await runInlineBpCheck(args.bpCheck, args.objectType, finalObjectName, context);
                            return {
                                content: [
                                    {
                                        type: 'text',
                        ...
                        console.error(`[create_d365fo_file] createSmartTable returned ${JSON.stringify(smartResult)} — falling back to generic bridge create`);
                    }
                    catch (smartErr) {
                        console.error(`[create_d365fo_file] createSmartTable failed, falling back to generic bridge create: ${smartErr}`);
                    }
                }
                const createAttempt = await timer.time('C# bridge Create()', () => bridgeCreateObject(context.bridge, bridgeParams));
                if (isBridgeFailure(createAttempt))
                    bridgeFailure = createAttempt;
                ...
                if (bridgeResult?.success && bridgeResult.filePath) {
                    ...
                    if (args.addToProject !== false) {
                        if (projectPathToUse) {
                            try {
                                const projectManager = new ProjectFileManager();
                                await projectManager.addToProject(projectPathToUse, args.objectType, finalObjectName, bridgeResult.filePath);
                                projectMsg = `\n✅ Added to project: ${path.basename(projectPathToUse)}`;
                            }
                            catch (projErr) {
                                projectMsg = `\n⚠️ Could not add to project: ${projErr}`;
                            ...
                            try {
                                ...
                                if (detectedPath) {
                                    const projectManager = new ProjectFileManager();
                                    await projectManager.addToProject(detectedPath, args.objectType, finalObjectName, bridgeResult.filePath);
                                    projectMsg = `\n✅ Added to project: ${path.basename(detectedPath)}`;
                                }
                                else {
                                    projectMsg = `\n⚠️ Could not find .rnrproj for model '${actualModelName}' in ${solutionPathToUse}`;
                    ...
                    const indexNote = await timer.time('symbol index upsert', () => upsertWrittenFileIntoIndex(bridgeResult.filePath, context));
                    // Verify the write — see the smart-table path above.
                    const verifyNote = renderWriteVerification(await timer.time('write verification', () => verifyWrittenFile(bridgeResult.filePath, projectPathToUse, membershipOf(args.objectType, finalObjectName, actualModelName))));
                    const bpNote = await timer.time('inline BP check', () => runInlineBpCheck(args.bpCheck, args.objectType, finalObjectName, context));
        ...
        if (args.objectType === 'view' &&
            ...
            const queryFile = path.join(path.dirname(modelPath), 'AxQuery', `${String(args.properties.query)}.xml`);
            try {
                ...
                console.error(`[create_d365fo_file] ⚠️ Could not read query '${args.properties.query}' at ${queryFile} — ` +
        ...
        let xmlContent = args.xmlContent
            ? args.xmlContent
            : XmlTemplateGenerator.generate(args.objectType, finalObjectName, args.sourceCode, effectiveProperties);
        ...
        if (args.xmlContent && /&lt;\?xml|&lt;Ax\w/.test(args.xmlContent) && !args.xmlContent.trimStart().startsWith('<')) {
            throw new Error('xmlContent appears to be HTML-entity-escaped (contains "&lt;" but does not start with a literal "<"). ' +
        ...
        if (finalObjectName !== args.objectName && (args.xmlContent || args.sourceCode)) {
            ...
            const classPattern = new RegExp(`\\b(public\\s+|private\\s+|protected\\s+|internal\\s+|final\\s+)?class\\s+${escapedOrig}\\b`, 'g');
            ...
            if (replaced !== xmlContent) {
                console.error(`[create_d365fo_file] ✅ Fixed class name inconsistency: ` +
        ...
        xmlContent = xmlContent.replace(/<\/Method>\n(\t*)<Method>/g, '</Method>\n\n$1<Method>');
        ...
        if (args.objectType === 'form') {
            const gate = await gateOnFormPatternErrors(xmlContent, `d365fo_file(action="create", form ${finalObjectName})`);
            if (gate.blocked) {
                return gate.blocked;
            }
        ...
        console.error(`[create_d365fo_file] XML content (${xmlSource}): ${xmlContent.length} bytes`);
        console.error(`[create_d365fo_file] XML preview: ${xmlContent.substring(0, 200)}...`);
        ...
        try {
            ...
            if (errorMessage.includes('EINVAL') || errorMessage.includes('ENOENT')) {
                throw new Error(`Failed to write file to ${normalizedFullPath}.\n\n` +
                    `Possible causes:\n` +
        ...
        console.error(`[create_d365fo_file] ✅ Written: ${normalizedFullPath}  (${fileSizeKb} KB)`);
        ...
        bridgeValidateAfterWrite(context?.bridge, args.objectType, finalObjectName).then(validationMsg => {
            if (validationMsg) {
        ...
        if (args.addToProject) {
            ...
            if (!projectPath && solutionPathToUse) {
                ...
                console.error(`[create_d365fo_file] Searching for .rnrproj in solution: ${solutionPathToUse}, model: ${actualModelName}`);
                const detectedPath = await ProjectFileFinder.findProjectInSolution(solutionPathToUse, actualModelName);
                if (!detectedPath) {
                    console.error(`[create_d365fo_file] No .rnrproj found in solution directory`);
                    ...
                    console.error(`[create_d365fo_file] Found project file: ${detectedPath}`);
            ...
            if (projectPath) {
                try {
                    ...
                    // Add to project
                    const projectManager = new ProjectFileManager();
                    const wasAdded = await projectManager.addToProject(projectPath, args.objectType, finalObjectName, absoluteXmlPath);
                    if (wasAdded) {
                    ...
                    console.error(`[create_d365fo_file] Failed to add to project:`, projectError);
        ...
        const verifyNote = renderWriteVerification(await verifyWrittenFile(normalizedFullPath, args.addToProject ? projectPathToUse : undefined, membershipOf(args.objectType, finalObjectName, actualModelName)));
        const bpNote = await runInlineBpCheck(args.bpCheck, args.objectType, finalObjectName, context);
...
//# sourceMappingURL=createD365File.js.map