...
import { UNKNOWN_PARAMETER_LIST, parseXppDeclaration, renderMethodSignature, } from '../../metadata/xppDeclaration.js';
import { getModelVisibility, packagesRootFromPath, } from '../../metadata/modelDescriptor.js';
import { getConfigManager } from '../../utils/configManager.js';
export const resolveReferencesArgsSchema = z.object({
    code: z.string().describe('X++ source code to resolve. Paste the full generated method/class text.'),
...
function symbolTypes(deps, name) {
    try {
...
function menuItemExists(deps, name) {
    try {
        const exact = deps.db
...
function findMethod(deps, ownerName, methodName, depth = 0) {
    if (depth > 10)
        return undefined;
    try {
        ...
        const row = deps.db.prepare(`SELECT signature, source FROM symbols
       WHERE parent_name = ? AND type = 'method' AND name = ? COLLATE NOCASE
       LIMIT 1`).get(owner, methodName);
        if (row)
            return row;
        // Extension-added methods (CoC wrappers, augmentation classes)
        let extRows = deps.db.prepare(`SELECT added_methods, coc_methods FROM extension_metadata
       WHERE base_object_name = ?`).all(owner);
        if (extRows.length === 0 && !ownerHit) {
            ...
            extRows = deps.db.prepare(`SELECT added_methods, coc_methods FROM extension_metadata
         WHERE base_object_name = ? COLLATE NOCASE`).all(ownerName);
        }
        ...
        for (const ext of extRows) {
            for (const col of [ext.added_methods, ext.coc_methods]) {
                if (!col)
                    continue;
                try {
                    const names = JSON.parse(col);
                    if (names.some(m => (typeof m === 'string' ? m : m?.name ?? '')
                        .toLowerCase() === target)) {
 ...
 */
function checkModelVisibility(deps, typeName, line) {
    ...
    for (const hit of hits) {
        ...
        if (!pkg)
            return undefined; // cannot tell
        if (vis.visiblePackages.has(pkg.toLowerCase()))
            return undefined; // reachable
        packages.add(pkg);
    }
    return {
        ...
        line,
        identifier: typeName,
        detail: `"${typeName}" is indexed, but only in package ${[...packages].join(' / ')}, which model ` +
...
function fieldExists(deps, tableName, fieldName) {
    if (TABLE_SYSTEM_FIELDS.has(fieldName.toLowerCase()))
        return true;
    try {
        ...
        const row = deps.db.prepare(`SELECT 1 AS x FROM symbols
       WHERE parent_name = ? AND type = 'field' AND name = ? COLLATE NOCASE
       LIMIT 1`).get(table, fieldName);
        if (row !== undefined)
            return true;
        let extRows = deps.db.prepare(`SELECT added_fields FROM extension_metadata
       WHERE base_object_name = ? AND extension_type = 'table-extension'`).all(table);
        if (extRows.length === 0 && !tableHit) {
            extRows = deps.db.prepare(`SELECT added_fields FROM extension_metadata
         WHERE base_object_name = ? COLLATE NOCASE AND extension_type = 'table-extension'`).all(tableName);
        }
        ...
        for (const ext of extRows) {
            if (!ext.added_fields)
                continue;
            try {
                const names = JSON.parse(ext.added_fields);
                if (names.some(f => (typeof f === 'string' ? f : f?.name ?? '')
                    .toLowerCase() === target)) {
                    return true;
                }
            }
...
function parseSignatureArity(signature) {
    ...
    if (inner === UNKNOWN_PARAMETER_LIST)
        return undefined;
    if (inner === '')
        return { min: 0, max: 0 };
 ...
 */
function arityOf(method, methodName) {
    if (method.source) {
...
function extractCallArgs(code, openIdx) {
    let depth = 0;
...
function collectLocals(cleaned) {
    ...
    for (const m of cleaned.matchAll(declRe)) {
        const typeName = m[2];
        if (DECL_STOPWORDS.has(typeName.toLowerCase()))
            continue;
        for (const varName of m[3].split(',').map(v => v.trim())) {
            if (!varName || XPP_KEYWORDS.has(varName.toLowerCase()))
                continue;
    ...
    for (const m of cleaned.matchAll(/\(([^()]*)\)/g)) {
        for (const param of splitTopLevel(m[1])) {
            ...
            if (!pm)
                continue;
            if (XPP_KEYWORDS.has(pm[1].toLowerCase()))
                continue;
...
export function resolveXppReferences(code, deps) {
    const violations = [];
    let verifiedCount = 0;
    const { cleaned, strings } = cleanCode(code);
    const locals = collectLocals(cleaned);
    const typeExistsCache = new Map();
    const lookupTypes = (name) => {
    ...
    const visibilityVerdicts = new Map();
    const reportIfInvisible = (name, line) => {
        if (!deps.visibility)
            return false;
        const lower = name.toLowerCase();
        if (XPP_BUILTIN_TYPES.has(lower) || KERNEL_TYPES.has(lower))
            return false;
    ...
    for (const m of cleaned.matchAll(intrinsicRe)) {
        ...
        if (expected === undefined)
            continue; // not an intrinsic we know (e.g. subStr)
        const target = m[2];
        const member = m[3];
        const line = lineOf(cleaned, m.index ?? 0);
        if (locals.declaredNames.has(target.toLowerCase())) {
            verifiedCount++;
            continue;
        }
    ...
    for (const m of cleaned.matchAll(staticRe)) {
        ...
        if (KERNEL_TYPES.has(lower)) {
            verifiedCount++;
            continue;
        } // no metadata for kernel statics
        ...
        if (reportIfInvisible(typeName, line))
            continue;
                ...
                if (argsText !== undefined) {
                    ...
                    if (n < arity.min || n > arity.max) {
                        violations.push({
                            ...
                            line,
                            identifier: `${typeName}::${member}`,
                            detail: `Call passes ${n} argument(s), but the declaration expects ${arity.min === arity.max ? arity.min : `${arity.min}–${arity.max}`}: ${shown}`,
                        });
                    }
                }
            }
    ...
    for (const [, typeName] of locals.bindings) {
        const lower = typeName.toLowerCase();
        if (reportedTypes.has(lower))
            continue;
        reportedTypes.add(lower);
        if (isKnownType(typeName)) {
            if (!reportIfInvisible(typeName, 0))
                verifiedCount++;
        }
        else {
            violations.push({
 ...
 */
export function resolverModelVisibility() {
    try {
 ...
 */
function checkLabelPlaceholders(hits, identifier, cleaned, index, code) {
    ...
    if (needed === 0 && supplied !== null && supplied > 0) {
        return {
            ...
            line,
            identifier,
            detail: `${identifier} is ${quoted} — no %1 placeholder, so the ${supplied} strFmt argument(s) are discarded. ` +
    ...
    if (needed > 0 && supplied === null) {
        return {
            ...
            line,
            identifier,
            detail: `${identifier} is ${quoted} — it takes ${needed} argument(s) and must be wrapped: ` +
    ...
    if (needed > 0 && supplied !== null && supplied !== needed) {
        return {
            ...
            line,
            identifier,
            detail: `${identifier} is ${quoted} — it takes ${needed} argument(s), strFmt supplies ${supplied}.`,
        };
    }
    return null;
}
 ...
 */
function strFmtArgs(cleaned, index) {
    let depth = 0;
    let i = index - 1;
    for (; i >= 0; i--) {
        const c = cleaned[i];
    ...
    if (!callee)
        return null;
    if (/^literalStr$/i.test(callee[1]))
        return strFmtArgs(cleaned, callee.index);
    if (!/^strFmt$/i.test(callee[1]))
        return null;
...
function labelFileExists(deps, fileId) {
    try {
 ...
 */
export function gateOnReferenceErrors(code, symbolIndex, operationDescription) {
    if (process.env.GROUNDING_ENFORCE !== 'true')
        return null;
    if (!code || !symbolIndex)
        return null;
    let result;
    ...
    return {
        ...
        content: [{
                type: 'text',
                text: `❌ Unresolved references in ${operationDescription} (GROUNDING_ENFORCE=true).\n\n` +
...
export async function resolveReferencesTool(request, context) {
    ...
    if (result.violations.length === 0) {
        return {
            content: [{
                    type: 'text',
                    text: `✅ validate_code(references): all ${result.verifiedCount} reference(s) verified against the index${suffix}.\n` +
    ...
    for (const v of result.violations) {
        lines.push(`${v.severity === 'error' ? '❌' : '⚠️'} **${v.kind}**` +
            `${v.line > 0 ? ` (line ${v.line})` : ''}: \`${v.identifier}\``);
        lines.push(`   ${v.detail}`);
        lines.push('');
    }
    if (errors.length > 0) {
        lines.push('**Fix all errors before writing** — these identifiers could not be resolved against the ' +
