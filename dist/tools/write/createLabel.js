...
const CreateLabelArgsSchema = z.object({
    labelId: z
        ...
        .describe('Label identifier — must be unique within the label file. ' +
    ...
    labelFileId: z
        .string()
        .describe('Label file ID to add the label to (e.g. ContosoExt). Must exist in the model, or be creatable ' +
    ...
    translations: z
        ...
        .min(1)
        .describe('Label text for each language. At minimum provide en-US. ' +
        'For languages without a translation the en-US text is used as fallback.'),
    languages: z
        .array(z.string())
        .optional()
        .describe('Restrict which language .label.txt files are written/created. ' +
    ...
    description: z
        .string()
        .optional()
        .describe('Label description written as the comment line in .label.txt. ' +
    ...
    createLabelFileIfMissing: z
        ...
        .optional()
        .default(true)
        .describe('If true (default) and the AxLabelFile does not exist yet, create it with the provided ' +
    ...
    allowExtensionLabelFile: z
        ...
        .optional()
        .default(false)
        .describe('Escape hatch to allow writing into a label file EXTENSION (a labelFileId ' +
    ...
    sortLabels: z
        .boolean()
        .optional()
        .describe('Sort labels alphabetically when writing .label.txt files. ' +
    ...
    overwriteExisting: z
        ...
        .optional()
        .default(false)
        .describe('Update mode (set automatically by labels(action="update")): overwrite the text of an ' +
    ...
    createIfMissing: z
        ...
        .optional()
        .default(false)
        .describe('Upsert-lite for action="create": create the label when it is absent, and when it already ' +
...
function compareLabelIdsOrdinalCI(a, b) {
...
function serializeLabelMap(map, sort = true, eol = '\r\n') {
    const entries = sort
...
function buildAxLabelFileXml(labelFileId, language, packageName, model) {
 ...
 */
function normalizeCreateLabelArgs(raw) {
    if (raw === null || typeof raw !== 'object')
        return {};
    const args = { ...raw };
    // labelFile / labelfile → labelFileId
    if (args.labelFileId === undefined) {
        const alias = args.labelFile ?? args.labelfile ?? args.labelFileID;
        if (typeof alias === 'string')
            args.labelFileId = alias;
    }
 ...
 */
export function extractBulkLabels(raw) {
    if (raw === null || typeof raw !== 'object')
        return null;
    const arr = raw.labels;
    if (!Array.isArray(arr) || arr.length === 0)
        return null;
    return arr.filter((e) => e !== null && typeof e === 'object');
}
/**
 ...
 */
export async function createLabelsBulk(entries, raw, context, runSingle = createLabelTool) {
    ...
    const header = `${failed === 0 ? '✅' : '⚠️'} labels(action="create", labels=[…]): ` +
...
// Tool implementation
export async function createLabelTool(request, context) {
    ...
    if (bulk) {
        return createLabelsBulk(bulk, request.params.arguments, context);
    }
    try {
        const parsed = CreateLabelArgsSchema.safeParse(normalizeCreateLabelArgs(request.params.arguments));
        if (!parsed.success) {
            ...
            const wrongShape = (raw.language !== undefined || raw.labelFileId !== undefined) &&
            ...
            return {
                content: [{
                        type: 'text',
                        text: `❌ labels(action="create"/"update"): invalid arguments — ${issues}.\n` +
        ...
        const { labelId, labelFileId, model, translations, description, defaultComment, packagePath, createLabelFileIfMissing, updateIndex, } = args;
        ...
        if (isExtensionLabelFile(labelFileId) && !args.allowExtensionLabelFile) {
            return {
                content: [
                    {
                        type: 'text',
                        text: `❌ "${labelFileId}" is a label file EXTENSION, not an original label file.\n\n` +
        ...
        try {
            ...
            const existing = symbolIndex.labelsDb
                .prepare(`SELECT label_id, label_file_id, model, text FROM labels
           WHERE label_id IN (${spellings.map(() => '?').join(', ')})
             AND language = 'en-US' AND label_file_id != ?
           LIMIT 10`)
                .all(...spellings, labelFileId);
            if (existing.length > 0) {
                ...
                for (const r of existing) {
                    ...
                    lines.push(`  ${formatLabelReference(r.label_file_id, r.label_id)}  [${r.model}]  "${r.text}"${flag}`);
                }
                if (msCollisions.length > 0) {
                    lines.push('');
                    lines.push('  ⛔ Collision with Microsoft standard label file detected! ' +
        ...
        if (args.packageName) {
            ...
            const roots = [customPath, msPath].filter(Boolean);
            ...
            if (resolved?.rootPath)
                resolvedPackagePath = resolved.rootPath;
        }
        else {
        ...
        const createLangDirectory = async (lang) => {
            ...
            try {
                ...
                await writeFileWithBom(txtPath, '');
            }
        ...
        if (labelFileMissing && !createLabelFileIfMissing) {
            return {
                content: [
                    {
                        type: 'text',
                        text: `AxLabelFile "${labelFileId}" not found in model "${model}" ` +
            ...
            if (!modelDirExists) {
                return {
                    content: [
                        {
                            type: 'text',
                            text: `❌ Cannot create label file "${labelFileId}" — the model directory does not exist at the resolved path:\n` +
                                `    ${modelDir}\n\n` +
        ...
        if (written.length === 0 && skipped.length > 0) {
            ...
            if (crossModelNotice)
                skipLines.push(crossModelNotice.trim());
