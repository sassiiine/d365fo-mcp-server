...
import { checkAddControlAgainstParentPattern, checkAddControlAgainstDataGroup, findDataGroupRenderers, listDataGroupRenderers, isFormPatternEnforceEnabled, } from '../analysis/validateFormPattern.js';
...
import { getRequiredParams, renderOpSpec, OP_PARAM_ALIASES, findIgnoredParams, renderIgnoredParamsWarning, findMissingMutationParams, } from '../specs/d365foFileOpSpecs.js';
...
import { crossModelWriteRefusal, standDownNotice, baseObjectOf, } from '../../utils/crossModelWriteGuard.js';
 ...
 */
export function describeBridgeFallbackReason(bridge, objectType, operation, bridgeResult) {
...
async function readForMatching(filePath) {
    try {
...
function occurrenceLines(content, needle) {
    const lines = [];
    let from = 0;
    for (;;) {
...
export function preflightReplaceCode(content, oldCode, newCode) {
    ...
    if (hits.length === 0) {
        ...
        if (normNew.length > 0 && content.includes(normNew)) {
            return {
                kind: 'noop',
                message: `Nothing to do — the file already contains newCode ` +
    ...
    if (normNew.includes(normOld) && content.includes(normNew)) {
        return {
            kind: 'noop',
            message: `Nothing to do — this edit is already applied, and repeating it would nest the text.\n` +
    ...
    if (hits.length > 1) {
        return {
            kind: 'refuse',
            message: `⛔ replace-code refused — oldCode matches ${hits.length} times (lines ${hits.join(', ')}).\n` +
 ...
 */
export function renderChangedLines(before, after, context = 3) {
    if (before === after)
        return '';
    const b = before.split('\n');
    const a = after.split('\n');
    let head = 0;
    while (head < b.length && head < a.length && b[head] === a[head])
        head++;
    let tail = 0;
    while (tail < b.length - head &&
        tail < a.length - head &&
 ...
 */
const directXmlReplaceCode = serializedOnFile(async (filePath, oldCode, newCode, reason) => {
    try {
 ...
 */
const directXmlModifyProperty = serializedOnFile(async (filePath, propertyPath, propertyValue, reason) => {
    try {
        const rawContent = await fs.readFile(filePath, 'utf-8');
        ...
        if (!/^[A-Za-z_][A-Za-z0-9_.:-]*$/.test(tagName)) {
            return {
                success: false,
                message: `❌ directXmlModifyProperty: '${propertyPath}' does not name a plain XML element ` +
        ...
        if (totalMatches === 0) {
            ...
            return {
                success: true,
                message: `✅ Property '${propertyPath}'='${propertyValue}' added via direct XML fallback ` +
        ...
        if (openMatches.length === 1) {
            ...
            if (inner.includes('<')) {
                return {
                    success: false,
                    message: `❌ directXmlModifyProperty: <${tagName}> in ${filePath} contains child elements — ` +
 ...
 */
const directXmlAddMenuItemToMenu = serializedOnFile(async (filePath, menuItemToAdd, menuItemToAddType) => {
    try {
        const rawContent = await fs.readFile(filePath, 'utf-8');
        const content = rawContent.replace(/^﻿/, '').replace(/\r\n/g, '\n');
        ...
        // unambiguous).
        const newElement = `\t\t<AxMenuElement xmlns="" i:type="AxMenuElementMenuItem">\n` +
 ...
 */
function viaXmlFallback(result) {
    return (result ? { ...result, viaXmlFallback: true } : result);
}
 ...
 */
const directXmlAddControl = serializedOnFile(async (filePath, controlName, parentControl, controlType, dataSource, dataField, label, previousSibling, positionType) => {
    try {
        const rawContent = await fs.readFile(filePath, 'utf-8');
        const content = rawContent.replace(/^﻿/, '').replace(/\r\n/g, '\n');
        ...
        return {
            success: true,
            message: `✅ Control '${controlName}' (${iType}) added to '${parentControl}' via direct XML fallback ` +
 ...
 */
export function coerceNoYesFlag(value) {
 ...
 * dropped (cluster #35).
 */
const directXmlAddIndex = serializedOnFile(async (filePath, indexName, fields, allowDuplicates, alternateKey) => {
    ...
    if (namedFields.length === 0) {
        return {
            success: false,
            message: `Index '${indexName}' has no fields — pass indexFields as [{ fieldName: "<field>" }]. ` +
    ...
    try {
        ...
        if (!/<AxTable\b/.test(content) && !/<AxTableExtension\b/.test(content))
            return null;
        ...
        const fieldElements = namedFields
            .map(f => `\t\t\t\t<AxTableIndexField>\n` +
            `\t\t\t\t\t<DataField>${f}</DataField>\n` +
            `\t\t\t\t</AxTableIndexField>`)
            .join('\n');
        const newElement = `\t\t<AxTableIndex>\n` +
            `\t\t\t<Name>${indexName}</Name>\n` +
 ...
 */
export function findTopLevelCollection(content, rootElement, collection) {
 ...
 */
function upsertDataEntityFieldGroupExtension(content, groupName, fieldName) {
    ...
    const blockRe = new RegExp(`<AxTableFieldGroupExtension>\\s*<Name>${escapeRe(groupName)}</Name>[\\s\\S]*?</AxTableFieldGroupExtension>`);
    ...
    const newGroup = `\t\t<AxTableFieldGroupExtension>\n` +
        `\t\t\t<Name>${groupName}</Name>\n` +
        `\t\t\t<Fields>\n${entry}\n\t\t\t</Fields>\n` +
 ...
 */
const directXmlAddDataEntityExtensionField = serializedOnFile(async (filePath, fieldName, dataField, dataSource, label, fieldGroupName) => {
    try {
        const rawContent = await fs.readFile(filePath, 'utf-8');
        const content = rawContent.replace(/^﻿/, '').replace(/\r\n/g, '\n');
        // Only data-entity extensions carry a <Fields> collection of mapped fields
        // shaped like this — bail on any other file shape.
        if (!/<AxDataEntityViewExtension\b/.test(content))
            return null;
        ...
        return {
            success: true,
            message: `✅ Mapped field '${fieldName}' (${dataSource}.${dataField}) added via direct XML fallback ` +
 ...
 * DeleteAction.
 */
const directXmlDeleteAction = serializedOnFile(async (filePath, mode, name, table, deleteAction) => {
    try {
        const rawContent = await fs.readFile(filePath, 'utf-8');
        const content = rawContent.replace(/^﻿/, '').replace(/\r\n/g, '\n');
 ...
 */
export const directXmlEnsureRelationProperties = serializedOnFile(async (filePath, relationName, cardinality, relatedTableCardinality, relationshipType) => {
    try {
        const rawContent = await fs.readFile(filePath, 'utf-8');
        const content = rawContent.replace(/^﻿/, '').replace(/\r\n/g, '\n');
        ...
        for (const m of content.matchAll(relRegex)) {
            if (new RegExp(`<Name>${relationName}</Name>`).test(m[0])) {
                block = m[0];
                break;
            }
        }
        ...
        if (!/<Cardinality>/.test(patched)) {
            patched = patched.replace(new RegExp(`(<Name>${relationName}</Name>)`), `$1\n${indent}<Cardinality>${cardinality}</Cardinality>`);
        ...
        if (applied.length === 0 || patched === block)
            return { applied: [] };
        const updated = content.replace(block, patched);
        await writeFileAtomic(filePath, normalizeD365Xml(updated));
        console.error(`[modify_d365fo_file] ✅ directXmlEnsureRelationProperties: ${applied.join(', ')} on '${relationName}' in ${filePath}`);
        return { applied };
    }
    catch (err) {
...
const ModifyD365FileArgsSchema = z.object({
    ...
    parentControl: z.string().optional().describe('Name of the existing parent control/tab/group to insert into. ' +
    ...
    positionType: z.string().optional().describe('Optional positioning: AfterItem (needs previousSibling) | Begin | End. Omit to append at the ' +
        'end of the parent. Other values are refused — these are the ones D365FO metadata carries.'),
    previousSibling: z.string().optional().describe('Name of the sibling control to position after. Implies positionType=AfterItem when that is omitted.'),
    ...
    indexAllowDuplicates: z.union([z.boolean(), z.string()]).optional().describe('Whether index allows duplicates (default: false = unique). Accepts true/false or the XML spelling "Yes"/"No".'),
    indexAlternateKey: z.union([z.boolean(), z.string()]).optional().describe('Whether index is an alternate key. Accepts true/false or the XML spelling "Yes"/"No".'),
    indexEnabled: z.union([z.boolean(), z.string()]).optional().describe('Whether index is enabled (default: true). Accepts true/false or the XML spelling "Yes"/"No".'),
    ...
    dataField: z.string().optional().describe('Source table field name for add-field on a data-entity-extension (e.g. "MyField"). Required alongside dataSource.'),
    dataSource: z.string().optional().describe('Source data-source/table name on the entity for add-field on a data-entity-extension (e.g. "MyTable"). Required alongside dataField.'),
    // For modify-property
    ...
    // Options
    autoCorrect: z.boolean().optional().default(true).describe('Apply a correction the server has already fully determined (exactly one valid reading, derivable ' +
    ...
    // to contain it.
    addToProject: z.boolean().optional().default(true).describe('Add the modified file to the active .rnrproj when it is not already listed there. ' +
    ...
    // its warnings.
    peerOperations: z.array(z.string()).optional().describe('Internal: the operation names travelling in the same operations[] batch.'),
});
    ...
    try {
        ...
        if (missingMutation.length > 0) {
            return {
                content: [{
                        type: 'text',
                        text: `❌ '${args.operation}' was called with no parameter that changes anything — nothing would be written.\n` +
        ...
        const { objectType, operation, createBackup, modelName, workspacePath, filePath: explicitFilePath, } = args;
        ...
        if (operation === 'add-control' &&
            ...
            if (baseXml) {
                const dgVerdict = await checkAddControlAgainstDataGroup(baseXml, args.parentControl, args.controlDataField, args.controlName);
                if (dgVerdict) {
                    const field = args.controlDataField;
                    const onTable = dgVerdict.dataSource ? ` on \`${dgVerdict.dataSource}\`` : '';
                    if (isFormPatternEnforceEnabled()) {
                        return {
                            content: [{
                                    type: 'text',
                                    text: `⛔ add-control blocked — parent control "${args.parentControl}" renders field group ` +
        ...
        if (autoCorrect) {
            const retargetNote = await resolveFieldNameForFieldGroup(args, objectType, operation, containment.modelSegment || resolvedModelFromPath || modelName || getConfigManager().getModelName() || '', actualFilePath, symbolIndex);
            if (retargetNote)
                noteAutoCorrection(autoCorrectNotes, retargetNote);
        }
        ...
        _bridgeRetry: do {
            ...
            switch (operation) {
                ...
                case 'add-field': {
                    ...
                    if (objectType === 'data-entity-extension' && args.fieldName) {
                        ...
                        if (!dataField || !dataSource) {
                            return {
                                content: [{
                                        type: 'text',
                                        text: `❌ add-field on a data-entity-extension needs BOTH dataField and dataSource — ` +
                                            `nothing was written.\n` +
                        ...
                        bridgeResult = await bridgeAddField(context.bridge, objectName, args.fieldName, '', // no base type — the mapped-field path ignores it
                        undefined, // no EDT
                        args.fieldMandatory, args.fieldLabel, { dataField, dataSource, fieldGroupName: args.fieldGroupName });
                        ...
                        if (!bridgeResult || !bridgeResult.success) {
                            const xmlFallbackResult = await directXmlAddDataEntityExtensionField(actualFilePath, args.fieldName, dataField, dataSource, args.fieldLabel, args.fieldGroupName);
                    ...
                    let enumTypeArg = args.fieldEnumType?.trim() || undefined;
                    ...
                    if (args.fieldType && /^Ax(Table|View|Query|Map|DataEntityView)[A-Za-z]*Field[A-Za-z0-9]*$/i.test(args.fieldType)) {
                        ...
                        const derivedEnum = autoCorrect
                            ...
                            : undefined;
                        if (derivedEnum) {
                            noteAutoCorrection(autoCorrectNotes, `fieldType="${args.fieldType}" is an XML element name; treated as fieldEnumType="${derivedEnum}". ` +
                ...
                case 'add-index': {
                    if (args.indexName) {
                        ...
                        const allowDuplicates = coerceNoYesFlag(args.indexAllowDuplicates);
                        const alternateKey = coerceNoYesFlag(args.indexAlternateKey);
                        bridgeResult = await bridgeAddIndex(context.bridge, objectName, args.indexName, indexFieldNames, allowDuplicates, alternateKey);
                        ...
                        if (!bridgeResult || !bridgeResult.success) {
                            const xmlFallbackResult = await directXmlAddIndex(actualFilePath, args.indexName, indexFieldNames, allowDuplicates, alternateKey);
                            if (xmlFallbackResult)
                                bridgeResult = viaXmlFallback(xmlFallbackResult);
                        }
                    }
                ...
                case 'add-full-text-index': {
                    if (args.indexName) {
                        ...
                        const fullTextFields = Array.isArray(args.indexFields)
                            ? args.indexFields.map((f) => (typeof f === 'string' ? f : f?.fieldName)).filter(Boolean)
                            : undefined;
                        bridgeResult = await bridgeAddFullTextIndex(context.bridge, objectName, args.indexName, fullTextFields);
                    }
                    break;
                }
                case 'remove-full-text-index': {
                    if (args.indexName) {
                        bridgeResult = await bridgeRemoveFullTextIndex(context.bridge, objectName, args.indexName);
                    }
                    break;
                }
                case 'add-table-mapping': {
                    if (args.mapName) {
                        bridgeResult = await bridgeAddTableMapping(context.bridge, objectName, args.mapName, args.mappingTable, args.mappingConnections);
                    }
                    break;
                }
                case 'remove-table-mapping': {
                    if (args.mapName) {
                        bridgeResult = await bridgeRemoveTableMapping(context.bridge, objectName, args.mapName);
                    }
                    break;
                }
                case 'add-relation': {
                    if (args.relationName && args.relatedTable) {
                        ...
                        const relationProperties = {
                            relationCardinality: args.relationCardinality ?? 'ZeroMore',
                            relatedTableCardinality: args.relatedTableCardinality ?? 'ExactlyOne',
                            relationshipType: args.relationshipType ?? 'Association',
                        };
                        bridgeResult = await bridgeAddRelation(context.bridge, objectName, args.relationName, args.relatedTable, constraints, relationProperties);
                        ...
                        if (bridgeResult?.success && !bridgeResult.propertiesWritten) {
                            const relProps = await directXmlEnsureRelationProperties(actualFilePath, args.relationName, relationProperties.relationCardinality, relationProperties.relatedTableCardinality, relationProperties.relationshipType);
                ...
                case 'remove-delete-action': {
                    ...
                    if (daName) {
                        bridgeResult = viaXmlFallback(await directXmlDeleteAction(actualFilePath, operation === 'add-delete-action' ? 'add' : 'remove', daName, args.deleteActionTable, args.deleteActionType));
                    }
                    break;
                }
                ...
                case 'add-field-to-field-group': {
                    if (args.fieldGroupName && args.fieldName) {
                        const groupName = args.fieldGroupName;
                        const extendFlag = args.extendBaseFieldGroup;
                        bridgeResult = await bridgeAddFieldToFieldGroup(context.bridge, objectName, groupName, args.fieldName, extendFlag);
                        // ── AUTO-CORRECT ────────────────────────────────────────────────────
                        ...
                        if (autoCorrect &&
                            ...
                            if (await baseObjectDefinesFieldGroup(baseTable, groupName, symbolIndex)) {
                                const retried = await bridgeAddFieldToFieldGroup(context.bridge, objectName, groupName, args.fieldName, true);
                                if (retried?.success) {
                                    bridgeResult = retried;
                                    noteAutoCorrection(autoCorrectNotes, `'${groupName}' is defined by the base table "${baseTable}"; extended it through ` +
                ...
                case 'modify-property': {
                    if (args.propertyPath && args.propertyValue !== undefined) {
                        ...
                        if (!bridgeResult || !bridgeResult.success) {
                            const xmlFallbackResult = await directXmlModifyProperty(actualFilePath, args.propertyPath, String(args.propertyValue), describeBridgeFallbackReason(context.bridge, objectType, 'modify-property', bridgeResult));
                            if (xmlFallbackResult) {
                                bridgeResult = viaXmlFallback(xmlFallbackResult);
                            }
                        }
                ...
                case 'replace-code': {
                    ...
                    if (hasOldNew) {
                        ...
                        if (beforeContent !== null) {
                            ...
                            if (verdict?.kind === 'noop') {
                                ...
                                return {
                                    content: [{
                                            type: 'text',
                                            text: `✅ ${operation} on ${objectType} "${objectName}" — no change needed.\n\n` +
                                                `**File:** ${actualFilePath}\n${verdict.message}`,
                                        }],
                                };
                            }
                        ...
                        if (!bridgeResult || !bridgeResult.success) {
                            const xmlFallbackResult = await directXmlReplaceCode(actualFilePath, args.oldCode, args.newCode, describeBridgeFallbackReason(context.bridge, objectType, 'replace-code', bridgeResult));
                            if (xmlFallbackResult) {
                                bridgeResult = viaXmlFallback(xmlFallbackResult);
                            }
                        }
                ...
                case 'add-control': {
                    if (args.controlName && args.parentControl) {
                        const resolvedControlType = args.controlType
                            ?? resolveControlTypeForField(args.controlDataSource, args.controlDataField, symbolIndex?.getReadDb?.())
                            ?? 'String';
                        ...
                        if (isFormExtension && (!bridgeResult || !bridgeResult.success)) {
                            const xmlFallbackResult = await directXmlAddControl(actualFilePath, args.controlName, args.parentControl, resolvedControlType, args.controlDataSource, args.controlDataField, args.controlLabel, args.previousSibling, args.positionType);
                            if (xmlFallbackResult)
                                bridgeResult = viaXmlFallback(xmlFallbackResult);
                        }
                    }
        ...
        if (!bridgeResult.success) {
            ...
            if (bridgeResult.viaXmlFallback)
        ...
        if (args.addToProject) {
            ...
            projectMessage = await registerFileInActiveProject(objectType, objectName, modelName || configManager.getModelName() || undefined, resolvedProjectPath);
        }
        ...
        const writtenXpp = args.sourceCode ?? args.methodCode ?? args.newCode;
        ...
        if (operation === 'add-field' && (objectType === 'table' || objectType === 'table-extension')) {
            ...
            if (!groupEntryInBatch) {
                notes.push(`⚠️ BP: a table field must belong to a field group (BPErrorTableFieldNotInFieldGroup). ` +
        ...
        if ((operation === 'add-field-to-field-group' || operation === 'add-field-group') &&
            ...
            if (operation === 'add-field-to-field-group' && args.fieldName) {
                fieldGroupRenderNote = await timer.time('field-group render probe', () => describeFieldGroupRendering(baseTableName, groupName, args.fieldName, symbolIndex));
            }
            if (!fieldGroupRenderNote) {
                fieldGroupRenderNote = await timer.time('field-group reach probe', () => describeUnrenderedFieldGroup(baseTableName, groupName, symbolIndex));
            }
        }
        ...
        // was pure waste.
        const indexNote = await timer.time('symbol index upsert', () => upsertWrittenFileIntoIndex(actualFilePath, context));
        ...
        const verifyProjectPath = args.projectPath || (await getConfigManager().getProjectPath()) || undefined;
        const verifyNote = renderWriteVerification(await timer.time('write verification', () => verifyWrittenFile(actualFilePath, verifyProjectPath, membershipOf(objectType, objectName, modelName || getConfigManager().getModelName()))));
        const bpNote = await timer.time('inline BP check', () => runInlineBpCheck(args.bpCheck, objectType, objectName, context));
        return {
            content: [
                {
                    type: 'text',
                    text: `✅ ${operation} on ${objectType} "${objectName}" — applied via ${bridgeResult.viaXmlFallback
 ...
 */
const EXTENSION_MEMBER_NAME_ARG = {
    'add-field': 'fieldName',
    'add-index': 'indexName',
...
export function applyExtensionMemberPrefix(args, objectType, operation, modelName) {
    ...
    if (lower.startsWith(token.toLowerCase()) || lower.startsWith(bare.toLowerCase()))
        return '';
...
function extensionFieldNames(xml) {
    const names = [];
...
function baseTableDeclaresField(xml, fieldName) {
 ...
 * a failed modify.
 */
export async function resolveFieldNameForFieldGroup(args, objectType, operation, modelName, actualFilePath, symbolIndex) {
    if (operation !== 'add-field-to-field-group' || objectType !== 'table-extension')
        return '';
    ...
    try {
        ...
        if (fields.some(n => n.toLowerCase() === lower))
            return '';
        ...
        if (baseTable) {
            ...
            if (baseXml && baseTableDeclaresField(baseXml, given))
                return '';
        }
        args.fieldName = match;
        console.error(`[modifyD365File] Field-group entry retargeted: ${given} → ${match}`);
        return (`'${given}' is not a field of this extension, but '${match}' is — the group entry now points at it. ` +
...
async function resolveD365FileByName(symbolIndex, objectType, objectName, modelName, packagePath) {
    ...
    if (symbolType) {
      ...
      `);
        ...
        const looksLikeAotSourceFile = (p) => /\.(xml|xpp)$/i.test(p);
 ...
 */
export async function createFileBackup(filePath) {
 ...
 * Returns raw XML content, or null if not accessible.
 */
export async function findBaseObjectXml(objectType, objectName, symbolIndex) {
    ...
    try {
        const rdb = symbolIndex.getReadDb();
        const row = rdb.prepare(`SELECT file_path FROM symbols WHERE type = ? AND name = ? LIMIT 1`).get(objectType, objectName);
        if (row?.file_path)
...
function noteAutoCorrection(notes, note) {
 ...
 */
function resolveEnumForFieldElement(fieldTypeElement, explicitEnumType, fieldNameCandidates, symbolIndex) {
    if (!/Enum$/i.test(fieldTypeElement))
        return undefined;
    if (explicitEnumType)
        return explicitEnumType;
    for (const candidate of fieldNameCandidates) {
        if (!candidate)
            continue;
        try {
            const row = symbolIndex.getReadDb().prepare(`SELECT name FROM symbols WHERE type = 'enum' AND name = ? COLLATE NOCASE LIMIT 1`).get(candidate);
...
function isBaseFieldGroupMissError(message) {
    if (!message)
        return false;
 ...
 */
export async function describeFieldGroupRendering(baseTableName, groupName, fieldName, symbolIndex) {
    try {
        const rdb = symbolIndex?.getReadDb?.();
        if (!rdb)
            return '';
        ...
        const sql = (collate) => `SELECT DISTINCT form_name, datasource_name FROM form_datasources ` +
            `WHERE table_name = ?${collate} LIMIT ${DATA_GROUP_FORM_PROBE_LIMIT}`;
        let rows = rdb.prepare(sql('')).all(baseTableName);
        if (rows.length === 0) {
            rows = rdb.prepare(sql(' COLLATE NOCASE')).all(baseTableName);
        }
        for (const row of rows) {
            ...
            const hit = renderers.find(r => !r.dataSource || r.dataSource.toLowerCase() === row.datasource_name.toLowerCase());
            if (!hit)
                continue;
 ...
 */
export async function describeUnrenderedFieldGroup(baseTableName, groupName, symbolIndex) {
    try {
        const rdb = symbolIndex?.getReadDb?.();
        if (!rdb)
            return '';
        ...
        for (const row of rows) {
            ...
            for (const r of await listDataGroupRenderers(xml)) {
                ...
                if (r.dataSource && r.dataSource.toLowerCase() !== row.datasource_name.toLowerCase())
                    continue;
                // Rendered after all — describeFieldGroupRendering owns this case.
                if (r.dataGroup.toLowerCase() === needle)
                    return '';
...
async function baseObjectDefinesFieldGroup(baseTableName, groupName, symbolIndex) {
    ...
    for (const m of xml.matchAll(/<AxTableFieldGroup>[\s\S]*?<Name>([^<]+)<\/Name>/g)) {
        if (m[1].trim().toLowerCase() === groupName.trim().toLowerCase())
            return true;
    }
    return false;
}
 ...
 */
export function resolveControlTypeForField(dataSource, dataField, db) {
    if (!dataField)
        return undefined;
    ...
    const asControlType = (baseType) => baseType === 'Enum' ? 'ComboBox' : baseType;
    if (declared && db) {
        if (isEnumName(declared, db))
            return enumControlType(declared);
        ...
        if (enumType)
            return enumControlType(enumType);
    ...
    if (db) {
        if (isEnumName(dataField, db))
            return enumControlType(dataField);
...
function enumControlType(enumName) {
