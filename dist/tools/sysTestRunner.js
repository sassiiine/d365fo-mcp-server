...
function assertSafePath(value, label) {
    if (/[&|<>^`!;$%"'\n\r]/.test(value)) {
        throw new Error(`${label} contains potentially dangerous characters and cannot be used in a command: ${value}`);
    }
}
// Tool registration (name, description, inputSchema) lives inline in
// src/server/mcpServer.ts - the single source of truth for tool instructions.
export const sysTestRunnerTool = async (params, _context) => {
    ...
    try {
        ...
        try {
            ...
            if (testMethod)
                assertSafePath(testMethod, 'testMethod');
        }
        catch (validationErr) {
            return {
        ...
        const { stdout, stderr } = await withOperationLock(`systest:${resolvedModelName}:${className}`, () => execFileAsync(runnerPath, args, {
...
//# sourceMappingURL=sysTestRunner.js.map