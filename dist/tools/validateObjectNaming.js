    ...
    try {
        ...
        if (isExtension) {
            ...
            if (!baseObjectName) {
                ...
                const baseExists = db.prepare(`SELECT name FROM symbols WHERE name = ? AND type IN (${dbTypes.map(() => '?').join(',')}) LIMIT 1`).get(baseObjectName, ...dbTypes);
                if (!baseExists) {
                    ...
                    args.objectType === 'enum-extension' ? 'enum-extension' :
                        args.objectType === 'edt-extension' ? 'edt-extension' :
                            args.objectType === 'data-entity' ? 'view' :
                                args.objectType;
        const exactConflict = db.prepare(`SELECT name, type, model FROM symbols WHERE name = ? ORDER BY model LIMIT 5`).all(name);
        // Near-match search (same prefix, similar name)
        const similarSymbols = db.prepare(`SELECT name, type, model FROM symbols WHERE name LIKE ? AND type = ? ORDER BY name LIMIT 5`).all(`${name.slice(0, Math.max(4, name.length - 3))}%`, dbType);
        // ══════════════════════════════════════════════════════════════════
        // FORMAT OUTPUT
        // ══════════════════════════════════════════════════════════════════
...
function detectModelPrefix(db, proposedName) {
    ...
    try {
        ...
        const sample = db.prepare(`SELECT name FROM symbols WHERE type = 'class' AND name LIKE ? LIMIT 20`).all(`${prefix3}%`);
        if (sample.length >= 3)
            return prefix3;
        // Try 2 chars
