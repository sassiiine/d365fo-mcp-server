 ...
 */
import { D365FO_FILE_OP_SPECS, renderOpSpec, renderCreatePropertySpec, } from './d365foFileOpSpecs.js';
import { GENERATE_OBJECT_MODE_SPECS, renderGenerateObjectSpec, } from './generateObjectOpSpecs.js';
 ...
 */
const D365FO_FILE_OBJECT_TYPES = d365foFileTool.inputSchema.properties.objectType.enum;
...
function findKey(registry, needle) {
 ...
 */
const D365FO_FILE_OVERRIDE_PARAMS = {
    ...
    bpCheck: '[create|modify] true = run the best-practice check on the object in THIS call, ' +
...
export function renderOpSpecIndex(unknownTopic) {
...
export function renderPrepareOpSpec(args) {
    ...
    if (mode === 'create') {
        ...
        if (!op) {
            lines.push('### Write contract', 'Pick the operation, then get_knowledge(kind="op-spec", topic="<operation>") for its parameters — ' +
 ...
 * key would hit.
 */
export function lookupOpSpec(topic) {
    if (!topic || !topic.trim())
        return renderOpSpecIndex();
    const needle = normalize(topic);
    const operation = findKey(D365FO_FILE_OP_SPECS, needle);
    if (operation)
        return renderOpSpec(operation);
    const mode = findKey(GENERATE_OBJECT_MODE_SPECS, needle);
    if (mode)
        return renderGenerateObjectSpec(mode);
    const objectType = D365FO_FILE_OBJECT_TYPES.find(t => t.toLowerCase() === needle);
    if (objectType)
        return renderCreatePropertySpec(objectType);
    if (LABELS_TOPICS.includes(needle))
        return renderLabelsOpSpec();
    const redirect = TOPIC_REDIRECTS[needle];
    if (redirect && REDIRECT_ANSWERS[redirect])
        return REDIRECT_ANSWERS[redirect];
    return renderOpSpecIndex(topic);
}
//# sourceMappingURL=opSpecs.js.map