...
export const D365FO_FILE_PARAM_SPECS = {
    // methods
    methodName: {
        type: 'string',
        description: 'Method name. add-method/add-display-method: derived from the source signature when omitted; ' +
    ...
    methodCode: {
        type: 'string',
        description: 'Alias of sourceCode — full X++ method source incl. modifiers/attributes. A bare body gets its ' +
            'signature assembled from methodModifiers/methodReturnType/methodName/methodParameters.',
    },
    sourceCode: {
        type: 'string',
        description: 'Full X++ method source incl. modifiers/attributes (top-level core param; alias: methodCode). ' +
    ...
    oldCode: {
        type: 'string',
        description: 'Exact existing X++ snippet to find (whitespace-trimmed match); methodName scopes the search to ' +
    ...
    fieldType: {
        type: 'string',
        description: 'EDT name for the field (e.g. "InventQty", "WHSZoneId", "TransDate"). For modify-field: new EDT to set.',
    },
    fieldBaseType: {
        type: 'string (String | Integer | Real | Date | DateTime | Int64 | GUID | Enum)',
        description: 'Base type selecting the XML element for add-field (e.g. edt "InventQty" + "Real" → AxTableFieldReal). ' +
    ...
    fieldLabel: {
        type: 'string',
        description: 'Field label. On an ENUM field it must be a different label ID than the enum\'s own label — ' +
    ...
    autoCorrect: {
        ...
        description: 'Apply a correction the server has already fully determined — one valid reading only — and report ' +
    ...
    dataField: {
        type: 'string',
        description: 'add-field on a data-entity-extension: source table field name for the mapped field ' +
    ...
    dataSource: {
        type: 'string',
        description: 'add-field on a data-entity-extension: source data-source/table name on the entity for the ' +
            'mapped field (e.g. "MyTable"). Required alongside dataField.',
    },
    fields: {
        type: 'array of { name, edt?, type?, mandatory?, label? }',
        description: 'Full replacement field list (atomic — replaces ALL fields; for corrupted field names). ' +
    ...
    propertyPath: {
        type: 'string',
        description: 'Property name to set. Supported names by objectType:\n' +
    ...
    controlName: {
        type: 'string',
        description: 'Name of the new form control — MUST match the field name in the table extension so the binding works.',
    },
    parentControl: {
        type: 'string',
        description: 'Existing parent tab/group in the base form (e.g. "TabGeneral"). Fuzzy names are auto-resolved ' +
    ...
    controlType: {
        type: 'string',
        description: 'String (default), Integer, Real, CheckBox (NoYes/boolean), ComboBox (enums), Date, DateTime, ' +
    ...
    positionType: {
        type: 'string',
        description: 'AfterItem (needs previousSibling) | Begin | End. Omit to append at the end of the parent. ' +
            'These are the values D365FO form-extension metadata actually carries; anything else is refused.',
    },
    previousSibling: {
        type: 'string',
        description: 'Name of the sibling control to position after. Implies positionType=AfterItem when that is omitted.',
    },
    baseFormName: {
        type: 'string',
        description: 'Base form name for resolving parentControl — pass only when auto-detection from the extension name fails.',
    },
    // table methods / display methods
    tableMethodType: {
        type: 'string (find | exist | findByRecId | validateWrite | validateDelete | initValue)',
        description: 'Standard method to auto-generate; find/exist also need tableKeyField. ' +
    ...
    displayMethodReturnEdt: {
        type: 'string',
        description: 'Return EDT (e.g. "Name") — auto-generates a stub with methodName. Omit and pass sourceCode for a custom body.',
    },
    // indexes
    ...
    relationCardinality: {
        type: 'string',
        description: 'Local-side cardinality: ZeroOne | ExactlyOne | ZeroMore | OneMore | NotSpecified (default: ZeroMore).',
    },
    ...
    relationshipType: {
        type: 'string',
        description: 'Association | Composition | Aggregation | Link | Specialization | NotSpecified (default: Association).',
    },
    // field groups
    ...
    extendBaseFieldGroup: {
        type: 'boolean',
        description: 'table-extension only: true = extend an existing base-table group (<FieldGroupExtensions>); ' +
    ...
    enumValueHelpText: {
        type: 'string',
        description: 'NOT WRITABLE — an enum value has no help text in the metamodel (AxEnumValue has no ' +
...
export const D365FO_FILE_OP_SPECS = {
    ...
    'replace-code': {
        ...
        optional: ['methodName'],
        note: 'Surgical oldCode→newCode replacement — NOT sourceCode/methodCode. Preferred for rewriting a known ' +
    ...
    'add-display-method': {
        ...
        note: 'Display method with [SysClientCacheDataMethodAttribute]. Pass methodName + displayMethodReturnEdt ' +
    ...
    'add-table-method': {
        ...
        note: 'Canonical find/exist/findByRecId/validateWrite/validateDelete/initValue boilerplate. Pass ' +
    ...
    'add-field-to-field-group': {
        ...
        note: 'table-extension: a group owned by the BASE table is detected and extended through ' +
 ...
 */
export const D365FO_FILE_CREATE_PROPERTY_SPECS = {
    class: 'extends, implements, isFinal, isAbstract',
    table: 'label, tableGroup, tableType, titleField1/2, cacheLookup?, primaryIndex?, ' +
    ...
    'table-extension': 'fields[{name,edt?,enumType?,label?,mandatory?,fieldType?}] — enum fields need ' +
    ...
    'security-privilege': 'label, targetObject, objectType (MenuItemDisplay|MenuItemAction|MenuItemOutput|ServiceOperation), ' +
    ...
    'data-entity': 'primaryTable + fields[{name,dataField?}] — BOTH REQUIRED (without them the entity has no query ' +
    ...
    map: 'label?, developerDocumentation?, fields[{name,type?,edt?,enumType?,stringSize?}] — type is ' +
    ...
    service: 'serviceClass (defaults to the service name), externalName?, namespace?, description?, ' +
 ...
 */
export function renderCreatePropertySpec(objectType) {
    const spec = D365FO_FILE_CREATE_PROPERTY_SPECS[objectType];
    if (!spec) {
        return (`objectType '${objectType}' takes no extra \`properties\` beyond objectName/sourceCode ` +
            `(or is not a d365fo_file objectType). Types that do: ` +
            `${Object.keys(D365FO_FILE_CREATE_PROPERTY_SPECS).join(', ')}.`);
    }
    return (`d365fo_file(action="create", objectType="${objectType}") \`properties\` contract — ` +
 ...
 */
export const D365FO_FILE_CORE_PARAMS = new Set([
    // routing / dispatch
 ...
 */
export const OP_UNHONOURED_PARAMS = {
    'add-enum-value': {
        enumValueHelpText: 'an enum VALUE has no help text in the D365FO metamodel. Verified by reflection on this ' +
...
function allKnownParamNames() {
    const names = new Set(Object.keys(D365FO_FILE_PARAM_SPECS));
 ...
 */
function suggestParam(operation, key) {
    const k = key.toLowerCase();
    const candidates = opParamNames(operation);
    return (candidates.find(p => p.toLowerCase() === k) ??
 ...
 */
export function findIgnoredParams(operation, providedKeys) {
    if (!D365FO_FILE_OP_SPECS[operation])
        return [];
    ...
    for (const key of providedKeys) {
        if (D365FO_FILE_CORE_PARAMS.has(key))
            continue;
        if (mine.has(key)) {
            if (unhonoured[key])
                ignored.push({ name: key, reason: 'not-honoured', detail: unhonoured[key] });
            continue;
        }
        ignored.push({
            name: key,
...
export function renderIgnoredParamsWarning(operation, ignored) {
    if (ignored.length === 0)
        return '';
 ...
 */
export function findMissingMutationParams(operation, providedKeys) {
...
export function getRequiredParams(operation) {
    return D365FO_FILE_OP_SPECS[operation]?.required ?? [];
}
function renderParamLine(name, marker) {
 ...
 */
export function renderOpSpec(operation) {
    const op = D365FO_FILE_OP_SPECS[operation];
    if (!op)
        return `Unknown operation '${operation}'. Valid operations: ${Object.keys(D365FO_FILE_OP_SPECS).join(', ')}.`;
    const lines = [
...
//# sourceMappingURL=d365foFileOpSpecs.js.map