...
export const GENERATE_OBJECT_PARAM_SPECS = {
    // shared identity / placement
    name: {
        type: 'string',
        description: 'REQUIRED. [pattern] element name (extensions: base element; form-datasource/control-extension: ' +
    ...
    pattern: {
        type: 'string',
        description: 'REQUIRED. CoC skeletons: class/table-extension, form-handler, form-datasource-extension ' +
    ...
    baseName: {
        type: 'string',
        description: 'event-handler: base class/table. form-datasource-extension: data source name (defaults to form name). ' +
    ...
    tableGroup: {
        type: 'string',
        description: 'Business role (TableGroup enum): Main, Transaction, Parameter, Group, WorksheetHeader/WorksheetLine, ' +
    ...
    formPattern: {
        type: 'string',
        description: 'Form pattern: SimpleList, SimpleListDetails, DetailsMaster, DetailsTransaction, Dialog, DropDialog, ' +
            'TableOfContents, Lookup, ListPage, Workspace.',
    },
    cloneFrom: {
        type: 'string',
        description: 'PREFERRED: clone a reference form\'s XML re-bound via tableMapping (methods stripped; fields missing ' +
    ...
    fields: {
        ...
        description: 'Structured field specs; takes priority over fieldsHint. PREFER for enum-backed fields or an explicit ' +
    ...
    additionalDatasets: {
        ...
        description: 'Multi-dataset report: each entry adds a TempDB TmpTable + a get<Table>() DP method. name = suffix ' +
    ...
    fieldsHint: {
        type: 'string',
        description: 'Comma-separated field names; EDTs auto-suggested from the index. ⚠️ EDTs/enums created this session ' +
 ...
 */
export const GENERATE_OBJECT_MODE_SPECS = {
    pattern: {
        required: ['name', 'pattern'],
        optional: ['menuItemType', 'baseName', 'targetObject', 'serviceMethod', 'modelName'],
        note: 'Text only, no write. Call analyze_code(mode="patterns") first, then generate_object(mode="pattern"), ' +
    ...
    scaffold: {
        ...
        note: 'objectType selects the scaffold; ask for the one you need — topic="scaffold:table", ' +
...
export function getGenerateObjectRequiredParams(mode) {
 ...
 */
export function renderGenerateObjectSpec(mode) {
    const spec = GENERATE_OBJECT_MODE_SPECS[mode];
    if (!spec) {
        return (`Unknown generate_object mode '${mode}'. Valid topics: ` +
...
//# sourceMappingURL=generateObjectOpSpecs.js.map