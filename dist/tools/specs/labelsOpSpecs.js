...
export const LABELS_OVERRIDE_PARAMS = {
    packageName: '[create|rename] Package name for the model. Auto-resolved if omitted.',
    packagePath: '[create|rename] Root packages path. Auto-detected from environment config if omitted.',
    projectPath: '[create] Path to the .rnrproj project file. Auto-detected from .mcp.json if omitted.',
    solutionPath: '[create] Path to the .sln solution directory. Fallback to find .rnrproj if projectPath is not set.',
    addToProject: '[create] Add label file XML descriptors to the VS project (default: true).',
    createIfMissing: '[create] Upsert-lite: create the label when absent, and when it already exists reuse it ' +
    ...
    createLabelFileIfMissing: '[create] Create the AxLabelFile structure if missing (default: true). A wrong-path guard still ' +
        'fails loudly when the model directory is not found, so no phantom file is produced. ' +
        'Set false to fail fast instead.',
    sortLabels: '[create] Sort labels alphabetically in .label.txt (default true, from LABEL_SORT_ORDER env; ' +
        'false = append at end).',
    languages: '[create] string[] — restrict which language .label.txt files are written (e.g. ["en-US"]). ' +
        'Omitted = every language folder present in the model.',
    defaultComment: '[create] Developer comment for languages without an explicit comment.',
    description: '[create] Label description (comment line in .label.txt). Defaults to the VS project name from ' +
        '.rnrproj when omitted, then falls back to labelFileId. Per-translation comment and ' +
        'defaultComment take priority.',
    searchPaths: '[rename] string[] — additional absolute directory paths to scan for X++ / XML references.',
    updateIndex: '[create|rename] Update the MCP label index after writing (default: true).',
    allowExtensionLabelFile: '[create|rename] Allow writing to a label file EXTENSION ("_Extension" marker). Default false — ' +
        "new labels belong in the model's ORIGINAL label file.",
};
/** The contract rendered for get_knowledge(kind="op-spec", topic="labels"). */
export function renderLabelsOpSpec() {
    return [
...
//# sourceMappingURL=labelsOpSpecs.js.map