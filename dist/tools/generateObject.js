...
import { GENERATE_OBJECT_MODE_SPECS, getGenerateObjectRequiredParams, renderGenerateObjectSpec, } from './specs/generateObjectOpSpecs.js';
function err(text) {
 ...
 */
function specKey(mode, args) {
    if (mode === 'scaffold' && typeof args.objectType === 'string') {
        const key = `scaffold:${args.objectType}`;
        if (GENERATE_OBJECT_MODE_SPECS[key])
            return key;
    }
    return mode;
}
export async function generateObjectTool(request, context) {
    ...
    const a = params && typeof params === 'object' && !Array.isArray(params) ? { ...flat, ...params } : flat;
    const mode = a.mode;
    if (mode && GENERATE_OBJECT_MODE_SPECS[mode]) {
        ...
        if (missing.length > 0) {
            return err(`❌ generate_object(mode="${mode}"): missing required parameter(s) ${missing.join(', ')}.\n\n` +
