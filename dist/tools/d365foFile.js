 ...
 * and lose edits.
 */
async function runModifyBatch(rest, context) {
    const { operations, ...shared } = rest;
    ...
    const peerOperations = operations
        .map(e => (e && typeof e === 'object' ? String(e.operation ?? '') : ''))
        .filter(Boolean);
    for (let i = 0; i < operations.length; i++) {
        ...
        const { params: entryParams, ...entryFlat } = entry;
        const entryArgs = {
            ...shared,
            ...entryFlat,
        ...
        const opName = String(entry.operation ?? '(missing operation)');
        if (!entry.operation) {
    ...
    const head = `${failed === 0 ? '✅' : '⚠️'} d365fo_file(action="modify") — ${succeeded}/${operations.length} operation(s) applied` +
...
export async function d365foFileTool(request, context) {
    ...
    const rest = params && typeof params === 'object' && !Array.isArray(params)
        ? { ...flat, ...params }
        : flat;
...
//# sourceMappingURL=d365foFile.js.map