 ...
 */
async function bridgeEdtBaseType(bridge, edtName) {
    if (!bridge?.isReady)
        return undefined;
    try {
...
export function isEnumName(name, db) {
    try {
        const row = db.prepare(`SELECT 1 FROM symbols WHERE name = ? COLLATE NOCASE AND type IN ('enum', 'enum-extension') LIMIT 1`).get(name);
        return !!row;
    }
    catch {
        return false;
...
export function resolveBestEdt(fieldName, db) {
    try {
        const exact = db.prepare(`SELECT edt_name FROM edt_metadata WHERE edt_name = ? COLLATE NOCASE LIMIT 1`).get(fieldName);
        if (exact)
