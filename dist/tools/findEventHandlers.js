...
export async function findEventHandlersTool(request, context) {
    try {
        ...
        const bridgeResult = await tryBridgeEventHandlers(context.bridge, targetName, args.eventName, args.handlerType);
        if (bridgeResult)
            return bridgeResult;
        ...
        if (args.eventName)
            output += `Filtering by event: ${args.eventName}\n`;
        output += '\n';
        // ── 1. Static event handlers via extension_metadata.event_subscriptions ──
        let metaHandlers = [];
        if (args.handlerType !== 'delegate') {
            try {
                metaHandlers = rdb.prepare(`SELECT extension_name, model, event_subscriptions
           FROM extension_metadata
           WHERE (event_subscriptions LIKE ? OR event_subscriptions LIKE ?)
           ORDER BY model, extension_name`).all(`%classStr(${targetName}%`, `%tableStr(${targetName}%`);
            }
            catch (e) {
                // extension_metadata table may not exist in older databases — non-fatal
                if (process.env.DEBUG_LOGGING === 'true')
                    console.warn('[findEventHandlers] extension_metadata query failed:', e);
            }
        }
        ...
        if (args.handlerType !== 'delegate') {
           ...
           LIMIT 50`).all(`%SubscribesTo(classStr(${targetName}%`, `%SubscribesTo(tableStr(${targetName}%`, `%SubscribesTo(formStr(${targetName}%`);
            }
            catch {
                // FTS5 query failed — fallback to LIKE
                ftsHandlers = rdb.prepare(`SELECT name, parent_name, model, source_snippet FROM symbols
           WHERE type = 'method'
        ...
        if (args.handlerType !== 'static') {
            delegateHandlers = rdb.prepare(`SELECT name, parent_name, model, source_snippet FROM symbols
         WHERE type = 'method'
         AND source_snippet LIKE ?
         ORDER BY model, parent_name, name
         LIMIT 20`).all(`%${targetName}.on%+=% `);
            // Also: find methods with body referencing delegate attach
            const delegateHandlers2 = rdb.prepare(`SELECT name, parent_name, model, source_snippet FROM symbols
         WHERE type = 'method'
         AND source_snippet LIKE ?
         ORDER BY model, parent_name, name
         LIMIT 20`).all(`%${targetName}%.on%`);
        ...
        for (const h of ftsHandlers) {
            ...
            if (args.eventName && eventN && eventN.toLowerCase() !== args.eventName.toLowerCase())
                continue;
            addHandler({
        ...
        if (byEvent.size === 0) {
            ...
            const staticByEvent = new Map();
            const delegateByEvent = new Map();
            for (const [event, handlers] of byEvent) {
                ...
                if (statics.length > 0)
                    staticByEvent.set(event, statics);
                if (delegates.length > 0)
                    delegateByEvent.set(event, delegates);
            }
...
//# sourceMappingURL=findEventHandlers.js.map