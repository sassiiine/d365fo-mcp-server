...
const CodeGenArgsSchema = z.object({
    ...
    name: z.string().describe('For NEW objects (class, runnable, data-entity, batch-job, sysoperation): the object name WITHOUT prefix — prefix is auto-applied from EXTENSION_PREFIX env var or modelName. ' +
    ...
    modelName: z.string().optional().describe('Actual model name from .mcp.json — used to derive the naming infix when EXTENSION_PREFIX env var is not set (e.g. "ContosoExt", "WHSExt", "ApplicationSuite"). ' +
    ...
    serviceMethod: z.string().optional()
        .describe('For sysoperation pattern: the name of the method on the Service class that the Controller will call. ' +
    ...
    groundingToken: z.string().optional()
        .describe('Provenance token from prepare(mode="change"). Required for extension patterns when ' +
...
function formHandlerTemplate(baseName, prefix) {
...
function tableExtensionTemplate(baseName, prefix) {
...
function classExtensionTemplate(baseName, prefix) {
...
function formDataSourceExtensionTemplate(formName, prefix, dataSourceName) {
...
function formControlExtensionTemplate(formName, prefix, controlName) {
...
function mapExtensionTemplate(baseName, prefix) {
...
function sysOperationTemplate(name, serviceMethod = 'process') {
    return `
// ── 1. DataContract ─────────────────────────────────────────────────────
/// <summary>
...
function eventHandlerTemplate(baseName, _prefix) {
    return `
/// <summary>
...
function securityPrivilegeXmlTemplate(name, targetMenuItemName) {
...
function menuItemXmlTemplate(name, itemType, targetObject) {
...
function lookupFormTemplate(name) {
    return `/// <summary>
...
// ── Dialog Box pattern ─────────────────────────────────────────────────────
function dialogBoxTemplate(name) {
    return `/// <summary>
...
function dimensionControllerTemplate(name) {
    return `/// <summary>
...
function numberSeqHandlerTemplate(name) {
    return `/// <summary>
...
function displayMenuControllerTemplate(name) {
    return `/// <summary>
...
function serviceClassAisTemplate(name) {
    return `/// <summary>
...
function businessEventTemplate(name) {
    return `// ══════════════════════════════════════════════════════════════════
// Business Event: ${name}
...
function customTelemetryTemplate(name) {
    return `/// <summary>
...
function featureClassTemplate(name) {
    return `/// <summary>
...
function compositeEntityTemplate(name) {
    return `// ══════════════════════════════════════════════════════════════════
...
function customServiceTemplate(name) {
    return `// ══════════════════════════════════════════════════════════════════
// Custom Service: ${name}
...
function erCustomFunctionTemplate(name) {
    return `/// <summary>
...
export async function codeGenTool(request) {
    try {
        ...
        if (XML_PATTERNS.has(args.pattern)) {
            ...
            const groundingError = enforceGrounding(args.groundingToken, `generate_code(pattern="${args.pattern}", name="${args.name}")`, args.name);
            if (groundingError)
            ...
            if (args.pattern === 'form-datasource-extension') {
                ...
                if (args.pattern === 'event-handler') {
                    ...
                    const exampleClass = args.pattern === 'table-extension' ? `${baseName}${extensionInfix}_Extension`
