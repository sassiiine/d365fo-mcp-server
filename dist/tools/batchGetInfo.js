...
export async function batchGetInfoTool(request, context) {
    ...
    const results = await Promise.all(parsed.data.objects.map(async (obj) => {
    ...
    const sections = results.map((r, i) => `## ${i + 1}. ${r.name} [${r.type.toUpperCase()}] ${r.success ? '' : '❌'}\n\n${r.text}`);
    const header = `# Batch Get Info\n\n` +
...
//# sourceMappingURL=batchGetInfo.js.map