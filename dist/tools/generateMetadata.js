...
// ---------------------------------------------------------------------------
function exeCachePath(frameworkBinDir) {
...
async function compileHelper(cscExe, frameworkBinDir, exePath) {
...
// ---------------------------------------------------------------------------
async function syncCompilerMetadata(compilerMetadataRoot, metadataRoot, modelName) {
 ...
 */
export async function generateRuntimeMetadata(microsoftPackagesPath, customPackagesPath, modelName) {
    ...
    // Run the helper
    try {
        const { stdout, stderr } = await execFileAsync(exePath, [customPackagesPath, modelName, outputDir], { timeout: 60_000 });
