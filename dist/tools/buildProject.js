...
const XPPC_DIAG_LINE_RE = /^(Compile Fatal Error|Compile Error|Compile Warning|Generation Warning|Best Practice Warning):\s*(?:(.*?)\s+)?dynamics:\/\/([^/\s:]+)\/([^/\s:]+)(?:\/([^\s:]+))?\s*:?\s*\[\((\d+),(\d+)\)(?:,\(\d+,\d+\))?\]\s*:\s*(.*)$/;
...
async function findXppcExe(microsoftPackagesPath) {
    ...
    candidates.push('C:\\AOSService\\PackagesLocalDirectory\\bin\\xppc.exe', 'K:\\AOSService\\PackagesLocalDirectory\\bin\\xppc.exe', 'J:\\AOSService\\PackagesLocalDirectory\\bin\\xppc.exe', 'I:\\AOSService\\PackagesLocalDirectory\\bin\\xppc.exe');
...
// ---------------------------------------------------------------------------
async function waitForBuildCompletion(targetModel, customPackagesPath, timeoutMs) {
...
// Tool handler
// ---------------------------------------------------------------------------
export const buildProjectTool = async (params, _context) => {
    try {
        ...
        if (!microsoftPackagesPath) {
            for (const candidate of [
                ...
                try {
                    ...
                    microsoftPackagesPath = candidate;
                    break;
                }
                catch { /* next */ }
            }
        ...
        if (existingState && !force) {
            ...
            if (buildModeChanged) {
                ...
                if (existingState.status === 'running' && alive) {
                    ...
                    if (waitForFinish) {
                        const timeoutMs = (typeof params.waitTimeoutMs === 'number' && params.waitTimeoutMs > 0)
