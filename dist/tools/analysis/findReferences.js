 ...
 */
export function resolveLabelTarget(targetName, targetType) {
    const t = targetName.trim();
    if (/^\/Labels\//i.test(t))
        return t; // already an xref label path
    if (t.startsWith('@'))
        return `/Labels/${t}`; // "@WAX2194" / "@ApplicationPlatform:Foo"
    if (targetType === 'label')
        return `/Labels/@${t}`; // bare id + explicit targetType
    return null;
}
/**
 ...
 */
function resolveXrefContainers(db, ownerName) {
 ...
 */
function buildScopedEmptyResult(displayName, bridgeTargets) {
...
export async function findReferencesTool(request, context) {
    try {
        ...
        if (labelPath) {
            ...
            if (outcome.status === 'empty') {
                return { content: [{ type: 'text', text: `# References to label \`${targetName}\`\n\n**Total References Found:** 0\n` +
        ...
        const parentObjectName = owner ?? ((targetType === 'class' || targetType === 'method') ? memberName : null);
        ...
        if (wantsMethod)
            memberSegments.push('Methods');
        if (!targetType || targetType === 'field' || targetType === 'all')
            memberSegments.push('Fields');
        ...
        let bridgeTargets = [cleanTargetName];
 ...
 */
function ftsMethodSearch(db, term, limit, extraColumns) {
...
function findMethodReferences(symbolIndex, methodName, _scope, limit) {
    const references = [];
...
function findClassReferences(symbolIndex, className, _scope, limit) {
    const references = [];
    const rdb = symbolIndex.getReadDb();
...
function findTableReferences(symbolIndex, tableName, _scope, limit) {
    const references = [];
    const rdb = symbolIndex.getReadDb();
...
function findFieldReferences(symbolIndex, fieldName, _scope, limit) {
    const references = [];
    const rdb = symbolIndex.getReadDb();
...
function findEnumReferences(symbolIndex, enumName, _scope, limit) {
    const references = [];
    const rdb = symbolIndex.getReadDb();
...
function extractMethodCallContext(source, methodName) {
    if (!source)
        return null;
...
function extractInstantiationContext(source, className) {
    if (!source)
        return null;
...
function extractTableReferenceContext(source, tableName) {
    if (!source)
        return null;
    const lines = source.split('\n');
    for (let i = 0; i < lines.length; i++) {
...
function extractFieldAccessContext(source, fieldName) {
    if (!source)
        return null;
    const lines = source.split('\n');
    for (let i = 0; i < lines.length; i++) {
...
function extractEnumReferenceContext(source, enumName) {
    if (!source)
        return null;
    const lines = source.split('\n');
    for (let i = 0; i < lines.length; i++) {
...
function generateReferenceSummary(references) {
...
function groupByReferenceType(references) {
    const groups = {};
    for (const ref of references) {
