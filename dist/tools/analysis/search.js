...
import { generateRelatedSearches, detectCommonPatterns, generateContextualTips, formatRichContext } from '../../utils/richContext.js';
import { generateSearchSuggestions, formatSuggestions } from '../../utils/suggestionEngine.js';
 ...
 */
export async function probeExactMatches(index, query, types) {
 ...
 */
export async function probeCustomMatches(index, query, types, limit = 15) {
    try {
 ...
 */
async function dropStaleRows(rows) {
 ...
 */
async function markStaleRows(rows) {
    const missing = await Promise.all(rows.map(r => indexedPathIsMissing(r.filePath)));
...
export async function searchTool(request, context) {
    try {
        ...
        const bridgeResult = await tryBridgeSearch(context.bridge, args.query, args.type === 'all' ? undefined : args.type, args.limit, { exactMatches, customMatches });
        if (bridgeResult)
            return bridgeResult;
 ...
 */
async function performHybridSearch(args, context) {
    ...
    const results = await hybridSearch.search(args.query, {
        types: args.type === 'all' ? undefined : [args.type],
        limit: args.limit,
    ...
    if (results.length === 0) {
        ...
        try {
            ...
            const suggestions = generateSearchSuggestions(args.query, allSymbolNames, symbolsByTerm, 5 // max suggestions
            );
 ...
 */
async function performExternalSearch(args, symbolIndex, index) {
    try {
        ...
        const raw = (await index.searchSymbols(args.query, args.limit, types)) || [];
        ...
        for (const hit of missingExact)
            seen.add(`${hit.name.toLowerCase()} ${hit.type}`);
        ...
        const missingCustom = customProbe
            .filter(hit => !seen.has(`${hit.name.toLowerCase()} ${hit.type}`));
        for (const hit of missingCustom)
            seen.add(`${hit.name.toLowerCase()} ${hit.type}`);
        ...
        const isCustomHit = (r) => (r.model ? isCustomModel(String(r.model)) : false) ||
            customKeys.has(`${String(r.name).toLowerCase()} ${r.type}`);
        const ranked = rankCustomFirst(args.query, combined, r => String(r.name), isCustomHit);
