...
import { crossModelLabelWarning, formatLabelReference, isLabelLikelyResolvable, labelProvenanceWarning, } from '../../utils/labelReference.js';
 ...
 */
const CREATE_CALL_ADVICE = `      labels(action="create", labelFileId="<your model's label file>", model="<your model>",\n` +
...
export const NO_REUSE_ADVICE = `➡️  Nothing reusable here — create your own label and move on:\n` + CREATE_CALL_ADVICE;
/**
 ...
 */
export const SOME_REUSE_ADVICE = `➡️  If none of the hits above says what you need, create your own label and move on:\n` + CREATE_CALL_ADVICE;
const SearchLabelsArgsSchema = z.object({
    query: z
        .string()
        .describe('Search text — searches label ID, label text and comments (e.g. "customer name", "MyFeature", "batch")'),
    language: z
        .string()
        .optional()
        .default('en-US')
    ...
    labelFileId: z
        .string()
        .optional()
        .describe('Restrict results to ONE label file ID (e.g. ContosoExt, SYS). Omitting it searches every label ' +
...
function resolveCurrentModel(explicit) {
    if (explicit)
        return explicit;
    try {
...
export async function searchLabelsTool(request, context) {
    try {
        ...
        for (const r of normalised) {
            ...
            if (!resolvable) {
                flaggedCount++;
                if (!flaggedModels.includes(r.model))
                    flaggedModels.push(r.model);
            }
            if (verbose) {
        ...
        if (flaggedCount > 0 && !verbose)
            lines.push(crossModelLabelWarning(flaggedModels, flaggedCount));
        ...
        if (recommended) {
            ...
            lines.push(`⚠️ None of these labels is in a core label file (SYS/…) or in your own model, so none is ` +
...
//# sourceMappingURL=searchLabels.js.map