...
import { validateFormPatternXml, hasPatternErrors, } from '../../validation/formPatternValidator.js';
...
export const validateFormPatternArgsSchema = z.object({
    xml: z.string().optional().describe('Complete AxForm XML to validate. Provide this OR formName/filePath.'),
    formName: z.string().optional().describe('Name of an indexed form — its XML is loaded from the metadata store via the symbol index.'),
    filePath: z.string().optional().describe('Explicit path to an AxForm XML file (e.g. a freshly created form not yet indexed).'),
});
...
function formatReport(report, source) {
    ...
    if (report.violations.length === 0) {
        ...
        lines.push(`${errors.length > 0 ? '❌' : '⚠️'} object_patterns(domain="form", action="validate"): ` +
        ...
        report.violations.forEach((v, idx) => {
            ...
            if (idx < report.violations.length - 1)
                lines.push('');
        });
    }
    lines.push('');
    lines.push(`Pattern coverage: ${report.coverage.containersPatterned}/${report.coverage.containersTotal} containers carry a sub-pattern.`);
    if (errors.length > 0) {
        lines.push('⛔ Fix all errors before calling d365fo_file(action="create") — they will block the write.');
    }
    return lines.join('\n');
}
export async function validateFormPatternTool(request, context) {
...
export function isFormPatternEnforceEnabled() {
 ...
 */
export async function checkAddControlAgainstParentPattern(baseFormXml, parentControlName, controlType) {
    let parsed;
    try {
    ...
    const allowedTypes = new Set(Array.isArray(extra) ? extra : []);
    for (const node of sp.root)
        node.controlTypes.forEach((t) => allowedTypes.add(t));
 ...
 */
export async function checkAddControlAgainstDataGroup(baseFormXml, parentControlName, controlDataField, controlName) {
    if (!controlDataField)
        return null;
    let parsed;
    try {
 ...
 */
export async function listDataGroupRenderers(baseFormXml) {
    let parsed;
    try {
        const parser = new Parser({ explicitArray: false, mergeAttrs: true, trim: true });
        parsed = await parser.parseStringPromise(baseFormXml);
        ...
        for (const n of nodes) {
            ...
            if (dataGroup) {
                found.push({
                    ...
                    generatedNameFor: (fieldName) => `${dataGroup}_${fieldName}`,
                    dataGroup,
                });
            }
            visit(n.children);
 ...
 */
export async function findDataGroupRenderers(baseFormXml, fieldGroupName) {
 ...
 */
export async function gateOnFormPatternErrors(xmlContent, operationDescription) {
    ...
    if (errors.length === 0 || !isFormPatternEnforceEnabled()) {
        if (errors.length > 0) {
            const downgraded = `⚠️ FORM_PATTERN_ENFORCE is disabled — ${errors.length} pattern error(s) NOT blocking:\n` +
    ...
    return {
        blocked: {
            ...
            content: [{
                    type: 'text',
                    text: `⛔ ${operationDescription} blocked — the form XML violates its declared pattern ` +
...
//# sourceMappingURL=validateFormPattern.js.map