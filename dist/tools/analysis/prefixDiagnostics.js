 ...
 */
import { prefixConflictWarning, resolveEffectivePrefix, sameModel, } from '../../utils/effectivePrefix.js';
 ...
 */
export function buildPrefixDiagnostics(writeModel, readModel) {
    ...
    const disagreeNote = `⚠️  The model's own objects use "${learned?.regular}", which overrides EXTENSION_PREFIX="${extensionPrefixEnv}" — new objects will be named "${effectivePrefix}…". If that is wrong, the model's existing objects are the thing to check; set naming.prefixSource=config (env: EXTENSION_PREFIX_SOURCE=config) to pin the configured value instead.`;
    ...
    verboseLines.push(resolution.conflict
        ? disagreeNote
        : learned?.regular
