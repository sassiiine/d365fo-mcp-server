...
export const BatchSearchArgsSchema = z.object({
    ...
    globalTypeFilter: z.array(z.string()).max(5).optional()
        .describe('Default type filter applied to queries that have no per-query type set. ' +
    ...
    deduplicate: z.boolean().optional().default(true)
        .describe('Remove symbols that appear in more than one query result. ' +
    ...
    crossReference: z.boolean().optional().default(true)
        .describe('Append a cross-reference summary at the end listing symbols that appeared in multiple queries. ' +
 ...
 */
function annotateDuplicates(text, seenKeys, currentQueryIdx) {
 ...
 */
function extractSymbolKeys(text) {
    const keys = [];
 ...
 */
function makeSearchRequest(queryArgs, overrideType) {
    return {
        method: 'tools/call',
        params: {
            name: 'search',
 ...
 */
export async function batchSearchTool(request, context) {
    const startTime = Date.now();
    try {
        const args = BatchSearchArgsSchema.parse(request.params.arguments);
        // Validate globalTypeFilter values
        const validGlobalTypes = (args.globalTypeFilter ?? []).filter((t) => SYMBOL_TYPES.includes(t));
        const subSearches = [];
        ...
        const rawResults = await Promise.all(subSearches.map(async (sub) => {
        ...
        const mergedResults = args.queries.map((q, i) => {
        ...
        if (args.deduplicate) {
            const seenKeys = new Map(); // key → 1-based query index
            for (let i = 0; i < mergedResults.length; i++) {
                const r = mergedResults[i];
                if (!r.success || !r.result?.content?.[0]?.text)
                    continue;
                const { text, dupCount } = annotateDuplicates(r.result.content[0].text, seenKeys, i + 1);
        ...
        const output = formatBatchResults(mergedResults, executionTime, args.queries.length, validGlobalTypes, args.deduplicate ? dedupStats.total : -1, crossRefEntries);
 ...
 */
function formatBatchResults(results, executionTime, totalQueries, globalTypeFilter, dedupCount, // -1 = dedup disabled
crossRefEntries = []) {
