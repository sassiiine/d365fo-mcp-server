...
import { matchPrefixCandidate, modelWritesLandIn, prefixCandidates, prefixConflictWarning, resolveEffectivePrefix, } from '../../utils/effectivePrefix.js';
 ...
 */
function reinterpretExtensionType(objectType, proposedName) {
    ...
    return {
        ...
        note: `Read as **${counterpart}**, not "${objectType}" — "${proposedName}" is an extension name. ` +
            `Pass objectType="${counterpart}" directly next time.`,
    };
}
export async function validateObjectNamingTool(request, context) {
    try {
        ...
        if (reinterpreted) {
            args.objectType = reinterpreted.objectType;
        ...
        const modelTokenPhrase = modelToken === modelName
        ...
        const candidates = explicitPrefix
                ...
                if (args.objectType === 'class-extension') {
                    ...
                    if (!name.includes('.')) {
                        ...
                        if (extPart.toLowerCase() !== modelToken.toLowerCase()) {
                            warnings.push(`Extension token (after '.') should be the ${modelTokenPhrase} (EXTENSION_NAMING_STYLE=model-name).\n  Current: ${extPart}\n  Recommended: ${modelToken}`);
                        }
                    }
                }
                else {
        ...
        if (!isExtension) {
            ...
            if (name.includes('_')) {
                ...
                if (!separator) {
                    errors.push(`Non-extension objects must not contain underscores. ` +
            ...
            if (prefix) {
                ...
                if (!leading) {
                    ...
                    warnings.push(`"${name}" carries "${leading.token}" (${leading.label}), not the prefix this server ` +
        ...
        const dbType = args.objectType === 'class-extension'
            ? 'class-extension'
        ...
        const similarSymbols = db
            ...
            .all(`${name.slice(0, Math.max(4, name.length - 3))}%`, dbType);
        let output = `Validation: "${name}" as ${args.objectType}\n`;
        if (reinterpreted)
            output += `ℹ️ ${reinterpreted.note}\n`;
 ...
 */
function detectModelPrefix(db, proposedName) {
    const stdPrefixes = [
        'Cust',
        'Vend',
        'Sales',
...
//# sourceMappingURL=validateObjectNaming.js.map