...
const calls = [];
function prune(now) {
    while (history.length > 0 && now - history[0].at > HISTORY_TTL_MS)
        history.shift();
    while (calls.length > 0 && now - calls[0] > HISTORY_TTL_MS)
        calls.shift();
}
/** Record one search and whether it came back empty-handed. */
export function recordLabelSearch(query, fruitless) {
    const now = Date.now();
    prune(now);
...
export function fruitlessLabelSearches() {
    prune(Date.now());
 ...
 */
export function recordLabelSearchCall() {
    const now = Date.now();
    prune(now);
    calls.push(now);
}
/** Search calls made in this session. */
export function labelSearchCallCount() {
    prune(Date.now());
    return calls.length;
}
/**
...
export function searchBudgetNotice() {
    prune(Date.now());
    if (calls.length <= SEARCH_CALL_BUDGET)
        return '';
    return (`🛑 **STOP — that was label search call ${calls.length} this session** ` +
 ...
 */
export function repeatSearchNotice(excluding = []) {
    ...
    const listed = prior.length > MAX_LISTED
        ...
        : prior.map(q => `"${q}"`).join(', ');
    return (`🛑 **This is not new information.** ${prior.length} phrasing(s) already came back empty here: ` +
...
export function resetLabelSearchHistory() {
    history.length = 0;
    calls.length = 0;
}
//# sourceMappingURL=labelSearchHistory.js.map