...
function findMethod(deps, ownerName, methodName, depth = 0) {
    if (depth > 10)
        return undefined;
    try {
        const row = deps.db.prepare(`SELECT signature FROM symbols
       WHERE parent_name = ? COLLATE NOCASE AND name = ? COLLATE NOCASE AND type = 'method'
       LIMIT 1`).get(ownerName, methodName);
        if (row)
            return row;
        // Extension-added methods (CoC wrappers, augmentation classes)
        const extRows = deps.db.prepare(`SELECT added_methods, coc_methods FROM extension_metadata
       WHERE base_object_name = ? COLLATE NOCASE`).all(ownerName);
        ...
        const parent = deps.db.prepare(`SELECT extends_class FROM symbols
       WHERE name = ? COLLATE NOCASE AND type IN ('class','table') AND extends_class IS NOT NULL
       LIMIT 1`).get(ownerName);
...
function fieldExists(deps, tableName, fieldName) {
    ...
    try {
       ...
       LIMIT 1`).get(tableName, fieldName);
        if (row !== undefined)
            return true;
        const extRows = deps.db.prepare(`SELECT added_fields FROM extension_metadata
...
export function resolveXppReferences(code, deps) {
    ...
    for (const m of cleaned.matchAll(staticRe)) {
        ...
        if (locals.declaredNames.has(lower))
            continue;
                ...
                if (argsText !== undefined) {
                    ...
                    if (n < arity.min || n > arity.max) {
                        violations.push({
                            ...
                            line,
                            identifier: `${typeName}::${member}`,
                            detail: `Call passes ${n} argument(s), but the indexed signature expects ${arity.min === arity.max ? arity.min : `${arity.min}–${arity.max}`}: ${method.signature.trim()}`,
                        });
                    }
                }
            }
...
export async function resolveReferencesTool(request, context) {
    ...
    if (result.violations.length === 0) {
        return {
            content: [{
                    type: 'text',
                    text: `✅ resolve_references: all ${result.verifiedCount} reference(s) verified against the index${suffix}.\n` +
