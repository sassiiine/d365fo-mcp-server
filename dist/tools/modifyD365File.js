            ...
            switch (operation) {
                ...
                case 'add-control': {
                    if (args.controlName && args.parentControl) {
                        bridgeResult = await bridgeAddControl(context.bridge, objectName, args.controlName, args.parentControl, args.controlType ?? 'String', args.controlDataSource, args.controlDataField, args.controlLabel);
                        ...
                        if (objectType === 'form-extension' && (!bridgeResult || !bridgeResult.success)) {
                            const xmlFallbackResult = await directXmlAddControl(actualFilePath, args.controlName, args.parentControl, args.controlType ?? 'String', args.controlDataSource, args.controlDataField, args.controlLabel);
                            if (xmlFallbackResult)
                                bridgeResult = xmlFallbackResult;
                        }
                    }
...
function resolveFieldEdt(tableName, fieldName, db) {
    try {
        const row = db.prepare(`SELECT signature FROM symbols WHERE type = 'field' AND parent_name = ? COLLATE NOCASE AND name = ? COLLATE NOCASE LIMIT 1`).get(tableName, fieldName);
