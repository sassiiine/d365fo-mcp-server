 ...
 */
import { z } from 'zod';
import { FORM_PATTERN_CATALOG, resolvePattern, resolveSubPattern, } from '../knowledge/formPatterns/index.js';
...
export const getFormPatternSpecArgsSchema = z.object({
    pattern: z.string().describe('Pattern name (id, xmlName, or alias) — e.g. "SimpleList", "DetailsMaster", "Dialog", ' +
...
function renderNode(node, indent, lines) {
    ...
    if (node.nameHint)
        parts.push(`name≈${node.nameHint}`);
    if (node.properties) {
        parts.push(Object.entries(node.properties).map(([k, v]) => `${k}=${v}`).join(' '));
    }
    if (node.requiresSubPattern) {
        parts.push(node.allowedSubPatterns?.length
            ? `sub-pattern: ${node.allowedSubPatterns.join('/')}`
            : 'sub-pattern required');
    }
    if (node.extraChildren === 'none')
        parts.push('[no extra children]');
    else if (Array.isArray(node.extraChildren))
        parts.push(`[extras: ${node.extraChildren.join(', ')}]`);
 ...
 */
function axIType(normalized) {
    if (!normalized || normalized === '*')
        return null;
    return `AxForm${normalized}Control`;
}
function renderSkeletonNode(node, indent, lines) {
    ...
    if (node.children?.length) {
        ...
        for (const child of node.children)
            renderSkeletonNode(child, indent + '    ', lines);
        lines.push(`${indent}  </Controls>`);
    }
    else {
 ...
 */
function renderXmlSkeleton(spec) {
    const lines = [];
    lines.push('');
    ...
    for (const node of spec.root)
        renderSkeletonNode(node, '    ', lines);
    lines.push('  </Controls>');
    lines.push('</Design>');
    lines.push('```');
...
function renderTopLevel(spec, db) {
    const lines = [];
    ...
    if (spec.variantOf)
        lines.push(`**Variant of:** ${spec.variantOf}`);
    lines.push('');
    lines.push(spec.purpose);
    lines.push('');
    lines.push('## When to use');
    for (const w of spec.whenToUse)
        lines.push(`- ${w}`);
    if (spec.whenNotToUse?.length) {
        lines.push('');
        lines.push('## When NOT to use');
        for (const w of spec.whenNotToUse)
            lines.push(`- ${w}`);
    }
    lines.push('');
    ...
    if (spec.extraRootChildren === 'none')
        lines.push('(no other root controls allowed)');
    else if (Array.isArray(spec.extraRootChildren))
        lines.push(`(extra root controls allowed: ${spec.extraRootChildren.join(', ')})`);
    else
        lines.push('(additional root controls tolerated)');
    lines.push('```');
    ...
    if (spec.requiresDataSource && spec.requiresDataSource !== 'none') {
        lines.push(`**Datasources:** ${spec.requiresDataSource === 'headerLines'
    ...
    if (spec.lifecycleGuidance?.length) {
        ...
        for (const g of spec.lifecycleGuidance)
            lines.push(`- ${g}`);
...
function renderSubPattern(sp) {
    ...
    if (sp.parentPatterns?.length)
        lines.push(`**Only inside patterns:** ${sp.parentPatterns.join(', ')}`);
    lines.push('');
    ...
    for (const node of sp.root)
        renderNode(node, '', lines);
    if (sp.extraRootChildren === 'none')
        lines.push('(no other children allowed)');
    else if (Array.isArray(sp.extraRootChildren))
        lines.push(`(allowed children: ${sp.extraRootChildren.join(', ')})`);
    else
        lines.push('(additional children tolerated)');
    lines.push('```');
...
export async function getFormPatternSpecTool(request, context) {
    ...
    const KNOWLEDGE_TOPIC_REDIRECTS = {
    ...
    if (knowledgeTopic) {
        return {
            ...
            content: [{
                    type: 'text',
                    text: `❌ "${name}" is not a form pattern — it is a knowledge topic.\n\n` +
...
//# sourceMappingURL=getFormPatternSpec.js.map