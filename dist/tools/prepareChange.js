...
async function resolveObjectType(objectName, context) {
    try {
...
async function fetchMethodSignature(objectName, methodName, context) {
    try {
        const db = context.symbolIndex.getReadDb();
        const row = db.prepare(`SELECT signature, tags FROM symbols
       WHERE parentName = ? AND name = ? AND type = 'method'
       COLLATE NOCASE LIMIT 1`).get(objectName, methodName);
        if (row) {
...
async function fetchEligibility(objectName, methodName, context) {
    if (!methodName)
...
async function fetchNamingValidation(proposedName, context) {
    ...
    try {
        const db = context.symbolIndex.getReadDb();
        const existing = db.prepare(`SELECT name, model FROM symbols WHERE name = ? COLLATE NOCASE LIMIT 1`).get(proposedName);
        if (existing) {
...
async function fetchPatterns(objectName, objectType, context) {
    try {
        const db = context.symbolIndex.getReadDb();
        const rows = db.prepare(`SELECT name, description FROM symbols
