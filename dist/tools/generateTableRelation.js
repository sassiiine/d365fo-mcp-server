...
export function buildTableRelationXml(rel) {
...
export async function generateTableRelationTool(request, context) {
    let args;
    try {
    ...
    if (bridge?.isReady && bridge.metadataAvailable) {
        try {
            ...
            if (t)
                tableFields = t.fields.map((f) => ({ name: f.name, edt: f.extendedDataType }));
        }
        catch {
            /* fall through to DB */
        }
    ...
    if (!tableFields) {
        ...
        const rows = db
            ...
            .all(name);
        if (rows.length === 0) {
            return text(`❌ Table "${name}" not found via bridge or symbol index.\n\nIf it was just created, call update_symbol_index first.`, true);
        }
        tableFields = rows.map((r) => ({ name: r.name, edt: r.signature ?? undefined }));
    }
    if (!db) {
        return text(`❌ table-relation needs the symbol index (edt_metadata) to resolve EDT reference tables. Build/refresh the index, then retry.`, true);
    }
    const wanted = fieldFilter && fieldFilter.length > 0
        ? new Set(fieldFilter.map((f) => f.toLowerCase()))
        : null;
    const refStmt = db.prepare(`SELECT reference_table FROM edt_metadata WHERE edt_name = ? AND reference_table IS NOT NULL AND reference_table != '' LIMIT 1`);
    const relations = [];
    const skipped = [];
    for (const f of tableFields) {
        if (SYSTEM_FIELDS.has(f.name.toLowerCase()))
            continue;
        if (wanted && !wanted.has(f.name.toLowerCase()))
            continue;
        if (!f.edt) {
            if (wanted)
                skipped.push(`${f.name}: no EDT on the field`);
            continue;
        }
        const row = refStmt.get(f.edt);
        if (!row?.reference_table) {
            if (wanted)
                skipped.push(`${f.name} (${f.edt}): EDT carries no reference table`);
            continue;
        }
    ...
    if (relations.length === 0) {
        ...
        return text(`ℹ️ No EDT-backed table relations found for **${name}**.${detail}\n\n` +
            `Only fields whose EDT declares a reference table (edt_metadata.reference_table) produce a relation.`, true);
    }
    ...
    if (skipped.length > 0)
        parts.push('', 'Skipped:', ...skipped.map((s) => `  • ${s}`));
    parts.push('', `Insert each relation with \`d365fo_file(action="modify", objectType="table", objectName="${name}", ...)\`, ` +
        `or add to the table's <Relations> block.`, '', '```xml', fragments, '```');
    return text(parts.join('\n'));
}
//# sourceMappingURL=generateTableRelation.js.map