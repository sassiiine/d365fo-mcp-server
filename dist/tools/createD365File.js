...
import { controlForTableField } from '../utils/fieldControlTypes.js';
...
async function withProjectFileLock(projectPath, fn) {
...
export class ProjectFileFinder {
     ...
     */
    static async findProjectInSolution(solutionPath, modelName) {
        return ProjectFileFinder.findRecursive(solutionPath, modelName, 0, 3);
    }
    static async findRecursive(dir, modelName, currentDepth, maxDepth) {
        ...
        const projectFiles = files.filter(file => file.endsWith('.rnrproj') &&
 ...
 */
function fieldTypeToAxType(fieldType, edtName) {
    const typeMap = {
    ...
    if (hint) {
        ...
        if (e === 'recid' || e.endsWith('recid') || e.includes('refrecid'))
            return 'AxTableFieldInt64';
        if (e.includes('utcdatetime') || (e.includes('datetime') && !e.includes('transdate')))
            return 'AxTableFieldUtcDateTime';
        if (e.includes('date') && !e.includes('time') && !e.includes('update'))
            return 'AxTableFieldDate';
        if (e.includes('amount') || e.includes('mst') || e.includes('price') || e.includes('qty')
            || e.includes('percent') || e === 'real')
            return 'AxTableFieldReal';
        if (e === 'noyesid' || e.endsWith('noyesid') || e === 'noyes')
            return 'AxTableFieldEnum';
        if ((e.endsWith('int') || e.includes('count') || e.includes('level'))
            && !e.includes('account') && !e.includes('name'))
            return 'AxTableFieldInt';
    }
 ...
 */
function buildFieldControlMap(specs) {
...
export function normalizeIndexSpecsForBridge(indexes) {
    return indexes.map((ix) => {
        const rawFields = ix.fields;
        const out = { name: ix.name };
        if (Array.isArray(rawFields)) {
            out.fields = rawFields
                .map(f => (typeof f === 'string' ? f : (f?.fieldName ?? f?.name ?? f?.dataField)))
                .filter((f) => typeof f === 'string' && f !== '');
        }
        if (ix.allowDuplicates != null)
            out.allowDuplicates = ix.allowDuplicates;
        if (ix.alternateKey != null)
            out.alternateKey = ix.alternateKey;
        return out;
    });
}
/** Field-group specs: the bridge's WriteFieldGroupParam.Fields is also a List<string>. */
export function normalizeFieldGroupSpecsForBridge(groups) {
    return groups.map((g) => {
        const rawFields = g.fields;
...
export class XmlTemplateGenerator {
     ...
     */
    static parseSourceForBridge(sourceCode) {
...
`;
     ...
     */
    static generateAxTableXml(tableName, properties) {
        ...
        if (fieldSpecs.length === 0) {
            ...
            for (const f of fieldSpecs) {
                ...
                if (f.helpText)
                    fieldsXml += `\t\t\t<HelpText>${f.helpText}</HelpText>\n`;
                if (f.label)
                    fieldsXml += `\t\t\t<Label>${f.label}</Label>\n`;
                if (f.mandatory)
        ...
        const groupSpecs = Array.isArray(properties?.fieldGroups) ? properties.fieldGroups : [];
        const fieldRefName = (f) => typeof f === 'string' ? f : (f.fieldName ?? f.name ?? '');
        ...
        const indexSpecs = Array.isArray(properties?.indexes) ? properties.indexes : [];
    ...
    static generateAxEnumXml(enumName, properties) {
        ...
        if (enumValueSpecs.length === 0) {
            ...
            for (const v of enumValueSpecs) {
                ...
                if (v.label)
                    enumValuesXml += `\t\t\t<Label>${v.label}</Label>\n`;
                if (v.helpText)
                    enumValuesXml += `\t\t\t<HelpText>${v.helpText}</HelpText>\n`;
...
\t<TableReferences />${stringSize}${enumTypeXml}
     ...
     */
    static generateAxSimpleExtensionXml(rootElement, name) {
        return `<?xml version="1.0" encoding="utf-8"?>
    ...
    static generateAxTableExtensionXml(name, properties) {
        ...
        if (fgSpecs.length === 0) {
            ...
            for (const fg of fgSpecs) {
                ...
                if (fg.label)
                    fieldGroupsXml += `\t\t\t<Label>${fg.label}</Label>\n`;
...
\t<PropertyModifications />
     ...
     */
    static normalizeNameList(value) {
        if (!value)
            return [];
     ...
     */
    static securityRefContainer(container, childTag, names) {
        if (names.length === 0)
            return `\t<${container} />`;
...
export class ProjectFileManager {
     ...
     */
    async addToProject(projectPath, objectType, objectName, _absoluteXmlPath // kept for API compatibility
    ) {
        return withProjectFileLock(projectPath, () => this._addToProjectLocked(projectPath, objectType, objectName));
    }
    async _addToProjectLocked(projectPath, objectType, objectName) {
        ...
        let folderGroup = project.Project.ItemGroup.find((group) => group.Folder !== undefined);
        if (!folderGroup) {
            folderGroup = { Folder: [] };
            project.Project.ItemGroup.push(folderGroup);
        }
        // Find or create Content ItemGroup
        let contentGroup = project.Project.ItemGroup.find((group) => group.Content !== undefined);
        if (!contentGroup) {
        ...
        const folderExists = folderGroup.Folder.some((folder) => folder.$ && folder.$.Include === `${displayFolderName}\\`);
        if (!folderExists) {
        ...
        const fileExists = contentGroup.Content.some((content) => content.$ && content.$.Include === contentInclude);
        if (fileExists) {
            console.error(`[ProjectFileManager] File ${objectName} is already in the project - skipping`);
        ...
        console.error(`[ProjectFileManager] Added file reference to project, Content items: ${contentGroup.Content.length}`);
     ...
     */
    async addLabelToProject(projectPath, labelFileId, languages) {
        return withProjectFileLock(projectPath, () => this._addLabelToProjectLocked(projectPath, labelFileId, languages));
    }
    async _addLabelToProjectLocked(projectPath, labelFileId, languages) {
        // Read project file (with retry for transient VS file locks)
        let projectXml = '';
        for (let attempt = 0; attempt < 5; attempt++) {
            try {
        ...
        let folderGroup = project.Project.ItemGroup.find((g) => g.Folder !== undefined);
        if (!folderGroup) {
            folderGroup = { Folder: [] };
            project.Project.ItemGroup.push(folderGroup);
        }
        let contentGroup = project.Project.ItemGroup.find((g) => g.Content !== undefined);
        if (!contentGroup) {
            contentGroup = { Content: [] };
            project.Project.ItemGroup.push(contentGroup);
        }
        ...
        const folderExists = folderGroup.Folder.some((f) => f.$ && f.$.Include === 'Label Files\\');
        if (!folderExists) {
            folderGroup.Folder.push({ $: { Include: 'Label Files\\' } });
        }
        const added = [];
        let newEntries = 0;
        const existingIncludes = new Set(contentGroup.Content.map((c) => c.$?.Include).filter(Boolean));
     ...
     */
    async extractModelName(projectPath) {
        try {
            console.error(`[ProjectFileManager] Extracting model name from: ${projectPath}`);
            // Read project file
            ...
            if (project.Project && project.Project.PropertyGroup) {
                ...
                for (const group of propertyGroups) {
                    ...
                    if (group.Model) {
                        ...
                        console.error(`[ProjectFileManager] Found Model in project: ${modelName}`);
                        return modelName;
                    }
                    // Fallback to <ModelName> tag (alternative format)
                    if (group.ModelName) {
                        const modelName = group.ModelName;
                        console.error(`[ProjectFileManager] Found ModelName in project: ${modelName}`);
                        return modelName;
                    }
                }
            }
            console.error(`[ProjectFileManager] No Model or ModelName found in project file`);
            return null;
        }
        catch (error) {
            console.error(`[ProjectFileManager] Error extracting model name:`, error);
            return null;
        }
    }
}
    ...
    try {
        ...
        if (namingStyle !== 'model-name' &&
            ...
            if (suffixPart.toLowerCase().startsWith(actualModelName.toLowerCase())) {
                ...
                console.error(`[create_d365fo_file] Stripped model name from dot-notation extension: ` +
        ...
        if (namingStyle !== 'model-name' &&
            ...
            if (baseName.toLowerCase().endsWith(actualModelName.toLowerCase())) {
                ...
                console.error(`[create_d365fo_file] Stripped model name infix "${actualModelName}" from extension class: ` +
                    `${args.objectName} → ${effectiveObjectName}`);
            }
        }
        ...
        if (DOT_NOTATION_EXTENSION_TYPES.has(args.objectType) && !effectiveObjectName.includes('.')) {
            ...
            console.error(`[create_d365fo_file] Bare extension name auto-converted to dot-notation: ` +
                `${args.objectName} → ${effectiveObjectName}`);
        }
        ...
        if (args.objectType === 'class-extension' && !effectiveObjectName.endsWith('_Extension')) {
            ...
            console.error(`[create_d365fo_file] Bare class-extension name auto-converted to _Extension form: ` +
                `${args.objectName} → ${effectiveObjectName}`);
        }
        ...
        const isExtensionObjectType = args.objectType === 'class-extension' || DOT_NOTATION_EXTENSION_TYPES.has(args.objectType);
        ...
        const skipBridgeForExtensibleEnum = args.objectType === 'enum'
            && Boolean(args.properties?.isExtensible);
            ...
            try {
                ...
                if ((args.objectType === 'table' || args.objectType === 'table-extension') && args.properties) {
                    ...
                    if (props.fieldGroups)
                        bridgeParams.fieldGroups = normalizeFieldGroupSpecsForBridge(props.fieldGroups);
                    if (props.indexes)
        ...
        let xmlContent = args.xmlContent
            ? args.xmlContent
            : XmlTemplateGenerator.generate(args.objectType, finalObjectName, args.sourceCode, args.properties);
        ...
        if (finalObjectName !== args.objectName && (args.xmlContent || args.sourceCode)) {
            ...
            if (replaced !== xmlContent) {
                console.error(`[create_d365fo_file] ✅ Fixed name inconsistency: ` +
