...
function normalizeOperations(raw) {
    if (!Array.isArray(raw))
        return [];
    return raw.map(op => (typeof op === 'string' ? { name: op } : op));
}
/** Accepts ["MyService"] shorthand as well as the full object form. */
function normalizeGroupServices(raw) {
    if (!Array.isArray(raw))
        return [];
    return raw.map(s => (typeof s === 'string' ? { service: s } : s));
}
/**
 ...
 */
export function buildAxServiceXml(serviceName, properties) {
    const serviceClass = properties?.serviceClass || properties?.class || serviceName;
    const externalName = properties?.externalName || serviceName;
 ...
 */
export function buildAxServiceGroupXml(groupName, properties) {
...
//# sourceMappingURL=serviceXml.js.map