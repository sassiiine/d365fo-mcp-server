...
export async function repairFormControlsTool(request, context) {
    ...
    if (!design.pattern) {
        return text(`ℹ️ ${source} declares no <Pattern> on Design — nothing to repair against.\n\n` +
    ...
    if (!spec) {
        return text(`❌ Form declares an unknown pattern "${design.pattern}" — cannot repair. ` +
            `Fix the <Pattern> value first (object_patterns(domain="form", action="validate")).`, true);
    }
    ...
    if (!result.changed) {
        const unfixNote = result.unfixable.length > 0
            ? `\n\nCould not auto-add:\n` + result.unfixable.map((u) => `  • ${u.id} — ${u.reason}`).join('\n')
            : '';
        return text(`✅ ${source} — no missing required top-level controls for pattern **${spec.xmlName}**. Nothing to repair.` +
            (beforeErrors > 0
    ...
    lines.push('', `**Next step:** review the XML, then write it with ` +
        `\`d365fo_file(action="create", objectType="form", objectName="${opt.formName}", overwrite=true, xmlContent="…")\`.`, '', '```xml', result.xml, '```');
    return text(lines.join('\n'), afterErrors > beforeErrors);
}
//# sourceMappingURL=repairFormControls.js.map