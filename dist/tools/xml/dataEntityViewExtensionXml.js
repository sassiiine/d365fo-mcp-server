 ...
 */
const FIELD_PRESENTATION_PROPERTIES = [
 ...
 * meets out of order.
 */
export function buildAxDataEntityViewFieldXml(field, defaultDataSource) {
 ...
 */
export function buildAxDataEntityViewExtensionXml(name, properties) {
    const defaultDataSource = properties?.dataSource;
    const fieldSpecs = Array.isArray(properties?.fields)
        ? properties.fields.filter((f) => f && f.name)
        : [];
    ...
    const groupSpecs = Array.isArray(properties?.fieldGroupExtensions)
        ? properties.fieldGroupExtensions.filter((g) => g && g.name && Array.isArray(g.fields) && g.fields.length > 0)
        : [];
    ...
    const mods = Array.isArray(properties?.propertyModifications)
        ? properties.propertyModifications.filter((m) => m && m.name !== undefined && m.name !== null && String(m.name) !== '')
        : [];
...
//# sourceMappingURL=dataEntityViewExtensionXml.js.map