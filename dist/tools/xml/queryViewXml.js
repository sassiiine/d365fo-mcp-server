 ...
 */
export function buildAxQueryXml(queryName, properties) {
    ...
    const title = properties?.title || properties?.label;
    const titleXml = title ? `\t<Title>${escapeXml(title)}</Title>\n` : '';
    const dataSource = properties?.dataSource || properties?.table;
    const dataSourceName = properties?.dataSourceName || dataSource || '';
...
`;
    ...
    const explicitDynamic = typeof properties?.dynamicFields === 'boolean'
...
\t\t\t\t</AxQuerySimpleDataSourceField>`).join('\n')}\n\t\t\t</Fields>\n`
    ...
    const ranges = Array.isArray(properties?.ranges) ? properties.ranges : undefined;
 ...
 */
export function extractQueryRootDataSourceName(queryXml) {
    const root = queryXml.match(/<AxQuerySimpleRootDataSource>[\s\S]*?<Name>([^<]+)<\/Name>/)?.[1]?.trim();
 ...
 */
export function buildAxViewXml(viewName, properties) {
...
//# sourceMappingURL=queryViewXml.js.map