 ...
 */
export function assertDataEntityIsFunctional(entityName, properties) {
    const primaryTable = properties?.primaryTable;
    const fields = properties?.fields;
    const missing = [];
    if (typeof primaryTable !== 'string' || !primaryTable.trim())
        missing.push('primaryTable');
    if (!Array.isArray(fields) || fields.length === 0)
        missing.push('fields');
    if (missing.length === 0)
        return;
    throw new Error(`Data entity '${entityName}': missing ${missing.join(' and ')} — nothing was written. ` +
...
function buildFieldGroupsXml(groups) {
    if (groups.length === 0)
        return '\t<FieldGroups />\n';
    const body = groups
        .map(g => {
...
function buildSourceCodeXml(entityName, declaration, methods) {
    const decl = (declaration && declaration.trim())
        ? declaration.replace(/\s+$/, '')
        : `public class ${entityName} extends common\n{\n}`;
    const methodsXml = methods && methods.length > 0
        ? '\t\t<Methods>\n' +
            methods
                .map(m => `\t\t\t<Method>\n\t\t\t\t<Name>${m.name}</Name>\n\t\t\t\t<Source><![CDATA[\n${(m.source ?? '').replace(/\s+$/, '')}\n\n]]></Source>\n\t\t\t</Method>`)
                .join('\n') +
...
export function buildAxDataEntityXml(entityName, properties) {
    ...
    const entityCategory = assertKnownEnumValue(`Data entity '${entityName}': entityCategory`, properties?.entityCategory, ENTITY_CATEGORIES, 'Transaction');
    const primaryTable = properties?.primaryTable;
    const fields = Array.isArray(properties?.fields) ? properties.fields : undefined;
    ...
    const changeTracking = isYes(properties?.allowRowVersionChangeTracking ?? properties?.changeTrackingEnabled);
    ...
    const publicNamesXml = isPublic
        ...
        : '';
    const declaration = typeof properties?.declaration === 'string' ? properties.declaration : undefined;
    const methods = Array.isArray(properties?.methods) ? properties.methods : undefined;
    const fieldGroups = Array.isArray(properties?.fieldGroups) ? properties.fieldGroups : undefined;
    const standardStructure = properties?.standardStructure === true ||
...
`;
    ...
    const keyName = properties?.primaryKey || properties?.entityKeyName || 'EntityKey';
    const keyFields = Array.isArray(properties?.primaryKeyFields) && properties.primaryKeyFields.length > 0
        ? properties.primaryKeyFields.map((f) => String(f))
...
//# sourceMappingURL=dataEntityXml.js.map