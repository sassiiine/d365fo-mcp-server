...
const ACCESS_LEVELS = ['view', 'read', 'maintain'];
export function buildAxSecurityPrivilegeXml(name, properties) {
    const label = properties?.label || '@TODO:LabelId';
    const targetObject = properties?.targetObject;
    ...
    const objType = assertKnownEnumValue(`Security privilege '${name}': objectType`, properties?.objectType, SECURITY_ENTRY_POINT_TYPES, 'MenuItemDisplay');
    ...
    if (!ACCESS_LEVELS.includes(rawAccess)) {
        throw new Error(`Security privilege '${name}': accessLevel "${properties?.accessLevel}" is not supported — ` +
    ...
    const dataEntity = properties?.dataEntity;
    let dataEntityPermissionsXml;
    if (dataEntity) {
...
//# sourceMappingURL=securityPrivilegeXml.js.map