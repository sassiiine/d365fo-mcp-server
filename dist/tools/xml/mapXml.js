...
import { axMapFieldElement, normalizeFieldBaseType, unknownFieldTypeMessage, } from '../../utils/axFieldTypes.js';
/**
...
export function buildAxMapXml(mapName, properties) {
    ...
    const fieldsXml = fields.length
        ? fields.map(f => {
            ...
            if (!base)
                throw new Error(unknownFieldTypeMessage(`Map field "${f.name}"`, String(f.type)));
            const axType = axMapFieldElement(base);
            const inner = [`\t\t\t<Name>${f.name}</Name>`];
            const edt = f.extendedDataType || f.edt;
            if (edt)
                inner.push(`\t\t\t<ExtendedDataType>${edt}</ExtendedDataType>`);
