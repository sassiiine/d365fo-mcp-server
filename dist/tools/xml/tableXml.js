 ...
 */
export function fieldTypeToAxType(fieldType, edtName) {
...
export function buildAxTableFieldsXml(fieldSpecs) {
 ...
 */
export function buildAxTableXml(tableName, properties, parsedSource) {
    ...
    const propertiesXml = renderAxTableProperties({
        ...
        AllowRowVersionChangeTracking: isYes(properties?.allowRowVersionChangeTracking) ? 'Yes' : undefined,
    ...
    const overviewGroupXml = fieldSpecs.length === 0 ? '' :
        `\t\t<AxTableFieldGroup>\n` +
            ...
            `\t\t\t<Label>@SYS9039</Label>\n` +
            `\t\t\t<Fields>\n` +
            fieldSpecs.map(f => `\t\t\t\t<AxTableFieldGroupField>\n\t\t\t\t\t<DataField>${f.name}</DataField>\n\t\t\t\t</AxTableFieldGroupField>\n`).join('') +
            `\t\t\t</Fields>\n` +
            `\t\t</AxTableFieldGroup>\n`;
    ...
    const declarationXpp = parsedSource?.declaration?.trim()
...
//# sourceMappingURL=tableXml.js.map