 ...
 */
import { AX_TABLE_ELEMENT_ORDER, AX_TABLE_NON_EXISTENT_PROPERTIES, upsertAxTableProperty, } from '../../utils/axTablePropertyOrder.js';
 ...
 */
const AX_TABLE_OMITTED_DEFAULTS = {
...
function sectionBody(xml, container) {
    if (new RegExp(`<${container}\\s*/>`).test(xml))
        return '';
...
function collectionMemberName(item) {
 ...
 */
export function findDroppedTableCollections(xml, properties) {
    if (!properties)
        return [];
    const dropped = [];
    for (const { key, container, plural, repair } of TABLE_COLLECTIONS) {
        ...
        const missing = requested
            ...
            .filter((name) => typeof name === 'string' && !new RegExp(`<Name>\\s*${escapeRegExp(name)}\\s*</Name>`, 'i').test(body));
        if (missing.length > 0)
            dropped.push({ key, plural, missing, repair });
    }
    return dropped;
}
...
function canonicalElement(key) {
    const lower = key.toLowerCase();
    const known = AX_TABLE_ELEMENT_ORDER.find((e) => e.toLowerCase() === lower);
    if (known)
        return known;
...
function normaliseValue(element, raw) {
    const s = String(raw);
    if (element && NO_YES_PROPERTIES.has(element)) {
        if (/^(true|yes|1)$/i.test(s))
            return 'Yes';
...
function elementPresent(xml, element) {
 ...
 */
export function reconcileTableCreateProperties(xml, properties) {
    const result = { xml, patched: [], unhonoured: [], droppedCollections: [] };
    if (!properties || !/<AxTable[\s>]/.test(xml))
        return result;
    ...
    for (const [key, rawValue] of Object.entries(properties)) {
        ...
        if (!['string', 'number', 'boolean'].includes(typeof rawValue))
            continue;
        if (STRUCTURAL_TABLE_KEYS.has(key.toLowerCase()))
            continue;
        if (String(rawValue).length === 0)
            continue;
        const element = canonicalElement(key);
        const value = normaliseValue(element, rawValue);
        ...
        if (elementPresent(result.xml, probe))
            continue;
        if (!element) {
            result.unhonoured.push({
                name: key,
                value,
                detail: `'${key}' is not a known AxTable property, so the writer could not place it in the ` +
        ...
        if (omitted?.some(d => d.toLowerCase() === value.toLowerCase()))
            continue;
        ...
        if (patchedXml) {
            ...
            result.unhonoured.push({
                name: key,
                value,
                detail: `<${element}> has no defined position in the AxTable element order, so writing it ` +
 ...
 */
export function renderTableCreateHonestyReport(result) {
    const parts = [];
...
//# sourceMappingURL=createTablePropertyHonesty.js.map