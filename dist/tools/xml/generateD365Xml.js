...
import { buildAxTableXml, fieldEdt, fieldTypeToAxType } from './tableXml.js';
import { buildAxFormXml } from './formXml.js';
import { buildAxSecurityDutyXml, buildAxSecurityRoleXml, buildAxSecurityDutyExtensionXml, buildAxSecurityRoleExtensionXml, } from './securityDutyRoleXml.js';
import { z } from 'zod';
...
import { makeEdtTypeLookup } from '../../validation/edtTypeLookup.js';
import { buildAxDataEntityXml, assertDataEntityIsFunctional } from './dataEntityXml.js';
import { assertKnownEnumValue, resolveEnumValueMode, RELATED_TABLE_CARDINALITIES, RELATION_CARDINALITIES, RELATIONSHIP_TYPES, } from '../../utils/axEnumProperties.js';
...
import { buildAxMenuItemExtensionXml } from './menuItemExtensionXml.js';
        ...
        switch (objectType) {
            ...
            case 'data-entity': {
                ...
                if (typeof entitySource === 'string' && entitySource.trim()) {
                    const split = XmlTemplateGenerator.splitXppClassSource(decodeXmlEntitiesFromXppSource(entitySource));
                    entityProps = {
    ...
    static generateAxTableExtensionXml(name, properties) {
        ...
        if (fieldSpecs.length === 0) {
            ...
            for (const f of fieldSpecs) {
                ...
                if (edt)
                    fieldsXml += `\t\t\t<ExtendedDataType>${edt}</ExtendedDataType>\n`;
                if (f.label)
 ...
 */
let edtLookup;
function getEdtTypeLookup() {
    if (edtLookup === undefined)
        edtLookup = makeEdtTypeLookup();
    return edtLookup;
}
export async function handleGenerateD365Xml(request) {
    try {
        ...
        console.error(`[generate_d365fo_xml] Generating XML for ${args.objectType}: ${args.objectName}, model: ${modelName}`);
        ...
        if (Array.isArray(args.properties?.fields)) {
            args.properties.fields = await resolveFieldElementTypes(args.properties.fields, getEdtTypeLookup());
        }
        // Generate XML content
        let xmlContent = XmlTemplateGenerator.generate(args.objectType, args.objectName, args.sourceCode, args.properties);
        ...
        if (args.objectType === 'report') {
            xmlContent = xmlContent.replace(/<Text><!\[CDATA\[([\s\S]*?)\]\]><\/Text>/g, (_m, inner) => `<Text>${escapeXml(inner)}</Text>`);
        }
        console.error(`[generate_d365fo_xml] Generated XML content: ${xmlContent.length} bytes`);
...
//# sourceMappingURL=generateD365Xml.js.map