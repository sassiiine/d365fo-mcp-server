...
export async function completionTool(request, context) {
    try {
        ...
        const bridgeResult = await tryBridgeCompletion(context.bridge, args.className, args.prefix || undefined, ancestors);
        if (bridgeResult)
            return bridgeResult;
 ...
 */
function formatCompletions(completions, className, prefix) {
 ...
 */
async function getWorkspaceCompletions(args, scanner) {
    try {
