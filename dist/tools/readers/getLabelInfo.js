...
const GetLabelInfoArgsSchema = z.object({
    labelId: z
        .string()
        .optional()
        .describe('Exact label ID to look up (e.g. MyFeature, BatchGroup). ' +
...
export async function getLabelInfoTool(request, context) {
    try {
            ...
            for (const f of files) {
                ...
                if (labelFileId) {
                    ...
                    if (paths.length > 0) {
                        ...
                        lines.push(`  Files       : (no physical path indexed — label rows may predate path tracking; ` +
                            `re-run build-database with INCLUDE_LABELS=true to populate file paths)`);
                    }
                }
                lines.push('');
            }
            lines.push(`💡 Use labels(action="search") to find a label by text, or labels(action="info") with labelId to see all translations.`);
        ...
        if (rows.length === 0) {
            ...
            return {
                content: [
                    {
                        type: 'text',
                        text: `No label matched the ID "${labelId}"` +
...
//# sourceMappingURL=getLabelInfo.js.map