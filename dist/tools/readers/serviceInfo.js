...
export async function getServiceInfoTool(request, context) {
    try {
        ...
        const symbol = db.prepare(`SELECT name, signature, description, model, file_path FROM symbols WHERE name = ? AND type = 'service' LIMIT 1`).get(serviceName);
        if (!symbol) {
            return {
                content: [{
                        type: 'text',
        ...
        const operations = db.prepare(`SELECT operation_name, method_name, idempotent FROM service_operations WHERE service_name = ? ORDER BY operation_name`).all(serviceName);
        const groups = db.prepare(`SELECT DISTINCT group_name FROM service_group_members WHERE service_name = ? ORDER BY group_name`).all(serviceName);
        ...
        if (externalName)
            lines.push(`**External name:** ${externalName}`);
