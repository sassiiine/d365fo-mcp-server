...
function candidateTypes(objectType) {
    if (objectType === 'menu-item')
        return MENU_ITEM_TYPES;
    if (objectType === 'auto')
        return ['form', 'table', 'class', ...MENU_ITEM_TYPES];
    return [objectType];
}
/**
 ...
 */
const OLS_UNKNOWN = `Row-level security (OLS): ⚠️ unknown — AxSecurityPolicy objects are not indexed in this database.\n` +
 ...
 */
function policiesIndexed(db) {
    return db.prepare('SELECT 1 FROM security_policies LIMIT 1').get() !== undefined;
}
export async function securityCoverageInfoTool(request, context) {
    try {
        ...
        if (resolvedType === 'auto') {
            const sym = db.prepare(`SELECT type FROM symbols WHERE name = ? AND type IN ('form','table','class','menu-item-display','menu-item-action','menu-item-output')
         ORDER BY CASE type WHEN 'form' THEN 0 WHEN 'table' THEN 1 WHEN 'class' THEN 2 ELSE 3 END LIMIT 1`).get(objName);
            if (sym) {
                resolvedType = sym.type.startsWith('menu-item') ? 'menu-item' : sym.type;
            }
        }
        ...
        if (resolvedType === 'table' || resolvedType === 'auto') {
            try {
                const policies = db.prepare(`SELECT policy_name, query_name, operation, constrained_table, label
           FROM security_policies WHERE primary_table = ? ORDER BY policy_name`).all(objName);
                if (policies.length > 0) {
                    ...
                    for (const p of policies) {
                        ...
                        if (p.query_name)
                            olsSection += ` via query ${p.query_name}`;
                        if (p.constrained_table)
                            olsSection += ` (constrained)`;
                        olsSection += '\n';
                    }
                    olsSection += '\n';
                }
                ...
                if (process.env.DEBUG_LOGGING === 'true')
                    console.warn('[securityCoverageInfo] security_policies query failed:', e);
                olsSection += OLS_UNKNOWN;
            }
        }
        ...
        let menuItems = [];
        try {
            menuItems = db.prepare(`SELECT menu_item_name, menu_item_type, target_object, target_type FROM menu_item_targets
         WHERE target_object = ?
         ORDER BY menu_item_type, menu_item_name`).all(objName);
        }
        catch (e) {
            // menu_item_targets table may not exist in older databases — non-fatal
            if (process.env.DEBUG_LOGGING === 'true')
                console.warn('[securityCoverageInfo] menu_item_targets query failed:', e);
        }
        // Fallback: if the object IS a menu item, use it directly
        if (menuItems.length === 0 && (resolvedType === 'menu-item' || resolvedType === 'auto')) {
            const directMenuItem = db.prepare(`SELECT name as menu_item_name, type as menu_item_type FROM symbols
         WHERE name = ? AND type IN ('menu-item-display','menu-item-action','menu-item-output') LIMIT 1`).get(objName);
            if (directMenuItem) {
        ...
        if (menuItems.length === 0) {
            const sameNameMenuItems = db.prepare(`SELECT name as menu_item_name, type as menu_item_type FROM symbols
         WHERE name = ? AND type IN ('menu-item-display','menu-item-action','menu-item-output')`).all(objName);
        ...
        const allPrivEntries = db.prepare(`SELECT DISTINCT entry_point_name, privilege_name, object_type, access_level
       FROM security_privilege_entries
       WHERE entry_point_name IN (${miPH})
       ORDER BY entry_point_name, privilege_name`).all(...miNames);
        const privilegesByMi = new Map();
        for (const pe of allPrivEntries) {
            if (!privilegesByMi.has(pe.entry_point_name))
                privilegesByMi.set(pe.entry_point_name, []);
        ...
        const dutiesByPrivilege = new Map();
        const dutiesCounts = new Map();
        const rolesByDuty = new Map();
        const rolesCounts = new Map();
        if (allPrivNames.length > 0) {
            const privPH = allPrivNames.map(() => '?').join(',');
            const allDutyRows = db.prepare(`SELECT privilege_name, duty_name FROM security_duty_privileges
         WHERE privilege_name IN (${privPH})
         ORDER BY duty_name`).all(...allPrivNames);
            for (const d of allDutyRows) {
                if (!dutiesByPrivilege.has(d.privilege_name))
                    dutiesByPrivilege.set(d.privilege_name, []);
            ...
            if (allDutyNames.length > 0) {
                ...
                const allRoleRows = db.prepare(`SELECT duty_name, role_name FROM security_role_duties
           WHERE duty_name IN (${dutyPH})
           ORDER BY role_name`).all(...allDutyNames);
                for (const r of allRoleRows) {
                    if (!rolesByDuty.has(r.duty_name))
                        rolesByDuty.set(r.duty_name, []);
...
//# sourceMappingURL=securityCoverageInfo.js.map