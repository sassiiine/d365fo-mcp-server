...
export async function dataEntityInfoTool(request, context) {
    try {
        ...
        const viaView = await getViewInfoTool({ method: 'tools/call', params: { name: 'get_view_info', arguments: { viewName: args.entityName } } }, context);
        if (!viaView?.isError)
            return viaView;
        ...
        try {
            ...
            const suggestions = db.prepare(`SELECT name, model FROM symbols WHERE type = 'view' AND name LIKE ? ORDER BY name LIMIT 10`).all(`%${args.entityName}%`);
...
//# sourceMappingURL=dataEntityInfo.js.map