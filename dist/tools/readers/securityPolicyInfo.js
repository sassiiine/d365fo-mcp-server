...
export async function getSecurityPolicyInfoTool(request, context) {
    try {
        ...
        const policy = db.prepare(`SELECT policy_name, primary_table, query_name, operation, constrained_table, label, model
       FROM security_policies WHERE policy_name = ? COLLATE NOCASE LIMIT 1`).get(policyName);
        if (!policy) {
            return {
        ...
        const file = db.prepare(`SELECT file_path FROM symbols WHERE name = ? AND type = 'security-policy' LIMIT 1`).get(policy.policy_name);
        const lines = [];
        lines.push(`# AxSecurityPolicy: \`${policy.policy_name}\``);
        lines.push('');
        lines.push(`**Model:** ${policy.model}`);
        if (policy.label)
            lines.push(`**Label:** ${policy.label}`);
        ...
        if (file?.file_path)
            lines.push(`**File:** \`${file.file_path}\``);
...
//# sourceMappingURL=securityPolicyInfo.js.map