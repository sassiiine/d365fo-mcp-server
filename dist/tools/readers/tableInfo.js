...
export async function tableInfoTool(request, context) {
    try {
        ...
        const bridgeResult = await tryBridgeTable(context.bridge, args.tableName, args.methodOffset, args.fieldsOffset, args.fieldFilter);
        if (bridgeResult) {
            return bridgeResult;
        }
        // 2. DB index fallback — serves offline / write-only / build-agent scenarios.
        const { symbolIndex } = context;
        const dbResponse = buildTableResponseFromDb(symbolIndex, args.tableName, args.methodOffset, args.fieldsOffset, args.fieldFilter);
        if (dbResponse) {
            return dbResponse;
        }
 ...
 */
function buildTableResponseFromDb(symbolIndex, tableName, methodOffset, fieldsOffset = 0, fieldFilter) {
    const tableSym = symbolIndex.getSymbolByName?.(tableName, 'table');
    if (!tableSym)
        return null;
    if (tableSym.filePath && !fs.existsSync(tableSym.filePath)) {
        console.error(`[tableInfo] Stale symbol-index entry for table '${tableName}' — indexed file ` +
    ...
    const fields = rdb.prepare(`SELECT name, signature FROM symbols WHERE parent_name = ? AND type = 'field' ORDER BY name`).all(tableName);
    const methods = rdb.prepare(`SELECT name, signature FROM symbols WHERE parent_name = ? AND type = 'method' ORDER BY name`).all(tableName);
