...
import { resolveIndexedObject, readXmlFile, indexedSourceNote, bridgeUnavailableNote, } from '../../utils/indexedXmlLookup.js';
...
export async function getEnumInfoTool(request, context) {
    try {
        ...
        const ref = await resolveIndexedObject(context.symbolIndex.getReadDb(), args.enumName, ['enum'], args.modelName);
        if (ref) {
        ...
        if (diskPath) {
            ...
            const formatted = xml && await formatEnumXml(args.enumName, args.modelName ?? 'Unknown', xml, `live file (not yet in bridge metadata): ${diskPath}`, args.includeLabels);
            if (formatted)
                return formatted;
        }
...
async function formatEnumXml(enumName, model, xml, source, includeLabels, ref) {
    let axEnum;
    try {
    ...
    if (!axEnum)
        return null;
    const values = (axEnum.EnumValues?.[0]?.AxEnumValue ?? []).map((v) => ({
