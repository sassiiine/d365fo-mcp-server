...
export async function getMethodSourceTool(request, context) {
    try {
        ...
        try {
            ...
            const candidates = db.prepare(`SELECT name, signature FROM symbols
         WHERE type = 'method' AND parent_name = ? AND name LIKE ?
         ORDER BY name LIMIT 10`).all(className, `%${methodName.replace(/^parm/i, '')}%`);
            if (candidates.length > 0) {
                hint = '\n\n**Similar methods on this class:**\n' +
                    candidates.map(c => `- \`${c.signature || c.name}\``).join('\n');
            }
            else if (/^parm/i.test(methodName)) {
                const parmMethods = db.prepare(`SELECT name, signature FROM symbols
           WHERE type = 'method' AND parent_name = ? AND name LIKE 'parm%'
           ORDER BY name LIMIT 15`).all(className);
 ...
 */
function resolveClassName(context, requested) {
    try {
 ...
 */
async function tryInheritedMethodSource(context, className, methodName) {
    let rdb;
    try {
    ...
    for (const ancestor of inheritedOwnerCandidates(rdb, className, methodName, bridgeUsable)) {
        ...
        if (fromBridge)
            return annotateInherited(fromBridge, className, ancestor, methodName);
        const fromXml = await tryXmlMethodSource(context, ancestor, methodName);
        if (fromXml)
            return annotateInherited(fromXml, className, ancestor, methodName);
    }
    return null;
}
/** Insert the "inherited from" note under the result's `## Class.method` heading. */
function annotateInherited(result, requestedClass, declaringClass, methodName) {
 ...
 */
async function tryXmlMethodSource(context, className, methodName) {
    ...
    try {
        ...
        const text = `## ${className}.${method.name}\n\n` +
...
//# sourceMappingURL=getMethodSource.js.map