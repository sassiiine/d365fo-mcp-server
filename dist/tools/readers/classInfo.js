...
export async function classInfoTool(request, context) {
    try {
 ...
 */
async function buildDbOnlyResponse(className, classSymbol, symbolIndex, methodOffset) {
    const methods = symbolIndex.getClassMethods(className);
    // Build a concise header
    let output = `# Class: ${className}`;
    if (classSymbol.extendsClass)
        output += ` extends ${classSymbol.extendsClass}`;
    ...
    if (classSymbol.visibility)
        output += `  **Access:** ${classSymbol.visibility}`;
    if (classSymbol.implementsInterfaces)
        output += `  **Implements:** ${classSymbol.implementsInterfaces}`;
    output += '\n\n';
 ...
 */
async function searchInWorkspace(args, scanner) {
    try {
