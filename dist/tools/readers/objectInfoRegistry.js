 ...
 */
export function withNotFoundGuidance(result, name, objectType) {
    if (!result?.isError)
        return result;
    const text = result.content?.[0]?.text;
    if (!isNotFoundResultText(text))
        return result;
    if (/Type Mismatch/i.test(text) || /Resolve `.*` .* with the right tool/.test(text))
        return result;
    return {
        ...result,
...
const byName = (key) => (name, options) => ({ [key]: name, ...(options ?? {}) });
/**
 ...
 */
const byBaseName = (key) => (name, options) => ({ [key]: name.includes('.') ? name.split('.')[0] : name, ...(options ?? {}) });
export const READER_DISPATCH = {
