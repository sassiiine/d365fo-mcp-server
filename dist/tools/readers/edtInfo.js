...
export async function getEdtInfoTool(request, context) {
    try {
        ...
        return {
            content: [{
                    type: 'text',
                    text: `No data available for EDT "${edtName}".\n\n` +
 ...
 */
function getEdtFromIndex(symbolIndex, edtName, modelName) {
    let db;
    try {
    ...
    const readRow = (name) => {
        try {
            return db.prepare(`SELECT edt_name, extends, enum_type, reference_table, relation_type,
                string_size, database_string_size, display_length, label, model
         FROM edt_metadata WHERE edt_name = ?${modelName ? ' AND model = ?' : ''} LIMIT 1`).get(...(modelName ? [name, modelName] : [name]));
        }
        catch {
            return undefined;
        }
    ...
    if (!row) {
        ...
        return {
            content: [{
                    type: 'text',
                    text: `# Extended Data Type: ${hit.name}\n\n` +
    ...
    if (row.enum_type)
        out += `| Enum Type | ${row.enum_type} |\n`;
    if (row.reference_table)
        out += `| Reference Table | ${row.reference_table} |\n`;
    if (row.relation_type)
        out += `| Relation Type | ${row.relation_type} |\n`;
    if (row.string_size)
        out += `| String Size | ${row.string_size} |\n`;
    if (row.database_string_size)
        out += `| Database String Size | ${row.database_string_size} |\n`;
    if (row.display_length)
        out += `| Display Length | ${row.display_length} |\n`;
    if (row.label)
        out += `| Label | ${row.label} |\n`;
...
function probeEdtRow(db, name, modelName) {
    try {
        return !!db.prepare(`SELECT 1 FROM edt_metadata WHERE edt_name = ?${modelName ? ' AND model = ?' : ''} LIMIT 1`).get(...(modelName ? [name, modelName] : [name]));
    }
    catch {
        return false;
    }
 ...
 */
function findFieldsTypedAs(db, edtName) {
    const safe = edtName.replace(/["\(\)\\]/g, '').trim();
    if (safe) {
        try {
            return db.prepare(`SELECT parent_name, name, model FROM symbols
          WHERE type = 'field'
            AND id IN (SELECT rowid FROM symbols_fts WHERE symbols_fts MATCH ?)
            AND signature = ? COLLATE NOCASE
          ORDER BY model, parent_name LIMIT 50`).all(`{signature} : "${safe}"`, edtName);
        }
        catch {
    ...
    try {
        ...
        ORDER BY model, parent_name LIMIT 50`).all(edtName);
    }
    catch {
        return [];
    }
 ...
 */
function getEdtHierarchy(db, edtName, modelName) {
    ...
    while (current && !visited.has(current)) {
      ...
      ${modelName ? 'AND model = ?' : ''}
      LIMIT 1
    `).get(...(modelName ? [current, modelName] : [current]));
        if (!row)
            break;
    ...
    const children = db.prepare(`SELECT edt_name, model, label FROM edt_metadata WHERE extends = ? ORDER BY model, edt_name`).all(edtName);
    ...
    for (const e of chain) {
        ...
        if (e.stringSize)
            output += `, StringSize: ${e.stringSize}`;
        if (e.extends)
            output += ` [extends ${e.extends}]`;
        output += '\n';
    }
    // Children
    ...
    if (children.length === 0) {
        ...
        if (children.length > 20)
            output += `  ... and ${children.length - 20} more\n`;
    }
    // Field usages
    ...
    if (fieldUsages.length === 0) {
        ...
        if (fieldUsages.length > 10)
            output += `  ... and ${fieldUsages.length - 10} more (${fieldUsages.length} total)\n`;
    }
...
//# sourceMappingURL=edtInfo.js.map