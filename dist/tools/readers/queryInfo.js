...
import { readIndexedXml, readXmlFile, indexedSourceNote, bridgeUnavailableNote, } from '../../utils/indexedXmlLookup.js';
...
export async function getQueryInfoTool(request, context) {
    try {
        ...
        const indexed = await readIndexedXml(context.symbolIndex.getReadDb(), args.queryName, ['query'], args.modelName);
        if (indexed) {
            const formatted = await formatQueryXml(indexed.ref.name, indexed.ref.model, indexed.xml, `XML: ${indexed.ref.localPath}`, args);
            if (formatted)
                return formatted;
        }
        // 3. Disk scan — a query created this session and not yet indexed
        ...
        if (diskPath) {
            ...
            const formatted = xml && await formatQueryXml(args.queryName, args.modelName ?? 'Unknown', xml, `live file (not yet in bridge metadata): ${diskPath}`, args);
            if (formatted)
                return formatted;
        }
...
async function formatQueryXml(queryName, model, xml, source, opts) {
    let axQuery;
    try {
    ...
    if (axQuery.Description?.[0])
        out += `**Description:** ${axQuery.Description[0]}\n`;
...
function extractDataSources(node, opts) {
    const out = [];
...
function extractRanges(node) {
    return (node.AxQuerySimpleDataSourceRange ?? []).map((r) => ({
...
function extractFields(node) {
    return (node.AxQuerySimpleDataSourceField ?? []).map((f) => f.Field?.[0] ?? 'Unknown');
}
function formatHierarchy(dataSources, indent, opts) {
    let out = '';
    ...
    for (const ds of dataSources) {
        ...
        if (opts.includeFields && ds.fields.length > 0) {
            ...
            for (const f of ds.fields.slice(0, 10))
                out += `${pad}- ${f}\n`;
            if (ds.fields.length > 10)
                out += `${pad}- ... (${ds.fields.length - 10} more fields)\n`;
            out += '\n';
        }
...
function countDataSources(dataSources) {
    return dataSources.reduce((n, ds) => n + 1 + countDataSources(ds.joins), 0);
}
function countRanges(dataSources) {
