...
import { createControlBudget, chargeControl, chargeSkippedSubtree, controlsFooter, DEFAULT_MAX_CONTROLS, } from '../../utils/payloadBudget.js';
const GetFormInfoArgsSchema = z.object({
    ...
    filePath: z.string().optional().describe('Absolute path to the form XML file on disk. ' +
    ...
    searchControl: z.string().optional().describe('Case-insensitive substring search for a control by name. ' +
    ...
    maxControls: z.number().optional().describe(`Cap on how many controls the tree renders (default ${DEFAULT_MAX_CONTROLS}). ` +
        'Prefer searchControl over raising this — a platform form has >1000 controls.'),
});
export async function getFormInfoTool(request, context) {
    try {
        const args = GetFormInfoArgsSchema.parse(request.params.arguments);
        const { formName, filePath: explicitFilePath, includeControls, includeDataSources, includeMethods, searchControl, maxControls, } = args;
        ...
        if (explicitFilePath) {
            ...
            if (!containment.ok) {
                return {
                    content: [{ type: 'text', text: `❌ get_object_info(form): filePath rejected — ${containment.reason}`,
                        }],
                    isError: true,
                };
            }
            ...
            try {
                ...
                return {
                    content: [{ type: 'text', text: `❌ get_object_info(form): cannot read form XML at explicit filePath="${explicitFilePath}": ` +
        ...
        const indexed = await readIndexedXml(context.symbolIndex.getReadDb(), formName, ['form'], args.modelName);
        if (indexed) {
            try {
                return await parseAndFormatForm(indexed.ref.name, indexed.ref.model, indexed.xml, includeControls, includeDataSources, includeMethods, searchControl, maxControls);
            }
 ...
 */
async function parseAndFormatForm(formName, modelName, xmlContent, includeControls, includeDataSources, includeMethods, searchControl, maxControls) {
 ...
 */
function searchControlsInHierarchy(controls, query, path = [], parentName = null) {
    const results = [];
 ...
 */
function formatControlSearchResults(formName, modelName, results, query) {
    ...
    for (let i = 0; i < results.length; i++) {
        ...
        if (r.control.children.length > 0) {
            ...
            for (const child of shown) {
                const extras = [];
                if (child.properties.DataSource)
                    extras.push(`DS: ${child.properties.DataSource}`);
                if (child.properties.DataField)
                    extras.push(`Field: ${child.properties.DataField}`);
                if (child.properties.Caption)
                    extras.push(`Caption: ${child.properties.Caption}`);
...
function extractDataSources(dataSourcesNode) {
    const dataSources = [];
    ...
    for (const dsNode of dsArray) {
        ...
        if (dsNode.Methods && dsNode.Methods[0] && dsNode.Methods[0].Method) {
            ds.methods = dsNode.Methods[0].Method.map((m) => m.Name ? m.Name[0] : 'Unknown');
        }
        dataSources.push(ds);
    }
    return dataSources;
}
function extractDataSourceFields(fieldsNode) {
    const fields = [];
...
function extractControls(designNode) {
    const controls = [];
...
function formatFormOutput(formInfo, includeControls, includeDataSources, includeMethods, maxControls) {
...
function formatControlHierarchy(controls, indent, budget) {
    let output = '';
