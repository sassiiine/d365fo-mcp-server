...
export async function getWorkspaceInfoTool(request, context) {
    ...
    if (args.projectName && !args.projectPath) {
        ...
        const selection = selectProject(args.projectName, allProjects);
    ...
    const { modelName, modelSource, isModelSourceAutoDetected, projectPath, projectSource, packagePath, packageSource, customPackagesPath, customPackagesSource, } = await configManager.getWorkspaceInfoDiagnostics();
    ...
    if (allProjects.length > 1) {
        if (diagnostics) {
            ...
            lines.push(`Projects    : ${allProjects.length} in solution` +
    ...
    if (toolSwitch) {
        lines.push(``, `## ⚠️  Project switched — writes are NOT switched`, ``, `"${toolSwitch.forcedModel}" is now the ACTIVE project. This did not change what you ` +
            ...
            `environment — that applies to the next attempt, no restart.`, `That setting is the USER'S to make, in their own editor. Do not write it yourself: editing ` +
    ...
    if (allowance) {
        lines.push(``, `## ⚠️  Cross-model writes are currently ALLOWED`, ``, `${allowance}.`, `Writes into that model will succeed and will NOT appear in this workspace's project or ` +
...
//# sourceMappingURL=getWorkspaceInfo.js.map