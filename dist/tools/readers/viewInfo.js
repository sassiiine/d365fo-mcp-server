...
import { resolveIndexedObject, indexedSourceNote, bridgeUnavailableNote, } from '../../utils/indexedXmlLookup.js';
...
export async function getViewInfoTool(request, context) {
    try {
 ...
 */
async function buildViewResponseFromIndex(context, viewName, modelName) {
    let ref;
    try {
...
async function buildViewResponseFromDisk(context, viewName, modelName) {
    for (const objectType of ['view', 'data-entity']) {
...
function formatViewLike(v, model, source, ref) {
    ...
    if (fields.length > 0) {
        ...
        if (mapped.length > 0) {
            ...
            for (const f of mapped)
                out += `| ${f.name} | ${f.dataSource ?? '—'} | ${f.dataField ?? '—'} |\n`;
            out += '\n';
        }
    ...
    if (methods.length > 0) {
        ...
        for (const m of methods)
            out += `- ${typeof m === 'string' ? m : m.name}\n`;
    }
    return out;
}
/** Build a minimal view report from the field/method rows stored in the symbol index. */
function formatViewFromSymbols(context, viewName, model) {
    let fields = [];
    let methods = [];
    try {
        const db = context.symbolIndex.getReadDb();
        fields = db.prepare(`SELECT name, signature FROM symbols WHERE parent_name = ? AND type = 'field' ORDER BY name`).all(viewName);
        methods = db.prepare(`SELECT name, signature FROM symbols WHERE parent_name = ? AND type = 'method' ORDER BY name`).all(viewName);
    }
...
function buildNotFoundText(context, viewName) {
