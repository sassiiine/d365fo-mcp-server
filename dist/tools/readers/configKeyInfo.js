...
export async function getConfigKeyInfoTool(request, context) {
    try {
        ...
        const key = db.prepare(`SELECT name, description, signature, model, file_path FROM symbols WHERE name = ? AND type = 'configuration-key' LIMIT 1`).get(name);
        if (key) {
            const lines = [];
            ...
            if (key.description)
                lines.push(`**Label:** ${key.description}`);
            lines.push(`**File:** \`${key.file_path}\``);
            // Walk up the parent chain (bounded to avoid cycles).
            const chain = [];
            let cursor = key.signature || undefined;
            const seen = new Set([key.name]);
            while (cursor && !seen.has(cursor) && chain.length < 20) {
                chain.push(cursor);
                seen.add(cursor);
                const parent = db.prepare(`SELECT signature FROM symbols WHERE name = ? AND type = 'configuration-key' LIMIT 1`).get(cursor);
                cursor = parent?.signature || undefined;
            }
            lines.push(`**Parent chain:** ${chain.length > 0 ? chain.map(c => `\`${c}\``).join(' → ') : '— (root key)'}`);
            const children = db.prepare(`SELECT name FROM symbols WHERE type = 'configuration-key' AND signature = ? ORDER BY name`).all(key.name);
            lines.push('');
        ...
        const lic = db.prepare(`SELECT name, description, signature, model, file_path FROM symbols WHERE name = ? AND type = 'license-code' LIMIT 1`).get(name);
        if (lic) {
            const lines = [];
            ...
            if (lic.description)
                lines.push(`**Label:** ${lic.description}`);
            if (lic.signature)
                lines.push(`**Group / Type:** ${lic.signature}`);
...
//# sourceMappingURL=configKeyInfo.js.map