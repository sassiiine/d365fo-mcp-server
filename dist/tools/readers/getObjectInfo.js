...
const OBJECT_TYPE_DESCRIPTION = 'Kind of object to read: class, table, form, query, view, enum, edt, report, ' +
...
const OPTIONS_DESCRIPTION = 'Optional type-specific flags forwarded to the reader. ' +
...
const GetObjectInfoArgsSchema = z.object({
    objects: z.array(ObjectRefSchema).min(1).max(MAX_OBJECTS).optional().describe(`Read 2+ objects in ONE call (max ${MAX_OBJECTS}) — all lookups run in parallel and come back as ` +
 ...
 */
function toObjectRefs(args) {
    const shared = { ...args.options };
    if (args.methodOffset !== undefined)
        shared.methodOffset = args.methodOffset;
    if (args.compact !== undefined)
        shared.compact = args.compact;
...
async function readObject(ref, context) {
 ...
 */
function joinContentText(result) {
 ...
 */
async function readObjects(refs, context) {
    ...
    const sections = results.map((r, i) => `## ${i + 1}. ${r.name} [${r.objectType.toUpperCase()}] ${r.success ? '' : '❌'}\n\n${r.text}`);
    const header = `# Object Info\n\n` +
...
export async function getObjectInfoTool(request, context) {
...
//# sourceMappingURL=getObjectInfo.js.map