...
if (LOG_FILE) {
    try {
        ...
        const teeWrite = function (chunk, ...rest) {
            try {
...
console.error = (...args) => {
    ...
    const hasErrorIndicator = firstArg.includes('Failed') ||
...
const isStdioMode = !process.env.WEBSITES_PORT &&
...
async function initializeServices() {
    ...
    if (SERVER_MODE === 'write-only') {
        ...
        const context = { symbolIndex, parser, workspaceScanner, hybridSearch };
...
async function initializeBridge(targetContext) {
    try {
...
async function main() {
     ...
     */
    function createInitializeParamsSniffer() {
    ...
    if (isStdioMode) {
        ...
        console.warn = (...args) => process.stdout.write('[WARN] ' + args.join(' ') + '\n');
    }
    if (isStdioMode) {
        ...
        let resolveDbReady;
        let rejectDbReady;
        const dbReadyPromise = new Promise((res, rej) => {
            resolveDbReady = res;
            rejectDbReady = rej;
        });
        ...
        const transport = new StdioServerTransport(createInitializeParamsSniffer(), process.stdout);
        ...
        stubContext.bridgeStartup = trackBridgeStartup(initializeBridge(stubContext).then(s => (s.ok ? log.ok(s.summary) : log.warn(s.summary))));
        ...
        initializeServices().then(async ({ mcpServer, symbolIndex, parser, workspaceScanner, hybridSearch, context }) => {
            ...
            if (context) {
                ...
                const status = await Promise.race([
                    attempt,
                    new Promise(r => setTimeout(() => r({ ok: true, summary: 'C# bridge still connecting in the background' + glyph.ellipsis }), 6000)),
                ]);
                if (status.ok)
                    log.ok(status.summary);
                else
                    log.warn(status.summary);
                if (status.detail)
            ...
            const nameW = Math.min(26, Math.max(...filteredCatalog.flatMap(cat => cat.tools.map(t => t.name.length))) + 2);
