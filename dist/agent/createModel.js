...
export function buildProjectXml(modelName, guid) {
    return [
        '<?xml version="1.0" encoding="utf-8"?>',
...
export async function createModel(req) {
    ...
    if (!packages)
        throw new Error('No packages root. Pass packagesPath or set D365FO_PACKAGE_PATH.');
    ...
    for (const f of SKELETON)
        await fs.mkdir(join(modelRoot, f), { recursive: true });
    ...
    if (useRepo) {
        ...
        try {
            ...
            warnings.push(`Created the model under ${metadataRoot} but could not link it into ${packages} ` +
