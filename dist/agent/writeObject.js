 ...
 */
export function normalizeMetadataXml(content) {
    return content.replace(/\r\n/g, '\n').replace(/\n/g, '\r\n').replace(/\s+$/, '');
}
/** Absolute path an object of this type/name/model belongs at. */
export function resolveObjectPath(req) {
    const folder = aotFolder(req.objectType);
    if (!folder)
        throw new Error(`Unknown object type '${req.objectType}'.`);
    const packages = req.packagePath || process.env.D365FO_PACKAGE_PATH;
    if (!packages) {
        throw new Error('No packages root. Pass packagePath, or set D365FO_PACKAGE_PATH to the ' +
...
export async function writeObject(req) {
    const warnings = [];
    if (!req.xmlContent?.trim()) {
        throw new Error('xmlContent is required. This agent writes XML; it does not generate it - ' +
    ...
    if (!req.overwrite) {
        try {
            ...
            if (e instanceof Error && !/ENOENT/.test(String(e.code ?? e.message)))
                throw e;
        }
    }
    ...
    if (req.projectPath) {
        ...
        if (!addedToProject)
            warnings.push(`Could not add the file to ${req.projectPath}; the model-level build is unaffected.`);
    }
    else {
...
//# sourceMappingURL=writeObject.js.map