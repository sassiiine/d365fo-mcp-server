...
const TOOLS = [
    {
        name: 'd365fo_file',
        description: 'Write an AOT object to this machine. action="create" (or "modify") with xmlContent authored by the hosted ' +
    ...
    },
    {
        ...
        description: 'Compile a model with xppc. Returns the compiler output verbatim — diagnosis of the errors belongs to the ' +
        ...
        inputSchema: {
            ...
            properties: {
                ...
                fullBuild: {
                    type: 'boolean',
                    description: 'Full recompile of the target model. Default false is INCREMENTAL, which only compiles changed ' +
...
async function handle(name, args) {
    ...
    switch (name) {
        case 'd365fo_file': {
            if (!model)
                return text('No model. Pass modelName or set D365FO_MODEL_NAME.', true);
            ...
            return text(`Wrote ${args.objectType} ${args.objectName} (${r.bytes} bytes)\n` +
                `Path: ${r.path}\n` +
        ...
        case 'build_d365fo_project': {
            ...
            return text(`${failed ? 'BUILD FAILED' : 'Build finished'} — ${model}, ${full ? 'FULL' : 'incremental'}\n` +
