...
function representativeRun(cluster, runs) {
...
export function topPriorityCluster(runs) {
...
export function renderFixBrief(cluster, runs) {
    ...
    lines.push('', '## Task', '1. Reproduce this as a minimal FAILING test under `tests/` (mirroring the root cause above) — a real server-side gap must be demonstrable without the D365FO VM.', '2. Fix the root cause in `src/`.', '3. Run `npx vitest run` — the full suite must pass, including the new regression test.', '4. Run `npm run eval:clusters` against the corpus (if available) and confirm this symptom no longer ranks — or note that the corpus is VM-side evidence not present in this checkout, and the regression test is the durable proof instead.', '5. Open a PR citing this brief. Do NOT merge — a human reviews and merges.');
    return lines.join('\n');
}
/** Build the brief for the single top-priority cluster, or null if the corpus has nothing actionable. */
export function buildTopFixBrief(runs) {
    const top = topPriorityCluster(runs);
    if (!top)
        return null;
    return renderFixBrief(top, runs);
}
//# sourceMappingURL=fixBrief.js.map