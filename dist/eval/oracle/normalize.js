...
import { canonicalizePrefix } from './prefix.js';
 ...
 */
export const GOLDEN_CAPTURE_PREFIXES = ['Contoso', 'Con'];
export { canonicalizePrefix, PREFIX_PLACEHOLDER } from './prefix.js';
 ...
 */
function isXppSourcePath(pathPrefix) {
 ...
 */
function canonicalizeXppSourceText(s) {
 ...
 */
function canonicalizeXppDocComments(s) {
    const out = [];
    let inRun = false;
    for (const line of s.split('\n')) {
        ...
        if (m) {
            if (!inRun)
                out.push(`${m[1]}${XMLDOC_PLACEHOLDER}`);
            inRun = true;
            continue;
        }
        inRun = false;
 ...
 */
function normalizeText(s, pathPrefix) {
 ...
 */
function looksAutoGenerated(name, tag) {
    const unprefixed = tag.replace(/^Ax/, '');
    if (unprefixed.length === 0 || !name.startsWith(unprefixed))
        return false;
 ...
 */
function nestedName(node) {
    for (const [key, value] of Object.entries(node)) {
        if (key === '$' || key === '_' || key === 'Name' || key === 'DataField')
            continue;
        const candidates = Array.isArray(value) ? value : [value];
        for (const child of candidates) {
            if (child && typeof child === 'object' && !Array.isArray(child)) {
                const childName = ownName(child);
                if (childName !== undefined && !looksAutoGenerated(childName, key))
                    return childName;
            }
        }
    }
...
function emitAttrs(attrs, pathPrefix, out, matchers, prefix) {
    ...
    for (const [name, value] of Object.entries(attrs)) {
        ...
        if (!isIgnored(path, matchers))
            out.set(path, canonicalizePrefix(String(value).trim(), prefix));
    }
}
function walk(node, pathPrefix, out, matchers, prefix) {
    if (node == null)
        return;
    // Leaf: a plain string is element text.
    if (typeof node === 'string') {
        const v = normalizeText(node, pathPrefix);
        if (v !== '' && !isIgnored(pathPrefix, matchers))
            out.set(pathPrefix, canonicalizePrefix(v, prefix));
        return;
    }
    ...
    if ('_' in obj || '$' in obj) {
        emitAttrs(obj.$, pathPrefix, out, matchers, prefix);
    ...
    for (const [key, value] of Object.entries(obj)) {
        ...
        for (const child of children) {
            let segment = key;
            if (child && typeof child === 'object' && !Array.isArray(child)) {
                const disc = discriminator(child, key, prefix);
                if (disc != null && (isCollection || disc !== ''))
                    segment = `${key}[${disc}]`;
            }
 ...
 */
export async function normalizeAotXml(xml, ignore = [], prefix = '') {
    ...
    for (const [rootTag, rootVal] of Object.entries(parsed)) {
        const children = Array.isArray(rootVal) ? rootVal : [rootVal];
        for (const child of children)
            walk(child, rootTag, out, matchers, prefix);
    }
...
export function renderNormalized(map) {
 ...
 */
export async function normalizeMultiArtifact(artifacts, ignore = [], prefix = '') {
    const out = new Map();
    ...
    for (const name of names) {
        ...
        for (const [path, value] of single)
            out.set(`${canonName}::${path}`, value);
    }
    return out;
}
