...
export function parseSysTestResult(output) {
    ...
    let passed;
    if (/✅\s*Tests passed/i.test(text))
        passed = true;
    else if (/❌\s*Tests FAILED/.test(text))
        passed = false;
    else if (typeof counts.failed === 'number')
        passed = counts.failed === 0;
    else if (/⚠️\s*Tests completed/i.test(text))
        passed = null;
    else
        passed = null;
    const failures = [];
    if (passed === false) {
        ...
        for (const raw of text.split(/\r?\n/)) {
            ...
            if (COUNT_SUMMARY.test(line) && !/::/.test(line))
                continue;
...
//# sourceMappingURL=systest.js.map