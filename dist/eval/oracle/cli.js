...
import { evaluate, evaluateMulti, renderDiff, renderNormalized, normalizeAotXml, parseSysTestResult, scoreRun, GOLDEN_CAPTURE_PREFIXES, } from './index.js';
...
async function main() {
    ...
    const caseSpec = JSON.parse(fs.readFileSync(path.join(REPO_ROOT, 'eval', 'cases', `${caseId}.json`), 'utf8'));
    ...
    if (!bpChecked) {
        console.error('note: --bp-warnings not supplied → bp_clean: null (BP NOT CHECKED). ' +
    ...
    const goldenPrefix = arg('--golden-prefix') ?? GOLDEN_CAPTURE_PREFIXES;
    const actualPrefix = arg('--actual-prefix') ?? (resolveRegularObjectPrefixToken() || GOLDEN_CAPTURE_PREFIXES);
    let goldenDiff;
    let score;
    let systestOut, generatedArtifacts, debugLabel;
    ...
    const goldenUnavailable = !explicitGoldenExists && (caseSpec.golden_pending === true || listGoldenArtifacts(caseId).length === 0);
    if (goldenUnavailable) {
        ...
        systestOut = systest && 'ran' in systest ? systest : { ran: false, passed: null, failures: [] };
        if (actualDir) {
        ...
        if (artifactNames.length === 0)
            throw new Error(`No *.metadata.xml goldens in ${goldenDir(caseId)}`);
        const goldenArtifacts = {};
        for (const name of artifactNames) {
            goldenArtifacts[name] = fs.readFileSync(path.join(goldenDir(caseId), name), 'utf8');
        }
        const { actualArtifacts, matchedActualFiles } = buildActualArtifactsMap(resolvedActualDir, artifactNames, goldenPrefix, actualPrefix);
    ...
    if (goldenDiff)
        ...
        console.error('golden_match: null — golden not evaluated (build and bp_clean scored; golden diff skipped).');
    if (systestFile)
        console.error(`SysTest: ran=${systestOut.ran} passed=${systestOut.passed} failures=${systestOut.failures.length}`);
    ...
    if (flagSet('--write')) {
        ...
        if (suppliedClassification && !CLASSIFICATIONS.includes(suppliedClassification)) {
            console.error(`--classification "${suppliedClassification}" is not one of ${CLASSIFICATIONS.join(', ')} ` +
