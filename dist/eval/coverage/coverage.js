 ...
 */
function summarise(tier, leaves) {
...
export function computeCoverage(taxonomy, inputs) {
    ...
    const leaves = taxonomy.map(leaf => {
        ...
        for (const id of [...declaredCases, ...byTag])
            claimedCases.add(id);
...
export function renderMarkdown(report, generatedAt) {
    ...
    out.push('');
    out.push('A taxonomy leaf counts as covered only when all three hold: **K** a knowledge entry ' +
    ...
    out.push('');
    out.push('**core** = anything done at least once per project — the hard commitment. ' +
    ...
    out.push('## Orphans');
    out.push('');
    out.push(`- Knowledge entries no leaf claims (**unproven knowledge**): ${report.orphans.knowledge.length ? report.orphans.knowledge.join(', ') : 'none'}`);
    out.push(`- Eval cases no leaf claims (**unmapped proof**): ${report.orphans.cases.length ? report.orphans.cases.join(', ') : 'none'}`);
    out.push('');
...
export function danglingReferences(report) {
