...
import { renderMarkdown, danglingReferences } from './coverage.js';
...
function main() {
    ...
    if (check) {
        ...
        if (normalizeEol(readme) !== normalizeEol(readmeNext))
            stale.push('README.md (coverage badge)');
        if (stale.length > 0) {
            console.error(`❌ stale coverage artifact(s): ${stale.join(', ')} — run \`npm run eval:coverage\` and commit.`);
            return 1;
        }
        console.log(`✅ coverage up to date — core ${report.core.covered}/${report.core.total} (${report.core.percent}%), ` +
    ...
    if (readme !== readmeNext)
        fs.writeFileSync(README_PATH, readmeNext, 'utf8');
    console.log(`core ${report.core.covered}/${report.core.total} (${report.core.percent}%) · ` +
        `total ${report.total.covered}/${report.total.total} (${report.total.percent}%)`);
...
process.exit(main());
//# sourceMappingURL=coverageCli.js.map