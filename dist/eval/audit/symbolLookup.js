 ...
 */
export async function openSymbolLookup(dbPath) {
    ...
    for (const r of rows) {
        ...
        if (hit)
            hit.types.add(r.type);
        else
            byLower.set(key, { canonical: r.name, types: new Set([r.type]) });
    }
    ...
    for (const r of baseRows) {
        if (r.extends_class)
            referenced.add(r.extends_class.trim().toLowerCase());
    ...
    const memberStmt = db.prepare(`SELECT name FROM symbols WHERE parent_name = ? AND type = 'method'`);
    ...
    const lookup = {
        ...
        hasMember(canonical, member) {
            ...
            if (!set) {
                set = new Set(memberStmt.all(canonical).map(r => r.name.toLowerCase()));
                memberCache.set(canonical, set);
            }
            return set.has(member.toLowerCase());
        },
    };
    const meta = db.prepare(`SELECT value FROM _index_meta WHERE key = 'last_indexed_at'`).get();
