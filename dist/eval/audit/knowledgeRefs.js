...
function locallyDeclared(texts) {
    const out = new Set();
...
function push(acc, entryId, field, kind, name, member) {
    if (NON_AOT.has(name.toLowerCase()))
        return;
    if (name.length < 3)
        return;
    ...
    if (!acc.has(key))
        acc.set(key, { entryId, name, member, kind, field, key });
}
/** Extract every AOT reference from one knowledge entry. */
export function extractEntryRefs(entry) {
    const acc = new Map();
    ...
    for (const { field, text } of sources) {
        ...
        for (const m of text.matchAll(new RegExp(`\\b(?:extends|implements)\\s+(${IDENT}(?:\\s*,\\s*${IDENT})*)`, 'g'))) {
            for (const one of m[1].split(','))
                push(acc, entry.id, field, 'extends', one.trim());
        }
        // new Foo(
        ...
        for (const m of text.matchAll(/\[([^\]]+)\]/g)) {
            ...
            if (attr)
                push(acc, entry.id, field, 'attribute', attr[1]);
        }
        ...
        if (field.endsWith('.code')) {
            for (const line of text.split('\n')) {
                ...
                if (m)
                    push(acc, entry.id, field, 'declaration', m[1]);
            }
        }
    }
...
export function extractKnowledgeRefs(base) {
    return base.flatMap(extractEntryRefs);
}
//# sourceMappingURL=knowledgeRefs.js.map