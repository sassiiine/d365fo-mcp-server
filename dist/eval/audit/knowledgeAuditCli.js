...
import { auditRefs, renderFindings, buildSnapshot, verifyAgainstSnapshot, } from './knowledgeAudit.js';
...
async function main() {
    ...
    const allow = readJson(ALLOW_PATH, {});
    if (!capture) {
        ...
        if (asJson) {
            ...
            console.log(`Knowledge audit (verify) — ${refs.length} reference(s) vs snapshot captured ` +
            ...
            if (missing.length === 0) {
                ...
                for (const m of missing)
                    console.log(`   ${m.entryId} · ${m.field} · ${m.kind} · ${m.name}${m.member ? `::${m.member}` : ''}`);
            }
        }
...
//# sourceMappingURL=knowledgeAuditCli.js.map