...
export function demoBasesInCase(c) {
    const bases = new Set();
    const hay = `${c.title ?? ''}\n${c.instruction}`;
    for (const m of hay.matchAll(DEMO_TOKEN))
        bases.add(m[1]);
    return bases;
}
export const SHARED_DECISIONS = {
 ...
 */
export function classifyDemoObjects(cases) {
    const casesByBase = new Map();
    for (const c of cases) {
        for (const base of demoBasesInCase(c)) {
            const set = casesByBase.get(base) ?? new Set();
            set.add(c.id);
    ...
    for (const [base, caseSet] of [...casesByBase.entries()].sort((a, b) => a[0].localeCompare(b[0]))) {
        ...
        if (decided.decision === 'INPUT')
            provisioned.push(`Con${base}`);
    }
...
export function fixtureNames(dir = FIXTURES_DIR) {
 ...
 */
export function fixturesForCase(caseId, cases) {
    ...
    for (const [base, decided] of Object.entries(SHARED_DECISIONS)) {
        if (decided.decision !== 'INPUT')
            continue;
        if (decided.origin === caseId)
            continue; // do not re-provision into the origin case
        if (bases.has(base))
            needed.push(`Con${base}`);
    }
    return needed.sort();
}
/**
 ...
 */
export function partitionForRollback(written, fixtures = fixtureNames()) {
    const undo = [];
    const keep = [];
    for (const obj of written)
        (fixtures.has(obj) ? keep : undo).push(obj);
    return { undo, keep };
}
