...
import { workspacePanorama, workspaceOperational, formPartSectionList, formPartSectionListDouble, hubPartChart, hubPartGrid, } from './catalog/topLevel/workspace.js';
...
export const FORM_PATTERN_CATALOG = {
    patterns: [
        simpleList,
...
for (const p of FORM_PATTERN_CATALOG.patterns) {
    ...
    for (const alias of p.xmlAliases ?? [])
        patternByKey.set(alias.toLowerCase(), p);
}
const subPatternByKey = new Map();
for (const sp of FORM_PATTERN_CATALOG.subPatterns) {
    ...
    for (const alias of sp.xmlAliases ?? [])
        subPatternByKey.set(alias.toLowerCase(), sp);
}
/**
 ...
 */
export function resolvePattern(name) {
    if (!name)
        return undefined;
    ...
    for (const alias of PATTERN_ALIASES) {
        if (alias.test(normalized))
            return patternByKey.get(alias.id.toLowerCase());
    }
    return undefined;
}
/**
 ...
 */
export function subPatternsFor(controlType, parentPatternId) {
    return FORM_PATTERN_CATALOG.subPatterns.filter((sp) => {
        if (!sp.appliesToControlTypes.includes(controlType))
            return false;
