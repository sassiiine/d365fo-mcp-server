...
const FORM_EXECUTE_QUERY = (dsName) => ({
    name: 'executeQuery',
...
const LINES_INIT_VALUE = (headerDsName) => ({
    name: 'initValue',
    source: `        public void initValue()
        {
            super();
 ...
 */
export function methodStubsForPattern(patternName, dsName, linesDsName) {
...
function methodXml(stub, indent) {
 ...
 */
export function injectMethodStubs(xml, stubs, dsName, linesDsName) {
    let result = xml;
    const injected = [];
    ...
    const injectDsMethods = (src, targetDs, methods) => {
        if (methods.length === 0 || !targetDs)
            return src;
        ...
        if (selfClosed) {
            ...
            const dataSourceBlock = `<DataSources${dsAttrs}>\n` +
                `\t\t\t<DataSource>\n` +
                `\t\t\t\t<Name>${targetDs}</Name>\n` +
                methodsBlock +
                `\t\t\t</DataSource>\n` +
        ...
        if (dsCloseIdx === -1 || dsCloseIdx > scEnd)
            return src;
        ...
        const existingMethodsMatch = /^\s*<Methods\s*\/>|^\s*<Methods>([\s\S]*?)<\/Methods>/.exec(src.slice(afterNameAbs, dsCloseIdx));
...
//# sourceMappingURL=methodStubs.js.map