...
const NEXT_ONCE = '`next <method>()` must be reached exactly once and unconditionally — not inside an `if`, not after a ' +
    '`return`. The compiler rejects the alternative with SYS10028 (rule COC004).';
const VALIDATION_RETURN = 'Report a failure by RETURNING false, not by throwing: `ret = checkFailed("@MyModel:MyLabel");` ' +
...
export const TABLE_DATA_METHODS = {
    validatewrite: {
        name: 'validateWrite',
...
export function lookupTableDataMethod(methodName) {
 ...
 */
export function hasTableDataMethods(objectType) {
    return objectType?.toLowerCase() === 'table';
}
/** The `### Method signature` body when only this fallback knows the method. */
export function renderTableDataMethodSignature(method, objectName) {
    return [
...
export function renderTableDataMethodEligibility(method, objectName) {
    return [
...
//# sourceMappingURL=tableDataMethods.js.map