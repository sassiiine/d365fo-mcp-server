...
export async function tryBridgeTable(bridge, tableName, methodOffset = 0, fieldsOffset = 0, fieldFilter) {
...
function formatTable(t, methodOffset, fieldsOffset = 0, fieldFilter) {
    let out = `# Table: ${t.name}\n\n`;
    if (t.label)
        out += `**Label:** ${t.label}\n`;
    if (t.tableGroup)
        out += `**Table Group:** ${t.tableGroup}\n`;
    if (t.model)
        out += `**Model:** ${t.model}\n`;
    if (t.extends)
        out += `**Extends:** ${t.extends}\n`;
    if (t.cacheLookup)
        out += `**CacheLookup:** ${t.cacheLookup}\n`;
    if (t.clusteredIndex)
        out += `**ClusteredIndex:** ${t.clusteredIndex}\n`;
    if (t.primaryIndex)
        out += `**PrimaryIndex:** ${t.primaryIndex}\n`;
    ...
    for (const idx of t.indexes) {
        ...
        const fieldNames = idx.fields.map((f) => typeof f === 'string' ? f : f.dataField ?? f.name).join(', ');
...
export async function tryBridgeClass(bridge, className, compact, methodOffset = 0) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
...
// METHOD SOURCE
export async function tryBridgeMethodSource(bridge, className, methodName) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
        const ms = await bridge.getMethodSource(className, methodName);
        if (!ms.found || !ms.source)
            return null;
        ...
        const obsoleteWarning = obsoleteMatch
            ...
            : '';
        const text = `## ${ms.className}.${ms.methodName}\n\n` +
...
// ENUM
export async function tryBridgeEnum(bridge, enumName) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
...
// EDT
export async function tryBridgeEdt(bridge, edtName) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
...
function formatEdt(edt) {
    ...
    if (edt.enumType)
        out += `| Enum Type | ${edt.enumType} |\n`;
    if (edt.referenceTable)
        out += `| Reference Table | ${edt.referenceTable} |\n`;
    if (edt.relationType)
        out += `| Relation Type | ${edt.relationType} |\n`;
    if (edt.stringSize)
        out += `| String Size | ${edt.stringSize} |\n`;
    if (edt.displayLength)
        out += `| Display Length | ${edt.displayLength} |\n`;
    if (edt.label)
        out += `| Label | ${edt.label} |\n`;
    if (edt.helpText)
        out += `| Help Text | ${edt.helpText} |\n`;
    if (edt.formHelp)
        out += `| Form Help | ${edt.formHelp} |\n`;
    if (edt.configurationKey)
        out += `| Configuration Key | ${edt.configurationKey} |\n`;
    if (edt.alignment)
        out += `| Alignment | ${edt.alignment} |\n`;
    if (edt.noOfDecimals != null)
        out += `| No. of Decimals | ${edt.noOfDecimals} |\n`;
    if (edt.decimalSeparator)
        out += `| Decimal Separator | ${edt.decimalSeparator} |\n`;
    if (edt.signDisplay)
        out += `| Sign Display | ${edt.signDisplay} |\n`;
    return out;
}
// FORM
export async function tryBridgeForm(bridge, formName, maxControls) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
...
function formatForm(form, maxControls) {
    let out = `# Form: ${form.name}\n\n`;
    if (form.model)
        out += `**Model:** ${form.model}\n`;
    if (form.formPattern)
        out += `**Pattern:** ${form.formPattern}\n`;
    out += `_Source: C# bridge (IMetadataProvider)_\n\n`;
    ...
    for (const ds of form.dataSources) {
        ...
        const perms = [];
        if (ds.allowEdit)
            perms.push(`Edit: ${ds.allowEdit}`);
        if (ds.allowCreate)
            perms.push(`Create: ${ds.allowCreate}`);
        if (ds.allowDelete)
            perms.push(`Delete: ${ds.allowDelete}`);
        if (perms.length > 0)
            out += `  Permissions: ${perms.join(', ')}\n`;
        out += '\n';
    }
...
function buildControlTreeV2(controls, depth, budget) {
    if (!controls || depth > 10)
        return '';
    let out = '';
    ...
    for (const c of controls) {
        ...
        const props = [];
        if (c.caption)
            props.push(`Caption: ${c.caption}`);
        if (c.visible && c.visible !== 'Yes')
            props.push(`Visible: ${c.visible}`);
        if (c.enabled && c.enabled !== 'Yes')
            props.push(`Enabled: ${c.enabled}`);
        if (c.label)
            props.push(`Label: ${c.label}`);
        if (props.length > 0)
...
export async function tryBridgeReferences(bridge, target, limit = 50, displayName, formatAs = 'default') {
    if (!bridge?.isReady || !bridge.xrefAvailable)
    ...
    if (merged.length === 0)
        return errored ? { status: 'error' } : { status: 'empty' };
...
function friendlyXrefSourceType(container) {
    const key = container.toLowerCase();
    if (XREF_SOURCE_TYPE_LABELS[key])
        return XREF_SOURCE_TYPE_LABELS[key];
    if (key.startsWith('edt'))
        return 'EDT'; // EdtString, EdtEnum, EdtReal, EdtInt64, …
    if (key.startsWith('workflow'))
        return 'Workflow'; // WorkflowTemplate, WorkflowApproval, …
 ...
 */
function parseLabelSource(sourcePath) {
    ...
    if (objQ >= 0)
        objectName = objectName.substring(0, objQ);
    let detail;
 ...
 */
function formatLabelReferences(references, label, limit) {
...
export async function tryBridgeSearch(bridge, query, objectType, maxResults = 50, opts) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
        ...
        for (const cand of opts?.exactMatches ?? []) {
            if (!isExactNameMatch(query, cand.name))
                continue;
            if (known.has(`${cand.name.toLowerCase()}\0${cand.type}`))
                continue;
...
// QUERY
export async function tryBridgeQuery(bridge, queryName) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
...
// VIEW
export async function tryBridgeView(bridge, viewName) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
...
function formatView(v) {
    ...
    if (v.query)
        out += `**Query:** ${v.query}\n`;
    if (v.isPublic != null)
        out += `**Public:** ${v.isPublic ? 'Yes' : 'No'}\n`;
    if (v.isReadOnly != null)
        out += `**Read-Only:** ${v.isReadOnly ? 'Yes' : 'No'}\n`;
    if (v.primaryKey)
        out += `**Primary Key:** ${v.primaryKey}\n`;
    out += `_Source: C# bridge (IMetadataProvider)_\n\n`;
    // DataSources
...
// DATA ENTITY
export async function tryBridgeDataEntity(bridge, entityName) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
...
function formatDataEntity(e) {
    ...
    if (e.entityCategory)
        out += `Category: ${e.entityCategory}\n`;
    if (e.publicEntityName)
        out += `Public Name: ${e.publicEntityName} (OData resource name)\n`;
    if (e.publicCollectionName)
        out += `Collection: ${e.publicCollectionName}\n`;
    out += `OData Enabled: ${e.isPublic ? 'Yes' : 'No'}\n`;
    if (e.isReadOnly != null)
        out += `Read-Only: ${e.isReadOnly ? 'Yes' : 'No'}\n`;
    if (e.dataManagementEnabled != null)
        out += `Data Management (DMF): ${e.dataManagementEnabled ? 'Yes' : 'No'}\n`;
    if (e.stagingTable)
        out += `Staging Table: ${e.stagingTable}\n`;
    out += `_Source: C# bridge (IMetadataProvider)_\n`;
    ...
    if (e.fields && e.fields.length > 0) {
        ...
        out += fieldNames.join(', ');
        if (e.fields.length > 8)
            out += ` ... (+${e.fields.length - 8} more)`;
        out += '\n';
    }
    // Field mappings
    if (e.fieldMappings && e.fieldMappings.length > 0) {
        ...
        if (e.fieldMappings.length > 20)
            out += `  ... (+${e.fieldMappings.length - 20} more)\n`;
    }
    // Computed columns
...
export async function tryBridgeReport(bridge, reportName) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
    ...
    if (r.dataSets.length > 0) {
        ...
        for (const ds of r.dataSets) {
            ...
            if (ds.dataSourceType)
                out += `  DataSourceType: ${ds.dataSourceType}\n`;
            if (ds.query)
                out += `  Query: ${ds.query}\n`;
            if (ds.fields && ds.fields.length > 0) {
 ...
 */
export async function bridgeRefreshProvider(bridge) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
 ...
 */
export async function bridgeValidateAfterWrite(bridge, objectType, objectName) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
        ...
        if (result.valid) {
            ...
            if (result.fieldCount != null && result.fieldCount > 0)
                parts.push(`${result.fieldCount} fields`);
            if (result.methodCount != null && result.methodCount > 0)
                parts.push(`${result.methodCount} methods`);
            if (result.indexCount != null && result.indexCount > 0)
                parts.push(`${result.indexCount} indexes`);
            if (result.valueCount != null && result.valueCount > 0)
                parts.push(`${result.valueCount} values`);
            return parts.join(' | ');
        }
        else {
 ...
 */
export async function bridgeResolveObject(bridge, objectType, objectName) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
 ...
 */
export function unappliedSuffix(result) {
 ...
 */
export function canBridgeCreate(objectType) {
 ...
 */
function documentAndFormat(params) {
    return {
        ...params,
 ...
 * the bridge does).
 */
export async function bridgeCreateObject(bridge, params) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    if (!canBridgeCreate(params.objectType))
        return null;
    try {
 ...
 */
export async function bridgeCreateSmartTable(bridge, params) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
 ...
 */
export async function bridgeAddMethod(bridge, objectType, objectName, methodName, sourceCode) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    if (!BRIDGE_MODIFY_TYPES.has(objectType.toLowerCase()))
        return null;
    try {
        ...
        const result = await bridge.addMethod(objectType, objectName, methodName, xppMethodSourceForXml(ensureXppDocComment(sourceCode)));
        return {
 ...
 */
export async function bridgeAddField(bridge, tableName, fieldName, fieldType, edt, mandatory, label, mapped) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
 ...
 */
export async function bridgeSetProperty(bridge, objectType, objectName, propertyPath, propertyValue) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    if (!BRIDGE_MODIFY_TYPES.has(objectType.toLowerCase()))
        return null;
 ...
 */
export async function bridgeReplaceCode(bridge, objectType, objectName, methodName, oldCode, newCode) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    if (!BRIDGE_MODIFY_TYPES.has(objectType.toLowerCase()))
        return null;
 ...
 */
export async function bridgeRemoveMethod(bridge, objectType, objectName, methodName) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    if (!BRIDGE_MODIFY_TYPES.has(objectType.toLowerCase()))
        return null;
 ...
 */
export async function bridgeAddIndex(bridge, tableName, indexName, fields, allowDuplicates, alternateKey) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
 ...
 */
export async function bridgeRemoveIndex(bridge, tableName, indexName) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
 ...
 */
export async function bridgeAddFullTextIndex(bridge, tableName, indexName, fields) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
...
export async function bridgeRemoveFullTextIndex(bridge, tableName, indexName) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
...
export async function bridgeAddTableMapping(bridge, tableName, mapName, mappingTable, connections) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
...
export async function bridgeRemoveTableMapping(bridge, tableName, mapName) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
 ...
 */
export async function bridgeRemoveRelation(bridge, tableName, relationName) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
 ...
 */
export async function bridgeAddFieldGroup(bridge, tableName, groupName, label, fields) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
 ...
 */
export async function bridgeRemoveFieldGroup(bridge, tableName, groupName) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
 ...
 */
export async function bridgeAddFieldToFieldGroup(bridge, tableName, groupName, fieldName, extendBaseFieldGroup) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
        ...
        const target = result.extendBaseFieldGroup === true
 ...
 */
export async function bridgeModifyField(bridge, tableName, fieldName, properties) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
 ...
 */
export async function bridgeRenameField(bridge, tableName, oldName, newName) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
 ...
 */
export async function bridgeRemoveField(bridge, tableName, fieldName) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
 ...
 */
export async function bridgeReplaceAllFields(bridge, tableName, fields) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
 ...
 */
export async function bridgeAddEnumValue(bridge, enumName, valueName, value, label, countryRegionCodes) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
 ...
 */
export async function bridgeModifyEnumValue(bridge, enumName, valueName, properties) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
 ...
 */
export async function bridgeRemoveEnumValue(bridge, enumName, valueName) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
 ...
 */
export async function bridgeAddControl(bridge, formName, controlName, parentControl, controlType, dataSource, dataField, label) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
 ...
 */
export async function bridgeAddDataSource(bridge, objectType, objectName, dsName, table, joinSource, linkType) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
 ...
 */
export async function bridgeAddFieldModification(bridge, extensionName, fieldName, fieldLabel, fieldMandatory) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
 ...
 */
export async function bridgeAddMenuItemToMenu(bridge, menuName, menuItemToAdd, menuItemToAddType) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
 ...
 */
export async function bridgeBatchModify(bridge, objectType, objectName, operations) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
 ...
 * Returns a formatted ToolResult or null if bridge unavailable.
 */
export async function bridgeGetCapabilities(bridge) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
 ...
 * Returns a formatted ToolResult or null if bridge unavailable.
 */
export async function bridgeDiscoverFormPatterns(bridge) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
...
export async function tryBridgeSecurityArtifact(bridge, name, artifactType, includeChain) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
...
function formatSecurityPrivilege(priv, _includeChain) {
    let out = `SecurityPrivilege: ${priv.name}\n`;
    if (priv.label)
        out += `Label: ${priv.label}\n`;
    if (priv.description)
        out += `Description: ${priv.description}\n`;
    if (priv.model)
        out += `Model: ${priv.model}\n`;
    out += `_Source: C# bridge (IMetadataProvider)_\n`;
...
function formatSecurityDuty(duty, _includeChain) {
    let out = `SecurityDuty: ${duty.name}\n`;
    if (duty.label)
        out += `Label: ${duty.label}\n`;
    if (duty.description)
        out += `Description: ${duty.description}\n`;
    if (duty.model)
        out += `Model: ${duty.model}\n`;
    out += `_Source: C# bridge (IMetadataProvider)_\n`;
...
function formatSecurityRole(role, _includeChain) {
...
// MENU ITEM (Phase 6)
export async function tryBridgeMenuItem(bridge, name, itemType) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
...
function formatMenuItem(mi) {
    ...
    if (mi.openMode && mi.openMode !== 'Auto')
        out += `Open Mode: ${mi.openMode}\n`;
    if (mi.linkedPermissionObject) {
        out += `Security Privilege: ${mi.linkedPermissionObject}`;
        if (mi.linkedPermissionType)
            out += ` [${mi.linkedPermissionType}]`;
        out += '\n';
    }
    if (mi.helpText)
        out += `Help: ${mi.helpText}\n`;
    return out;
}
// TABLE EXTENSIONS (Phase 6)
export async function tryBridgeTableExtensions(bridge, baseTableName) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return null;
    try {
...
function formatTableExtensions(r) {
...
export async function tryBridgeCocExtensions(bridge, baseClassName, methodName) {
    if (!bridge?.isReady || !bridge.xrefAvailable)
        return null;
    try {
...
function formatCocExtensions(r, methodNameFilter) {
    ...
    if (methodNameFilter) {
        filtered = r.extensions.filter(ext => ext.wrappedMethods?.some(m => m.toLowerCase() === methodNameFilter.toLowerCase())
...
export async function tryBridgeEventHandlers(bridge, targetName, eventName, handlerType) {
    if (!bridge?.isReady || !bridge.xrefAvailable)
        return null;
    try {
...
function formatEventHandlers(r) {
...
export async function tryBridgeApiUsageCallers(bridge, apiName, limit = 200) {
    if (!bridge?.isReady || !bridge.xrefAvailable)
        return null;
    try {
        const result = await bridge.findApiUsageCallers(apiName, limit);
        if (!result || result.totalCallers === 0)
            return null;
...
function formatApiUsageCallers(r) {
