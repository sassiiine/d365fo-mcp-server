...
export function isBridgeFailure(value) {
    return typeof value === 'object' && value !== null && value.bridgeFailed === true;
}
...
const failureScope = new AsyncLocalStorage();
/**
 ...
 */
export function runWithBridgeFailureScope(sink, fn) {
 ...
 */
export function recordBridgeFailure(operation, error, options) {
    const failure = {
        bridgeFailed: true,
        operation,
...
export function describeBridgeFailure(failure) {
 ...
 */
export function renderBridgeFailureNote(failures) {
    const shown = failures.slice(0, 3);
    const rest = failures.length - shown.length;
    const lines = shown.map(f => `>    • ${f.operation} — ${f.reason}`);
    if (rest > 0)
        lines.push(`>    • …and ${rest} more bridge call${rest === 1 ? '' : 's'}`);
    return (`> ⚠️ ${BRIDGE_FAILURE_MARKER} the C# metadata bridge threw during this call, so anything ` +
...
//# sourceMappingURL=bridgeFailure.js.map