 ...
 */
export function trackBridgeStartup(attempt) {
    const handle = {
        settled: false,
        done: attempt.then(() => { handle.settled = true; }, () => { handle.settled = true; }),
    };
    return handle;
}
/**
 ...
 */
export function bridgeStartupState(context) {
    ...
    if (context.bridgeStartup && !context.bridgeStartup.settled)
        return 'starting';
    return 'unavailable';
}
/**
 ...
 */
export async function awaitBridgeReady(context, timeoutMs = READY_TIMEOUT_MS) {
    if (context.bridge?.isReady)
        return 'ready';
    ...
    try {
        const raced = await Promise.race([startup.done.then(() => 'settled'), timeout]);
        if (raced === 'timeout')
            return 'timeout';
    }
    finally {
 ...
 */
export function describeBridgeStartup(context) {
    switch (bridgeStartupState(context)) {
        case 'starting':
            return (`The C# bridge is still starting — this is a cold-start race, NOT a configuration problem, ` +
                `and NOT evidence that the object does not exist. Retry the identical call in a few seconds.`);
        case 'unavailable':
            return (`The C# bridge is not connected. Ensure the bridge exe is built and D365FO metadata ` +
...
//# sourceMappingURL=bridgeReadiness.js.map