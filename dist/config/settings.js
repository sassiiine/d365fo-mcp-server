...
export const SETTINGS = [
    // ── environment ──────────────────────────────────────────────────────────
    {
        ...
        description: 'Classic AOSService VM ("traditional") or Unified Developer Experience / Power Platform Tools ("ude"). ' +
    ...
    },
    {
        ...
        description: 'Your own (non-Microsoft) models, comma-separated. They are indexed with priority and treated as writable. ' +
    ...
    },
    {
        ...
        description: 'Name of a config file in %LOCALAPPDATA%\\Microsoft\\Dynamics365\\XPPConfig. Pinning one keeps the server on a ' +
    ...
    },
    {
        ...
        description: 'Where custom model XML is written and tracked by git. Normally read from the XPP config — override only when ' +
    ...
    // ── workspace ────────────────────────────────────────────────────────────
    {
        ...
        description: 'The model new objects are created in. Leave empty to let the server detect it from the IDE workspace or the ' +
    ...
    },
    {
        ...
        description: 'Two-level AOT path of the model being worked on. Used to resolve the package root and the write target when ' +
    ...
    },
    {
        ...
        description: 'Scanned once at startup so the server can switch model automatically when you open another solution or ' +
    ...
    },
    {
        ...
        description: 'Comma-separated models this workspace may write into besides its own. By default any create/modify/label write ' +
    ...
    // ── naming ───────────────────────────────────────────────────────────────
    {
        ...
        description: 'Your ISV/customer prefix. Prepended to every generated object, field and method name and enforced by the ' +
    ...
    },
    {
        ...
        description: 'Optional suffix appended to new object names (MyTableZZ with suffix "ZZ"). Most projects use only a prefix — ' +
    ...
    },
    {
        ...
        description: 'Whether extension classes/elements embed the prefix (per the Microsoft prefix guideline) or the model name ' +
    ...
    // ── index ────────────────────────────────────────────────────────────────
    {
        ...
        type: 'enum',
        label: 'What to index',
        description: 'Scope of the metadata extraction. "all" gives full cross-reference search over the standard application but ' +
    ...
    },
    {
        ...
        description: 'Builds the labels database so labels(action="search") and label reuse work. Disabling it speeds up the build ' +
    ...
    },
    {
        ...
        description: 'Comma-separated language codes, or `all` for every language shipped with the model. Each extra language ' +
    ...
    },
    {
        ...
        description: 'Second SQLite file for labels (dual-database architecture keeps label writes from locking metadata reads). ' +
    ...
    },
    {
        ...
        description: 'Alphabetical keeps .label.txt files sorted (smaller diffs, matches most teams); append adds new labels at the ' +
    ...
    // ── server ───────────────────────────────────────────────────────────────
    {
        ...
        type: 'enum',
        label: 'Server mode',
        description: 'Which half of the toolset this process exposes. "full" is a single local server; the hybrid deployment splits ' +
    ...
    },
    {
        ...
        type: 'enum',
        label: 'Tool profile',
        description: 'How many tools this server advertises. "full" publishes all 23. "core" publishes only the plan → discover → ' +
    ...
    },
    {
        ...
        description: 'Tool names to publish in addition to the core profile, e.g. security_info,run_systest_class. Ignored when ' +
    ...
    },
    {
        ...
        type: 'int',
        label: 'HTTP port',
        description: 'Port for the HTTP transport. Only relevant when clients connect over http://localhost:<port>/mcp/ — an IDE that ' +
    ...
    },
    {
        ...
        description: 'How long SIGTERM/SIGINT handling waits for in-flight work (bridge writes, database checkpoints) before the ' +
    ...
    },
    {
        ...
        description: 'How often the holder of a long-running operation lock (build, DB sync) touches it so the stale-lock reaper ' +
    ...
    },
    {
        ...
        description: 'Absolute path; the server appends everything it writes to stderr. The way to get logs out of an IDE that hides ' +
    ...
    },
    {
        ...
        description: 'The server picks stdio when its stdin is piped. Set this to keep HTTP anyway — e.g. when running under a ' +
    ...
    },
    {
        ...
        description: 'How long a per-customer key lookup is trusted before Neon is consulted again. This IS the revocation ' +
    ...
    },
    {
        ...
        description: 'Writes one line per tool call that exceeds this, with the tool name and a short argument digest. ' +
    ...
    },
    {
        ...
        description: 'Read calls are retried after a health-checked restart of the child process. Writes are never retried — a ' +
    ...
    },
    {
        ...
        description: 'Absolute path to D365MetadataBridge.exe. Leave empty to auto-detect inside the installation — ' +
    ...
    // ── behavior ─────────────────────────────────────────────────────────────
    {
        ...
        description: 'Structural form-pattern violations (unknown pattern, missing container, wrong control order) block the write. ' +
    ...
    },
    {
        ...
        description: 'Write tools only accept a token issued by prepare(), proving the model actually inspected the real object ' +
    ...
    },
    {
        ...
        description: 'Set the SAME random string on both halves of a hybrid deployment (and on every scaled-out App Service ' +
            'instance) so tokens issued by one process validate in another. Without it, tokens are memory-local.',
    },
];
const BY_PATH = new Map(SETTINGS.flatMap(s => (s.path ? [[s.path, s]] : [])));
const BY_ENV = new Map(SETTINGS.map(s => [s.env, s]));
export function settingByPath(path) {
    return BY_PATH.get(path);
}
export function settingByEnv(env) {
    return BY_ENV.get(env);
}
export function settingsInSection(section, tier) {
...
export function serializeValue(setting, value) {
...
export function parseValue(setting, raw) {
