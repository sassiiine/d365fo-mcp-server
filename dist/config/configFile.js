...
export function configCandidates(baseDir) {
 ...
 */
export function resolveConfigFiles(baseDir, opts) {
 ...
 */
export function toEnvRecord(files) {
    const out = {};
...
export function defaultPathEnv(baseDir) {
    const out = {};
    for (const setting of SETTINGS) {
        if (!setting.path || setting.type !== 'path' || typeof setting.default !== 'string' || setting.default === '')
            continue;
...
export function writeConfigFile(configPath, config) {
    ...
    const ordered = { version: CONFIG_VERSION };
    for (const setting of SETTINGS) {
        if (setting.tier === 'secret')
            continue;
        const value = getAtPath(config, setting.path);
        if (value !== undefined && value !== null && value !== '')
            setAtPath(ordered, setting.path, value);
    }
...
export function writeSecretsFile(secretsPath, secrets) {
