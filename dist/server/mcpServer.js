...
import { ListToolsRequestSchema, InitializedNotificationSchema, RootsListChangedNotificationSchema, ListRootsResultSchema, } from '@modelcontextprotocol/sdk/types.js';
 ...
 */
function applyRootsToConfig(roots) {
    if (!roots?.length) {
    ...
    getConfigManager().setRuntimeContextFromRoots(paths).then(() => {
        if (DEBUG_LOGGING) {
            const { modelName, source, projectPath, solutionPath, workspacePath } = getConfigManager().getDetectionSummary();
            process.stderr.write(`[mcpServer] ✅ Project detection result:\n` +
...
export function createXppMcpServer(context) {
    ...
    server.setNotificationHandler(InitializedNotificationSchema, async () => {
        if (DEBUG_LOGGING) {
            process.stderr.write(`[mcpServer ${new Date().toISOString().slice(11, 23)}] ⚡ 'initialized' notification received — requesting roots/list\n`);
        }
        ...
        try {
            const result = await server.request({ method: 'roots/list', params: {} }, ListRootsResultSchema);
    ...
    server.setNotificationHandler(RootsListChangedNotificationSchema, async () => {
        ...
        if (DEBUG_LOGGING) {
            process.stderr.write(`[mcpServer ${new Date().toISOString().slice(11, 23)}] 🔄 'roots/list_changed' notification — re-requesting roots/list\n`);
        }
        const isHttpMode = !!process.env.WEBSITES_PORT || process.env.MCP_FORCE_HTTP === 'true';
    ...
    server.setRequestHandler(ListToolsRequestSchema, async () => {
        ...
        const modeNote = SERVER_MODE === 'read-only' ? ' · read-only (local tools excluded)' :
