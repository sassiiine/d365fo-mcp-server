...
import { openInstanceStore, openStore } from './settingsStore.js';
...
export function targetEnv(target) {
    const env = {};
    if (target.envFile)
        env.ENV_FILE = target.envFile;
    if (fs.existsSync(target.store.configPath))
        env.D365FO_CONFIG = target.store.configPath;
...
export function instanceTarget(inst) {
    return {
        name: inst.name,
 ...
 */
export async function pickTarget(instanceName, message) {
