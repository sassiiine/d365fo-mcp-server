...
export const installMode = isGitCheckout ? 'git' : 'npm';
/** Bootstrap one-liner printed when a command needs a full installation. */
export const installOneLiner = 'irm https://raw.githubusercontent.com/dynamics365ninja/d365fo-mcp-server/main/install.ps1 | iex';
...
function stateDir() {
    ...
    if (isWindows && local)
        return resolve(local, 'd365fo-mcp');
    const xdg = process.env.XDG_CONFIG_HOME;
    if (xdg)
        return resolve(xdg, 'd365fo-mcp');
 ...
 */
export function readInstallPointer(file) {
    try {
...
export function writeInstallPointer(file, dir) {
...
export const paths = {
    ...
    get rootEnv() { return resolve(currentDataRoot, '.env'); },
    get rootConfig() { return resolve(currentDataRoot, 'config', 'd365fo-mcp.json'); },
    get rootSecrets() { return resolve(currentDataRoot, 'config', 'secrets.json'); },
 ...
 */
export const DOTNET_MISSING = 'The .NET SDK is not on PATH, so the C# bridge cannot be built — the server will run read-only.\n' +
...
export function bridgeBuildCommand() {
