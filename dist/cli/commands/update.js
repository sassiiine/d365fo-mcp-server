...
export function bridgeAction(hadBridge, existsNow) {
    if (!hadBridge)
        return 'none';
...
export async function updateCommand(opts) {
    ...
    if (action !== 'none') {
        ...
        if (gone)
            p.log.warn('The update replaced the package, so the C# bridge binary is gone — writes stay unavailable until it is rebuilt.');
        const rebuild = opts.yes || await askConfirm(gone
