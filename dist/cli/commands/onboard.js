...
async function findPackagesRoot() {
...
async function findCustomModels(packages) {
    const out = [];
    let entries;
    try {
        entries = await fs.readdir(packages);
    }
    catch {
        return out;
    }
...
export async function onboardCommand(opts) {
    ...
    const localEnv = { MCP_SERVER_MODE: 'write-only' };
    if (packages)
        localEnv.D365FO_PACKAGE_PATH = packages;
    if (model) {
        localEnv.D365FO_MODEL_NAME = model;
        localEnv.CUSTOM_MODELS = model;
    }
    const cfg = {
        servers: {
