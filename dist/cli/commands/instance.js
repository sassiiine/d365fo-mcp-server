...
export async function instanceAddCommand(name, portArg) {
    ...
    p.note(`Config: ${store.configPath}\n\n` +
        ...
        `2. Start it:         d365fo-mcp instance run ${instanceName}   (only needed for the http entry)`, 'Next steps');
    p.outro('');
}
/** Interactive XPP config picker; returns null when none are available. */
async function pickXppConfig(currentConfig) {
    ...
    const fullName = await askSelect('Select the XPP config', configs.map((cfg, i) => ({
        value: cfg.fullName,
...
export async function instanceUpgradeCommand(name) {
    ...
    if (!inst) {
        const picked = await askSelect('Which instance do you want to repoint?', instances.map(i => {
