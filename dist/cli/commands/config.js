 ...
 */
import { SECTIONS, settingByPath, settingsInSection } from '../../config/settings.js';
import { askSecrets, askSetting, askSettings } from '../settingsPrompt.js';
import { readSetting, saveStore, writeSetting } from '../settingsStore.js';
...
export async function selectXppConfig(store) {
    ...
    const pick = await askSelect(`${xppConfigNameSetting.label}\n  ${xppConfigNameSetting.description}`, [
...
export async function configCommand(sectionArg, opts) {
    ...
    if (opts.xppConfig) {
        if (!await selectXppConfig(store))
            process.exitCode = 1;
        p.outro('Rebuild the index afterwards: d365fo-mcp index');
        return;
    }
    const section = sectionArg ?? await askSelect('Which area?', SECTIONS.map(s => ({ value: s.id, label: s.title, hint: s.description })));
