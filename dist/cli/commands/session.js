...
function attributionNote(a) {
    const at = a.attribution;
    const total = a.totals.cost;
    const share = (v) => `${aiu(v, a.costUnit).padStart(11)} ${pct(v / total).padStart(7)}`;
    return [
...
function toolTable(a, top) {
    ...
    const rows = shown.map(t => t.name.slice(0, width).padEnd(width + 1)
    ...
    if (a.tools.length > top)
        rows.push(`… ${a.tools.length - top} more tool(s)`);
    return [header, '-'.repeat(header.length), ...rows].join('\n');
}
function printHuman(a, logPath, top) {
    ...
    if (!a.fit.usable) {
        ...
        p.log.info('Attribution, prefix and carry cost are withheld — they are all priced with the fitted rates.\n' +
    ...
    if (a.roundTrips.pluralOpportunities.length > 0) {
        ...
        p.log.warn(`${a.roundTrips.pluralOpportunities.length} run(s) of consecutive turns reaching for the same pluralisable tool ` +
    ...
    if (a.failures.length > 0) {
        ...
        p.log.warn(`${total} failed tool call(s)${wasted}:\n` +
    ...
    if (a.truncation.cap !== null) {
        p.log.info(`${a.truncation.results} result(s) were cut off at the host log's ${int(a.truncation.cap)}-char cap — ` +
...
export async function sessionCommand(logPath, opts = {}) {
    if (!logPath) {
    ...
    try {
        ...
        if (opts.json)
            process.stdout.write(`${JSON.stringify({ schemaVersion: 1, log: logPath, error: message }, null, 2)}\n`);
        else
            p.log.error(message);
        process.exitCode = 1;
        return;
