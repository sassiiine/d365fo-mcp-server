...
async function probe(health, apiKey) {
    try {
...
function serverEntry(url, apiKey, alwaysLoad) {
    return {
        type: 'http',
        url,
 ...
 */
export function mergeConfig(existing, key, name, entry) {
    let doc = {};
...
async function connectClaudeCode(entry) {
...
export async function connectCommand(urlArg, opts) {
    ...
    // 4. Which editor.
    const client = opts.client ?? await askSelect('Which editor should use it?', [
    ...
    p.note(client === 'vs'
        ...
        : 'Reload the window to pick up the new server.', 'Next');
