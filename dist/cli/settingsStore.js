...
import { getAtPath, resolveConfigFiles, setAtPath, writeConfigFile, writeSecretsFile, } from '../config/configFile.js';
import { SETTINGS, parseValue } from '../config/settings.js';
 ...
 */
export function openStore(baseDir, legacyEnvFile, fallbackConfigPath) {
...
export function openInstanceStore(instanceDir) {
...
export function readSetting(store, setting) {
    ...
    if (store.legacyEnvFile) {
        const raw = readEnvValue(store.legacyEnvFile, setting.env);
        if (raw !== null && raw !== '')
            return parseValue(setting, stripInlineComment(raw));
    }
    return undefined;
}
/** Effective value, falling back to the documented default. */
export function readSettingOrDefault(store, setting) {
...
export function initialText(store, setting) {
 ...
 */
export function migrateLegacyEnv(store) {
    if (!store.legacyEnvFile)
        return [];
    const migrated = [];
    for (const setting of SETTINGS) {
        ...
        if (!setting.path)
            continue;
        if (getAtPath(store.config, setting.path) !== undefined)
            continue;
        const raw = readEnvValue(store.legacyEnvFile, setting.env);
        if (raw === null)
            continue;
        ...
        if (setting.type === 'enum' && !setting.choices?.some(c => c.value === parsed))
            continue;
 ...
 */
export function conflictingLegacyValues(store) {
    if (!store.legacyEnvFile)
        return [];
    const out = [];
    for (const setting of SETTINGS) {
        ...
        if (configValue !== envValue)
            out.push({ setting, envValue, configValue });
    }
    return out;
}
/**
