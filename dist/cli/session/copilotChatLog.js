...
export function sniffCopilotChatLog(lines) {
 ...
 */
function detectTruncationCap(lengths) {
    if (lengths.length === 0)
        return null;
    const max = Math.max(...lengths);
    if (max < 1000)
        return null; // too small to be a cap; just a chatty tool
    return lengths.filter(l => l === max).length >= 2 ? max : null;
}
export function readCopilotChatLog(lines) {
    ...
    for (const span of spans) {
        if (!sessionId && typeof span.sid === 'string')
            sessionId = span.sid;
    ...
    const toolCalls = rawToolCalls.map(({ span, text }) => ({
        ts: num(span.ts),
...
//# sourceMappingURL=copilotChatLog.js.map