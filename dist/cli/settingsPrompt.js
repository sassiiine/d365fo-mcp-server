...
import { initialText, readSetting, readSettingOrDefault, writeSetting } from './settingsStore.js';
 ...
 */
export async function askSetting(store, setting, opts) {
    ...
    if (setting.tier === 'secret') {
        ...
        if (entered)
            writeSetting(store, setting, entered);
    ...
    switch (setting.type) {
        ...
        case 'enum': {
            ...
            const current = String(readSetting(store, setting) ?? opts?.initial ?? setting.default ?? choices[0]?.value ?? '');
            const value = await askSelect(message(setting), choices.map(choice => ({ value: choice.value, label: choice.value, hint: choice.hint })), current);
            writeSetting(store, setting, value);
            return value;
        }
        case 'int': {
...
export async function askSettings(store, settings) {
 ...
 */
export async function askAdvanced(store, sections) {
    ...
    let initialValues = [];
    for (;;) {
        const picked = ensure(await multiselect({
        ...
        if (await askConfirm('Nothing selected — leave every advanced setting at its default?', true))
            return;
        initialValues = available;
    }
}
/** Ask for the secrets of the given sections, skipping any already set. */
export async function askSecrets(store, sections) {
...
//# sourceMappingURL=settingsPrompt.js.map