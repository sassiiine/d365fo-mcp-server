...
export function selectProject(projectName, allProjects) {
    ...
    for (const { matchedOn, hits } of STRATEGIES) {
        ...
        if (found.length === 1)
            return { kind: 'resolved', project: found[0], matchedOn };
        if (found.length > 1) {
...
export function renderProjectCandidates(candidates) {
    return candidates
        .map(c => {
...
export function renderSelectionFailure(selection, allProjects) {
    if (selection.kind === 'none') {
        ...
        return (`❌ No project matches "${selection.needle}".\n\n` +
    ...
    return (`❌ "${selection.needle}" is a ${what}, not one project — ${selection.candidates.length} projects match it.\n` +
