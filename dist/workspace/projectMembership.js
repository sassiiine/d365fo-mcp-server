 ...
 * the same function.
 */
const AX_FOLDER_BY_OBJECT_TYPE = {
    class: 'AxClass',
...
export function axFolderForObjectType(objectType) {
...
export function objectTypeForAxFolder(axFolder) {
 ...
 */
export function includeKey(axFolder, objectName) {
 ...
 */
export function projectDisplayName(projectPath) {
 ...
 */
export async function readProjectIncludes(projectPath) {
    ...
    for (const group of (parsed?.Project?.ItemGroup ?? [])) {
        ...
        for (const c of contents) {
            ...
            if (inc)
                includes.add(inc.replace(/\.xml$/i, '').toLowerCase());
        }
    }
 ...
 */
export async function resolveMembership(axFolder, objectName, activeProjectPath, siblingProjectPaths = []) {
    ...
    if (!readAny)
        return { status: 'unknown', owners: [] };
    if (activeProjectPath && owners[0] === activeProjectPath)
        return { status: 'active', owners };
    if (owners.length > 0)
        return { status: 'other', owners };
 ...
 */
export function renderMembership(m, axFolder, objectName) {
    switch (m.status) {
        case 'active':
        case 'unknown':
            return '';
