 ...
 */
async function getUncommittedXppFiles(workspacePath) {
    if (!workspacePath)
        return [];
    ...
    const untracked = await gitSafe(['ls-files', '--others', '--exclude-standard'], workspacePath);
    ...
    for (const block of [tracked, untracked]) {
        ...
        for (const line of block.split('\n')) {
            const rel = line.trim();
            if (rel && rel.toLowerCase().endsWith('.xml'))
                files.add(rel);
        }
    }
 ...
 */
export async function buildContextSnapshot(context) {
...
export function renderContextSnapshotSection(snapshot) {
    ...
    if (snapshot.activeObject) {
        ...
        lines.push(`Active object (most recently modified): ${a.name} [${a.type}] — ${a.modifiedAt.replace('T', ' ').slice(0, 16)}`, '');
    }
    if (snapshot.recentObjects.length === 0) {
