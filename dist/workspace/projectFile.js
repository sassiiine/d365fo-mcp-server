...
import { axFolderForObjectType, resolveMembership, projectDisplayName, } from './projectMembership.js';
 ...
 */
export async function registerFileInActiveProject(objectType, objectName, modelName, projectPath) {
    ...
    // ones stay quiet.
    if (membership.status === 'active' || membership.status === 'unknown')
        return '';
...
export class ProjectFileManager {
     ...
     */
    async removeFromProject(projectPath, objectType, objectName) {
        return withFileLock(projectPath, () => this._removeFromProjectLocked(projectPath, objectType, objectName));
    }
    async _removeFromProjectLocked(projectPath, objectType, objectName) {
        let projectXml = '';
        for (let attempt = 0; attempt < 5; attempt++) {
            try {
                projectXml = await fs.readFile(projectPath, 'utf-8');
        ...
        const project = await this.parser.parseStringPromise(projectXml);
        if (!project.Project || !project.Project.ItemGroup)
            return false;
        ...
        for (const group of itemGroups) {
            ...
            const kept = contents.filter((c) => c?.$?.Include !== contentInclude);
        ...
        const stillUsesFolder = itemGroups.some((group) => {
            if (group.Content === undefined)
                return false;
            const contents = Array.isArray(group.Content) ? group.Content : [group.Content];
            return contents.some((c) => typeof c?.$?.Include === 'string' && c.$.Include.startsWith(`${axFolderPrefix}\\`));
        });
        if (weAddedFolder && !stillUsesFolder) {
            for (const group of itemGroups) {
                ...
                group.Folder = folders.filter((f) => f?.$?.Include !== `${displayFolderName}\\`);
            }
        }
        const updatedXml = this.builder.buildObject(project);
        const output = hadBom ? '\uFEFF' + updatedXml : updatedXml;
    ...
    async addLabelToProject(projectPath, labelFileId, languages) {
        return withFileLock(projectPath, () => this._addLabelToProjectLocked(projectPath, labelFileId, languages));
    }
