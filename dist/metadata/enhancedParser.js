...
export class EnhancedXppParser {
    ...
    generateUsageExample(className, method) {
        ...
        const params = method.parameters.map(p => {
            if (p.type.toLowerCase().includes('int'))
                return '0';
            if (p.type.toLowerCase().includes('str'))
                return '""';
            if (p.type.toLowerCase().includes('bool'))
                return 'false';
            if (p.type.toLowerCase().includes('date'))
    ...
    generateExampleValue(typeName) {
        ...
        if (lower.includes('int') || lower.includes('recid'))
            return '0';
        if (lower.includes('str') || lower.includes('string'))
            return '""';
