 ...
 */
export function normalizeControlType(axType) {
    if (!axType)
        return '';
    let t = axType;
    if (t.startsWith('AxForm'))
        t = t.slice('AxForm'.length);
    if (t.endsWith('Control') && !EXTENSION_CONTROL_NAMES.has(t))
        t = t.slice(0, -'Control'.length);
    return t;
}
...
function resolveControlType(node) {
    ...
    if (fromAxType)
        return { type: fromAxType, axType };
    const typeElement = asString(node.Type);
    if (typeElement)
        return { type: typeElement, axType };
 ...
 */
export function walkFormDesign(designNode) {
    const design = { properties: {}, controls: [] };
    if (!designNode || typeof designNode !== 'object')
        return design;
            ...
            for (const node of asArray(designNode[key])) {
                ...
                if (control) {
                    ...
                    if (control.type === 'Control')
                        control.type = normalizeControlType(key) || control.type;
                    design.controls.push(control);
                }
            }
        }
 ...
 */
export function collectPatternNodes(design) {
    const records = [];
    if (design.pattern) {
        records.push({
            nodePath: 'Design',
    ...
    for (const control of design.controls)
        visit(control, 'Design');
    return records;
}
//# sourceMappingURL=formPatternMiner.js.map