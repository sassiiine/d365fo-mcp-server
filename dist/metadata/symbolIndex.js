...
export class XppSymbolIndex {
    db; // Public for direct pragma access in build scripts
    labelsDb; // Separate DB for labels (performance optimization)
    stmtCache = new Map();
    labelsStmtCache = new Map();
    ...
    constructor(dbPath, labelsDbPath) {
        ...
        if (dbPath !== ':memory:') {
            const poolSize = Math.min(8, Math.max(1, parseInt(process.env.READ_POOL_SIZE || '3', 10) || 3));
    ...
    `);
        ...
        {
            const existingCols = new Set(this.db.pragma('table_info(symbols)').map(r => r.name));
            const newCols = [
                ['package_name', 'TEXT'],
                ['description', 'TEXT'],
                ['tags', 'TEXT'],
    ...
    `);
        ...
        {
            const existingCols = new Set(this.db.pragma('table_info(edt_metadata)').map(r => r.name));
    ...
    static LABEL_SECONDARY_INDEX_SQL = XppSymbolIndex.LABEL_SECONDARY_INDEXES
    ...
    migrateLabelPathsToLabelFiles() {
        ...
        try {
            ...
            if (version >= LABEL_FILES_NORMALISED)
                return;
            ...
            if (n > 0) {
                log.detail(`Normalising ${n.toLocaleString('en-US')} label file paths into label_files ` +
      ...
      `);
            ...
            this.labelsDb.pragma(`user_version = ${LABEL_FILES_NORMALISED}`);
            if (n > 0)
                log.detail(`Label file paths normalised (${n.toLocaleString('en-US')} rows).`);
        }
        catch (e) {
    ...
    migrateLabelsFtsLanguageCoverage() {
        ...
        try {
            const version = Number(this.labelsDb.pragma('user_version', { simple: true }) ?? 0);
            if (version >= LABELS_FTS_ALL_LANGUAGES)
                return;
            ...
            const legacyPathColumn = this.labelsDb.pragma('table_info(labels)')
            ...
            if (n > 0) {
                ...
                log.detail(`Re-tokenising ${n.toLocaleString('en-US')} labels so every language is searchable (one-off, ~20 s)…`);
                this.rebuildLabelsFts();
            }
            this.labelsDb.pragma(`user_version = ${LABELS_FTS_ALL_LANGUAGES}`);
        }
    ...
    addSymbol(symbol) {
      ...
      `);
        ...
        stmt.run(symbol.name, symbol.type, symbol.parentName || null, symbol.signature || null, symbol.filePath, symbol.model, symbol.packageName || symbol.model, symbol.description || null, symbol.tags || null, symbol.sourceSnippet || null, symbol.source || null, symbol.complexity || null, symbol.usedTypes || null, symbol.methodCalls || null, symbol.inlineComments || null, symbol.extendsClass || null, symbol.implementsInterfaces || null, symbol.visibility || null, symbol.usageExample || null, symbol.usageFrequency || 0, symbol.patternType || null, symbol.typicalUsages || null, symbol.calledByCount || 0, symbol.relatedMethods || null, symbol.apiPatterns || null);
    }
    /**
    ...
    removeSymbolsByFile(filePath) {
        ...
        const rows = this.db.prepare(`SELECT DISTINCT name FROM symbols WHERE file_path COLLATE NOCASE IN (${placeholders}) AND parent_name IS NULL`).all(...forms);
    ...
    removeLabelsByFile(filePath) {
        ...
        const result = this.labelsDb.prepare(`DELETE FROM labels
    ...
    removeLabelById(labelId, model) {
        const result = this.labelsDb.prepare(`DELETE FROM labels WHERE label_id = ? AND model = ?`).run(labelId, model);
        return result.changes;
    }
    /**
    ...
    getSymbolByName(name, type) {
        const db = this.getReadDb();
        const stmt = this.getReadStmt(db, 'getSymbolByName', () => `SELECT * FROM symbols WHERE name = ? AND type = ? LIMIT 1`);
     ...
     */
    getAllClasses() {
        const stmt = this.getReadDb().prepare(`SELECT * FROM symbols WHERE type = 'class' ORDER BY name`);
     ...
     */
    getSymbolCount() {
        if (this.symbolCountsCache)
            return this.symbolCountsCache.total;
    ...
    getSymbolCountByType() {
        if (this.symbolCountsCache)
            return this.symbolCountsCache.byType;
    ...
    async getSymbolCounts() {
        if (this.symbolCountsCache)
            return this.symbolCountsCache;
        if (this.symbolCountsPromise)
            return this.symbolCountsPromise;
    ...
    `).all();
        ...
        const insertStmt = this.db.prepare('INSERT INTO temp_method_calls (caller_method, called_method) VALUES (?, ?)');
        ...
        if (!inCI)
            console.log(''); // newline after the overwriting progress line
    ...
    async indexMetadataDirectory(metadataPath, modelNames, opts) {
        ...
        for (const model of models) {
            ...
            const tx = this.db.transaction(() => {
                ...
                if (fs.existsSync(edtsPath))
                    this.indexEdts(edtsPath, model);
                const reportsPath = path.join(modelPath, 'reports');
                if (fs.existsSync(reportsPath))
                    this.indexReports(reportsPath, model);
                // Security artifacts
                const secPrivPath = path.join(modelPath, 'security-privileges');
                if (fs.existsSync(secPrivPath))
                    this.indexSecurityPrivileges(secPrivPath, model);
                const secDutyPath = path.join(modelPath, 'security-duties');
                if (fs.existsSync(secDutyPath))
                    this.indexSecurityDuties(secDutyPath, model);
                const secRolePath = path.join(modelPath, 'security-roles');
                if (fs.existsSync(secRolePath))
                    this.indexSecurityRoles(secRolePath, model);
                // Menu items
                const menuDisplayPath = path.join(modelPath, 'menu-item-displays');
                if (fs.existsSync(menuDisplayPath))
                    this.indexMenuItems(menuDisplayPath, model, 'display');
                const menuActionPath = path.join(modelPath, 'menu-item-actions');
                if (fs.existsSync(menuActionPath))
                    this.indexMenuItems(menuActionPath, model, 'action');
                const menuOutputPath = path.join(modelPath, 'menu-item-outputs');
                if (fs.existsSync(menuOutputPath))
                    this.indexMenuItems(menuOutputPath, model, 'output');
                // Extensions
                const tableExtPath = path.join(modelPath, 'table-extensions');
                if (fs.existsSync(tableExtPath))
                    this.indexExtensions(tableExtPath, model, 'table-extension');
                const classExtPath = path.join(modelPath, 'class-extensions');
                if (fs.existsSync(classExtPath))
                    this.indexExtensions(classExtPath, model, 'class-extension');
                const formExtPath = path.join(modelPath, 'form-extensions');
                if (fs.existsSync(formExtPath))
                    this.indexExtensions(formExtPath, model, 'form-extension');
                const enumExtPath = path.join(modelPath, 'enum-extensions');
                if (fs.existsSync(enumExtPath))
                    this.indexExtensions(enumExtPath, model, 'enum-extension');
                const edtExtPath = path.join(modelPath, 'edt-extensions');
                if (fs.existsSync(edtExtPath))
                    this.indexExtensions(edtExtPath, model, 'edt-extension');
                const deExtPath = path.join(modelPath, 'data-entity-extensions');
                if (fs.existsSync(deExtPath))
                    this.indexExtensions(deExtPath, model, 'data-entity-extension');
                const viewExtPath = path.join(modelPath, 'view-extensions');
                if (fs.existsSync(viewExtPath))
                    this.indexExtensions(viewExtPath, model, 'view-extension');
                const queryExtPath = path.join(modelPath, 'query-extensions');
                if (fs.existsSync(queryExtPath))
                    this.indexExtensions(queryExtPath, model, 'query-extension');
                const mapExtPath = path.join(modelPath, 'map-extensions');
                if (fs.existsSync(mapExtPath))
                    this.indexExtensions(mapExtPath, model, 'map-extension');
                const menuExtPath = path.join(modelPath, 'menu-extensions');
                if (fs.existsSync(menuExtPath))
                    this.indexExtensions(menuExtPath, model, 'menu-extension');
                const secDutyExtPath = path.join(modelPath, 'security-duty-extensions');
                if (fs.existsSync(secDutyExtPath))
                    this.indexExtensions(secDutyExtPath, model, 'security-duty-extension');
                const secRoleExtPath = path.join(modelPath, 'security-role-extensions');
                if (fs.existsSync(secRoleExtPath))
                    this.indexExtensions(secRoleExtPath, model, 'security-role-extension');
                const miDisplayExtPath = path.join(modelPath, 'menu-item-display-extensions');
                if (fs.existsSync(miDisplayExtPath))
                    this.indexExtensions(miDisplayExtPath, model, 'menu-item-display-extension');
                const miActionExtPath = path.join(modelPath, 'menu-item-action-extensions');
                if (fs.existsSync(miActionExtPath))
                    this.indexExtensions(miActionExtPath, model, 'menu-item-action-extension');
                const miOutputExtPath = path.join(modelPath, 'menu-item-output-extensions');
                if (fs.existsSync(miOutputExtPath))
                    this.indexExtensions(miOutputExtPath, model, 'menu-item-output-extension');
                // Services + service groups
                const servicesPath = path.join(modelPath, 'services');
                if (fs.existsSync(servicesPath))
                    this.indexServices(servicesPath, model);
                const serviceGroupsPath = path.join(modelPath, 'service-groups');
                if (fs.existsSync(serviceGroupsPath))
                    this.indexServiceGroups(serviceGroupsPath, model);
                // Maps, feature gating, security policies, macros
                const mapsPath = path.join(modelPath, 'maps');
                if (fs.existsSync(mapsPath))
                    this.indexMaps(mapsPath, model);
                const configKeysPath = path.join(modelPath, 'configuration-keys');
                if (fs.existsSync(configKeysPath))
                    this.indexConfigurationKeys(configKeysPath, model);
                const licenseCodesPath = path.join(modelPath, 'license-codes');
                if (fs.existsSync(licenseCodesPath))
                    this.indexLicenseCodes(licenseCodesPath, model);
                const securityPoliciesPath = path.join(modelPath, 'security-policies');
                if (fs.existsSync(securityPoliciesPath))
                    this.indexSecurityPolicies(securityPoliciesPath, model);
                const macrosPath = path.join(modelPath, 'macros');
                if (fs.existsSync(macrosPath))
                    this.indexMacros(macrosPath, model);
    ...
    sortModelsBySize(metadataPath, models) {
        const sized = models.map(model => {
            ...
            try {
                ...
                const entries = fs.readdirSync(modelPath, { recursive: true });
                    ...
                    for (const relation of tableData.relations) {
                        if (relation.name && relation.relatedTable) {
                            ...
                            const constraintFields = relation.constraints
                                ...
                                : null;
                            stmt.run(tableData.name, relation.relatedTable, relation.name, constraintFields, model);
                        }
                    }
                }
            }
    ...
    indexEdts(edtsPath, model) {
        ...
        for (const file of files) {
          ...
          `);
                    stmt.run(edtName, edtData.extends || null, edtData.enumType || null, edtData.referenceTable || null, edtData.relationType || null, edtData.stringSize || null, edtData.databaseStringSize || null, edtData.displayLength || null, edtData.label || null, model);
                }
            }
            catch (error) {
        ...
        for (const file of files) {
          ...
          `);
                    for (const node of formData.patternNodes) {
                        if (!node?.pattern || !node?.nodePath)
                            continue;
                        patternStmt.run(formName, model, node.nodePath, node.controlName ?? '', node.controlType ?? '', node.pattern, node.patternVersion ?? null, JSON.stringify(node.childSequence ?? []));
                        ...
                        if (node.nodePath === 'Design' && this.isMineableModel(model)) {
                            ...
                            this.recordPropertyStat('AxFormDesign', `PatternVersion:${node.pattern}`, node.patternVersion ?? '(absent)', model);
                        }
                    }
                }
          ...
          `);
                    for (const ds of formData.dataSources) {
                        if (ds.name && ds.table) {
                            stmt.run(formName, ds.name, ds.table, ds.allowEdit ? 1 : 0, ds.allowCreate ? 1 : 0, ds.allowDelete ? 1 : 0, model);
                        }
                    }
                }
            }
                ...
                for (const ep of entryPoints) {
                    ...
                    const accessLevelStr = ep.accessLevel == null
                        ...
                        : typeof ep.accessLevel === 'object'
                            ? Object.entries(ep.accessLevel).map(([k, v]) => `${k}:${v}`).join(',')
                            : String(ep.accessLevel);
                    insertEntry.run(name, ep.name, ep.objectType ?? null, accessLevelStr, model);
                }
            }
            catch (error) {
    ...
    `);
        for (const file of files) {
            try {
                ...
                insertTarget.run(name, menuItemType, data.targetObject || data.object || null, data.targetType || data.objectType || null, data.securityPrivilege || null, data.label || null, model);
            }
            catch (error) {
    ...
    `);
        for (const file of files) {
            try {
                ...
                insertPolicy.run(name, data.primaryTable || null, data.query || null, data.operation || null, data.constrainedTable ? 1 : 0, data.label || null, model);
            }
            catch (error) {
    ...
    `);
        for (const file of files) {
            try {
                ...
                insertMeta.run(name, extensionType, baseObjectName, data.addedFields ? JSON.stringify(data.addedFields) : null, data.addedMethods ? JSON.stringify(data.addedMethods) : null, data.addedIndexes ? JSON.stringify(data.addedIndexes) : null, data.cocMethods ? JSON.stringify(data.cocMethods) : null, data.eventSubscriptions ? JSON.stringify(data.eventSubscriptions) : null, model);
            }
            catch (error) {
    ...
    upsertExtensionMetadata(record) {
        ...
        INSERT INTO extension_metadata
          (extension_name, extension_type, base_object_name, added_fields, added_methods,
           added_indexes, coc_methods, event_subscriptions, model)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(record.extensionName, record.extensionType, record.baseObjectName, json(record.addedFields), json(record.addedMethods), json(record.addedIndexes), json(record.cocMethods), json(record.eventSubscriptions), record.model);
        })();
    }
    ...
    removeExtensionMetadata(extensionName, extensionType, model) {
        try {
            return this.db.prepare(`DELETE FROM extension_metadata
         WHERE extension_name = ? AND extension_type = ? AND model = ?`).run(extensionName, extensionType, model).changes;
        }
        catch {
            return 0;
        }
    ...
    touchLastIndexed() {
        try {
            this.db.prepare(`INSERT OR REPLACE INTO _index_meta (key, value) VALUES ('last_indexed_at', ?)`).run(new Date().toISOString());
        }
        catch {
    ...
    getLastIndexedAt() {
        try {
            const row = this.getReadDb().prepare(`SELECT value FROM _index_meta WHERE key = 'last_indexed_at'`).get();
    ...
    getPropertyPresenceRatio(nodeType, property) {
        const rows = this.getReadDb().prepare(`SELECT value, SUM(count) AS c FROM property_stats
       WHERE node_type = ? AND property = ? GROUP BY value`).all(nodeType, property);
        let present = 0;
        let total = 0;
        for (const row of rows) {
            total += row.c;
    ...
    getPropertyValueDistribution(nodeType, property, limit = 10) {
        return this.getReadDb().prepare(`SELECT value, SUM(count) AS count FROM property_stats
       WHERE node_type = ? AND property = ? AND value NOT IN ('(present)', '(absent)')
       GROUP BY value ORDER BY count DESC LIMIT ?`).all(nodeType, property, limit);
    }
    /**
    ...
    purgeNonMineableStats(opts = {}) {
        const models = this.db.prepare('SELECT DISTINCT model FROM property_stats').all()
            .map(r => r.model);
        const toPurge = models.filter(m => !this.isMineableModel(m));
        if (toPurge.length === 0 || opts.dryRun)
            return toPurge;
    ...
    recordTablePropertyStats(tableData, model) {
        ...
        try {
            ...
            this.recordPropertyStat('AxTable', 'AlternateKeyIndex', presence(indexes.some((i) => i?.unique)), model);
            const fields = Array.isArray(tableData.fields) ? tableData.fields : [];
            for (const field of fields) {
                this.recordPropertyStat('AxTableField', 'ExtendedDataType', presence(field?.extendedDataType || field?.enumType), model);
            }
        }
        catch {
    ...
    getCompletions(objectName, prefix) {
        ...
        if (!stmt) {
            stmt = this.db.prepare(`SELECT name, type, signature FROM symbols
    ...
    analyzeCodePatterns(scenario, classPattern, limit = 20) {
        ...
        const buildLikeSql = () => {
            const likeSql = keywords.length > 0
                ? `SELECT DISTINCT s.* FROM symbols s WHERE s.type = 'class' AND (${keywords.map(() => 's.name LIKE ? OR s.tags LIKE ? OR s.description LIKE ?').join(' OR ')})`
      ...
      `;
        ...
        if (classes.length > 0) {
            ...
            const allMethods = this.db.prepare(`SELECT name FROM symbols WHERE type = 'method' AND parent_name IN (${placeholders})`).all(...classNames);
    ...
    detectPatternTypes(classes) {
        ...
        for (const cls of classes) {
            ...
            if (name.endsWith('Helper'))
                ...
                patternType = 'Handler';
            else if (name.endsWith('Repository') || name.endsWith('Repo'))
                ...
                patternType = 'Builder';
            else if (name.endsWith('Processor'))
                patternType = 'Processor';
        ...
        for (const method of methods) {
            if (method.method_calls) {
                for (const call of method.method_calls.split(',')) {
                    const c = call.trim();
                    if (c)
                        methodCallPatterns[c] = (methodCallPatterns[c] || 0) + 1;
                }
            }
            ...
            if (method.source_snippet && method.source_snippet.includes('new ' + className)) {
                ...
                if (!initPatterns.includes(snippet))
                    initPatterns.push(snippet);
            }
        }
    ...
    `;
        ...
        if (similarClasses.length > 0) {
            ...
            const placeholders = classNames.map(() => '?').join(',');
            const methodCounts = this.db.prepare(`SELECT name, COUNT(DISTINCT parent_name) AS class_count
         FROM symbols
         WHERE type = 'method' AND parent_name IN (${placeholders})
         GROUP BY name
         ORDER BY class_count DESC
         LIMIT 50`).all(...classNames);
     ...
     */
    getAllSymbolNames(query, limit = 2000) {
        const trimmed = query?.trim();
        ...
        if (this.suggestionNamesCache.size >= SUGGESTION_CACHE_ENTRIES) {
            ...
            if (oldest !== undefined)
                this.suggestionNamesCache.delete(oldest);
        }
     ...
     */
    getSymbolsByTerm() {
        if (this.symbolsByTermCache)
            return this.symbolsByTermCache;
    ...
    addLabel(entry) {
      ...
      `);
        ...
        stmt.run(entry.labelId, entry.labelFileId, entry.model, entry.language, entry.text, entry.comment ?? null, this.labelFilePathId(entry.filePath));
    }
    /**
    ...
    bulkAddLabels(entries, opts) {
      ...
      `);
            const insertMany = this.labelsDb.transaction((rows) => {
                for (const e of rows) {
                    insert.run(e.labelId, e.labelFileId, e.model, e.language, e.text, e.comment ?? null, this.labelFilePathId(e.filePath));
                }
            });
            insertMany(entries);
    ...
    getLabelFileIds(model, labelFileId) {
        const run = (extraWhere, params) => this.labelsDb.prepare(`
    ...
    clearLabelsForModels(models, opts) {
        ...
        if (opts?.ftsStrategy !== 'incremental')
            this.rebuildLabelsFts();
    }
    /**
     * Total label count
