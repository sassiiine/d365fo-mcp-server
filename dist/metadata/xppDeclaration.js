...
function parseParameter(raw, blanked) {
    ...
    if (!isNameToken(name) || !typeTokens.every(isTypeToken))
        return null;
    if (eq >= 0 && !defaultValue)
        return null;
...
function tryDeclarationAt(source, blanked, nameStart, name) {
    ...
    if (!returnType || STATEMENT_KEYWORDS.includes(returnType.toLowerCase()))
        return null;
 ...
 */
export function parseXppClassHeader(declaration) {
 ...
 */
export function visibilityFromModifiers(modifiers) {
    return modifiers.find(m => m === 'public' || m === 'private' || m === 'protected' || m === 'internal');
}
/**
 ...
 */
export function parseExtensionOfAttribute(declaration) {
    if (!declaration || typeof declaration !== 'string')
        return null;
 ...
 */
export function callsNext(source) {
 ...
 */
export function parseXppDeclaration(source, methodName) {
...
//# sourceMappingURL=xppDeclaration.js.map