 ...
 */
import { resolveObjectPrefix, applyObjectPrefix, applyObjectSuffix, deriveExtensionInfix, getObjectSuffix, getExtensionNamingStyle, } from './modelClassifier.js';
 ...
 */
export const DOT_NOTATION_EXTENSION_TYPES = new Set([
...
export function isExtensionObjectType(objectType) {
 ...
 */
function stripElementStyleExtensionWord(name, tokens) {
    if (!/extension$/i.test(name))
        return name;
 ...
 */
export function normalizeObjectName(objectName, objectType, modelName, onNote) {
    ...
    const modelDiffersFromPrefix = !!modelToken && objectPrefix.toLowerCase() !== modelToken.toLowerCase();
    ...
    if (objectType === 'class-extension' && !effective.endsWith('_Extension')) {
        const base = stripElementStyleExtensionWord(effective, [deriveExtensionInfix(objectPrefix, modelName), objectPrefix, modelToken]);
