 ...
 */
function leadingTokenCandidates(name) {
    const out = [];
    ...
    if (us >= 1 && us <= MAX_TOKEN_LEN)
        out.push(name.slice(0, us + 1));
    ...
    for (let i = 0; i < Math.min(MAX_TOKEN_SEGMENTS, segs.length); i++) {
        acc += segs[i];
        if (acc.length >= 2 && acc.length <= MAX_TOKEN_LEN)
            out.push(acc);
    }
    return out;
}
 ...
 */
function tokenAllowed(token, modelName, statedInfix) {
    const bare = token.replace(/_+$/, '');
    const segs = segments(bare);
    if (segs.length < 3)
        return true;
    if (!modelName && !statedInfix)
        return true;
    if (statedInfix && statedInfix.toLowerCase().startsWith(bare.toLowerCase()))
        return true;
 ...
 */
function modelNameCarries(modelName, segs) {
    ...
    for (let i = 0; i < segs.length; i++) {
        ...
        if (at < 0)
            return false;
        if (i === 0 && at !== 0)
            return false; // the prefix starts where the name does
 ...
 */
function inferInfixFromDotExtensions(dotNames) {
    const counts = new Map();
    for (const name of dotNames) {
        const suffix = name.slice(name.lastIndexOf('.') + 1);
        if (!/extension$/i.test(suffix))
            continue;
        const token = suffix.slice(0, -'Extension'.length);
        if (token.length < 2 || token.length > MAX_TOKEN_LEN)
            continue;
 ...
 */
function observedInfixCasing(names, token) {
    if (!token)
        return null;
    const needle = token.toLowerCase();
    const counts = new Map();
    const bump = (spelling) => {
        if (spelling.toLowerCase() !== needle)
            return;
 ...
 */
export function toExtensionInfixCase(bare) {
    if (!bare)
        return '';
    const segs = segments(bare);
    if (segs.length === 0)
 ...
 */
function deriveInfixFrom(regular) {
 ...
 */
export function inferPrefixFromObjectNames(names, modelName) {
    ...
    if (regularOk && infixFromElements) {
        ...
        const isTruncation = longer.length > bare.length && longer.toLowerCase().startsWith(bare.toLowerCase());
        const carriedByObjects = regulars.filter(n => n.toLowerCase().startsWith(longer.toLowerCase())).length / regulars.length;
 ...
 */
export function setModelObjectNameSource(source) {
    nameSource = source;
    inferred.clear();
}
/** Seed a model's inferred prefix directly, bypassing the source (tests, CLI). */
export function primeInferredModelPrefix(modelName, names) {
...
export function clearInferredModelPrefixes() {
    inferred.clear();
}
/**
 ...
 */
export function getInferredModelPrefix(modelName) {
    if (!modelName || inferenceDisabled())
        return null;
    ...
    try {
        ...
        if (result) {
            console.error(`[ModelPrefix] Model "${modelName}" uses prefix "${result.regular}" ` +
...
//# sourceMappingURL=modelPrefixInference.js.map