...
function parseNodes(xml) {
    ...
    while (i < xml.length) {
        ...
        if (raw.startsWith('</')) {
            ...
            if (!top || top.name !== name)
                return null; // mismatched close → decline
            top.closeStart = lt;
            top.end = gt + 1;
            i = gt + 1;
            continue;
    ...
    if (stack.length > 0)
        return null; // unclosed element → decline
    return root;
}
const firstChild = (n, name) => n.children.find(c => c.name === name);
const textOf = (xml, n) => n.selfClosing ? '' : xml.slice(n.openEnd, n.closeStart).trim();
/**
...
const textValueOf = (xml, n) => decodeXmlText(textOf(xml, n));
/**
...
function lineIndentOf(xml, offset) {
    const lineStart = xml.lastIndexOf('\n', offset - 1) + 1;
    const seg = xml.slice(lineStart, offset);
    return /^[ \t]*$/.test(seg) ? seg : '\t';
}
const indentBlock = (lines, indent) => lines.map(l => (l === '' ? '' : indent + l)).join('\n');
/**
 ...
 */
export function findFormExtensionPlacementProblems(xml) {
    const root = parseNodes(xml);
    if (!root || root.name !== 'AxFormExtension')
        return [];
    return findPlacementIssues(xml, root).map(i => i.problem);
}
function findPlacementIssues(xml, root) {
    const issues = [];
    const lineAt = (offset) => {
        let line = 1;
            ...
            if (child.name !== 'AxFormExtensionControl') {
                issues.push({
                    ...
                    problem: {
                        ...
                        detail: `<${child.name}> is in the extension's ROOT <Controls>, which holds ` +
                    ...
                    if (child.name !== 'AxFormControl') {
                        issues.push({
                            ...
                            problem: {
                                element: child.name,
                                line: lineAt(child.start),
                                detail: `<${child.name}> is nested inside the <Controls> of "${owner}", which is typed to ` +
 ...
 */
const problemKey = (p) => `${p.element} ${p.detail}`;
const tallyProblems = (problems) => {
    const counts = new Map();
    for (const p of problems)
        counts.set(problemKey(p), (counts.get(problemKey(p)) ?? 0) + 1);
    return counts;
};
/** True when `n` is the misplaced node itself or lives inside its discarded subtree. */
const isWithin = (n, container) => n.start >= container.start && n.end <= container.end;
/** Render placement problems as a blocking, self-explaining error. */
export function buildFormExtensionPlacementError(objectName, problems) {
    const rows = problems
        .map(p => `  • line ${p.line}: <${p.element}>\n    ${p.detail}`)
        .join('\n');
    return (`⛔ form-extension "${objectName}" — a control element is in a collection that cannot hold it.\n\n` +
        `${rows}\n\n` +
 ...
 */
export function insertFormExtensionControl(xml, spec) {
    const root = parseNodes(xml);
    ...
    if (preExisting.length > 0) {
        notes.push(`this file already contained ${preExisting.length} misplaced control ` +
    ...
    if (owned.live.has(spec.controlName.toLowerCase()))
        return { kind: 'exists' };
    const discardedTwin = owned.discarded.get(spec.controlName.toLowerCase());
    if (discardedTwin) {
        notes.push(`a control named "${spec.controlName}" already exists in this file but sits in a collection the ` +
    ...
    if (!parentNode && owned.discarded.has(spec.parentControl.toLowerCase())) {
        return {
            kind: 'refused',
            message: `Parent control "${spec.parentControl}" exists in this extension but sits inside a misplaced ` +
    ...
    if (parentNode) {
        ...
        if (dataGroup) {
            ...
            notes.push(dataGroupWarning(spec, textValueOf(xml, dataGroup), dsNode ? textValueOf(xml, dsNode) : undefined));
        }
        ...
        if (!controls) {
            ...
            if (!created) {
                return {
                    kind: 'refused',
                    message: `Parent control "${spec.parentControl}" is defined by this extension, has no <Controls> ` +
            ...
            if (spec.previousSibling) {
                notes.push(`previousSibling "${spec.previousSibling}" was ignored: "${spec.parentControl}" had no ` +
    ...
    if (position.kind === 'invalid')
        return { kind: 'refused', message: position.message };
    if (position.note)
...
function resolvePosition(spec) {
    ...
    if (normalized === 'afteritem') {
        if (!sibling) {
            return {
                kind: 'invalid',
                message: `positionType="AfterItem" needs previousSibling — the name of the existing control in ` +
    ...
    return {
        kind: 'invalid',
        message: `positionType="${requested}" is not a value D365FO form-extension metadata uses. Supported: ` +
 ...
 * through.
 */
function finish(updated, spec, representation, notes, baseline) {
    const reparsed = parseNodes(updated);
    if (!reparsed || reparsed.name !== 'AxFormExtension') {
        return {
            kind: 'refused',
            message: `Internal check failed: inserting "${spec.controlName}" produced XML that no longer parses as ` +
    ...
    if (!after.live.has(spec.controlName.toLowerCase())) {
        return {
            kind: 'refused',
            message: `Internal check failed: "${spec.controlName}" is not readable as a live control after ` +
 ...
 */
export function buildAbandonedWriteMessage(xml, controlName, introduced) {
    const root = parseNodes(xml);
    const objectName = root ? extensionNameOf(xml, root) : '(unparseable form extension)';
    return (`Internal check failed while adding "${controlName}" — the write was ABANDONED and the ` +
        `file left unchanged.\n\n` +
 ...
 */
function createControlsCollection(xml, parentNode, blockLines) {
 ...
 */
function dataGroupWarning(spec, dataGroup, dataSource) {
    const onTable = dataSource ? ` on \`${dataSource}\`` : '';
    const field = spec.dataField ?? spec.controlName;
    const generated = `${dataGroup}_${field}`;
    return (`parent "${spec.parentControl}" renders field group **${dataGroup}**${onTable} via \`<DataGroup>\`, ` +
...
const escapeXmlAttr = (value) => escapeXmlText(value).replace(/"/g, '&quot;');
/** The bare nested shape — no wrapper, no <Parent>; nesting encodes parentage. */
function nestedControlLines(spec) {
    return [
 ...
 */
function envelopeControlLines(spec, position) {
    const lines = [
        `<AxFormExtensionControl xmlns="">`,
 ...
 */
function innerControlLines(spec) {
    const lines = [
    ...
    if (spec.dataField)
        lines.push(`<DataField>${escapeXmlText(spec.dataField)}</DataField>`);
    if (spec.dataSource)
        lines.push(`<DataSource>${escapeXmlText(spec.dataSource)}</DataSource>`);
    if (spec.label)
        lines.push(`<Label>${escapeXmlText(spec.label)}</Label>`);
    if (spec.typeValue === 'ComboBox')
        lines.push(`<Items />`);
    return lines;
}
/**
 ...
 */
function insertIntoControls(xml, controls, blockLines, previousSibling, notes) {
    ...
    if (controls.selfClosing) {
        if (previousSibling) {
            notes.push(`previousSibling "${previousSibling}" was ignored: the parent's <Controls> collection is empty.`);
        }
        const openTag = xml.slice(controls.start, controls.openEnd).replace(/\s*\/>$/, '>');
        return (xml.slice(0, controls.start) +
    ...
    if (previousSibling) {
        ...
        if (sibling) {
            ...
            notes.push(`previousSibling "${previousSibling}" was not found among the parent's existing children — ` +
...
//# sourceMappingURL=formExtensionControlXml.js.map