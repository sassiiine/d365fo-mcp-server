 ...
 * much riskier thing.
 */
export function reloadWritePolicy() {
    const src = writePolicySource;
    if (!src)
        return;
    let stamp;
    try {
        stamp = String(statSync(src.envPath).mtimeMs);
    }
    catch {
        stamp = '-';
    }
    if (stamp === writePolicyStamp)
        return;
    writePolicyStamp = stamp;
    let parsed = {};
    try {
        parsed = dotenv.parse(readFileSync(src.envPath, 'utf-8'));
    }
    catch { /* no file, no policy */ }
    for (const key of WRITE_POLICY_VARS) {
        if (src.pinned.has(key))
            continue; // real environment still wins
