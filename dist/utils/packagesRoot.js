...
function plausibility(root, io) {
    ...
    if (entries.some(e => e.toLowerCase() === 'bin'))
        return 2; // real packages root
 ...
 */
export function scanPackagesRoots(io = realIo) {
    if (io.platform !== 'win32')
        return [];
    const hits = [];
    for (const letter of SCANNED_DRIVES) {
        if (!io.isDirectory(`${letter}:\\`))
            continue;
...
export function packagesRoots() {
    if (cached === null)
        cached = scanPackagesRoots();
    return cached;
}
/** The most plausible packages root on this machine, or null when there is none. */
export function findPackagesRoot() {
 ...
 */
export function defaultPackagesRoot() {
 ...
 */
function joinXPlat(root, relative) {
 ...
 */
export function resolveIndexedFilePath(filePath, opts = {}) {
    if (!filePath || isAbsoluteXPlat(filePath))
        return filePath;
 ...
 * drawn from it.
 */
export function isAotSourcePath(filePath) {
    return !!filePath && /\.(xml|xpp)$/i.test(filePath.trim());
}
/** Test seam — drops the cached scan. */
export function resetPackagesRootCache() {
    cached = null;
}
//# sourceMappingURL=packagesRoot.js.map