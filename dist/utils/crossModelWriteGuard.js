 ...
 * kind — see the header.
 */
export function activeCrossModelAllowance() {
    reloadWritePolicy();
 ...
 */
export function standDownNotice(check) {
    const { objectName, owningModel, activeModel } = check;
    const verb = check.action ?? 'modify';
    if (!owningModel)
        return '';
    if (activeModel && (eq(owningModel, activeModel) || eq(check.owningPackage, activeModel)))
        return '';
    if (!activeModel) {
...
export function crossModelWriteRefusal(check) {
    ...
    if (eq(check.toolSwitchedModel, owningModel)) {
        lines.push(`A get_workspace_info(projectName="${check.toolSwitchedModel}") switch earlier in this ` +
    ...
    lines.push(`"${owningModel}" is a different model: the change would land in code that "${activeModel}" ` +
    ...
    lines.push(`Do NOT route around this guard: no retry with a different modelName, filePath, ` +
        ...
        `earlier run made this same mistake, NOT evidence that the feature belongs there.`, '', `Writing into "${owningModel}" is the user's decision, not yours. If they want it, they add ` +
