 ...
 */
export function extractBaseEdtName(extensionObjectName) {
    if (!extensionObjectName)
        return extensionObjectName;
    const dotIdx = extensionObjectName.indexOf('.');
    if (dotIdx > 0)
        return extensionObjectName.substring(0, dotIdx);
 ...
 */
export function lookupBaseEdtFromIndex(db, baseEdtName) {
    if (!db || !baseEdtName)
        return null;
    try {
 ...
 */
export function resolveEdtChain(db, baseEdtName, maxDepth = 16) {
    const chain = [];
    const visited = new Set();
    let current = baseEdtName;
 ...
 */
export function resolveEffectiveDatabaseStringSize(db, baseEdtName, maxDepth = 16) {
 ...
 */
async function enrichWithBridge(base, bridge) {
    if (!bridge?.isReady || !bridge.metadataAvailable)
        return base;
    try {
        const live = (await bridge.readEdt(base.edtName));
        if (!live)
            return base;
        return {
            ...base,
 ...
 */
export function validateEdtExtensionProperty(base, propertyPath, propertyValue, db) {
    const propLower = (propertyPath || '').trim().toLowerCase();
    if (FORBIDDEN_EXT_PROPERTIES.has(propLower)) {
        return {
            ok: false,
            message: `⛔ Cannot change property '${propertyPath}' on an EDT extension.\n` +
    ...
    if (INHERITED_PROPERTIES.has(propLower)) {
        if (base.extends && base.extends.trim().length > 0) {
            ...
            return {
                ok: false,
                message: `⛔ Cannot modify '${propertyPath}' via AxEdtExtension on derived EDT '${base.edtName}'.\n\n` +
        ...
        if (base.isExtensible === false) {
            return {
                ok: false,
                message: `⛔ Cannot modify '${propertyPath}' on EDT '${base.edtName}' — base EDT is **not extensible** ` +
        ...
        if (base.isExtensible == null) {
            return {
                ok: false,
                message: `⛔ Cannot verify whether EDT '${base.edtName}' is marked **IsExtensible = true**.\n` +
        ...
        if (propLower === 'stringsize') {
            ...
            if (Number.isFinite(newSize) && Number.isFinite(currentSize) && newSize < currentSize) {
                return {
                    ok: false,
                    message: `⛔ Refusing to shrink StringSize on '${base.edtName}' from ${currentSize} to ${newSize}. ` +
            ...
            if (Number.isFinite(newSize) && dbSize != null && dbSize > 0 && newSize > dbSize) {
                return {
                    ok: false,
                    message: `⛔ Cannot widen StringSize on '${base.edtName}' to ${newSize}: it exceeds the ` +
        ...
        if (propLower === 'databasestringsize') {
            ...
            if (Number.isFinite(newDb) && newDb !== -1 && newDb > 0 &&
                ...
                return {
                    ok: false,
                    message: `⛔ Cannot shrink DatabaseStringSize on '${base.edtName}' to ${newDb}: ` +
 ...
 */
export async function validateEdtExtensionChange(extensionObjectName, propertyPath, propertyValue, db, bridge) {
    ...
    if (!base) {
        ...
        if (bridge?.isReady && bridge.metadataAvailable) {
            try {
                const live = (await bridge.readEdt(baseName));
                if (live) {
                    base = {
                        edtName: live.name,
    ...
    if (!base) {
        ...
        if (INHERITED_PROPERTIES.has(propLower) || FORBIDDEN_EXT_PROPERTIES.has(propLower)) {
            return {
                ok: false,
                message: `⛔ Cannot modify '${propertyPath}' on EDT extension '${extensionObjectName}' — ` +
...
//# sourceMappingURL=edtExtensionValidator.js.map