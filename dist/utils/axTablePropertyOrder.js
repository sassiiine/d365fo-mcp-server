...
export const AX_TABLE_ELEMENT_ORDER = [
 ...
 */
export const AX_TABLE_NON_EXISTENT_PROPERTIES = {
    AlternateKey: 'AlternateKey is an INDEX property, not a table property. Put <AlternateKey>Yes</AlternateKey> ' +
        'inside the <AxTableIndex> that should act as the alternate key.',
    PrimaryKey: 'There is no table-level PrimaryKey property. Use <PrimaryIndex> (and <ReplacementKey>) ' +
        'naming an existing index.',
    Index: 'Indexes are declared as <AxTableIndex> entries inside <Indexes>, not as a table property.',
};
/** Canonical position of an element; unknown names sort last but keep relative order. */
export function axTableElementRank(name) {
 ...
 */
export function renderAxTableProperties(props, indent = '\t') {
 ...
 */
export function upsertAxTableProperty(xml, property, value) {
    if (!/<AxTable[\s>]/.test(xml))
        return null;
    if (AX_TABLE_NON_EXISTENT_PROPERTIES[property])
        return null;
    if (axTableElementRank(property) === Number.MAX_SAFE_INTEGER)
        return null;
    ...
    for (const candidate of AX_TABLE_ELEMENT_ORDER) {
        if (axTableElementRank(candidate) <= rank)
            continue;
...
//# sourceMappingURL=axTablePropertyOrder.js.map