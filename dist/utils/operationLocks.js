...
function getLockDirectory(normalizedKey) {
 ...
 */
async function claimAndRemoveLock(lockDir) {
...
async function tryRemoveStaleLock(lockDir, normalizedKey) {
    try {
        ...
        if (typeof owner?.pid === 'number') {
            if (!isProcessAlive(owner.pid)) {
                ...
                if (!(await claimAndRemoveLock(lockDir)))
                    return false;
 ...
 */
function startLockHeartbeat(lockDir, ownerFile) {
...
async function acquireFilesystemLock(normalizedKey) {
...
export async function withOperationLock(lockKey, fn) {
...
export function getOperationLockCount() {
 ...
 */
export async function isOperationLockHeld(lockKey) {
    const normalizedKey = lockKey.trim().toLowerCase();
    if (operationLocks.has(normalizedKey))
        return true;
    const lockDir = getLockDirectory(normalizedKey);
    try {
        const stat = await fs.stat(lockDir);
...
//# sourceMappingURL=operationLocks.js.map