...
function verifySignedToken(token) {
    ...
    if (!secret || !token.startsWith(SIGNED_PREFIX))
        return undefined;
...
export function createProvenanceToken(context) {
    prune();
    const now = Date.now();
...
export function getProvenanceBundle(token) {
    prune();
 ...
 */
export function tokenMatchesTarget(bundle, targetObjectName) {
...
export function enforceGrounding(groundingToken, operationDescription, targetObjectName) {
    ...
    if (SERVER_MODE === 'write-only' && !getGroundingSecret()) {
        if (!warnedWriteOnlyBypass) {
            ...
            console.error('[provenance] ⚠️ GROUNDING_ENFORCE=true is ignored in write-only mode — ' +
    ...
    if (groundingToken && isValidToken(groundingToken)) {
        ...
        if (tokenMatchesTarget(bundle, targetObjectName))
            return null;
        return {
            isError: true,
            content: [{
                    type: 'text',
                    text: `❌ Grounding token mismatch for ${operationDescription} (GROUNDING_ENFORCE=true).\n\n` +
    ...
    return {
        ...
        content: [{
                type: 'text',
                text: `❌ Grounding required for ${operationDescription} (GROUNDING_ENFORCE=true).\n\n` +
                    `Reason: ${reason}.\n\n` +
                    `**Required workflow:**\n` +
...
//# sourceMappingURL=provenanceStore.js.map