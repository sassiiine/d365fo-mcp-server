...
function parseSig(sigLine) {
    ...
    if (!hasParens) {
        ...
        if (!classMatch)
            return null;
        if (/\b(private|internal)\b/.test(sigLine))
            return null;
 ...
 */
function splitTopLevelCommas(paramStr) {
    const chunks = [];
    let depth = 0;
    let current = '';
 ...
 */
function inferClassSummary(name, baseClass, attributes) {
    ...
    if (name.endsWith('UIBuilder'))
        return `UI builder class that customizes the dialog for ${humanize(name.replace(/UIBuilder$/, ''))}.`;
    ...
    if (/_Extension$/.test(name))
        return `Extension class for ${humanize(name.replace(/_Extension$/, '').replace(/\w+$/, ''))}.`;
    if (name.endsWith('Helper'))
        return `Helper class providing utility methods for ${humanize(name.replace(/Helper$/, ''))}.`;
    if (name.endsWith('Entity'))
        return `Data entity for ${humanize(name.replace(/Entity$/, ''))}.`;
    if (name.endsWith('Provider'))
        return `Data provider for ${humanize(name.replace(/Provider$/, ''))}.`;
 ...
 */
function inferMethodSummary(name, returnType, params) {
    const n = name.toLowerCase();
    // Well-known D365FO method names
    if (n === 'main')
        return 'Entry point for the class.';
    if (n === 'run')
        return 'Executes the main processing logic.';
    if (n === 'construct')
        return 'Creates and returns a new instance.';
    if (n === 'new' || n === 'init')
        return 'Initializes a new instance of the class.';
    if (n === 'insert')
        return 'Inserts the record into the database.';
    if (n === 'update')
        return 'Updates the record in the database.';
    if (n === 'delete')
        return 'Deletes the record from the database.';
    if (n === 'validatewrite')
        return 'Validates the record before it is written to the database.';
    if (n === 'validatedelete')
        return 'Validates the record before it is deleted.';
    if (n === 'validatefield')
        return 'Validates the specified field value.';
    if (n === 'modifiedfield')
        return 'Handles logic when a field value is modified.';
    if (n === 'initvalue')
        return 'Initializes default field values for a new record.';
    if (n === 'close')
        return 'Handles cleanup when the form or object is closed.';
    if (n === 'pack')
        return 'Serializes the class state into a container.';
    if (n === 'unpack')
        return 'Restores the class state from a packed container.';
    if (n === 'dialog')
        return 'Builds the dialog for user parameter input.';
    if (n === 'getfromdialog')
        return 'Reads parameter values from the dialog after user confirmation.';
    if (n === 'canrun')
        return 'Determines whether the operation can be executed.';
    if (n === 'description')
        return 'Returns a user-readable description of the operation.';
    if (n === 'processreport')
        return 'Main processing method that populates the report data set.';
    if (n === 'caption')
        return 'Returns the caption displayed to the user.';
    if (n === 'defaultcaption')
        return 'Returns the default caption for the operation.';
    // Prefix-based patterns
    if (n.startsWith('find'))
        return `Finds a record matching the specified ${describeParams(params)}.`;
    if (n.startsWith('exist'))
        return `Checks whether a record exists for the specified ${describeParams(params)}.`;
    if (n.startsWith('validate'))
        return `Validates ${humanize(name.substring(8))}.`;
    if (n.startsWith('parm')) {
        const prop = humanize(name.substring(4));
        return `Gets or sets the ${prop} value.`;
    }
    if (n.startsWith('calc') || n.startsWith('compute'))
        return `Calculates ${humanize(name.replace(/^calc|^compute/i, ''))}.`;
    if (n.startsWith('process'))
        return `Processes ${humanize(name.substring(7))}.`;
    if (n.startsWith('populate'))
        return `Populates ${humanize(name.substring(8))}.`;
    if (n.startsWith('build'))
        return `Builds ${humanize(name.substring(5))}.`;
    if (n.startsWith('create'))
        return `Creates ${humanize(name.substring(6))}.`;
    if (n.startsWith('init'))
        return `Initializes ${humanize(name.substring(4))}.`;
    if (n.startsWith('get'))
        return `Gets ${humanize(name.substring(3))}.`;
    if (n.startsWith('set'))
        return `Sets ${humanize(name.substring(3))}.`;
    if (n.startsWith('is') || n.startsWith('can') || n.startsWith('has'))
        return `Determines whether ${humanize(name)}.`;
    if (n.startsWith('on'))
        return `Handles the ${humanize(name.substring(2))} event.`;
    if (returnType === 'boolean')
        return `${humanize(name)} and returns the result.`;
 ...
 */
function inferParamDescription(paramName, paramType) {
    const n = paramName.replace(/^_+/, '').toLowerCase();
    if (n === 'args')
        return 'The framework arguments.';
    if (n === 'sender')
        return 'The event sender object.';
    if (n === 'e' || n === 'eventargs')
        return 'The event arguments.';
    if (n === 'insertmode')
        return 'Indicates whether the operation is an insert.';
    if (n === 'ret' || n === 'result')
        return 'The validation result.';
    if (paramType) {
        const t = paramType.toLowerCase();
        if (t === 'boolean')
            return `A value indicating whether ${humanize(paramName)}.`;
        if (t === 'args')
            return 'The framework arguments.';
    }
 ...
 */
function inferReturnDescription(returnType, methodName) {
    ...
    if (r === 'container')
        return 'A packed container with the serialized state.';
    if (r === 'str')
        return 'The resulting string value.';
    if (r === 'int' || r === 'int64' || r === 'real')
        return `The ${humanize(methodName)} value.`;
 ...
 * unchanged.
 */
export function ensureBlankLineBeforeClosingBrace(declaration) {
 ...
 */
export function ensureXppDocComment(source) {
 ...
 */
function completeExistingDocBlock(source) {
    const lines = source.split('\n');
    // Leading /// block = consecutive /// lines at the top.
    let docEnd = 0;
    while (docEnd < lines.length && lines[docEnd].trim().startsWith('///'))
        docEnd++;
    if (docEnd === 0)
        return source;
    ...
    const needsReturns = !!parsed.returnType && parsed.returnType.toLowerCase() !== 'void' && !hasReturns;
    if (missingParams.length === 0 && !needsReturns)
        return source;
    // Anchor: after the last </param> line, else after the </summary> line,
    // else at the end of the doc block.
    let anchor = -1;
    for (let i = 0; i < docLines.length; i++) {
        if (docLines[i].includes('</param>'))
            anchor = i;
    }
    if (anchor === -1) {
    ...
    if (anchor === -1)
        anchor = docLines.length - 1;
    const newDoc = [...docLines];
    const paramAdditions = missingParams.map(p => `${indent}/// <param name="${p.name}">${inferParamDescription(p.name, p.type)}</param>`);
    newDoc.splice(anchor + 1, 0, ...paramAdditions);
    if (needsReturns) {
        const returnsLine = `${indent}/// <returns>${inferReturnDescription(parsed.returnType, parsed.name)}</returns>`;
 ...
 */
function normalizeDocBlockIndent(source) {
    const lines = source.split('\n');
    let sigIndent = '';
 ...
 */
function stripDocCommentGap(source) {
...
//# sourceMappingURL=xppDocGen.js.map