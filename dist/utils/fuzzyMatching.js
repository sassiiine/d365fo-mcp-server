...
export function findFuzzyMatches(query, candidates, minScore = 0.7, maxResults = 5) {
    const matches = [];
 ...
 */
export function isProbableTypo(query, bestMatch, score) {
    if (score >= 0.85)
        return true;
    if (score >= 0.75 && hasSingleCharDifference(query, bestMatch))
        return true;
    if (hasTransposition(query, bestMatch))
        return true;
    return false;
}
/**
 ...
 */
function hasSingleCharDifference(str1, str2) {
 ...
 */
function hasTransposition(str1, str2) {
 ...
 */
export function generateBroaderSearches(query) {
    const suggestions = [];
 ...
 */
export function generateNarrowerSearches(query) {
    const suggestions = [];
    const commonSuffixes = [
 ...
 */
export function extractRootTerm(term) {
    const commonSuffixes = [
        'Helper', 'Service', 'Manager', 'Controller', 'Handler',
        'Builder', 'Factory', 'Provider', 'Processor', 'Engine',
...
//# sourceMappingURL=fuzzyMatching.js.map