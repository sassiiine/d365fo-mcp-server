 ...
 */
const INHERITING_TYPES = ['class', 'table'];
/**
 ...
 */
export function inheritanceAncestors(db, name, maxDepth = MAX_DEPTH) {
    if (!name)
        return [];
    const chain = [];
    try {
        ...
        while (current?.extends_class && chain.length < maxDepth) {
            ...
            if (!parentName || seen.has(parentName.toLowerCase()))
                break;
 ...
 */
export function findDeclaringAncestor(db, className, methodName) {
    if (!className || !methodName)
        return undefined;
    try {
        const stmt = db.prepare(`SELECT 1 AS x FROM symbols
       WHERE type = 'method' AND parent_name = ? AND name = ? COLLATE NOCASE
       LIMIT 1`);
        for (const ancestor of inheritanceAncestors(db, className)) {
            if (stmt.get(ancestor, methodName) !== undefined)
                return ancestor;
        }
    }
 ...
 */
export function inheritedOwnerCandidates(db, className, methodName, bridgeAvailable) {
    const declaring = findDeclaringAncestor(db, className, methodName);
    if (declaring)
        return [declaring];
    return bridgeAvailable ? inheritanceAncestors(db, className) : [];
}
//# sourceMappingURL=inheritanceChain.js.map