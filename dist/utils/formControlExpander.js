...
export function isRequired(spec) {
 ...
 */
export function canExpandPattern(spec) {
    if (spec.versions.length === 0)
        return false;
    const requiredNodesConcrete = (nodes) => nodes
        .filter(isRequired)
...
function emitProperties(props, indent) {
    if (!props)
        return '';
 ...
 */
export function buildControlXml(spec, opt, depth) {
 ...
 */
function emitNode(spec, opt, depth) {
    if (!isConcrete(spec))
        return '';
    ...
    if (controlType === 'Grid' && opt.dsName && (opt.gridFields?.length ?? 0) > 0) {
        gridColumns = opt
            .gridFields.map((f) => FormPatternTemplates.fieldControl(f, opt.dsName, '\t'.repeat(depth + 2), 'Grid_', opt.fieldTypes))
            .join('')
            .replace(/\n$/, '');
    }
...
function emitDataSources(opt) {
    const ds = (name, table) => `\t\t<AxFormDataSource xmlns="">\n` +
        `\t\t\t<Name>${name}</Name>\n` +
        `\t\t\t<Table>${table}</Table>\n` +
        `\t\t\t<Fields />\n` +
        ...
        `\t\t</AxFormDataSource>`;
    const blocks = [];
    if (opt.dsName && opt.dsTable)
        blocks.push(ds(opt.dsName, opt.dsTable));
    if (opt.linesDsName && opt.linesDsTable)
        blocks.push(ds(opt.linesDsName, opt.linesDsTable));
    if (blocks.length === 0)
        return `\t<DataSources />`;
    return `\t<DataSources>\n${blocks.join('\n')}\n\t</DataSources>`;
}
 ...
 */
export function expandPatternToXml(spec, opt) {
...
//# sourceMappingURL=formControlExpander.js.map