 ...
 */
export function controlForTableField(tableFieldIType, enumType) {
 ...
 */
export function parseTableFieldControls(tableXml) {
    const map = new Map();
 ...
 */
export function parseViewFieldBindings(viewXml) {
    const out = [];
 ...
 */
export function parseQueryDataSourceTables(queryXml) {
    const map = new Map();
...
export function parseViewQueryName(viewXml) {
...
function readIndexedObjectXml(db, name, types) {
 ...
 */
function getViewFieldControlMap(db, viewXml, depth = 0) {
    const map = new Map();
    ...
    for (const b of bindings) {
        ...
        if (hit)
            map.set(b.name.toLowerCase(), hit);
    }
    return map;
}
/**
 ...
 */
export function getFieldControlMap(db, table) {
    try {
        ...
        if (/^\s*<AxView[\s>]/m.test(xml))
            return getViewFieldControlMap(db, xml);
 ...
 */
export function parseTableTitleField(tableXml) {
 ...
 */
export function getTableTitleField(db, table) {
    try {
        const canonical = lookupSymbolNocase(db, table)?.name ?? table;
        const row = db
            .prepare(`SELECT file_path FROM symbols
...
export function controlForField(field, types) {
    return types?.get(field.toLowerCase()) ?? DEFAULT_CONTROL;
}
//# sourceMappingURL=fieldControlTypes.js.map