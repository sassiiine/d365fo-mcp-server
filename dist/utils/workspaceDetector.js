 ...
 */
export function isMicrosoftDemoModel(modelName) {
 ...
 */
async function findProjectFiles(dir, maxDepth = 5, currentDepth = 0) {
 ...
 */
export async function extractModelNameFromProject(projectPath) {
    try {
 ...
 */
async function readModelNames(projectFiles) {
 ...
 */
function distinctCustomModels(models) {
    const seen = new Map();
    for (const m of models) {
        if (isMicrosoftDemoModel(m))
            continue;
 ...
 */
function resolvePrimaryProject(workspacePath, projectFiles) {
    if (projectFiles.length === 1)
        return projectFiles[0];
    const wpBase = path.basename(workspacePath).toLowerCase();
    const nameMatch = projectFiles.find(p => path.basename(path.dirname(p)).toLowerCase() === wpBase);
    if (nameMatch) {
 ...
 */
export async function detectD365Project(workspacePath, maxDepth = 5) {
    try {
        ...
        if (!primaryProject) {
            ...
            debugLog(`[WorkspaceDetector] ${projectFiles.length} .rnrproj files and none matches the workspace ` +
            ...
            if (owning.length === 1) {
                ...
                debugLog(`[WorkspaceDetector] ${owning.length} projects share model "${customModels[0]}" — model resolved, ` +
 ...
 */
export async function autoDetectD365Project(explicitWorkspacePath) {
    ...
    if (explicitWorkspacePath) {
        ...
        if (result)
            return won('the workspace path', result);
    }
    ...
    if (cwdIsNodeProject) {
        ...
        if (cwdResult)
            return won('the current working directory', cwdResult);
    }
    ...
    if (envWorkspace) {
        ...
        if (envResult)
            return won('the WORKSPACE_PATH env var', envResult);
    }
    ...
    if (solutionsRoot) {
        ...
        if (result)
            return won('D365FO_SOLUTIONS_PATH', result);
    }
                ...
                if (files.length > 0) {
                    ...
                    if (files.length > 1) {
                        ...
                        const owning = customModels.length === 1
                            ...
                            : [];
                        if (owning.length !== 1) {
                            debugLog(`[WorkspaceDetector] Skipping ${loggedSearchRoot}: ${files.length} .rnrproj files across ` +
    ...
    if (explicitWorkspacePath) {
        ...
        const twoLevelMatch = normalized.match(/^(.+[\\\/]PackagesLocalDirectory)[\\\/]([^\\\/]+)[\\\/]([^\\\/]+)\\?\/?$/i);
        if (twoLevelMatch) {
 ...
 */
export async function detectGitBranch(workspaceRoot) {
    try {
        const { execFile } = await import('child_process');
        const { promisify } = await import('util');
        const execFileAsync = promisify(execFile);
        const { stdout } = await execFileAsync('git', ['-C', workspaceRoot, 'rev-parse', '--abbrev-ref', 'HEAD'], { timeout: 5000, windowsHide: true, encoding: 'utf8' });
 ...
 */
export async function scanAllD365Projects(rootPath) {
    try {
