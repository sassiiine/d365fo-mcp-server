...
export function reportSlowCall(toolName, elapsedMs, args) {
    if (elapsedMs < SLOW_CALL_MS)
        return;
    console.error(`[toolMetrics] ⚠️ slow call: ${toolName} took ${(elapsedMs / 1000).toFixed(1)}s${digestArgs(args)}`);
}
/** Call before dispatching a tool. Returns a finish() callback. */
export function recordToolStart(toolName) {
    const t0 = Date.now();
    return (isEmpty) => {
 ...
 */
export function recordCallSequence(toolName, argsKey) {
    ...
    if (occurrences > 1)
        getStats(toolName).duplicateCalls++;
    return occurrences;
}
 ...
 */
export function startMetricsLogging(intervalMs = 5 * 60 * 1000) {
    ...
    logIntervalHandle = setInterval(() => {
        ...
        const lines = top10.map(r => `  ${r.tool.padEnd(40)} calls=${r.calls}  avgMs=${r.avgLatencyMs}  emptyRatio=${r.emptyRatio}`);
