 ...
 */
export function resolveEnumValueMode(enumName, properties, values) {
    ...
    if (offPositional.length > 0) {
        const shown = offPositional
            ...
            .join(', ');
        if (isExtensible) {
            throw new Error(`Enum '${enumName}': isExtensible=true cannot be combined with explicit enum values (${shown}) — ` +
        ...
        if (properties?.useEnumValue === false) {
            throw new Error(`Enum '${enumName}': useEnumValue=false contradicts the explicit enum values (${shown}) — ` +
    ...
    const useEnumValue = (isExtensible || properties?.useEnumValue === false)
        ? 'No'
 ...
 */
export function assertKnownEnumValue(propertyName, value, allowed, fallback) {
    ...
    if (match)
        return match;
    throw new Error(`${propertyName}: "${raw}" is not a valid value — nothing was written. ` +
...
//# sourceMappingURL=axEnumProperties.js.map