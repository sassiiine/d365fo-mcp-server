...
import { bridgeStartupState } from '../bridge/bridgeReadiness.js';
/** Look up a top-level object in the symbol index and resolve a readable local path. */
export async function resolveIndexedObject(db, name, types, modelName) {
    let hit;
    try {
        hit = lookupSymbolNocase(db, name, types);
    }
    catch {
    ...
    if (hit.name?.toLowerCase() !== name.toLowerCase())
        return null;
...
async function resolveLocalPath(indexedPath) {
    if (!indexedPath)
        return null;
    try {
 ...
 */
async function packagesRootReachable() {
    try {
 ...
 */
async function isStaleIndexedPath(indexedPath, localPath) {
    if (localPath !== null)
        return false;
    if (!indexedPath || !/PackagesLocalDirectory/i.test(indexedPath))
        return false;
...
export async function readIndexedXml(db, name, types, modelName) {
 ...
 */
export function indexedSourceNote(source, ref) {
 ...
 */
export function staleIndexNote(ref) {
    if (!ref.sourceFileMissing)
        return '';
 ...
 */
export async function indexedPathIsMissing(indexedPath) {
    if (!indexedPath || !/PackagesLocalDirectory/i.test(indexedPath))
        return false;
 ...
 */
export function renderStaleSearchRowsNote(count) {
...
export function renderStaleIndexNote(name, indexedPath) {
 ...
 */
export function bridgeUnavailableNote(context) {
...
//# sourceMappingURL=indexedXmlLookup.js.map