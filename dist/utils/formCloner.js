 ...
 */
export function findElementBlocks(xml, tagName, searchStart = 0, searchEnd) {
    const blocks = [];
...
function firstElementValue(content, tagName) {
...
function extPropValue(content, propName) {
...
function firstRemainingChildName(containerContent, removed) {
    for (const child of findElementBlocks(containerContent, 'AxFormControl', containerContent.indexOf('>') + 1)) {
        ...
        if (n && !removed.has(n.toLowerCase()))
            return n;
    }
    return undefined;
}
...
function replaceDsReferences(xml, from, to) {
    const tags = ['DataSource', 'TitleDataSource', 'JoinSource'];
    let result = xml;
    for (const tag of tags) {
        const re = new RegExp(`(<${tag}(?:\\s[^>]*)?>)${escapeRegExp(from)}(</${tag}>)`, 'g');
        result = result.replace(re, `$1${to}$2`);
    }
    return result;
}
export function cloneFormXml(sourceXml, opt) {
    const { targetFormName, tableMapping = {}, getTableFields, stripMethods = true, caption, } = opt;
    let xml = sourceXml;
    const result = {
        xml: sourceXml,
        sourceFormName: '',
    ...
    if (!rootNameMatch)
        throw new Error('Source XML has no <Name> element — not an AxForm?');
    ...
    xml = xml.replace(new RegExp(`\\bclass\\s+${escapeRegExp(sourceFormName)}\\b`, 'g'), `class ${targetFormName}`);
    ...
    // empty the body so the clone compiles. The form name was already retargeted.
    {
        ...
        if (sc) {
            for (const method of findElementBlocks(xml, 'Method', sc.start, sc.end)) {
                if (firstElementValue(method.content, 'Name') !== 'classDeclaration')
                    continue;
    ...
    for (const ds of [...dsBlocks].sort((a, b) => b.start - a.start)) {
        ...
        let newBlock = ds.content.replace(new RegExp(`(<Table(?:\\s[^>]*)?>)${escapeRegExp(dsTable)}(</Table>)`), `$1${targetTable}$2`);
    ...
    if (getTableFields) {
        ...
        for (const ds of findElementBlocks(xml, 'AxFormDataSource', scEnd2 === -1 ? 0 : scEnd2)) {
            ...
            if (!fields)
                continue; // unknown table — keep everything
            ...
            if (total > 0)
                result.fieldStats.push({ dataSource: dsName, total, dropped });
        }
        xml = removeRanges(xml, removals);
        // Remove controls bound to dropped fields
        if (result.droppedFields.length > 0) {
            const dropped = new Set(result.droppedFields.map((d) => `${d.dataSource.toLowerCase()}|${d.field.toLowerCase()}`));
            ...
            for (const control of findElementBlocks(xml, 'AxFormControl', designStart === -1 ? 0 : designStart)) {
                ...
                while (stack.length > 0) {
                    ...
                    const isBoundToDropped = dataField && dataSource && dropped.has(`${dataSource.toLowerCase()}|${dataField.toLowerCase()}`);
            ...
            for (const ext of [...findElementBlocks(xml, 'FormControlExtension')].sort((a, b) => b.start - a.start)) {
                ...
                if (!defCol || !removedSet.has(defCol.toLowerCase()))
                    continue;
                ...
                if (replacement) {
                    const newExt = ext.content.replace(new RegExp(`(<Value>)${escapeRegExp(defCol)}(</Value>)`), `$1${replacement}$2`);
...
//# sourceMappingURL=formCloner.js.map