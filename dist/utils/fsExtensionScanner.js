...
export const EXTENSION_FOLDER_CONFIG = {
 ...
 */
const SCAN_TIMEOUT_MS = Math.max(500, parseInt(process.env.D365FO_FS_SCAN_TIMEOUT_MS || '3000', 10) || 3000);
/**
...
function cacheKeyFor(objectName, extensionType, packagePath) {
 ...
 */
export async function scanFsExtensions(objectName, extensionType, packagePath) {
    if (FS_FALLBACK_DISABLED)
        return [];
        ...
        for (const mdl of modelDirs) {
            ...
            for (const xmlFile of xmlFiles) {
                ...
                if (config.isClassStyle) {
                    ...
                    if (!lowerBase.startsWith(lowerObjName + '_'))
                        continue;
                    if (!lowerBase.endsWith('_extension'))
                        continue;
                }
                else {
                    // BaseName.ModelName.xml
                    if (!lowerBase.startsWith(lowerObjName + '.'))
                        continue;
                }
                const fullPath = path.join(axDir, xmlFile);
                try {
                    const result = await parseExtensionFile(fullPath, baseName, mdl, extensionType, config);
                    if (result)
                        results.push(result);
                }
                catch {
...
function rootKeyFor(config) {
    return config.axFolder;
}
async function parseExtensionFile(fullPath, baseName, model, extensionType, config) {
    ...
    if (!root)
        return null;
    const extName = root.Name?.[0] ?? baseName;
    const addedFields = [];
    const addedIndexes = [];
    const addedMethods = [];
    const cocMethods = [];
    ...
    if (sourceCode) {
        ...
        if (methodsNode && typeof methodsNode === 'object') {
            const methodArr = methodsNode.Method ?? [];
 ...
 *   </Fields>
 */
function extractNamedChildren(node, out) {
...
//# sourceMappingURL=fsExtensionScanner.js.map