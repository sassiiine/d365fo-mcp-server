 ...
 */
export function buildProgressMessage(toolName, args) {
    const a = args ?? {};
    switch (toolName) {
        case 'search':
            if (Array.isArray(a.queries)) {
                return `🔍 Batch search: ${a.queries.map((q) => `"${q.query ?? q}"`).join(', ')}`;
            }
        ...
        case 'get_object_info':
            if (Array.isArray(a.objects) && a.objects.length > 1) {
                return `📦 Reading ${a.objects.length} objects: ${a.objects.map((o) => o.objectName ?? '').join(', ')}`;
            }
            if (Array.isArray(a.objects) && a.objects.length === 1) {
                    ...
                    switch (op) {
                        ...
                        case 'remove-index': {
                            const fields = Array.isArray(p.indexFields)
                                ? p.indexFields.map((f) => f.fieldName ?? f).join(', ')
                                : '';
                        ...
                        case 'remove-relation': {
                            const constraints = Array.isArray(p.relationConstraints)
                                ? p.relationConstraints.map((c) => `${c.fieldName ?? c.field} → ${c.relatedFieldName ?? c.relatedField}`).join(', ')
                                : '';
        ...
        case 'generate_object':
            if (a.mode === 'scaffold') {
                const kind = a.objectType ?? 'object';
        ...
        case 'run_bp_check':
            return `🔍 Running Best Practices check${Array.isArray(a.objects) && a.objects.length > 0
        ...
        case 'get_knowledge':
            ...
            if (a.kind === 'op-spec')
                return `📖 Reading parameter spec: "${a.topic ?? a.operation ?? a.mode ?? ''}"`;
 ...
 */
function deriveMethodNameForLabel(source) {
...
//# sourceMappingURL=toolProgressMessage.js.map