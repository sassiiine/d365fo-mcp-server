 ...
 */
export function pageFields(fields, offset = 0, filter, pageSize = TABLE_FIELD_PAGE_SIZE) {
...
export function fieldsHeading(page) {
    if (page.filter) {
 ...
 */
export function fieldsFooter(page) {
    if (!page.hasMore)
        return '';
...
export function createControlBudget(max = DEFAULT_MAX_CONTROLS) {
 ...
 */
export function chargeControl(budget) {
...
export function chargeSkippedSubtree(budget, count) {
    budget.omitted += count;
}
export function controlsFooter(budget) {
    if (budget.omitted <= 0)
        return '';
 ...
 */
export function truncateOnBlockBoundary(text, cap) {
    if (text.length <= cap)
        return text;
    ...
    if (lastOpen > lastClose)
        cut = lastClose >= 0 ? lastClose + 1 : 0;
    return text.slice(0, cut);
}
//# sourceMappingURL=payloadBudget.js.map