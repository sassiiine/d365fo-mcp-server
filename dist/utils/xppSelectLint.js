 ...
 */
export function lintXppSelect(source) {
    if (!source || !/\bselect\b/i.test(source))
        return [];
    ...
    while ((m = selectRe.exec(cleaned)) !== null) {
        const stmt = m[0];
        if (!/\bjoin\b/i.test(stmt))
            continue; // only join-bearing selects can have this bug
        ...
        for (let s = 1; s < segments.length; s++) {
            ...
            if (whereCount >= 2) {
                ...
                warnings.push(`⚠️ Possible X++ select error: a WHERE clause appears AFTER a join.\n` +
...
//# sourceMappingURL=xppSelectLint.js.map