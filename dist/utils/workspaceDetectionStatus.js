...
export function recordDetectionSuccess(source, modelName, projectPath) {
    ...
    if (status.resolved && status.source === source && status.modelName === modelName)
        return;
    ...
    status.warned = false;
    if (wasWarned) {
        console.error(`[WorkspaceDetector] ✅ Detection resolved after all: model "${modelName}" from ${source} ` +
 ...
 * being plausible.
 */
export function recordDetectionFailure(tried) {
    if (status.resolved)
        return;
    status.tried = tried;
    status.attempts++;
...
export function getWorkspaceDetectionStatus() {
    return { ...status };
}
/**
 ...
 */
export function reportUnresolvedDetection() {
    if (status.resolved || status.warned)
        return false;
    status.warned = true;
    console.error(`[WorkspaceDetector] ⚠️ Could not auto-detect a D365FO project after ${status.attempts} ` +
...
export function describeWorkspaceDetection() {
    if (status.resolved) {
...
export function resetWorkspaceDetectionStatus() {
...
//# sourceMappingURL=workspaceDetectionStatus.js.map