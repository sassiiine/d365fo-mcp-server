...
export class XppConfigProvider {
    ...
    constructor(configDir) {
        this.configDir = configDir ||
            path.join(process.env.LOCALAPPDATA || path.join(process.env.USERPROFILE || '', 'AppData', 'Local'), 'Microsoft', 'Dynamics365', 'XPPConfig');
    }
    /**
    ...
    async listConfigs() {
        ...
        const withStats = await Promise.all(jsonFiles.map(async (entry) => {
        ...
        for (const { entry } of valid) {
            ...
            try {
                ...
                if (!json.ModelStoreFolder || !json.FrameworkDirectory)
                    continue;
                configs.push({
    ...
    async getActiveConfig(configName) {
        ...
        if (configs.length === 0)
            return null;
        if (configName) {
            return configs.find(c => c.fullFilename === configName ||
                c.configName === configName) || null;
        }
        ...
        if (configs.length > 1) {
            ...
            console.warn(`[XppConfigProvider] XPP_CONFIG_NAME is not set and ${configs.length} configs were found ` +
...
//# sourceMappingURL=xppConfigProvider.js.map