...
export class FormPatternTemplates {
     ...
     * string control.
     */
    static fieldControl(field, dsName, indent, namePrefix = '', types) {
        const ctl = controlForField(field, types);
        return (`${indent}<AxFormControl xmlns=""\n` +
    ...
    static buildSimpleList(opt) {
        ...
        const fieldControls = gridFields.map(f => FormPatternTemplates.fieldControl(f, dsName, '\t\t\t\t\t', 'Grid_', opt.fieldTypes)).join('');
        const dsFields = gridFields.length > 0
            ? `\t\t\t<Fields>\n` +
                gridFields.map(f => `\t\t\t\t<AxFormDataSourceField>\n\t\t\t\t\t<DataField>${f}</DataField>\n\t\t\t\t</AxFormDataSourceField>\n`).join('') +
                `\t\t\t</Fields>\n`
            : `\t\t\t<Fields />\n`;
...
`;
    ...
    static buildSimpleListDetails(opt) {
        const { formName, dsName = formName, dsTable = dsName, caption, gridFields = [] } = opt;
        const captionXml = caption
            ? `\t\t<Caption xmlns="">${caption}</Caption>\n`
            : '';
        const defaultCol = gridFields.length > 0 ? `Grid_${gridFields[0]}` : `Grid_${dsName}`;
        const listFieldControls = gridFields.slice(0, 3).map(f => FormPatternTemplates.fieldControl(f, dsName, '\t\t\t\t\t\t\t\t', 'Grid_', opt.fieldTypes)).join('');
        const detailFieldControls = gridFields.map(f => FormPatternTemplates.fieldControl(f, dsName, '\t\t\t\t\t\t\t\t\t\t', 'Overview_', opt.fieldTypes)).join('');
        // xppc requires the Tab's FieldsFieldGroups page to hold real controls; an empty group is rejected.
        const tabFieldControls = gridFields.map(f => FormPatternTemplates.fieldControl(f, dsName, '\t\t\t\t\t\t\t\t\t', 'Tab_', opt.fieldTypes)).join('');
        return `<?xml version="1.0" encoding="utf-8"?>
<AxForm xmlns:i="http://www.w3.org/2001/XMLSchema-instance" xmlns="Microsoft.Dynamics.AX.Metadata.V6">
\t<Name>${formName}</Name>
\t<SourceCode>
...
`;
    ...
    static buildDetailsMaster(opt) {
        const { formName, dsName = formName, dsTable = dsName, caption, gridFields = [] } = opt;
        const captionXml = caption
            ? `\t\t<Caption xmlns="">${caption}</Caption>\n`
            : '';
        const overviewFieldControls = gridFields.map(f => FormPatternTemplates.fieldControl(f, dsName, '\t\t\t\t\t\t\t\t\t\t', 'Overview_', opt.fieldTypes)).join('');
        ...
        const navListFieldControls = gridFields.slice(0, 3).map(f => FormPatternTemplates.fieldControl(f, dsName, '\t\t\t\t\t\t', 'NavList_', opt.fieldTypes)).join('');
        const generalFieldControls = gridFields.map(f => FormPatternTemplates.fieldControl(f, dsName, '\t\t\t\t\t\t\t\t\t\t\t\t', 'General_', opt.fieldTypes)).join('');
        const gridPanelFieldControls = gridFields.slice(0, 5).map(f => FormPatternTemplates.fieldControl(f, dsName, '\t\t\t\t\t\t\t\t', 'GridPanel_', opt.fieldTypes)).join('');
...
`;
    ...
    // ---------------------------------------------------------------------------
    static buildDetailsTransaction(opt) {
        const { formName, dsName = formName, dsTable = dsName, caption, linesDsName = `${dsName.replace(/Table$/i, '')}Line`, linesDsTable = linesDsName, gridFields = [], linesFields = [], } = opt;
        const captionXml = caption
            ? `\t\t<Caption xmlns="">${caption}</Caption>\n`
            : '';
        // Read-only navigation list: up to 3 identifying header fields.
        const navListColumns = gridFields.slice(0, 3).map(f => FormPatternTemplates.fieldControl(f, dsName, '\t\t\t\t\t\t\t', 'Nav_', opt.fieldTypes)).join('');
        const headerFieldControls = gridFields.map(f => FormPatternTemplates.fieldControl(f, dsName, '\t\t\t\t\t\t\t\t\t', 'Header_', opt.fieldTypes)).join('');
        const linesColumns = linesFields.map(f => FormPatternTemplates.fieldControl(f, linesDsName, '\t\t\t\t\t\t\t\t\t', 'Line_', opt.linesFieldTypes)).join('');
        ...
        const lineDetailControls = linesFields.map(f => FormPatternTemplates.fieldControl(f, linesDsName, '\t\t\t\t\t\t\t\t\t\t', 'LineDtl_', opt.linesFieldTypes)).join('');
        ...
        const gridPanelColumns = gridFields.slice(0, 5).map(f => FormPatternTemplates.fieldControl(f, dsName, '\t\t\t\t\t\t\t\t', 'GridPanel_', opt.fieldTypes)).join('');
        ...
        const headerViewFieldControls = gridFields.map(f => FormPatternTemplates.fieldControl(f, dsName, '\t\t\t\t\t\t\t\t\t\t', 'HView_', opt.fieldTypes)).join('');
        const dsFieldList = (fields) => fields.length > 0
            ? `\t\t\t<Fields>\n` +
                fields.map(f => `\t\t\t\t<AxFormDataSourceField>\n\t\t\t\t\t<DataField>${f}</DataField>\n\t\t\t\t</AxFormDataSourceField>\n`).join('') +
                `\t\t\t</Fields>\n`
            : `\t\t\t<Fields />\n`;
...
`;
    ...
    static buildDialog(opt) {
        ...
        const sectionControls = sections.map(s => `\t\t\t\t<AxFormControl xmlns=""\n` +
            `\t\t\t\t\t\ti:type="AxFormTabPageControl">\n` +
            `\t\t\t\t\t<Name>${s.name}</Name>\n` +
            `\t\t\t\t\t<Pattern>FieldsFieldGroups</Pattern>\n` +
            `\t\t\t\t\t<PatternVersion>1.1</PatternVersion>\n` +
...
`;
    ...
    // ---------------------------------------------------------------------------
    static buildTableOfContents(opt) {
        ...
        const sectionFields = (s) => (dsName ? gridFields : []).map(f => FormPatternTemplates.fieldControl(f, dsName, '\t\t\t\t\t\t\t\t\t\t', `${s.name}_`, opt.fieldTypes)).join('');
        const tabPageControls = effectiveSections.map(s => `\t\t\t\t<AxFormControl xmlns=""\n` +
            `\t\t\t\t\t\ti:type="AxFormTabPageControl">\n` +
            `\t\t\t\t\t<Name>${s.name}</Name>\n` +
            `\t\t\t\t\t<Type>TabPage</Type>\n` +
            `\t\t\t\t\t<FormControlExtension\n\t\t\t\t\t\ti:nil="true" />\n` +
...
`;
    ...
    // ---------------------------------------------------------------------------
    static buildLookup(opt) {
        const { formName, dsName = formName, dsTable = dsName, caption, gridFields = [] } = opt;
        const captionXml = caption
            ? `\t\t<Caption xmlns="">${caption}</Caption>\n`
            : '';
        const fieldControls = gridFields.map(f => `\t\t\t\t\t<AxFormControl xmlns=""\n` +
            `\t\t\t\t\t\t\ti:type="AxFormStringControl">\n` +
            `\t\t\t\t\t\t<Name>Grid_${f}</Name>\n` +
            `\t\t\t\t\t\t<Type>String</Type>\n` +
            `\t\t\t\t\t\t<FormControlExtension\n\t\t\t\t\t\t\ti:nil="true" />\n` +
...
`;
    ...
    static buildWorkspace(opt) {
        ...
        const listSections = sections.map((sec, idx) => `\t\t\t\t\t<AxFormControl xmlns=""\n` +
            `\t\t\t\t\t\t\ti:type="AxFormTabPageControl">\n` +
...
`;
     ...
     */
    static normalizePattern(raw) {
        const s = raw.toLowerCase().replace(/[^a-z]/g, '');
        if (s.includes('simplelist') && s.includes('detail'))
            return 'SimpleListDetails';
        if (s.includes('simplelist'))
            return 'SimpleList';
        if (s.includes('listpage'))
            return 'ListPage';
        if (s.includes('detailmaster') || s.includes('detailsmaster'))
            return 'DetailsMaster';
        if (s.includes('detailtransaction') || s.includes('detailstransaction'))
            return 'DetailsTransaction';
        if (s.includes('dialog') || s.includes('dropdialog'))
            return 'Dialog';
        if (s.includes('tableofcontents') || s.includes('toc') || s.includes('parameter'))
            return 'TableOfContents';
        if (s.includes('lookup'))
            return 'Lookup';
        if (s.includes('workspace') || s.includes('panorama') || s.includes('operational'))
            return 'Workspace';
        return 'SimpleList'; // default — most common for new setup tables
    }
}
//# sourceMappingURL=formPatternTemplates.js.map