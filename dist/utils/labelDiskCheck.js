 ...
 */
function fileDeclaresLabel(content, labelId) {
 ...
 */
export async function labelMissingOnDisk(labelId, filePaths) {
    let readAny = false;
    let bytesRead = 0;
    for (const filePath of filePaths) {
        try {
            const stat = await fs.stat(filePath);
            if (!stat.isFile() || stat.size > MAX_LABEL_FILE_BYTES)
                continue;
            ...
            if (bytesRead + stat.size > MAX_TOTAL_READ_BYTES)
                return null;
...
//# sourceMappingURL=labelDiskCheck.js.map