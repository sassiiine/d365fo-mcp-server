 ...
 */
function scanLine(line, inBlockCommentAtStart = false) {
    const braces = [];
    const code = [];
    let leadingCloses = 0;
    ...
    for (let i = 0; i < line.length; i++) {
        ...
        if (stringQuote) {
            ...
            if (c === stringQuote && next === stringQuote) {
                code.push(next);
                i++;
                continue;
            } // doubled quote
 ...
 */
function terminatesStatement(code) {
    if (code === '')
        return true; // comment-only line, or the body of a block comment
    if (code.startsWith('#'))
        return true; // #define / #macrolib
    if (code.startsWith('[') && code.endsWith(']'))
        return true; // [ExtensionOf(…)]
    return /[;{}:]$/.test(code);
}
/**
 ...
 */
export function reindentXppSource(source, baseDepth = 1) {
    ...
    const innermostSwitch = () => {
        for (let i = blocks.length - 1; i >= 0; i--)
            if (blocks[i].isSwitch)
                return blocks[i];
        return undefined;
    };
    ...
    for (const raw of lines) {
        ...
        if (/^switch\b/.test(code) && !braces.includes('{'))
            pendingSwitch = true;
        if (startsCase) {
            const sw = innermostSwitch();
 ...
 */
export function xppMethodSourceForXml(source) {
    const body = reindentXppSource(source);
    return body === '' ? '' : `${body}\n`;
}
//# sourceMappingURL=xppFormat.js.map